...
var ThinxApp = function() {
    ...
    */
      ...
      userlib.get(owner, function(err, user) {
        ...
        client.get("ak:" + user._id, function(err, redis_keys) {
          if ((typeof(redis_keys) !== "undefined") || (redis_keys !==
              null)) {
            console.log("[WARNING-NEW]: fetched redis_keys" +
              JSON.parse(
                redis_keys));
          } else {
    ...
    app.post("/api/user/create", function(req, res) {
      ...
      https.createServer(options, app).listen(serverPort + 1); http.createServer(
      ...
      console.log(""); console.log("-=[ ☢ " + product + " v" + version +
        " rev. " +
        app.version() +
        " ☢ ]=-"); console.log(""); console.log("» Started on port " +
        serverPort +
        " (HTTP) and " + (serverPort +
          1) +
        " (HTTPS)");
