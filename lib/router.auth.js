...
const SessionToken = require("./thinx/session_token");
...
module.exports = function (app) {
    ...
    // SEC-CSRF-01: double-submit anti-CSRF token check for cookie-session login.
    ...
    let userlib = require("./thinx/couch")(db_uri).use(prefix + "managed_users"); // lgtm [js/unused-local-variable]
    ...
    function performTokenLogin(req, res, oauth) {
        ...
        redis.get(oauth, (error, userWrapper) => {
                ...
                userlib.get(owner_id, (gerr, doc) => {
                    ...
                    SessionToken.markLogin(req.session, owner_id);
                    ...
                    // Cookie attributes are owned by the global sessionConfig
                    // (thinx-core.js): secure:false on purpose, because Traefik
                    // terminates TLS and the app sees plain HTTP. Forcing
                    // cookie.secure=true here made express-session SUPPRESS the
                    // Set-Cookie header behind the proxy (req.secure===false), so
                    // OAuth/token logins established no session and the dashboard's
                    // /api/user/* calls 401'd straight back to login. Inherit the
                    // same cookie config the working password path already uses.

                    // Additionally mint JWTs so SPA consoles (Vue) that authenticate
                    // via `Authorization: Bearer` can complete OAuth through the
                    // /oauth-return page. The legacy auth.html ignores these fields
                    // and rides the session cookie set above.
                    app.login.sign_with_refresh(owner_id, (access_token, refresh_token) => {
                        Util.respond(res, {
                            redirectURL: "/app",
                            success: true,
    ...
    function respondRedirectWithToken(req, res, user_data) {
        ...
        console.log(`🔨 [debug] [auth] [OID:${user_data.owner}] Signing JWT access+refresh tokens`);
    ...
    function checkMqttKeyAndLogin(req, cached_response, user_data) {
        ...
        user.mqtt_key(user_data.owner, function (success, key) {
            ...
            user.create_default_acl(user_data.owner, () => {
                ...
                // audit
                console.log(`ℹ️ [info] Calling trackUserLogin on checkMqttKeyAndLogin...`);
    ...
    function logoutAction(req, res) {
        const owner = (req.session && req.session.owner) ? req.session.owner : "unknown";
        console.log(`ℹ️ [info] [OID:${owner}] Logout action`);
        if (typeof (req.session) !== "undefined") {
            req.session.destroy(function (err) {
                if (err) {
                    console.log(`☣️ [error] [OID:${owner}] logoutAction session destroy error: ${err}`);
                }
            });
        } else {
    ...
    function loginAction(req, res) {
        ...
        if ((typeof (token) !== "undefined") && (token !== null)) {
            console.log("🔨 [debug] [auth] OAuth token login attempt");
            return performTokenLogin(req, res, token);
        }
        ...
        user.validate(username, (db_body) => {
            ...
            // owner.validate() passes `false` when the user DB view errors
            // (e.g. CouchDB unavailable during a deploy). Without this guard,
    ...
    // SEC-CSRF-01: cold-prime endpoint — lets a cookieless console obtain the
    // XSRF-TOKEN cookie (already minted by the global ensureXsrfCookie for this
    // request) before its first protected POST. GET, NOT wrapped in verifyCsrfToken.
    ...
    app.post("/api/login", csrf.verifyCsrfToken, function (req, res) {
       loginAction(req, res);
    });
    ...
    // SEC-CSRF-01: same cold-prime endpoint under the Vue console's /api/v2 base.
    app.get("/api/v2/csrf-token", function (req, res) {
        csrf.issueCsrfToken(req, res);
    });

    // Front-end authentication, returns session on valid authentication
    app.post("/api/v2/login", csrf.verifyCsrfToken, function (req, res) {
        loginAction(req, res);
    });
    ...
    // Vue console: exchange the httpOnly session cookie for a fresh in-memory
    // access token after a page reload (see lib/thinx/session_token.js).
    app.post("/api/v2/session/token", csrf.verifyCsrfToken, SessionToken.createHandler(app));
