...
const testEnvi = process.env.ENVIRONMENT === "test" ? require("../spec/_envi.json") : null;
...
module.exports = function (app) {
    ...
    function ensureTestUser(username, callback) {
        if (process.env.ENVIRONMENT !== "test" || !testEnvi || username !== testEnvi.dynamic.username) {
            callback(false);
            return;
        }
        ...
        const passwordHash = sha256(prefix + dynamicUser.username);

        const ensurePassword = () => {
            user.apply_update(dynamicUser.owner, "password", passwordHash, () => {
                callback(true);
            });
        };

        user.profile(dynamicUser.owner, (success) => {
            if (success) {
                ensurePassword();
                return;
            }

            user.create(dynamicUserBody, true, {}, (_res, createSuccess, response) => {
                ...
                console.log(`⚠️ [warning] Failed to prepare dynamic test user on login: ${response}`);
                callback(false);
            });
        });
    }
    ...
    function performTokenLogin(req, res, oauth) {
        ...
        redis.get(oauth, (error, userWrapper) => {
                ...
                userlib.get(owner_id, (gerr, doc) => {
                    ...
                    // is this setting correctly the initial token value?
    ...
    function loginAction(req, res) {
        ...
        user.validate(username, (db_body) => {
            ...
            if ((typeof (db_body.rows) === "undefined") || (db_body.rows.length == 0)) {
                return ensureTestUser(username, (prepared) => {
                    if (prepared) {
