...
module.exports = function (app) {
    ...
    function performTokenLogin(req, res, oauth) {
        ...
        redis.v4.get(oauth).then((userWrapper) => {
            ...
            console.log("Token Login Error", e); // should return error!
        });
    }
    ...
    function newTokenWithUserData(user_data) {
        ...
        redis.v4.set(token, JSON.stringify(user_data)); // data copy, should expire soon or be deleted explicitly after use
        redis.v4.expire(token, 60);
        return token;
    }
