...
module.exports = function (app) {
    ...
    // AUTH-API-01 (b): respond 200 with an IDENTICAL envelope for both registered and
    // unregistered emails - no enumeration via status code OR response body. Production
    // path always returns {success: true, response: "password_reset_request_accepted"}
    // regardless of password_reset_init's internal result. Test env preserves the underlying
    // message (incl. reset_key from owner.js:165-167) so the spec round-trip at
    // ZZ-AppSessionUserSpec.js:191-198 can chain through GET /password/reset?reset_key=.
    function postPasswordReset(req, res) {
        // AUTH-RESET-ORIGIN: both consoles share the host, so the initiating console
        // tags the request with a `client` marker (Vue console sends "vue"). Stored as
        // user.reset_console and used by the GET handler to pick the redirect path.
        const client = (req.body && req.body.client) || (req.query && req.query.client);
        user.password_reset_init(req.body.email, client, (success, message) => {
            console.log("🔨 [debug] password_reset_init", success, message);
            if (process.env.ENVIRONMENT === "test") {
                return Util.responder(res, success, message);
            }
            Util.responder(res, true, "password_reset_request_accepted");
        });
    }
    ...
    function postChat(req, res) {
        ...
        app.messenger.slack(owner, message, function (err, response) {
            if (err) {
                ...
                return Util.failureResponse(res, 400, "slack_error");
            } else {
    ...
    app.post("/api/v2/password/set", csrf.verifyCsrfToken, function (req, res) {
    ...
    app.post("/api/user/create", csrf.verifyCsrfToken, function (req, res) {
        createUser(req, res);
    });
    ...
    app.post("/api/user/password/set", csrf.verifyCsrfToken, function (req, res) {
        postPasswordSet(req, res);
    });
    ...
    app.post("/api/user/password/reset", csrf.verifyCsrfToken, function (req, res) {
        postPasswordReset(req, res);
    });
