...
module.exports = function (app) {
    ...
    app.get("/api/slack/redirect", (req, xres) => {
        ...
        //TODO: validate code before use to prevent unintended API calls in tests
        ...
        var areq = https.get(options, function (res) {
            ...
            res.on('data', function (chunk) {
                ...
                try {
                    ...
                    if (!auth_data.ok) {
                        console.log("[warning] Slack OAuth login failed", { auth_data });
                        return;
                    }
                    var token = auth_data.bot_access_token;
                    if (typeof (token) !== "undefined") {
                        app.redis_client.set("__SLACK_BOT_TOKEN__", token); // used by messenger.js when user enables slack integration
                        console.log(`ℹ️ [info] Saving new Bot token ...${token.slice(-4)}`);
                    }
