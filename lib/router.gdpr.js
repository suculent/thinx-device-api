    ...
    function revokeGDPR(req, res) {
        ...
        userlib.get(owner_id, function (error, udoc) {
            ...
            devices.list(owner_id, (dsuccess, devicez) => {
                ...
                console.log("Deleting devices " + JSON.stringify(devicez));
                devicez.response.forEach(() => {
                    devices.revoke(owner_id, req.body, function (rsuccess, status) {
                       console.log("🔨 [debug] this error is ignored on device revocation while deleting GDPR:", rsuccess, status);
                    });
                });
            });
