...
const OwnerPurge = require("./thinx/owner_purge");
...
module.exports = function (app) {
    ...
    var devices = new Devices(app.messenger, app.redis_client);
    ...
    function revokeGDPR(req, res) {
        ...
        userlib.get(owner_id, function (error) {
            if (error) {
            ...
            // GDPR #353 / SEC-PII-03: single orchestrator removes every
            // owner-scoped artifact (devices, builds, RSA keys, deploy + repo
            // filesystem trees, Redis keys deleted not expired) and destroys the
            // user document LAST. Path-guarded + idempotent.
            const purger = new OwnerPurge(redis_client, devices);
            purger.purge(owner_id, function (success, report) {
                console.log("[gdpr] purge report for " + owner_id + ": " + JSON.stringify(report.steps));
                if (success) {
                    Util.responder(res, true, "Your personal data has been deleted from all systems.");
                } else {
                    Util.responder(res, false, "Your data deletion failed. Personal data may NOT have been deleted successfully. Please contact THiNX data processor in order to fix this GDPR issue for you.");
                }
            });
        });
    }
        ...
        userlib.get(owner_id, function (error, udoc) {
            ...
            devices.list(owner_id, function (dsuccess, ddevices) {
                apienv.list(owner_id, function (esuccess, envs) {
                    Util.respond(res,
                        {
                            success: (!error && dsuccess && esuccess) ? true : false,
                            user_data: udoc,
    ...
    function setGDPR(req, res) {
        ...
        if (typeof (gdpr_consent) === "undefined" || gdpr_consent === null) {
            return Util.failureResponse(res, 400, "consent_missing");
        }

        if (typeof (token) === "undefined" || token === null) {
            console.log("🔨 [debug] request is missing token");
            return Util.failureResponse(res, 401, "token_missing");
        }
