...
module.exports = function (app) {
  ...
  function transferResultRedirect(success, res, response) {
    ...
    /*
    // causes `Headers already set` issue...
  ...
  async function requestTransfer(req, res) {
    if (!Util.validateSession(req)) return Promise.resolve(res.status(401).end());
    let owner = sanitka.owner(req.session.owner);
    
    let response = await transfer.request(owner, req.body).catch( (error) => {
      console.log("[transferResultRedirect] in error", error);
      return Promise.resolve(transferResultRedirect(false, res, error));
    });

    console.log("[transferResultRedirect] with response", response);
    return Promise.resolve(transferResultRedirect(true, res, response));
  }
