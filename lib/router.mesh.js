...
function deleteMesh(req, res) {
    ...
    if (owner_id == null) {
        if (Util.isDefined(req.session.owner_id)) {
    ...
    if (!Util.isDefined(mesh_ids)) return Util.responder(res, false, "Parameter mesh_ids missing in request body.");
...
function createMesh(req, res) {
    ...
    if (!Util.validateSession(req)) return res.status(401).end();
    if (!Util.isDefined(req.body)) return Util.respond(res, { success: false, status: "body_missing" });

    let owner_id = sanitka.owner(req.body.owner_id);
    if (owner_id == null) {
        if (!Util.isDefined(req.session.owner_id)) return Util.responder(res, false, "owner_invalid");
        owner_id = req.session.owner_id;
    }

    if (!Util.isDefined(req.body.mesh_id)) return Util.respond(res, { success: false, status: "mesh_id_missing" });
