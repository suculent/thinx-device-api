...
function deleteMesh(req, res) {
    ...
    if (!Util.isDefined(req.body)) return Util.respond(res, { success: false, response: "Body missing." });

    if (typeof (req.body) !== "object") return Util.respond(res, { success: false, response: "Invalid request format." });
...
function createMesh(req, res) {
    ...
    if (!Util.validateSession(req)) return res.status(401).end();
    if (!Util.isDefined(req.body)) return Util.respond(res, { success: false, response: "body_missing" });
    ...
    if (!Util.isDefined(req.body.mesh_id)) return Util.respond(res, { success: false, response: "mesh_id_missing" });
    ...
    user.createMesh(owner_id, mesh_id, mesh_alias, (success, response) => {
        if (!success) return Util.respond(res, { success: success, response: "mesh_create_failed" });
