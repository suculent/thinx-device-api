...
module.exports = function (app) {
    ...
    app.post("/api/mesh/delete", function (req, res) {
        ...
        if (typeof (req.body) === "undefined") {
            return Util.respond(res, { success: false, status: "Body missing." });
        }

        if (typeof (req.body) !== "object") {
            return Util.respond(res, { success: false, status: "Invalid request format." });
        }
        ...
        user.deleteMeshes(owner_id, mesh_ids, function (success, status) {
            Util.respond(res, { success: success, status: status });
        });
