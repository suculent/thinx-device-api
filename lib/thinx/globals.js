    ...
    prefix: function() {
      ...
      try {
        ...
        if (fs.existsSync(pfx_path)) {
          _prefix = (fs.readFileSync(pfx_path).toString()).replace("\n", "");
          if (_prefix.length > 0) {
            // console.log("returning early prefix: "+_prefix);
            return _prefix; // this is the usual code-path now, rest is mess or workaround...
          }
        } else {
          // create .thx_prefix with random key on first run!
        ...
        console.log("[index] thx_prefix_exception (2) " + e);
      }
