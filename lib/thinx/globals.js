...
var Globals = (function() {
  ...
  var _public = {
    ...
    load: function() {
      ...
      if ((typeof(process.env.CIRCLE_USERNAME) !== "undefined") ||
        ...
        console.log("[globals.js] Starting server in Circle CI environment...");
        ...
        if (fs.existsSync(CONFIG_ROOT + '/google-oauth-test.json')) {
          ...
          console.log("[globals.js] Skipping Google OAuth, configuration not found...");
        }

        if (fs.existsSync(CONFIG_ROOT + '/github-oauth-test.json')) {
          _github_ocfg = require(CONFIG_ROOT + '/github-oauth-test.json');
        } else {
          console.log("[globals.js] Skipping GitHub OAuth, configuration not found...");
        }
    ...
    load_or_create_prefix: function() {
      ...
      if (!fs.existsSync(pfx_path)) {
        pfx_path = this.app_config.data_root + '/.thx_prefix'; // new
        if (!fs.existsSync(pfx_path)) {
          _prefix = null;
        }
      }
    ...
    generate_prefix: function() {
      ...
      // new implementation
      let pfx_path = this.app_config.data_root + '/.thx_prefix';
    ...
    prefix: function() {
      ...
      if (!fs.existsSync(pfx_path)) {
        pfx_path = this.app_config.data_root + '/.thx_prefix';
        if (!fs.existsSync(pfx_path)) {
          _prefix = null;
        }
      }
  ...
  console.log("Loading module: Globals");
