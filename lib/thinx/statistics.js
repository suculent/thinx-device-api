...
module.exports = class Statistics {
	...
	*/
	...
	aggregate(callback) {
		...
		let swarm_id = exec.execSync("docker info -f \'{{.Swarm.NodeID}}\'");
		...
		if (swarm_id.length < 8) {
			...
			if (container_id.length > 4) {
				LOGDATA += exec.execSync("docker logs --timestamp " + container_id);
			}
		} else {
			// swarm
			console.log("» Running Swarm Service Logs Aggregator...");
			LOGDATA += exec.execSync("docker service logs " + swarm_id);
		}
