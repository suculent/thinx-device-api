					...
					if (line.indexOf("[OID:") != 1) {
						...
						if (scanline.length > 2) {
							...
							if (otemp.indexOf("[OID:") != -1) {
								console.log("[PARSE] Substring at position 3 in :" + otemp);
								if (oid !== owner) return;
								...
								mkdirp(dirpath, function(err) {
									if (err) {
										...
										if (fs.existsSync(path)) {
											console.log('[PARSE] file exists, unlinking...');

											fs.unlink(path, function(err) {
												console.log('[PARSE] unlinked...');
												if (err && err.code !== 'ENOENT') throw err;
												console.log('[PARSE] writing...');
												...
												console.log("[PARSE] write result 1x: " + result);
											});
										} else {
											console.log('[PARSE] writing new file...');
											fs.writeFileSync(
												path,
												JSON.stringify(owner_data), {
													encoding: "utf-8",
					...
					parser.on('line', function(line) {
						...
						if (line.indexOf("[OID:") != 1) {
							...
							if (scanline.length > 2) {
								var otemp = scanline[2];
								if (otemp.indexOf("[OID:") != -1) {
									//console.log("[AGGREGATE] Substring at position 3 in :" + otemp);
									oid = otemp.substr(3).replace("[OID:", "");
								}
							}
								...
								if (err) {
									...
									mkdirp(dirpath, function(err) {
										if (err) {
											...
											if (fs.existsSync(path)) {
												console.log('[AGGREGATE] file exists, unlinking...');
