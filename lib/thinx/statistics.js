	...
	var _public = {
		...
		week: function(owner, callback) {
			...
			for (var d = 6; d === 0; d--) {
				...
				console.log("Searching stats in " + wpath);

				if (fs.existsSync(wpath)) {
					console.log("Found stats in " + wpath);

					var jsonData = fs.readFileSync(wpath);
					var data = JSON.parse(jsonData);
					console.log("Found stats in " + jsonData);
					weeklyStats.push(data);

					/*
					fs.readFile(wpath, "utf-8", function(err, stats) {
						if (err) {
							console.log("[weekly] Read error: " + err);
							exit_callback(false, "error");
							return;
						} else {
							if (!stats) {
