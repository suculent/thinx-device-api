...
var Statistics = (function() {
	...
	var _public = {
		...
		today: function(owner, exit_callback) {
			...
			console.log("today path:" + path);

			var parse_callback = function(path, exit_callback) {
				console.log("[parse_callback] Checking for file at: " + path);
				if (fs.existsSync(path)) {
					fs.readFile(path, "utf-8", function(err, statistics) {
						...
						if (!statistics) {
							...
							console.log("[parse_callback] Readfile: " + statistics);
			...
			if (!fs.existsSync(path)) {
				...
				parse_callback(path, parse_callback);
				return;
			}
		},
