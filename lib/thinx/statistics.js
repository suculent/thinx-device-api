...
module.exports = class Statistics {
	...
	write(path_with_filename, owner_data) {
		...
		let path_without_filename = path_without_filename_arr.join("/");
		mkdirp(path_without_filename); // fix for no such file or directory https://rollbar.com/keyguru/thinxguru/items/59/
		try {
			fs.unlinkSync(path_with_filename);
		} catch (unlinkError) {
			console.log({unlinkError}); // exception should be ignored, delete logging after test
		}
		try {
			fs.writeFileSync(
				path_with_filename,
