	...
	var _public = {
			...
			fs.readFile(LOG_PATH, 'utf8', function(err, data) {
				...
				parser.on('line', function(line) {
					...
					for (var owner_id in owners) {
						...
						var path = _private.ownerPath(owner_data.owner) +
							_private.todayPathElement() + "stats.json";
						console.log("Should save stats to path: " + path);
						mkdirp(path);
						var res = fs.writeFileSync(path, owner_stats);
						console.log(res);
					}
		...
		today: function(owner, callback) {
			...
			var path = _private.ownerPath(owner) +
				_private.todayPathElement() + "stats.json";
			var statistics = fs.readFileSync(path);
