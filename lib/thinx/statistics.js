...
module.exports = class Statistics {
	...
	*/
	...
	parse_oid(line) {
		var scanline = line.split("]");
		console.log("[debug] parse scanline", JSON.stringify(scanline));
		var oid = null;
		if (scanline.length > 2) {
			...
			if (oid === null) {
				otemp = scanline[3];
				if ((typeof(otemp) !== "undefined") && (otemp.indexOf("[OID:") !== -1)) {
					oid = sanitka.owner(otemp.substr(3).replace("[OID:", ""));
				}
			}
		}
	...
	write_stats(dirpath, filepath, owner_data) {
		...
		if (fs.existsSync(filepath)) {
			console.log("[debug] WARNING! ¨overwriting stats data", fs.readFileSync(filepath));
		}
