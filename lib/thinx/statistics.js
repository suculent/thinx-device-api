							...
							if (err) {
								...
								mkdirp(dirpath, function(err) {
									if (err) {
										...
										console.log("Attempt to write file:" + path);
										...
										if (fs.existsSync(path)) {
											fs.unlink(path, function(err) {
												...
												console.log("write result 1x: " + result);
											});
										} else {
											var result = fs.writeFileSync(
												path,
											...
											console.log("write result 1n: " + result);
										}
									}
								});
							}
								...
								if (err) {
									...
									mkdirp(dirpath, function(err) {
										if (err) {
											...
											// TODO: FIXME: Create or append file? Overwrite?
											console.log('before unlink2: ');

											// aargh prevent exception on unlinking non-existent file
											if (fs.existsSync(path)) {
												fs.unlink(path, function(err) {
