...
module.exports = class Statistics {
	...
	parse_oid_date_and_line(line) {
		...
		// NOTE: Was planned to use this to parse date for splitting day to hours; but it would be better to use Chronograf aggregator instead
	...
	parse_callback(path) {
		...
		if (fs.existsSync(path)) {
			fs.readFile(path, "utf8", (err, statistics) => {
				if (err) {
	...
	parse(owner, completed_callback) {
		...
		fs.copy(this.LOG_PATH, this.TEMP_PATH, (err) => {
			...
			if (typeof (this.parser) === "undefined") {
				console.log("☣️ [error] [parser] could not be instantiated in PARSE!");
		...
		this.extract_docker_logs(() => {
			...
			fs.copy(this.LOG_PATH, this.TEMP_PATH, (err) => {
				...
				if ((typeof (this.parser) === "undefined") || (this.parser === null)) {
					console.log("☣️ [error] [parser] could not be instantiated in AGGREGATE!");
	...
	*/
	...
	today(owner, callback) {
		...
		if (!fs.existsSync(xpath)) {
			console.log("ℹ️  [info] [today] FIXME: Statistics not found...");
			if (this.once === false) {
				this.once = true;
				...
				if (typeof (callback) === "function") {
					...
					console.log("ℹ️  [info] [today] No today parser callback, exiting...");
					return;
				}
			}
		} else {
			if (typeof (callback) === "function") {
				...
				console.log("ℹ️  [info] [today] No stats available, exiting...");
				return;
			}
		}
	}
