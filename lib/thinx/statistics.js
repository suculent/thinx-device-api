...
var Statistics = (function() {
	...
	var _public = {
		...
		parse: function(owner, completed_callback) {
			...
			console.log("[PARSE] Let's get log party started..."); // FIXME
			...
			fs.copy(LOG_PATH, TEMP_PATH, function(err) {
				...
				parser.on('close', function(line) {
					...
					console.log("[PARSE] Stop the music and go home."); // FIXME
				});
			});
		},
		...
		week: function(owner, callback) {
			...
			console.log("Aggregating weekly statistics for " + owner); // FIXME
			...
			for (var d = 6; d > 0; d--) {
				...
				var wpath = _private.ownerPath(owner) + "/" + _public.weekPathElement(d) +
					".json";

				console.log("Searching stats in " + wpath); // FIXME

				if (fs.existsSync(wpath)) {
					atLeastOneFileFound = true;
					console.log("Found stats in " + wpath); // FIXME

					var jsonData = fs.readFileSync(wpath);
					console.log("Found stats in " + jsonData); // FIXME
					...
					for (var kindex in skeys) {
						...
						results[keyname].push(data[keyname]);
					}
				}
			}

			console.log("[weekly] stats: " + JSON.stringify(results)); // FIXME
