...
var Statistics = (function() {
	...
	function parse_callback(path) {
		...
		if (fs.existsSync(path)) {
			...
			fs.readFile(path, "utf-8", function(err, statistics) {
				if (err) {
					console.error("[parse_callback] Read error: " + err);
			...
			console.error("[parse_callback] Stats file not found at: " + path);
	...
	var _public = {
		...
		write_stats: function(err, path, dirpath, owner_data) {
			...
			if (err) {
				console.error("[WRITE] Write-stats error: " + err);
			} else {
				mkdirp(dirpath, function(err) {
					if (err) {
						console.error("[AGGREGATE] Write-stats mkdirp error: " +
							err);
					} else {
						if (fs.existsSync(path)) {
