...
module.exports = class Statistics {
	...
	write_stats(err, path, filepath, owner_data) {
		if (err) {
			...
			fs.copy(this.LOG_PATH, this.TEMP_PATH, (err) => {
				...
				this.parser.on("close", (line) => {
					...
					for (var owner_id in this.owners) {
						...
						var path = dirpath + "/" + dateFormat(new Date(), "isoDate") + ".json";
					...
					if (this._owner !== null) {
						this.path = this.STATS_PATH + this._owner + "/" + dateFormat(new Date(), "isoDate") + ".json";
					}
