...
const app_config = Globals.app_config(); // for data_root
...
function parse_line(owner, line) {
	...
	// Increment the first matching event counter and stop, preserving the
	// original if/else-chain semantics (a line is attributed to a single event).
	const event_names = EventTaxonomy.names();
	for (let index = 0; index < event_names.length; index++) {
		const name = event_names[index];
...
module.exports = class Statistics {
	...
	constructor() {
		...
		if (!fs.existsSync(this.STATS_PATH)) {
			logger.info(`Stats path ${this.STATS_PATH} not found, creating...`);
	...
	static get_owner_template() {
		return EventTaxonomy.templateModel();
	}
	...
	*/
	...
	closeParsers() {
		for (var owner_id in this.owners) {
			...
			logger.info(`[closeParsers] writing stats to '${path}'`);
			var mkresult = fs.mkdirpSync(dirpath);
			if (mkresult) {
				logger.error(`[closeParsers] error creating statistic path for owner: ${mkresult}`);
			} else {
				try {
					this.write_stats(dirpath, path, owner_data);
				} catch (e) {
					logger.error(`[closeParsers] Error saving stats: ${e}`);
				}
			}
	...
	parse_oid(line) {
		...
		// Guards for legacy log lines (pre-Winston format used lowercase "[info]", "[warning]").
		// New Winston lines are filtered at source: the file transport only writes warn+.
		...
		const oid = Sanitka.owner(match[1]);

		// get_all_owners() already seeds one template per known owner, so the
		// isDefined() check is an allowlist: OIDs absent from the user DB are
		// ignored. Do NOT re-seed here -- parse_line() increments counters, and
		// re-templating per line threw away everything counted so far, leaving
		// each owner with only the last matching line of the whole log.
	...
	write(path_with_filename, owner_data) {
		...
		try {
			...
			logger.error("statsWriteError", { error: statsWriteError });
		}
	}
	...
	write_stats(dirpath, filepath, owner_data) {
		try {
			...
			logger.error("mkdirp(dirpath) failed", { error: d });
		}
	...
	*/
	...
	parse(owner, completed_callback) {
		...
		if (!fs.existsSync(this.LOG_PATH)) {
			logger.warn(`[PARSE] THiNX log not found at: ${this.LOG_PATH}`);
	...
	extract_docker_logs(callback) {
		...
		shell.on("exit", (code) => {
			if (code > 0) {
				logger.warn(`[stats] [dockerlog] result code non-null: ${code}`);
			}
			callback();
		}); // end shell on exit
	}
	...
	aggregate(callback) {
		...
		this.extract_docker_logs(() => {
			...
			if (!fs.existsSync(this.LOG_PATH)) {
				logger.warn(`[AGGREGATE] THiNX log not found at: ${this.LOG_PATH}`);
			...
			fs.copy(this.LOG_PATH, this.TEMP_PATH, (err) => {
				...
				this.parser.on("close", (/*line*/) => {
					...
					for (var owner_id in this.owners) {
						...
						logger.info(`aggregate parser closed: Writing stats '${filepath}'`);
	...
	*/
	...
	today(owner, callback) {
		...
		if (typeof (callback) !== "undefined") {
			logger.info("[today] Setting new exit_callback in today()...");
		...
		logger.debug(`[today] Looking for file at xpath ${xpath}`);
		this.path = xpath; // WTF? Side effect?

		if (!fs.existsSync(xpath)) {
			logger.info(`[today] Statistics not found on ${xpath}`);
			if (this.once === false) {
				this.once = true;
				this.aggregate(() => {
				...
				logger.info(`[today] [nonce] Stats not found in ${xpath}`);
