...
var Statistics = (function() {
		...
		aggregate: function() {
			...
			fs.readFile(LOG_PATH, 'utf8', function(err, data) {
				...
				parser.on('line', function(line) {
					...
					// Counts all events per OID per day..
					...
					if (line.indexOf("[OID:") != 1) {
						...
						// console.log("SCANNING LINE: " + line);
						// console.log(JSON.stringify(scanline));
						// console.log("SUBSTR OID:" + oid);
						...
						//var oids = Object.keys(owners);
						//console.log("oids: " + JSON.stringify(oids));

						var write_stats = function(err, path) {
							if (err) console.error(err);
							else {
								fs.writeFileSync(path + "/stats.json", JSON.stringify(
									owner_data));
							}
						};
	...
	// TEST ONLY - remove on reployment!
	// _public.aggregate();
