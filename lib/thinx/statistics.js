...
module.exports = class Statistics {
	...
	closeParsers() {
		for (var owner_id in this.owners) {
			...
			console.log("[info] [closeParsers] parser closed: Writing stats to path '" + path + "'");
			var mkresult = fs.mkdirpSync(dirpath);
			if (mkresult) {
				console.log("[error] ☣️ [closeParsers] error creating statistic path for owner: " + mkresult);
			} else {
				try {
					this.write_stats(dirpath, path, owner_data);
				} catch (e) {
					console.log("[error] ☣️ [closeParsers] Error saving stats: " + e);
				}
			}
			if (typeof (this.exit_callback) === "function") {
				console.log("[debug] calling exit_callback");
		...
		if (fs.existsSync(this.TEMP_PATH)) {
			try {
				...
				console.log("[debug] unlinking %s failed", this.TEMP_PATH);
			}
		}
	}
