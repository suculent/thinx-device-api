...
module.exports = class Statistics {
	...
	constructor() {
		...
		// The logs could be polled in swarm mode... maybe even in docker mode... but LOG_PATH is then not a file, or could be?
		...
		if (!fs.existsSync(this.STATS_PATH)) {
			console.log("Creating stats path " + this.STATS_PATH + " from " + __dirname);
			mkdirp(this.STATS_PATH.replace(".json", ""));
		}
	...
	write(path_with_filename, owner_data) {
		...
		mkdirp(path_without_filename); // fix for no such file or directory https://rollbar.com/keyguru/thinxguru/items/59/
		try {
			fs.writeFileSync(
				path_with_filename,
	...
	*/
	...
	aggregate(callback) {
		...
		let swarm_id = fs.execSync("docker info -f \'{{.Swarm.NodeID}}\'");
		console.log({swarm_id});

		let LOGDATA = "";
		if (swarm_id.length < 8) {
			// non-swarm
			console.log("» Running Docker Container Logs Aggregator...");
			let container_id = fs.execSync("docker info -f $(hostname)");
			if (container_id.length > 4) {
				LOGDATA += fs.execSync("docker logs --timestamp " + container_id);
			}
		} else {
			// swarm
			console.log("» Running Swarm Service Logs Aggregator...");
			LOGDATA += fs.execSync("docker service logs " + swarm_id);
		}

		if (LOGDATA.length > 8) {
			fs.writeFileSync(this.LOG_PATH, { mode: 0x776 });
		} else {
