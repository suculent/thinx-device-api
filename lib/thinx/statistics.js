...
var Statistics = (function() {
  ...
  var app_config = Globals.app_config();
	...
	var LOG_PATH = ROOT + "/../.pm2/logs/thinx-out.log";
	...
	if (!fs.existsSync(LOG_PATH)) {
		LOG_PATH = ROOT + "/../.pm2/logs/index-out-0.log";
	}

	if (!fs.existsSync(LOG_PATH)) {
		LOG_PATH = ROOT + "/../.pm2/logs/index-out-1.log";
	}
	...
	function parse_oid_date_and_line(line) {
	...
	var _public = {
		...
		write_stats: function(err, path, dirpath, owner_data) {
			...
			if (err) {
				...
				console.log("Writing stats to dirpath '"+dirpath+"'");
		...
		today: function(owner, callback) {
			...
			if (!fs.existsSync(path)) {
				...
				var today = true;
				_public.parse(owner, today, function(){ parse_callback(path); });
			}
		},
		...
		week: function(owner, callback) {
			...
			for (var d = 6; d >= 0; d--) {
				...
				var wpath = _private.ownerPath(owner) + "/" + _public.weekPathElement(d) + ".json";
