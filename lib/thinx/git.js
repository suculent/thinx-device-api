...
module.exports = class Git {
	...
	// noexec, in which case ssh-add cannot exec the helper at all and silently
	// loads no key.
	askpath(keypath) {
		return nodePath.join(os.tmpdir(), "thinx-askpass-" + nodePath.basename(keypath) + ".sh");
	}

	// Reads the passphrase from the environment it is exec'd with rather than
	// baking it into the file. A crashed fetch leaves this helper behind, and a
	// copy carrying the passphrase in cleartext is precisely what the stale
	// ssh_keys/askpass.sh (`echo "thinx"`) was.
	create_askfile(keypath) {
		let path = this.askpath(keypath);
		let contents = `#!/bin/sh\nprintf '%s\\n' "$GIT_KEY_PASSPHRASE"\n`;
		fs.writeFileSync(path, contents, { mode: 0o700 });
		fs.chmodSync(path, 0o700); // writeFileSync's mode is ignored if it already exists
	}
    ...
    fetch(owner, command, local_path) {
		...
		for (var kindex in key_paths) {
			...
			// SSH_ASKPASS_REQUIRE=force is the supported way to make ssh-add
			// consult the helper (OpenSSH >= 8.4); without it the DISPLAY=: hack
			// only works when ssh-add happens to find no controlling terminal.
			// These assignments prefix ssh-add only, so the clone that follows
			// is unaffected.
			const script = `DISPLAY=: SSH_ASKPASS_REQUIRE=force ` +
				`SSH_ASKPASS=${shellEscape([askpath])} GIT_ASKPASS=${shellEscape([askpath])} ` +
				`ssh-add ${shellEscape([keypath])} >/dev/null 2>&1; ${command}`;
			let prefixed_command = `ssh-agent sh -c ${shellEscape([script])} 2>&1`;

			this.create_askfile(keypath); // reads GIT_KEY_PASSPHRASE from the inherited environment
