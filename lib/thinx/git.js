...
module.exports = class Git {
    ...
    fetch(owner, command, local_path) {
		...
		for (var kindex in key_paths) {
			...
			try {
				result = exec.execSync(cmd, {stdio: 'inherit'});
				...
				if (typeof(local_path) !== "undefined") {
					success = fs.existsSync(local_path + "/*"); // may throw! but does not work.
					console.log("Fetch successful? : " + success, "in path", local_path);
				}
				if (success == true) {
					return success;
				}
