...
module.exports = class Git {
	...
	create_askfile(keypath, owner) {
		// let password = owner.keypass
		let path = this.askpath(keypath);
		fs.writeFileSync(path, "thinx"); // TODO: owner's keypass
		fs.chmodSync(path, 0o700);
		console.log("Creating askfile for ", owner);
	}

	delete_askfile(keypath, owner) {
    ...
    fetch(owner, command, local_path) {
		...
		if ((typeof(key_paths) === "undefined") || (key_paths.length < 1)) {
			...
			for (var kindex in key_paths) {
				...
				var gfpfx = `ssh-agent sh -c 'export SSH_ASKPASS=${askpath}; export GIT_ASKPASS=${askpath}; ssh-add ${keypath}; `;
				let prefixed_command = gfpfx + command + "' 2>&1";
				console.log("[git fetch] trying command:", prefixed_command); // REMOVE THIS!
				this.create_askfile(keypath, owner);
				success = this.tryShellOp(prefixed_command, local_path);
				this.delete_askfile(keypath, owner);
