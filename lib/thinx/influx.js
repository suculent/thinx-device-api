...
module.exports = class InfluxConnector {
    ...
    writePoint(point, callback) {
        ...
        point.measurement.timestamp = new Date();
    ...
    async today(owner_id, callback) {
        ...
        for (let measurement in InfluxConnector.measurements()) {
            results[measurement] = await this.influxdb.query(`SELECT mean("value") AS "mean_value" FROM "stats"."autogen"."${measurement}" WHERE "owner_id"='${owner_id} AND time > ${midnight}'`, {
                placeholders: {
                    owner_id: "placeholder"
                }
            }).catch(e => {
        ...
        callback(true, Promise.all(results));
    }
    ...
    async week(owner_id, callback) {
        ...
        for (let measurement in InfluxConnector.measurements()) {
            results[measurement] = await this.influxdb.query(`SELECT mean("value") AS "mean_value" FROM "stats"."autogen"."${measurement}" WHERE "owner_id"='${owner_id} AND time > ${week_ago}'`, {
                placeholders: {
                    owner_id: "placeholder"
                }
            }).catch(e => {
