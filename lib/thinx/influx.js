...
module.exports = class InfluxConnector {
    ...
    async today(owner_id, callback) {
        ...
        let measurements = InfluxConnector.measurements();
        for (let measurement in measurements) {
            // this should be actually array of results
            results[measurements[measurement]] = 0 || await this.influxdb.query(`SELECT mean("value") AS "mean_value" FROM "stats"."autogen"."${measurement}" WHERE "owner_id"='${owner_id} AND time > ${midnight}'`, {
                placeholders: {
                    owner_id: "placeholder"
                }
            }).catch(e => {
    ...
    async week(owner_id, callback) {
        ...
        for (let measurement in kpis) {
            ...
            // this should be actually array of results
            results[kpi] = 0 || await this.influxdb.query(`SELECT mean("value") AS "mean_value" FROM "stats"."autogen"."${kpi}" WHERE "owner_id"='${owner_id} AND time > ${week_ago}'`, {
                placeholders: {
                    owner_id: "placeholder"
                }
            }).catch((e) => {
