...
module.exports = class Transfer {
	...
	migrate_api_keys(original_owner, recipient, xudid, callback) {
		...
		this.devices.get(xudid, (success, dev) => {
			...
			// Get source keys
			this.client.v4.get(source_id).then((json_keys) => {
				...
				// Get recipient keys
				this.client.v4.get(recipient_id).then((recipient_keys) => {
					...
					this.client.v4.set(source_id, JSON.stringify(json_array));
					// Save new array with respective API Key added
					this.client.v4.set(recipient_id, JSON.stringify(recipient_array));
	...
	exit_on_transfer(udid, result_callback) {
		...
		this.client.v4.get("dtr:" + udid).then((reply) => {
			if ((reply === null) || (reply == [])) {
	...
	store_pending_transfer(udid, transfer_id) {
		this.client.v4.set("dtr:" + udid, transfer_id);
		this.client.v4.expire("dtr:" + udid, 86400); // expire pending transfer in one day...
	}
	...
	request(owner, body, callback) {
		...
		userlib.get(owner, (couch_err, ownerdoc) => {
			...
			userlib.get(recipient_id, (zerr/* , recipient */) => {
				...
				this.client.v4.set(transfer_id, JSON.stringify(body));
	...
	save_dtid(tid, keys, ac) {
		this.client.v4.set(tid, JSON.stringify(keys));
		console.log(`🔨 [debug] [transfer] Accepted udids ${keys.udids}`);
		if (keys.udids.length > 1) {
			ac(true, "transfer_partially_completed");
			this.client.v4.expire(tid, 3600); // 3600 seconds expiration for this transfer request; should be possibly more (like 72h to pass weekends)
		} else {
			ac(true, "transfer_completed");
			this.client.v4.del(tid);
		}
	}
	...
	async accept(body, accept_callback) {
		...
		this.client.v4.get(dtid).then((encoded_json_keys) => {
	...
	storeRemainingKeys(dtid, json_keys, callback) {
		this.client.v4.set(dtid, JSON.stringify(json_keys));
		console.log(`🔨 [debug] [transgfer] L4 Storing remaining keys: ${json_keys.udids}`);
		if (json_keys.udids.length > 1) {
			this.client.v4.expire(dtid, 3600); // 1 hour to let user accept/decline different devices
	...
	decline(body, callback) {
		...
		this.client.v4.get(dtid).then((json) => {
