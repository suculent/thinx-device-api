...
module.exports = class Transfer {
	...
	async is_pending(udid) {
		...
		try {
			...
			console.log("[debug] [is_pending] data:", {data});
	...
	request(owner, body, callback) {
		...
		// check whether those devices are not transferred already
		for (const udid in body.udids) {
			let state = this.is_pending(udid);
			if (state) {
		...
		userlib.get(owner, (couch_err, ownerdoc) => {
			...
			userlib.get(recipient_id, (zerr/* , recipient */) => {
				...
				if (typeof (udids) === "undefined") {
					...
					return callback(false, "udids_missing");
				}
