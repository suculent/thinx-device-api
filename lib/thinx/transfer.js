...
module.exports = class Transfer {
  ...
  migrate_device(original_owner, xudid, recipient, body, json_keys, callback) {
		...
		device.edit(original_owner, changes, (error, xchanges) => {
			console.log("🔨 [debug] [transfer] DEC", {error}, {xchanges});
		});
		...
		if (fs.existsSync(original_path)) {
			...
			console.log("Warning, original device path does not exist.");
		}
	...
	rename(from, to) {
		fs.rename(from, to, (err) => {
			if (err) {
				console.log("[transfer] caught rename error:", err);
			}
		});
	}
	...
	exit_on_transfer(udid, result_callback) {
		this.client.get("dtr:" + udid, (err, reply) => {
			if (!err && (reply === null)) {
				console.log("exit_on_transfer reply", {reply});
				console.log("Device already being transferred:", udid);
			...
			userlib.get(owner, (err, ownerdoc) => {
				...
				userlib.get(recipient_id, (zerr/* , recipient */) => {
					...
					if (zerr) {
						...
						console.log("Transfer target " + body.to + " id " + recipient_id + "not found?");
						return;
					}
					...
					this.client.set(transfer_id, JSON.stringify(body), (werr/* , ignored_result */) => {
						...
						if (werr) {
							console.log("Transfer redis save error: " + werr);
						...
						console.log(`[info] Sending transfer e-mail to ${recipientTransferEmail.from}`);

						mg.messages.create(app_config.mailgun.domain, recipientTransferEmail)
							.then((/* msg */) => {
								console.log("[info] Recipient transfer e-mail sent.");
							}) // logs response data
							.catch(merr => {
								console.log("[error] mail error:", merr);
							}); // logs any error
						...
						console.log("Sending transfer e-mail to sender: " + JSON.stringify(senderTransferEmail));
						...
						mg.messages.create(app_config.mailgun.domain, senderTransferEmail)
							.then((/* msg */) => {
								callback(true, "email_sent");
							}) // logs response data
							.catch(yerr => {
								console.log("yerr", yerr);
								if (process.env.ENVIRONMENT === "test") {
									// ignored in test
									callback(true, "test");
								} else {
	...
	accept(body, accept_callback) {
		...
		this.client.get(dtid, (err, encoded_json_keys) => {
			...
			console.log("dtid fetch results", {err}, {encoded_json_keys});

			if (err) {
				console.log("[transfer] transfer_id not found.", err);
			...
			console.log("L2 LEFT keys:", json_keys.udids);
			...
			for (var dindex in udids) {
				this.migrate_device(original_owner, udids[dindex], recipient, body, json_keys, () => {
					if (dindex != udids.length - 1) return;
					/* last callback */
					// Store remaining (not accepted) keys
					this.client.set(dtid, JSON.stringify(json_keys), (xerr, response) => {
						if (xerr) {
							console.log("accept set error: ", xerr, response);
							accept_callback(false, xerr);
						} else {
							console.log("L3 ACCEPTED keys:", json_keys.udids);
	...
	storeRemainingKeys(dtid, json_keys, callback) {
		this.client.set(dtid, JSON.stringify(json_keys), (err/* , response */) => {
			...
			console.log("L4 Storing remaining keys:", json_keys.udids);
			if (json_keys.udids.length > 1) {
	...
	decline(body, callback) {
		...
		this.client.get(dtid, (err, json) => {
			...
			// Check if there are some devices left
			console.log("L6", json_keys.udids);
