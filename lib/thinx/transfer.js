...
*/
			...
			client.get(dtid, function(err, json_keys) {
				...
				console.log("[transfer]Searching 'to' recipient in: " + JSON.stringify(json_keys));
				...
				function rename(from, to) {
					fs.rename(original_path, destination_path, function(err) {
						if (err) throw err;
						console.log("[transfer] Device builds artefacts transferred.");
					});
				}
				...
				for (var dindex in udids) {
					...
					if (body.mig_sources === true) {
						console.log(
							"[transfer] TODO #THX-396: Migrate sources for those devices from original owner to recipient!"
						);
					...
					if (body.api_keys === true) {
						console.log(
							"[transfer] TODO #THX-396: Migrate API Keys from original owner to recipient in Redis!"
						);
					}
