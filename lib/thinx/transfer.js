...
var Devices = require("./devices"); var devices = new Devices();
...
module.exports = class Transfer {
  ...
  migrate_device(original_owner, xudid, recipient) {
		...
		if (body.mig_sources === true) {
			...
			this.move_source(usid, original_owner, recipient);
		...
		if (body.api_keys === true) {
			...
			this.migrate_api_keys(original_owner, recipient, xudid);
		} // end device fetch
	}
	...
	accept(body, callback) {
		...
		this.client.get(dtid, (err, encoded_json_keys) => {
			...
			if (encoded_json_keys === null) {
				callback(false, "transfer_id_not_found");
				return;
			}
			...
			if (!this.transfer_valid(encoded_json_keys, dtid, callback)) return;
			...
			for (var dindex in udids) {
				this.migrate_device(original_owner, udids[dindex], recipient);
			}
