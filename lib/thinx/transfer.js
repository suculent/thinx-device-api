...
var Transfer = (function() {
  ...
  var prefix = Globals.prefix();
	...
	function transfer_valid(encoded_json_keys, callback) {
  ...
  function migrate_device(original_owner, xudid, recipient) {
		...
		devices.get(xudid, function(success, dev) {
			...
			client.get(source_id, function(err1, json_keys) {
				...
				client.get(recipient_id, function(err2, recipient_keys) {
					...
					client.set(source_id, JSON.stringify(json_array), function(err3, removal_response) {
						if (err3) callback(false, err3);
						// Save new array with respective API Key added
						client.set(recipient_id, JSON.stringify(recipient_array), function(err4, transfer_response) {
							if (err4) {
								callback(false, err4);
								return;
							}
