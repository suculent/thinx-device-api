  ...
  migrate_device(original_owner, xudid, recipient, body, json_keys, callback) {
	...
	accept(body, accept_callback) {
		...
		this.client.get(dtid, (err, encoded_json_keys) => {
			...
			console.log(`[debug] [transfer] Fetched DTID: ${dtid} with error ${err} and keys ${{encoded_json_keys}}`);

			if (err) {
				console.log("[warning] [transfer] transfer_id not found.", err);
			...
			for (var dindex in locked_udids) {
				...
				this.migrate_device(original_owner, list, recipient, body, json_keys, () => {
					if (index != locked_udids.length - 1) return;
					/* last callback */
					// Store remaining (not accepted) keys
					this.client.set(dtid, JSON.stringify(json_keys), (xerr, response) => {
						if (xerr) {
							console.log(`[error] [transfer] accept set error ${xerr} with response ${response}`);
	...
	decline(body, callback) {
		...
		console.log(`[transfer][decline] body: ${body}`);
