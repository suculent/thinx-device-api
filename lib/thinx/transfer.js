...
module.exports = class Transfer {
	...
	async pending_transfer_exists(encoded_json_keys, dtid) {
		...
		return false; // no callback called, continue with transfer...
	}
	...
	async request(owner, body) {
		...
		// Generic Checkr
		if (!Util.isDefined(body.to)) return Promise.reject(new Error("missing_recipient"));
		if (!Util.isDefined(body.udids)) return Promise.reject(new Error("missing_subject"));
		...
		for (const udid in body.udids) {
			...
			if (state) {
				...
				return Promise.reject(new Error("transfer_already_in_progress"));
			}
		}

		// Fetch original owner
		let ownerdoc = await userlib.get(owner).catch((e) => {
			console.log("Owner", owner, "unknown in transfer request!", e);
		...
		let recipient = await userlib.get(recipient_id).catch((e) => {
			// can return error:missing
			console.log("☣️ [error] Transfer target body.to id " + recipient_id + " not found with error:", e);
			return Promise.reject(new Error("recipient_unknown"));
		});

		console.log("[debug] fetched recipient", recipient.email, recipient.owner);
		...
		console.log("[debug] [transfer] returning transfer_uuid", transfer_uuid);

		// 5. respond with success/failure to the request
		return transfer_uuid;
	}

	// converts migrate_device to a promise
	...
	// accept can be specific (single dtid), or fully/partial (list of udids in dtid)
	async accept(body) {

		// minimum body should look like { "transfer_id":"dtid" }
		// optional body should look like { "transfer_id":"dtid", "udids" : [ ... ] }
		...
		console.log(`🔨 [debug] [transfer] accept incoming body`, { body });
		...
		if (keys.length == 0) {
			...
			return Promise.reject(new Error("accept_transfer_id_not_found_2"));
		}

		var json_keys = JSON.parse(encoded_json_keys);

		console.log(`🔨 [debug] [transfer] stringified DTID: ${dtid} with json_keys ${json_keys}`);

		if (this.pending_transfer_exists(encoded_json_keys, dtid) === true) {
			// false ^^^ = no callback called, continue with transfer...
			// true ^^^ = no remaining UDIDs, complete.
			console.log("[debug] [transfer] No remaining UDIDs.");
		...
		if (typeof (recipient_email) === "undefined" || recipient_email === null) {
			return Promise.reject(new Error("recipient_to_must_be_set"));
		}
		...
		if ((typeof (original_owner_email) === "undefined") || (original_owner_email === null)) {
			return Promise.reject(new Error("originator_from_must_be_set"));
		}
		...
		// if there are no devices left (dtid must NOT be deleted! (only expired?))
	...
	// used on accept only! duplicate code? calls accept_callback! otherwise similar to updateOrDeleteDTID below
	async updateOrDeleteDTIDWithCallback(dtid, keys, ac) {
		...
		if (keys.udids.length > 1) {
			...
			await this.redis.expire(dtid, 3600); // 3600 seconds expiration for this transfer request; should be possibly more (like 72h to pass weekends)
		} else {
			ac(true, "transfer_completed");
			console.log("[debug] [transfer] [updateOrDeleteDTIDWithCallback] deleting dtid: ", dtid); // deletes transfer's dtid after being saved on accept
			await this.redis.del(dtid);
		}
	}

	async updateOrDeleteDTID(dtid, keys) {
		await this.redis.set(dtid, JSON.stringify(keys));
		console.log(`🔨 [debug] [transgfer] L4 Storing remaining keys: ${keys.udids}`);
		if (keys.udids.length > 1) {
			// 1 hour to let user accept/decline different devices
			await this.redis.expire(dtid, 3600);
		} else {
			console.log("[debug] [transfer] [updateOrDeleteDTID] deleting dtid (3): ", dtid);
			await this.redis.del(dtid);
		}
	}
	...
	async decline(body) {
		...
		// this is maybe contraproductive, as it happens in updateOrDeleteDTID and otherwise is not checked...
		/*
