...
module.exports = class Transfer {
	...
	async request(owner, body) {
		...
		// Generic Check
		if (!Util.isDefined(body.to)) return Promise.reject("missing_recipient");
		...
		for (const udid in body.udids) {
			...
			if (state) {
				...
				return Promise.reject("transfer_already_in_progress");
			}
		}

		// Fetch original owner
		let ownerdoc = await userlib.get(owner).catch(() => {
			console.log("Owner", owner, "unknown in transfer request!");
			return Promise.reject("owner_unknown");
		});

		// Fetch new recipient
		await userlib.get(recipient_id).catch((e) => {
			// can return error:missing
			console.log("☣️ [error] Transfer target body.to id " + recipient_id + "not found", e);
			return Promise.reject("recipient_unknown");
		});
		...
		return Promise.resolve(transfer_uuid);
	...
	async accept(body) {
		...
		if (typeof (recipient_email) === "undefined" || recipient_email === null) {
			return Promise.reject("recipient_to_must_be_set");
		}
		...
		if ((typeof (original_owner_email) === "undefined") || (original_owner_email === null)) {
			return Promise.reject("originator_from_must_be_set");
		}
