...
module.exports = class Transfer {
	...
	async transfer_valid(encoded_json_keys, dtid) {
	...
	async accept(body) {
		...
		if (typeof (body.transfer_id) === "undefined") {
			return Promise.reject("missing_transfer_id");
		}
		...
		if (keys.length == 0) {
			...
			return Promise.reject("transfer_id_not_found");
		}
		...
		if (this.transfer_valid(encoded_json_keys, dtid) === true) {
	...
	async decline(body) {
		...
		if (json_keys === null) {
			...
			return Promise.reject("decline_complete_no_such_dtid");
		}
