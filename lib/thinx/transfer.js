...
module.exports = class Transfer {
	...
	migrate_api_keys(original_owner, recipient, xudid, callback) {
		...
		this.devices.get(xudid, (success, dev) => {
			...
			// Get source keys
			this.client.get(source_id).then((json_keys) => {
				...
				// Get recipient keys
				this.client.get(recipient_id).then((recipient_keys) => {
					...
					this.client.set(source_id, JSON.stringify(json_array));
					// Save new array with respective API Key added
					this.client.set(recipient_id, JSON.stringify(recipient_array));
	...
	rename(from, to) {
		fs.copy(from, to, (err) => {
			if (err) {
				console.log("☣️ [error] [transfer] caught COPY error:", err);
			} else {
				fs.remove(from);
			}
		});
	...
	exit_on_transfer(udid, result_callback) {
		...
		this.client.get("dtr:" + udid).then((reply) => {
			if ((reply === null) || (reply == [])) {
	...
	request(owner, body, callback) {
		...
		userlib.get(owner, (err, ownerdoc) => {
			...
			userlib.get(recipient_id, (zerr/* , recipient */) => {
				...
				this.client.set(transfer_id, JSON.stringify(body));
	...
	save_dtid(tid, keys, ac) {
		this.client.set(tid, JSON.stringify(keys));
	...
	async accept(body, accept_callback) {
		...
		this.client.get(dtid).then((encoded_json_keys) => {

			let keys = JSON.stringify(encoded_json_keys);

			if (keys.length == 0) err = true;
	...
	storeRemainingKeys(dtid, json_keys, callback) {
		this.client.set(dtid, JSON.stringify(json_keys));
	...
	decline(body, callback) {
		...
		this.client.get(dtid).then((json) => {
