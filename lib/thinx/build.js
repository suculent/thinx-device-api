...
var Build = (function() {
	...
	var _private = {
		pathForOwner: function(owner) {
			...
			var user_path = config.build_root + "/" + owner;
			return user_path;
		}
	};
			...
			_public.fetch(build_id, function(err, body) {
				...
				if (err) {
					...
					if (fs.existsSync(build_log_path)) {
						...
						console.log(
							"[tail][fake][DEBUG] Tailing application log in Fake-Mode!");
						...
						// FIXME: Debug only -->
						tail = new Tail("/var/log/thinx.log", xoptions);
