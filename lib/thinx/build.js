				...
				if (err) {
					...
					if (typeof(websocket) !== "undefined" && websocket !== null) {
						try {
							websocket.send(JSON.stringify({
								log: {
									message: build.date + " : " + build.message
								}
							}));
						} catch (e) { /* handle error */
							console.log(e);
					...
					if (fs.existsSync(build_log_path)) {
						...
						console.log(
							"[tail][fake][DEBUG] Tailing log in Fake-Mode!");

						// TODO: Should retry with timer instead...
						...
						// FIXME: Debug only -->
						xtail = new Tail("/root/thinx-device-api/vault.log", xoptions);

						xtail.on("line", function(data) {
							var logline = data.toString();
...
exports.pathForDevice = Build.pathForDevice;
