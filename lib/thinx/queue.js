...
module.exports = class Queue {
	...
	nextAvailableWorker() {
		for (let index in this.workers) {
            console.log("Checking worker", index, this.workers[index]);
	...
	parseSocketMessage(socket, message) {
        ...
        try {
            ...
            console.log("p.e:", e, "in:", message);
        }

        console.log("Parsing socket message status", msg);

        // Assign client id to new workers...
        if (0 === msg.status.indexOf("Hello from BuildWorker")) {
            socket.emit('client id', socket.id);
            let old_id = msg.id;
            let old_running = msg.running;
    ...
    setupSocket(socket) {
        ...
        socket.on('register', (msg) => {
            ...
            console.log("onregister workers", that.workers);
        });
            ...
            this.findNext((next) => {
                if (next) {
                    ...
                    this.io.emit('job', {
                        ...
                        owner_id: "baecb3124695efa1672b7e8d62e5b89e44713968f45eae6faa52066e87795a78",
