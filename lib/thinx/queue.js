...
module.exports = class Queue {
    ...
    constructor(builder) {
        ...
        if (typeof(Globals.app_config().builder) !== "undefined") {
            ...
            console.log("[warning] ⚠️ builder.concurrency integer not set in config.json, falling back to default 1 (disabled)");
        ...
        if (builder !== null) {
            ...
            this.http.listen(port, function () {
                console.log(`[info] ℹ️ BuildServer Manager listening on port ${port}`);
            });
            ...
            console.log("[error] ☣️ No builder defined for queue, not setting up IO!");
        }
    ...
    pruneIfCompleted(action) {
        if ((action.status == "success") || (action.status == "error")) {
            console.log("[info] ℹ️ Pruning completed build action...");
            action.delete();
            return true;
        }
        return false;
    ...
    runNext(action, worker) {
        ...
        if ((typeof(worker) === "undefined") || (worker === null) || (worker === false)) {
            console.log("[warning] ⚠️ runNext called with empty worker, skipping...");
            return true;
        }

        if ((typeof(action) === "undefined") || (action === null) || (action === false)) {
            console.log("[warning] ⚠️ runNext called with empty action, skipping...");
            return false;
        }
	...
	nextAvailableWorker() {
		for (let index in this.workers) {
            if (
                ...
                console.log("[info] ℹ️ Queue found available worker", index);
    ...
    setupSocket(socket) {
        ...
        socket.on('register', (msg) => {
            ...
            console.log("[info] ℹ️ Currently registered workers", Object.keys(that.workers));
        });

        socket.on('poll', (msg) => {
            console.log("[info] ℹ️ Worker is polling...", msg);
            this.findNext((next) => {
                if (next !== null) {
        ...
        socket.on('job-status', (job_status) => {
            this.notifier.process(job_status, (result) => {
                console.log("[info] ℹ️ Notifier's Processing result:", result);
            });
