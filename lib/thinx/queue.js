...
module.exports = class Queue {
    ...
    constructor(builder, di_app) {
        ...
        if (builder !== null) {
            ...
            this.initloop(this.http, this.port);
    ...
    initloop(server, port) {

        let sock_url = "http://api:" + this.port;
        console.log(`[info] [queue] checking socket availability at ${sock_url}`);

        this.checkSocketIoConnect(sock_url).then(function () {
            console.log(`ℹ️ [info] BuildServer Manager already listening on port ${port}`);
        }, function (reason) {
            console.log(`[debug] reason: ${reason}`); // TODO: remove
            try {
                ...
                port += 1;
                this.initloop(server, port);
                console.log(`[info] [queue] Socket started on port ${port}`);
            }
        });
    }
