...
module.exports = class Queue {
    ...
    initloop(client) {
        ...
        // TODO: Make the port override dependent on process executor?
        ...
        try {
            client.listen(this.port, function () {
                console.log(`ℹ️ [info] BuildServer Manager listening on port ${this.port}`);
            });
        } catch (e) {
            // In test, listen in port 3000 throws exception (already in use)
            this.port += 1;
            this.initloop(client, this.port);
            console.log(`[info] [queue] Socket started on port ${this.port}`);
        }
    }
