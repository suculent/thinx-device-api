...
module.exports = class Queue {
    ...
    add(udid, source, owner_id, opt_callback) {
        let action = new Action(udid, this.client);
    ...
    findNext(callback) {
        let action_keys = this.client.sendCommand(["KEYS", "queue:*"]);
        if (action_keys === null) {
            return callback(false, "sendCommand error");
        }
        ...
        for (var i = 0, len = action_keys.length; i < len; i++) {
            ...
            this.client.get(action_key).then((contents) => {
                
                let action = new Action(uaid, this.client).withString(contents);
        ...
        callback(true, null); // added to complete (in tests)
    }

    runNext(action, worker) {

        if ((typeof (worker) === "undefined") || (worker === null) || (worker === false)) {
            console.log("⚠️ [warning] runNext called with empty worker, skipping, will set error for action", action.action);
            action.setError();
            return;
        }
        ...
        console.log("ℹ️ [info] runNext:", action.toString());
    ...
    setupSocket(socket) {
        ...
        socket.on('poll', (msg) => {
            ...
            this.findNext((success, next) => {
                if ((success === true) && (next !== null)) this.runNext(next, socket.id);
            });
        });
