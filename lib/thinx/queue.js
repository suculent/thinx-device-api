...
module.exports = class Queue {
	...
	nextAvailableWorker() {
		for (let index in this.workers) {
            if (
                ...
                console.log("[DEBUG QUEUE] Assigning worker", index, ":", this.workers[index]);
    ...
    setupSocket(socket) {
        ...
        socket.on('disconnect', () => {
            console.log(new Date().getTime(), chalk.bold.red(`»`), chalk.white(`Unregistering disconnected worker ${socket.id}.`));
        ...
        socket.on('register', (msg) => {
            console.log(`» Registering new worker: ${socket.id}`);
            ...
            console.log("Currently registered workers", that.workers);
        });
        ...
        socket.on('job-status', (job_status) => {
            console.log("» Remote job-status received", job_status);
