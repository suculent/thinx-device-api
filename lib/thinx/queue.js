...
// Prevents collision when the class is instantiated 2nd time in test
...
module.exports = class Queue {
    ...
    constructor(builder) {
        ...
        if (typeof(Globals.app_config().builder) !== "undefined") {
            ...
            console.log("Builder Concurrency is:", this.maxRunningBuilds);
        } else {
            console.log("builder.concurrency integer not set in config.json, falling back to default 1 (disabled)");
            ...
            for (var i = 0, len = action_keys.length; i < len; i++) {
                ...
                this.client.get(action_key, (job_error, contents) => {
                    ...
                    if (action.isWaiting()) {
                        if (limit > 0) {
                            let a = action.action;
                            console.log("Scheduling build action", a.build_id, "with remaining concurrent job limit", limit);
                            limit--;
                            callback(action);
                        }
                    }
    ...
    runNext(action, worker) {
        ...
        this.builder.build(
            ...
            [], // notifiers
            (success, message) => {
                console.log("Build exit state", success, message);
                console.log("INFO: Deleting action after build request completed, set worker to not running...");
                action.delete();
	...
	nextAvailableWorker() {
		for (let index in this.workers) {
            if (
                ...
                console.log("[QUEUE] Found available worker", index);
