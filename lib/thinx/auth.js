...
module.exports = class Auth {
	...
	add_mqtt_credentials(_username, password, callback) {
		...
		bcrypt.genSalt(hasher_cost)
			...
			.then((hash) => {
				console.log(`ℹ️ [info] Preparing authentication state for username ${username}`);
				this.redis.set(username, hash).then((result) => {
			...
			.catch(err => { c
				onsole.log(`[error] Adding MQTT hash ${err}`); 
			});
	}

	revoke_mqtt_credentials(username) {
		this.redis.del(username).then((result) => {
			console.log("MQTT Revocation result:", result); // may be ignored
			return true;
		});
	}
