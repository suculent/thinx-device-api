...
module.exports = class Auth {
	...
	add_mqtt_credentials_go(username, password) {
		...
		bcrypt.hash(password, hasher_cost, (err, hash) => {
			if (err) {
				console.log(err);
				return;
			}
			if (err) {
				console.log("->bcrypt.hash error:", err, "hash:", hash);	
			}
			this.client.set(username, hash);

			// could be only conditional
			this.client.set(username+":su", "true"); // make it superuser to prevent issues with invalid ACL on login? this may be real issue for devices as they could receive others messages. rather not.
		});
	}
