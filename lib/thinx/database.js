...
module.exports = class Database {
	...
	constructor() {
		console.log("[info] ℹ️ (Will) init DBs...");
	...
	init(callback) {
		...
		console.log("[info] ℹ️ (Initializing) databases...");
		...
		this.nano.db.list((err, existing_dbs) => {
			...
			db_names.forEach((name) => {
				...
				console.log("[info] ℹ️ (Creating) database", name);
				...
				this.nano.db.create(dbprefix + "managed_" + name).then((/* body */) => {
					...
					console.log("[info] ℹ️ (managed)_" + name + " db is ready now.");
				}).catch((err2) => {
	...
	logCouchError(err, body, header, tag) {
		if (err !== null) {
			if (err.toString().indexOf("conflict") === -1) {
				console.log("[error] ☣️ Couch Init error: ", err, body, header, tag);
			}
		...
		if (typeof (body) !== "undefined") {
			console.log("[error] ☣️ Log Couch Insert body: " + body + " " + tag);
		}
		if (typeof (header) !== "undefined") {
			console.log("[error] ☣️ Log Couchd Insert header: " + header + " " + tag);
		}
	}
	...
	injectDesign(db, design, file) {
		...
		if (design_doc != null) {
			...
			console.log("[error] ☣️ Design doc injection issue at " + file);
		}
	}
	...
	injectReplFilter(db, file) {
		...
		if (filter_doc !== false) {
			...
			console.log("[error] ☣️ Filter doc injection issue (no doc) at " + file);
		}
	}
