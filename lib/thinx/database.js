...
module.exports = class Database {
	...
	constructor() {
		...
		//let env = process.env;
		//console.log("Debugging process.env:", {env});

		let db_uri;
		let user = process.env.COUCHDB_USER;
		let pass = process.env.COUCHDB_PASS;

		console.log(`Debugging CouchDB user ${sha256(user)} and pass ${sha256(pass)}`);

		if ((typeof (user) !== "undefined") && (typeof (pass) !== "undefined")) {
			db_uri = `http://${user}:${pass}@couchdb:5984`;
			console.log("ℹ️ [info] Using environment database credentials...");
		} else {
	...
	handleDatabaseErrors(err, name) {
		if (err.toString().indexOf("the file already exists") !== -1) {
			...
			console.log("🚫 [critical] Database " + name + " creation failed. " + err);
			setTimeout(() => {
				process.exit(2);
			}, 1000);
		}
