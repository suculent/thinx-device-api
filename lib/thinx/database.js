...
module.exports = class Database {
	...
	init(callback) {
		...
		this.nano.db.list((err, existing_dbs) => {
			...
			console.log("Creating DB with existing DBs:", { existing_dbs });

			if (existing_dbs === null) existing_dbs = [];
			...
			db_names.forEach((name) => {
				...
				console.log("DB create error", err3);
			});
	...
	compactDatabases(opt_callback) {
		this.nano.db.compact("managed_logs");
		this.nano.db.compact("managed_builds");
		this.nano.db.compact("managed_devices");
		this.nano.db.compact("managed_users", "owners_by_username", (err) => {
			if (err) {
