...
module.exports = class Database {
	...
	uri() {
		let db_uri;
		if ((typeof (process.env.COUCHDB_USER) !== "undefined") && (typeof (process.env.COUCHDB_PASSWORD) !== "undefined")) {
			db_uri = `http://${process.env.COUCHDB_USER}:${process.env.COUCHDB_PASSWORD}@couchdb:5984`
		} else {
	...
	handleDatabaseErrors(err, name) {
		...
		if (err.toString().indexOf("the file already exists") !== -1) {
			...
			console.log("🚫 [critical] Database connectivity issue. " + err.toString() + " URI: " + db_uri);
			// give some time for DB to wake up until next try, also prevents too fast restarts...
			setTimeout(() => {
				process.exit(1);
			}, 1000);
		} else {
			console.log("🚫 [critical] Database " + name + " creation failed. " + err + " URI: " + db_uri);
			setTimeout(() => {
				process.exit(2);
			}, 1000);
		}
