...
module.exports = class JWTLogin {
    ...
    fetchOrCreateSecretKey(callback) {
        this.redis.get(JWT_KEY, (err, result) => {
            if ((err !== null) || (result == null)) {
                this.createSecretKey((key) => {
                    // This should never leak, log only for debugging purposes! console.log("[jwt] Created signing key", key);
                    this.redis.set(JWT_KEY, key, (err) => {
                        if (err !== null) console.log("[jwt] save error!", err);
                        callback(key);
                    }); // could use error callback
                    
                });
            } else {
                // This should never leak, log only for debugging purposes! console.log("[jwt] Fetched signing key", result);
                callback(result);
            }
        });
    }
    ...
    sign(uid, callback) {
        ...
        this.fetchOrCreateSecretKey((secretkey) => {
            ...
            // This should never leak: console.log("[jwt] Fetched or created signing key", secretkey);
            ...
            // This should never leak: console.log("[jwt] signing payload", payload, "with secretkey", secretkey);
    ...
    verify_impl(req, callback) {
        ...
        // This should never leak: console.log("[jwt] verify token", token, "with secretkey", sck);
    ...
    verify(req, callback) {
        ...
        if ((typeof (req.headers['authorization']) !== "undefined") && (typeof (req.headers['Authorization']) !== "undefined")) {
            console.log("[error] Invalid request header in jwt-verify, req:", {req});
            callback(false);
        }
        // key guard
