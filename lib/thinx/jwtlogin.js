...
module.exports = class JWTLogin {
    ...
    constructor(redis) {
        if (typeof(redis) === "undefined") throw new Error("Bad JWTLogin initialization, needs valid/connected Redis.");
        this.redis = redis;
    ...
    revokeSecretKey(callback) {
        this.redis.del(JWT_KEY);
        callback(err);
    }

    fetchOrCreateSecretKey(callback) {
        if (!this.redis) throw new Error("Redis required in JWTLogin!");
        this.redis.get(JWT_KEY, (error, result) => {
            if (result) {
                callback(result);
            } else {
                this.createSecretKey((key) => {
