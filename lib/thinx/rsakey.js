	...
	generate(i_owner, date, callback) {
		...
		generateKeyPair('rsa', {
			...
			privateKeyEncoding: {
				...
				format: 'pem',
				cipher: 'aes-256-cbc',
				passphrase: 'thinx' // TODO: once we'll have Secure Storage (e.g. Vault), keys can be encrypted but anyway, it will be accessible from this container using vault key env...
			}
		}, (err, publicKey, privateKey) => {
			fs.writeFileSync(public_path, publicKey);
			fs.fchmodSync(fs.openSync(public_path), 0o644);
			fs.writeFileSync(private_path, privateKey);
			fs.fchmodSync(fs.openSync(private_path), 0o600);
