...
var RSAKey = (function() {
	...
	// TODO: Global imports, can exist outside class
  ...
  var app_config = Globals.app_config();
	...
	//
	// TODO: refactor to local/private class functions
	//

	function _getKeyPathsForOwner(owner) {
		return fs.readdirSync(app_config.ssh_keys).filter(
			file => ((file.indexOf(owner) !== -1) && (file.indexOf(".pub") === -1))
		);
	}

	function revokeUserKeys(revoked_filenames, owner) {
		var revoked_keys = [];
		const key_paths = _getKeyPathsForOwner(owner);
	...
	function exportKeysForList(key_paths, owner) {
		var exportedKeys = [];
	...
	function keyPathsExist(owner) {
		const key_paths = _getKeyPathsForOwner(owner);
		return (key_paths.count > 0);
	}

	function revokeUserKeysAndCallback(revoked_filenames, owner, callback) {
		const revoked_keys = revokeUserKeys(revoked_filenames, owner);
	...
	//
	// TODO: refactor to public class methods
	//
	...
	var _public = {
		...
		getKeyPathsForOwner: function(owner) {
			return _getKeyPathsForOwner(owner);
		}
	};
...
exports.getKeyPathsForOwner = RSAKey.getKeyPathsForOwner;
