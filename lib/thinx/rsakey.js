...
module.exports = class RSAKey {
	...
	generate(i_owner, date, callback) {
		...
		generateKeyPair('rsa', {
			...
			fs.writeFileSync(private_path, privKey);
			fs.writeFileSync(public_path, pubKey);
			callback(err, pubKey, privKey);
		});
	}
	...
	create(i_owner, callback) {
		...
		this.generate(owner, (err, pubKey /* , privKey */) => {
			if (err) {
				console.log(err);
