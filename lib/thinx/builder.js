...
const Sources = require("./sources"); var sources = new Sources();
...
module.exports = class Builder {
	...
	gitFetchCommand(BUILD_PATH, sanitized_url, sanitized_branch) {
		return (
			`cd ${BUILD_PATH}; rm -rf ./*; ` +
			`if $(git clone "${sanitized_url}"  -b "${sanitized_branch}); then` +
				`cd *; ` +
	...
	prefetchPrivate(br, SHELL_FETCH, BUILD_PATH) {
		...
		if (first_git_success === false) {
			...
			if (!gfr) {
				console.log("[error] Secondary git fetch failed. That's bad.");
				console.log("[info] Saving state FAILED for build_id", build_id, "owner", owner, "udid", udid);
	...
	run_build(br, notifiers, callback, transmit_key) {
		...
		devicelib.get(udid, (err, device) => {
			...
			this.getLastAPIKey(owner, (success, api_key) => {
				...
				if (!mkresult) console.log("mkdirp.sync ended with with:", mkresult);
				...
				const SHELL_FETCH = this.gitFetchCommand(BUILD_PATH, sanitized_url, sanitized_branch);

				// Attempts to fetch GIT repo, if not marked as private
				this.prefetchPublic(br, SHELL_FETCH);

				if (!this.prefetchPrivate(br, SHELL_FETCH, BUILD_PATH)) return;
				...
				Platform.getPlatform(XBUILD_PATH, (get_success, platform) => {
					...
					var commit_id = exec.execSync(`cd ${XBUILD_PATH}; git rev-list --all --max-count=1`).toString();
					var git_revision = exec.execSync(`cd ${XBUILD_PATH}; git rev-list --all --count`).toString();
