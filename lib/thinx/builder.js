...
module.exports = class Builder {
	...
	decrypt(cryptokey, inputHex) {
		...
		if (typeof (inputHex) === "undefined" || typeof (cryptokey) === "undefined") {
			throw new Error("Fatal Error - Invalid decryption data: [" + inputHex + "] [" + "***" + cryptokey.slice(cryptokey.length - 3) + "]");
		}

		try {
			// Transmitted data should be in base64
			let e64 = CryptoJS.enc.Base64.parse(inputHex);

			// After decoding, this should be encrypted data
			let bytes = CryptoJS.enc.Hex.parse(e64);
			...
			console.log("DEBUG decrypt using cryptokey", "***" + cryptokey.slice(cryptokey.length - 3), "***" + plain.slice(plain.length - 3));
	...
	// FIXME: unused?
	buildLogNotification(message, messageType, udid, build_id) {
		return {
			log: this.buildNotification(message, messageType, udid, build_id)
		};
	}
				...
				_platform.getPlatform(XBUILD_PATH, (get_success, platform) => {
					...
					if (isYAML) {
						...
						if (typeof (yml) !== "undefined") {
							...
							if (typeof(transmit_key) !== "undefined" && transmit_key !== null) {
								...
								console.log("[REM] CSSID/CPASS endecrypt with transmit_key", transmit_key);

								if (typeof(device.environment.cssid) !== "undefined") {
									console.log("CSSID found, attempt to decrypt using transmit_key...");
									yml["devsec"]["ssid"] = this.decrypt(transmit_key, device.environment.cssid);
								}
								if (typeof(device.environment.cpass) !== "undefined") {
									console.log("CPASS found, attempt to decrypt using transmit_key...");
									yml["devsec"]["pass"] = this.decrypt(transmit_key, device.environment.cpass);
								}
								console.log("[REM] YML:", {yml});
								...
								fs.writeFileSync(yml_path, insecure, 'utf-8');
							} else {
					...
					if (typeof (device_specific_environment) !== "undefined") {
						...
						if (fs.existsSync(XBUILD_PATH)) { // validate to prevent injection
							fs.writeFileSync(envFile, envString, 'utf-8');
						}
					}
