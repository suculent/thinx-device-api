...
var Builder = (function() {
	...
	try {
		...
		console.log("[builder] thx_prefix_exception" + e);
	}
	...
	var _private = {
		...
		buildCommand: function(build_id, owner, git, branch, udid, dryrun, notifiers, callback) {
			...
			devicelib.get(udid, function(err, device) {
				...
				var OWNER_ID_HOME = app_config.data_root + app_config.deploy_root + "/data/" + owner;
				...
				apikey.list(owner, function(success, json_keys) {
					...
					console.log("[builder] Injecting API Key" + api_key);
					...
					mkdirp(BUILD_PATH, function(err) {
						if (err) {
							...
							var SHELL_FETCH = "cd " + BUILD_PATH + "; if $(git clone " + sanitized_url +
								" -b " + sanitized_branch + "); then cd * then git pull origin " + sanitized_branch + " --recurse-submodules; fi";

							// TODO: Fixme
							//SHELL_FETCH = shellescape(SHELL_FETCH);
							...
							if ( git_success == false || git_success == [] ) {
								...
								for (var kindex in key_paths) {
									var prefix = "ssh-agent bash -c 'ssh-add /root/.ssh/" + key_paths[kindex] + "; ";
									console.log("git prefix: " + prefix);
									try {
										result = exec.execSync(prefix + SHELL_FETCH + "'").toString().replace("\n", "");
							...
							repository.getPlatform(XBUILD_PATH, function(success, platform) {
								...
								// TODO: Load from thinx.yml and validate (if thinx.yml)
								...
								if (typeof(yml) !== "undefined") {
									...
									console.log("Parsed YAML: "+JSON.stringify(yml, false, 4));
									...
									const y_platform = Object.keys(yml)[0];
									console.log("YAML-based platform: " + y_platform);
									const y_mcu = yml[y_platform].arch;

									if (y_mcu.indexOf(device_mcu) == -1) {
										callback(false, "MCU defined by thinx.yml (" + y_mcu + ") not compatible with this device MCU: " + device_mcu);
										return;
									}

									// Store platform + architecture if possible (o'rly?)
									...
									console.log("BuildCommand-Detected platform (no YAML): " + platform);
								}

								// <- feature/fix

								var platform_descriptor = require("../../platforms/" +
									device_platform + "/descriptor.json");
								console.log("[builder] Device Platform: " + device_platform);
								...
								apienv.list(owner, function(success, api_envs) {
										if (success) {
											...
											// OK console.log("Buiding from app_config: "+JSON.stringify(app_config, false, 4));
