			...
			this.getLastAPIKey(owner, udid, (success, api_key) => {
				...
				if (first_git_success === false) {
					...
					if (!gfr) {
						...
						// TODO (7):  If secondary git result is OK, mark this in DB and don't try to fetch repo again using public key anymore!
						// https://github.com/suculent/thinx-device-api/issues/300
	...
	build(owner, build, notifiers, callback, worker) {
		...
		devicelib.view("devices", "devices_by_owner", {
			...
			if ((typeof (device) === "undefined") || udid === null) {
				console.log(`[error] No device is currently using source ${build.source_id}.`);
			...
			userlib.get(owner, (user_get_err, doc) => {
				...
				if (user_get_err) {
					console.log(`[error] [builder] ${user_get_err}.`);
					callback(false, {
						success: false,
				...
				devicelib.atomic("devices", "modify", device.udid, { build_id: build_id }, (mod_error) => {
					if (mod_error) {
						console.log(`[error] [builder] Atomic update failed: ${mod_error}.`);
						callback(false, {
							success: false,
