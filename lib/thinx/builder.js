...
module.exports = class Builder {
	...
	processExitData(owner, build_id, udid, notifiers, data) {
		...
		this.wsError(notifiers.websocket, data.status);
	}
	...
	processShellData(owner, build_id, udid, notifiers, data) {
		...
		/* This is done by the builder, deprecated
		// Something must write to deploy_path/build_id/build.log where the file is tailed from to websocket...
	...
	run_build(br, notifiers, callback) {
		...
		devicelib.get(udid, (err, device) => {
			...
			this.getLastAPIKey(owner, udid, (success, api_key) => {
				...
				try {
					const fd = fs.openSync(BUILD_PATH, "r");
					fs.fchmodSync(fd, 0o777);
					console.log("BUILD_PATH permission change successful.");
				} catch (error) {
					console.log(error);
				}
				...
				try {
					...
					console.log("Attempting public git fetch...");
