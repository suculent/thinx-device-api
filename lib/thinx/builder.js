...
module.exports = class Builder {
	...
	notify(udid, notifiers, message, success) {
		var status = successStringFromBool(success);
		if (message.indexOf("build_running") !== -1) {
			status = "info"; // blue
		}
		var not = this.buildStatusNotification(message, status, udid);
		this.sendNotificationIfSocketAlive(notifiers.websocket, not);
	}
