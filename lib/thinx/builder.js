...
module.exports = class Builder {
	...
	runRemoteShell(worker, CMD, owner, build_id, udid, notifiers, source_id) {
		...
		if (this.io !== null) {
			...
			console.log("[error] [build] io socket null");
			return;
		}
		...
		if (typeof (worker.socket) === "undefined") {
			console.log(`[OID:${owner}] [BUILD_STARTED] REMOTE execution failed; no socket.`);
			return;
		}
	...
	processShellData(opts, data) {
		...
		if (logline.length > 1) { // skip empty lines in log
				...
				(logline.indexOf("status: OK") !== -1)) // new
			{
				...
				sources.updateLastBuild(opts.owner, opts.source_id, opts.data.version, (result) => {
	...
	run_build(br, notifiers, callback, transmit_key) {
		...
		devicelib.get(udid, (err, device) => {
			...
			this.getLastAPIKey(owner, udid, (success, api_key) => {
				...
				// useless: console.log("🔨 [debug] first_git_success", {first_git_success});
