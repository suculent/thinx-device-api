...
var ROOT = "/opt/thinx/thinx-device-api"; // = Dockerfile WORKDIR
...
const Sources = require("./sources.js"); var sources = new Sources();
...
module.exports = class Builder {
	...
	notify(udid, build_id, notifiers, message, success_status) {
		...
		console.log("notify udid:", udid, "message:", message, "status:", success_status);
	...
	runRemoteShell(worker, CMD, owner, build_id, udid, notifiers, source_id) {
		...
		worker.socket.on('log', (data) => {
			this.processShellData(owner, build_id, source_id, udid, notifiers, data);
		});
		...
		// TODO (6): Setup timeout for remote build executor to prevent hanging; 
		// https://github.com/suculent/thinx-device-api/issues/299
	...
	processShellData(owner, build_id, source_id, udid, notifiers, data) {
		...
		if (logline.length > 1) { // skip empty lines in log
			...
			{
				this.notify(udid, build_id, notifiers, "Completed", true);
				blog.state(build_id, owner, udid, "Success");
				this.wsOK(notifiers.websocket, "Build successful.", udid);

				sources.updateLastBuild(owner, source_id, data.version, (result) => {
	...
	runShell(CMD, owner, build_id, udid, notifiers) {
		...
		var shell = exec.spawn(command, { shell: true }); // lgtm [js/command-line-injection]
	...
	formatMacForDevSec(incoming) {
		console.log("[REM] formatMacForDevSec incoming", incoming);
		let outgoing;
		let no_colons = incoming.replace(/:/g, "");
		console.log("[REM] formatMacForDevSec no_colons", no_colons);
		if (no_colons.length == 12) {
			outgoing = no_colons.substr(6, 12);
		} else {
			outgoing = no_colons;
		}
		console.log("[REM] formatMacForDevSec outgoing", outgoing);
		return outgoing;
	}
	...
	run_build(br, notifiers, callback, transmit_key) {
		...
		if ((typeof (br.worker) === "undefined") || (typeof (br.worker.socket) === "undefined")) {
			...
			if (process.env.ENVIRONMENT !== "test") {
				callback(false, "workers_not_ready");
				return;
			}
		}
		...
		devicelib.get(udid, (err, device) => {
			...
			var sanitized_build = sanitka.udid(build_id);
			...
			this.getLastAPIKey(owner, udid, (success, api_key) => {
				...
				// prefetch without using SSH keys only for repos that have not been marked as is_private
				if (!br.is_private) {
					...
					try {
						let git_result = null;
						const SHELL_FETCH = "cd " + BUILD_PATH + "; rm -rf ./*; " +
						...
						// TODO: refactor to git.fetch(owner, SHELL_FETCH, BUILD_PATH); 
						git_result = exec.execSync(SHELL_FETCH).toString().replace("\n", "");
						console.log(`[info] [build] public prefetch git resut ${git_result}`);
					} catch (e) {
				...
				console.log("🔨 [debug] first_git_success", {first_git_success});
				if (first_git_success === false) {
					...
					if (!gfr) {
						...
						Sources.updatePrivate(owner, source_id, true, (xuccess, error) => {
							if (xuccess) {
								console.log(`[info] repo privacy status updated to is_private=true; should prevent future public fetches`);
							} else {
					...
					if (first_git_success !== "Already up to date.") {
				...
				Platform.getPlatform(XBUILD_PATH, (get_success, platform) => {
					...
					if (isYAML) {
						...
						if (typeof (yml) !== "undefined") {							
							...
							if (typeof(transmit_key) !== "undefined" && transmit_key !== null) {
								...
								if (device.environment !== "undefined") {
								...
								console.log("[REM] transmit_key is null or undefined on endecrypt");
							}
						}
					} else {
					...
					var git_tag = this.getTag(XBUILD_PATH);
					...
					apienv.list(owner, (env_list_success, api_envs) => {
						...
						try {
							const presult = exec.execSync(PRE); // lgtm [js/command-line-injection]
	...
	cleanupSecrets(cpath) {
		...
		var env_files = finder.in(cpath).findFiles("environment.json");
		env_files.forEach(env_file => {
			console.log("Cleaning up secrets:", env_file);
			fs.unlink(env_file);
		});

		var h_files = finder.in(cpath).findFiles("environment.h");
		h_files.forEach(h_file => {
			console.log("Cleaning up headers:", h_file);
			fs.unlink(h_file);
		});

		var yml_files = finder.in(cpath).findFiles("thinx.yml");
			...
			userlib.get(owner, (user_get_err, doc) => {
				...
				devicelib.atomic("devices", "modify", device.udid, { build_id: build_id }, (mod_error) => {
					if (mod_error) {
						...
						if (typeof (callback) === "undefined") {
							console.log(`🚫  [critical] [builder] Callback missing!`);
							return;
						}
	...
	supportedExtensions() {
		var languages_path = __dirname + "/../../languages";
		console.log("Loading from languages_path", languages_path);
		var languages = this.supportedLanguages();
		var extensions = [];
		for (var lindex in languages) {
			var dpath = languages_path + "/" + languages[lindex] + "/descriptor.json";
			console.log("Loading from dpath", dpath);
