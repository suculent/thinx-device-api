...
module.exports = class Builder {
	...
	decrypt(cryptokey, inputHex) {
		...
		try {
			...
			const encodedWord = CryptoJS.enc.Base64.parse(inputHex);
			const bytes = CryptoJS.enc.Utf8.stringify(encodedWord);

			console.log("[REM] DECODE BYTES", bytes);

			// Decrypt
			let decrypt = CryptoJS.AES.decrypt(bytes, cryptokey);

			console.log("[REM] DECRYPT DEC", decrypt);

			// Convert to UTF-String
			plain = decrypt.toString(CryptoJS.enc.Utf8);

			console.log("[REM] DECRYPT PLAIN", plain);
