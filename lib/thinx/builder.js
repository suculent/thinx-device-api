					...
					apienv.list(owner, (list_success, api_envs) => {
						...
						apienv.list(owner, (env_list_success, keys) => {
							...
							if (br.worker == false) {
								...
								this.runRemoteShell(br.worker, owner, build_id, udid);
							}
			...
			userlib.get(owner, (user_get_err, doc) => {
				...
				devicelib.atomic("devicelib", "modify", device.udid, changes, (mod_error) => {
					if (mod_error) {
						...
						console.log("Atomic update success, proceeding to callback...");

						if (typeof(callback) === "undefined") {
							console.log("Warning, callback mmissing!");
						}
						...
						console.log("builder.js is calling run_build with build_request:", buildRequest, "and callback", callback);
