...
var Builder = (function() {
	...
	var _private = {
		...
		buildCommand: function(build_id, owner, git, udid, dryrun, notifiers, callback) {
			...
			devicelib.get(udid, function(err, device) {
				...
				var OWNER_ID_HOME = app_config.deploy_root + "/data/" + owner;
				var BUILD_PATH = app_config.build_root + "/" + owner + "/" + udid + "/" + build_id;
				...
				apikey.list(owner, function(success, json_keys) {
					...
					console.log("Keys: " + JSON.stringify(json_keys));
					for (var key in json_keys) {
						...
						if ((typeof(kdata) !== "undefined") || (kdata !== null)) {
							if (sha256(kdata.hash) == last_key_hash) {
								api_key = kdata.key;
								console.log("[builder] Injecting API Key with hash: " + sha256(kdata));
								break;
							}
						}
					}
					...
					mkdirp(BUILD_PATH, function(err) {
						if (err) {
							...
							var fetch_result;
							var SHELL_FETCH = "cd " + BUILD_PATH + "; if $(git clone " + git +
								"); then cd * then git pull origin master --recurse-submodules; fi";
								...
								apienv.list(owner,
									function(success, api_envs) {
										if (success) {
											...
											thinx_json.THINX_MQTT_URL = app_config.mqtt_server.replace("mqtt://", ""); // due to problem with slashes in json and some libs on platforms
		...
		build: function(owner, build, notifiers, callback) {
			...
			devicelib.view("devicelib", "devices_by_owner", {
				...
				userlib.get(owner, function(err, doc) {
					...
					_private.buildCommand(build_id, owner, git, udid, dryrun, notifiers, callback);
