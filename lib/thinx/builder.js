			...
			devicelib.get(udid, function(err, device) {
				...
				mkdirp(BUILD_PATH, function(err) {
					if (err) {
						...
						repository.getPlatform(BUILD_PATH, function(error, result) {
							...
							apienv.list(owner,
								function(success, api_envs) {
									if (success) {
										...
										var thinx_json = require("builder.thinx.json"); // use as a template
										...
										// thinx_json.THINX_API_KEY = device.api_key;
										...
										fs.writeFile(BUILD_PATH + "/thinx_build.json", JSON.stringify(
											...
											if (err) {
												...
												// 1: pre-fetch repository to owner/udid/build_id
												// 2: var platform = // detect platform from repository
												// 3: fetch app version (where from?) and commit ID (1.0.revision || or tag)
												// 4: edit template data
												// 5: attach env vars
												// 6: set platform info from configuration
