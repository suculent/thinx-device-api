			...
			this.getLastAPIKey(owner, (success, api_key) => {
				...
				Platform.getPlatform(XBUILD_PATH, (get_success, platform) => {
					...
					apienv.list(owner, (env_list_success, api_envs) => {
						...
						console.log("TODO: Infer language and platform and inject those as well... we can see platform so only language for platform and language is unused!", JSON.stringify(thinx_json));
						...
						console.log("[builder] Generating headers from thinx_build.json...");

						if (fs.existsSync(header_file)) {
							JSON2H.convert(thinx_json, header_file, {});
						} else {
							console.log("[user-err] Should be reported: No header_file to write at", header_file);
						}
