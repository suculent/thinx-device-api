...
module.exports = class Builder {
	...
	getLastAPIKey(owner, callback) {
		...
		apikey.get_last_apikey(owner, callback);
	}
	...
	runRemoteShell(worker, CMD, owner, build_id, udid, notifiers, source_id) {
		...
		if (typeof (worker.socket) === "undefined") {
			...
			new InfluxConnector('stats').writePoint({
				measurement: 'BUILD_FAILED',
				tags: { owner_id: owner, build_id: build_id, socket: false },
				fields: { value: 1 },
			});
			return;
		}
	...
	processShellData(opts, data) {
		...
		if (logline.length > 1) { // skip empty lines in log
				...
				(logline.indexOf("status: OK") !== -1)) // new
			{
				...
				sources.update(opts.owner, opts.source_id, "last_build", opts.data.version, (result) => {
	...
	run_build(br, notifiers, callback, transmit_key) {
		...
		if ((typeof (br.worker) === "undefined") || (typeof (br.worker.socket) === "undefined")) {
			new InfluxConnector('stats').writePoint({
				measurement: 'BUILD_FAILED',
				tags: { owner_id: br.owner, build_id: br.build_id, socket: false },
				fields: { value: 1 },
			});
		...
		if (!this.buildGuards(callback, owner, git, branch)) {
			return new InfluxConnector('stats').writePoint({
				measurement: 'BUILD_FAILED',
				tags: { owner_id: owner, build_id: build_id, request: false },
				fields: { value: 1 },
			});
		}
		...
		devicelib.get(udid, (err, device) => {
			...
			if (err) {
				return callback(false, "no_such_udid");
			}
			...
			this.getLastAPIKey(owner, (success, api_key) => {
				...
				if (!br.is_private) {
					...
					try {
						fs.mkdirpSync(BUILD_PATH); // if not from prefetch
					} catch (e) {
						// ignore
					}
					...
					try {
						let git_result = null;
						// may fail if path already exists! why it does not exist?
				...
				if (first_git_success === false) {
					...
					if (!gfr) {
						...
						Sources.update(owner, source_id, "is_private", true, (xuccess, error) => {
							if (xuccess) {
