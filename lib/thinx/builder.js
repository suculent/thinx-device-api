						...
						if (err) {
							...
							repository.getPlatform(XBUILD_PATH, function(success, platform) {
								...
								apienv.list(owner, function(success, api_envs) {
									...
									// --> extract from here
									var thinx_json = JSON.parse(fs.readFileSync(app_config.project_root + "/builder.thinx.dist.json"));

									console.log("[builder] Preparing build template: "+JSON.stringify(thinx_json));

									if (api_envs.length() > 0) {
									...
									console.log("[builder] // Attach/replace with important data...");
									...
									console.log("[builder] // Attach/replace with more important data...");
