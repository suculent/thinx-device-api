...
module.exports = class Builder {
	...
	runRemoteShell(worker, CMD, owner, build_id, udid, notifiers, source_id) {
		...
		if (typeof (worker.socket) === "undefined") {
			...
			return InfluxConnector.statsLog(owner, EventTaxonomy.NAMES.BUILD_FAILED, build_id);
		}
		...
		InfluxConnector.statsLog(owner, EventTaxonomy.NAMES.BUILD_STARTED, build_id);
	}
	...
	writeBasenameMetadata(buildPath, repoPath, branch) {
		fs.writeFileSync(
	...
	prefetchPublic(buildPath, sanitized_url, sanitized_branch) {
		try {
			...
			exec.execFileSync("git", ["clone", sanitized_url, "-b", sanitized_branch], {
				cwd: buildPath,
				encoding: "utf8"
			});
			const directories = this.getDirectories(buildPath);
			if (directories.length < 1) {
				console.log("[builder] public git fetch did not create repository directory");
				return false;
			}
			...
			chmodr(repoPath, 0o766, (chmod_error) => {
				if (chmod_error) {
					console.log("[builder] chmodr failed after public git fetch:", chmod_error);
				}
			});
	...
	gitCloneAndPullCommand(BUILD_PATH, sanitized_url, sanitized_branch) {
		const escapedBuildPath = shellEscape([BUILD_PATH]);
		const escapedUrl = shellEscape([sanitized_url]);
		const escapedBranch = shellEscape([sanitized_branch]);
		return (
			`cd ${escapedBuildPath} && rm -rf ./* && ` +
			`git clone ${escapedUrl} -b ${escapedBranch} && ` +
			`cd * && ` +
			`git pull origin ${escapedBranch} --recurse-submodules --rebase && ` +
			`chmod -R 666 * && ` +
			`printf '{"basename":"%s","branch":"%s"}\n' "$(basename "$(pwd)")" ${escapedBranch} > ../basename.json`
		);
	}
	...
	run_build(br, notifiers, callback, transmit_key) {
		...
		if ((typeof (br.worker) === "undefined") || (typeof (br.worker.socket) === "undefined")) {
			InfluxConnector.statsLog(br.owner, EventTaxonomy.NAMES.BUILD_FAILED, br.build_id);
		...
		if (!this.buildGuards(callback, owner, git, branch)) {
			InfluxConnector.statsLog(owner, EventTaxonomy.NAMES.BUILD_FAILED, build_id);
		}
			...
			this.getLastAPIKey(owner, (success, api_key) => {
				...
				Platform.getPlatform(XBUILD_PATH, (get_success, platform) => {
					...
					this.apienv.list(owner, (env_list_success, api_envs) => {
						...
						const remoteCommand = shellEscape(["./builder", ...buildArgs]);
						console.log("[builder] Building with command: " + remoteCommand);
						...
						if (br.worker === false) {
							...
							this.runRemoteShell(br.worker, remoteCommand, owner, build_id, udid, notifiers, source_id);
						}
					}); // apienv.list
