					...
					mkdirp(BUILD_PATH, function(err) {
						if (err) {
							...
							repository.getPlatform(XBUILD_PATH, function(success, platform) {
								...
								console.log("[builder] REPO_VERSION (TAG+REV): '" + REPO_VERSION + "'");
		...
		build: function(owner, build, callback) {
			...
			devicelib.view("devicelib", "devices_by_owner", {
				...
				userlib.get(owner, function(err, doc) {
					...
					device.atomic("deviceslib", "modify", udid, device, function(error, body) {
						if (error) {
