...
module.exports = class Builder {
	...
	processShellData(opts, data) {
		...
		if (logline.length > 1) { // skip empty lines in log
				...
				(logline.indexOf("status: OK") !== -1)) // new
			{
				...
				sources.update(opts.owner, opts.source_id, "last_build", opts.version, (result) => {
	...
	prefetchPublic(SHELL_FETCH, BUILD_PATH) {
		try {
	...
	prefetchPrivate(SHELL_FETCH, BUILD_PATH) {
	...
	run_build(br, notifiers, callback, transmit_key) {
		...
		devicelib.get(udid, (err, device) => {
			...
			this.getLastAPIKey(owner, (success, api_key) => {
				...
				if (!this.prefetchPrivate(SHELL_FETCH, BUILD_PATH)) return callback(false, "git_fetch_failed");
				...
				Platform.getPlatform(XBUILD_PATH, (get_success, platform) => {
					...
					var d_filename = __dirname + "/../../platforms/" + y_platform + "/descriptor.json";
