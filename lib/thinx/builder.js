					...
					mkdirp(BUILD_PATH, function(err) {
						if (err) {
							...
							var SHELL_FETCH = "cd " + BUILD_PATH + "; git clone " + git +
								"; cd * ; git pull origin master --recurse-submodules";
							...
							if (fetch_result.indexOf("Please make sure you have the correct access rights") !== -1) {
								...
								for (var kindex in key_paths) {
									...
									try {
										var clone_result = exec.execSync(prefix + SHELL_FETCH + "'").toString().replace("\n", "");
										console.log("git rsa clone result: " + clone_result);
										break;
									} catch (e) {
								...
								console.log("GIT Fetch Result: " + fetch_result);
							}
										...
										if (success) {
											...
											fs.writeFile(XBUILD_PATH + "/thinx_build.json", JSON.stringify(
												...
												apienv.list(owner, function(success, keys) {
													...
													_private.notify(udid, notifiers, "build_running", false);
