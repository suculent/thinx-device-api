...
var Builder = (function() {
  ...
  // var rollbar = Globals.rollbar();
	...
	var notify = function(udid, notifiers, message, success) {
	...
	var getLastAPIKey = function(owner, callback) {
		...
		apikey.list(owner, function(success, json_keys) {
			if (!success) {
				...
				//notify(udid, notifiers, "error_api_key_list_failed", false);
	...
	var runShell = function(CMD, owner, build_id, owner_id, udid, notifiers) {
		...
		shell.on("exit", function(code) {
			...
			if (code > 0) {
				notify(udid, notifiers, "build_failed", false);
				blog.state(build_id, owner, udid, "failed");
				wsError(notifiers.websocket, "Build failed.");
			} else {
				notify(udid, notifiers, "build_completed", true);
	...
	var containsNullOrUndefined = function(array) {
	...
	var run_build = function(build_id, owner, git, branch, udid, dryrun, notifiers, callback) {
			...
			getLastAPIKey(owner, function(success, api_key) {
				...
				var mkresult = mkdirp.sync(BUILD_PATH, function(err) {
					if (err) {
						...
						notify(udid, notifiers, "error_io_failed", false);
						blog.state(build_id, owner, udid, "failed");
						callback(false, err);
					}
				});
				...
				notify(udid, notifiers, "fetching_git", true);
				...
				repository.getPlatform(XBUILD_PATH, function(success, platform) {
					...
					if (!success) {
						console.log("[builder] failed on unknown platform" +
							platform);
						notify(udid, notifiers, "error_platform_unknown", false);
						blog.state(build_id, owner, udid, "failed");
					...
					apienv.list(owner, function(success, api_envs) {
						...
						try {
							...
							blog.state(build_id, owner, udid, "failed");
							notify(udid, notifiers, "error_configuring_build", false);
							return;
						}
						...
						if (udid === null) {
							...
							notify(udid, notifiers, "error_starting_build", false);
							blog.state(build_id, owner, udid, "failed");
							return;
						}
						...
						apienv.list(owner, function(success, keys) {
							...
							notify(udid, notifiers, "build_running", true);
		...
		build: function(owner, build, notifiers, callback) {
			...
			devicelib.view("devicelib", "devices_by_owner", {
				...
				userlib.get(owner, function(err, doc) {
					...
					if (!containsNullOrUndefined([udid, build, owner, git])) {
						callback(false, {
							success: false,
