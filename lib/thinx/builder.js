			...
			devicelib.get(udid, function(err, device) {
				...
				apikey.list(owner, function(success, json_keys) {
					...
					if (!success) {
						...
						_private.notify(udid, notifiers, "API Key list failed.", false);
					...
					mkdirp(BUILD_PATH, function(err) {
						if (err) {
							...
							_private.notify(udid, notifiers, "IO operation failed.", false);
							blog.state(build_id, owner, udid, "failed");
							callback(false, err);
							return;
							...
							_private.notify(udid, notifiers, "Fetching repository...", true);
							...
							repository.getPlatform(XBUILD_PATH, function(success, platform) {
								...
								if (!success) {
									console.log("[builder] failed on unknown platform" +
										platform);
									_private.notify(udid, notifiers, "Unknown platform!", false);
									blog.state(build_id, owner, udid, "failed");
									...
									function(success, api_envs) {
										if (success) {
											...
											fs.writeFile(XBUILD_PATH + "/thinx_build.json", JSON.stringify(
												...
												if (err) {
													_private.notify(udid, notifiers, "Configuring build failed.", false);
													blog.state(build_id, owner, udid, "failed");
												...
												if (udid === null) {
													...
													_private.notify(udid, notifiers, "Starting build failed.", false);
													blog.state(build_id, owner, udid, "failed");
													return;
												}
												...
												apienv.list(owner, function(success, keys) {
													...
													shell.on("exit", function(code) {
														...
														if (code > 0) {
															...
															_private.notify(udid, notifiers, "Build failed.", false);
															blog.state(build_id, owner, udid, "failed");
															...
															_private.notify(udid, notifiers, "Build completed.", true);
