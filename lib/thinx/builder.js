...
module.exports = class Builder {
	...
	runRemoteShell(CMD, owner, build_id, udid, notifiers) {
		...
		socket.on(build_id, function (msg) {
            console.log("BuildSocket: ", chalk.bold.red("» ") + chalk.white("BLOG: "), msg);
            // TODO: forward log to web UI, no need to process.
        });

		console.log("[OID:" + owner + "] [BUILD_STARTED] REMOTE EXEC from " + __dirname);
		...
		shell.on("exit", (code) => {
			console.log("[OID:" + owner + "] [BUILD_COMPLETED] REMOTE [builder] with code " + code);
	...
	run_build(build_id, owner, git, branch, udid, dryrun, notifiers, worker, callback) {
					...
					apienv.list(owner, (list_success, api_envs) => {
						...
						apienv.list(owner, (env_list_success, keys) => {
							...
							if (worker == false) {
								...
								this.runRemoteShell(worker, owner, build_id, udid);
							}
	...
	build(owner, build, notifiers, callback) {
		...
		devicelib.view("devicelib", "devices_by_owner", {
			...
			userlib.get(owner, (user_get_err, doc) => {
				...
				this.run_build(build_id, owner, git, branch, udid, dryrun, notifiers, this.nextAvailableWorker(), callback);
