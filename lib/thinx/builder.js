...
var Builder = (function() {
	...
	var _private = {
		...
		notify: function(notifiers, message, success) {
			...
			devicelib.get(udid, function(err, device) {
				...
				apikey.list(owner, function(success, json_keys) {
					...
					if (!success) {
						...
						_private.notify(notifiers, "API Key list failed.", false);
					...
					mkdirp(BUILD_PATH, function(err) {
						if (err) {
							...
							_private.notify(notifiers, "IO operation failed.", false);
							blog.state(build_id, owner, udid, "failed");
							callback(false, err);
							return;
							...
							_private.notify(notifiers, "Fetching repository...", true);
							...
							repository.getPlatform(XBUILD_PATH, function(success, platform) {
								...
								if (!success) {
									console.log("[builder] failed on unknown platform" +
										platform);
									_private.notify(notifiers, "Unknown platform!", false);
									blog.state(build_id, owner, udid, "failed");
								...
								apienv.list(owner,
									function(success, api_envs) {
										if (success) {
											...
											thinx_json.THINX_FIRMWARE_VERSION = REPO_NAME + ":" + git_tag;
											...
											fs.writeFile(XBUILD_PATH + "/thinx_build.json", JSON.stringify(
												...
												if (err) {
													_private.notify(notifiers, "Configuring build failed.", false);
													blog.state(build_id, owner, udid, "failed");
												...
												if (udid === null) {
													...
													_private.notify(notifiers, "Starting build failed.", false);
													blog.state(build_id, owner, udid, "failed");
													return;
												}
												...
												apienv.list(owner, function(success, keys) {
													...
													shell.on("exit", function(code) {
														...
														if (code > 0) {
															...
															_private.notify(notifiers, "Build failed.", false);
															...
															_private.notify(notifiers, "Build completed.", true);
