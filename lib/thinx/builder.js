...
module.exports = class Builder {
	...
	cleanupDeviceRepositories(owner, udid, build_id) {
		...
		if ((s_owner == null) || (s_udid == null)) {
			console.log("[error] cleanup failed for udid/owner and keep_build_id", udid, owner, build_id);
			return;
		}
	...
	run_build(br, notifiers, callback, transmit_key) {
		...
		devicelib.get(udid, (err, device) => {
			...
			this.getLastAPIKey(owner, udid, (success, api_key) => {
				...
				if (first_git_success === false || first_git_success == []) {
				...
				_platform.getPlatform(XBUILD_PATH, (get_success, platform) => {
					...
					if (isYAML) {
						...
						// TODO: Extract with boolean retval where it may fail causing callback
