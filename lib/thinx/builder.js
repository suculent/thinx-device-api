...
module.exports = class Builder {
	...
	run_build(build_id, owner, git, branch, udid, dryrun, notifiers, callback) {
		...
		blog.log(build_id, owner, udid, "Build started..."); // may take time to save, initial record to be edited using blog.state
				...
				*/
				...
				_platform.getPlatform(XBUILD_PATH, (get_success, platform) => {
					...
					if (isYAML) {
						...
						if (typeof (yml) !== "undefined") {
							...
							if (typeof(device_specific_environment) !== "undefined") {
								// TODO: Export environment variables into environment.yml file to be imported by the builder for generating the file name defined by "keyguru.h"
								// file name should be ${custom_target} resp. `custom.target` String from thinx.yml (if any, otherwise will not work).
								
								// let filename = yml["custom"].target;
								//let content = YAML.

								let yamlStr = yaml.safeDump(device_specific_environment);
								fs.writeFileSync('environment.yml', yamlStr, 'utf8'); // or append to thinx.yml
								console.log(yamlStr);

								/*
								custom: 
									target: src/keyguru.h # should write following data to this file as CUSTOM_DATA_SSID... etc.
									data:
										ssid: KGR2
										pass: nadvojku
										role: master
								...
								*/
								...
								// TODO: create YML and save to filename
							}
					...
					apienv.list(owner, (list_success, api_envs) => {
						...
						let dry_run = (dryrun === true) ? " --dry-run" : "";
	...
	build(owner, build, notifiers, callback) {
		...
		devicelib.view("devicelib", "devices_by_owner", {
			...
			userlib.get(owner, (user_get_err, doc) => {
				...
				devicelib.atomic("devicelib", "modify", device.udid, device, (mod_error) => {
					if (mod_error) {
						console.log(mod_error);
