...
module.exports = class Builder {
	...
	processShellData(owner, build_id, udid, notifiers, data) {
		...
		if (logline.length > 1) { // skip empty lines in log
			let console_logline = logline.replace("\n\n", '\n'); // strip duplicate newlines in log, ignore lint warnings here
			console_logline = console_logline.replace("\n", ''); // strip last newline because console.log adds one...
			console.log("[" + build_id + "] »»", console_logline);
