...
module.exports = class Builder {
	...
	runShell(CMD, owner, build_id, udid, notifiers) {
		...
		shell.stdout.on("data", (data) => {
			...
			// causes document update conflicts... wrong way, delete everything and use flat file instead?
			// blog.log(build_id, owner, udid, logline);
		...
		shell.on("exit", (code) => {
			...
			if (code > 0) {
				...
				blog.state(build_id, owner, udid, "success"); // final success state
	...
	run_build(build_id, owner, git, branch, udid, dryrun, notifiers, callback) {
		...
		devicelib.get(udid, (err, device) => {
			...
			this.getLastAPIKey(owner, udid, (success, api_key) => {
				...
				blog.state(build_id, owner, udid, "started"); // initial state
				...
				const git_success = fs.existsSync(BUILD_PATH + "/*");
				console.log("Initial prefetch successful? : " + git_success);
				if ( git_success == false || git_success == [] ) {
					if (!sources.gitFetchWithKeys(owner, SHELL_FETCH)) {						
						blog.state(build_id, owner, udid, "failed");
						return;
				    }
				} else {
				...
				repository.getPlatform(XBUILD_PATH, (get_success, platform) => {
