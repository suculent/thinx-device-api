...
module.exports = class GDPR {
    ...
    guard() {
        ...
        if (process.env.ENVIRONMENT == "test") {
            cron_rule_15_min = "0/10 * * * * *"; // seconds
            cron_rule_daily = "0/10 * * * * *"; // seconds
        }
    ...
    purgeOldUsers(opt_callback) {
        ...
        this.userlib.view("users", "owners_by_id", {
            ...
            for (let index in body.rows) {
                ...
                let req = { query: { mindate: d, doc: doc } };
                this.userlib.atomic("users", "delete_expired", doc._id, req, (error, response) => {
                    if (error) {
