...
module.exports = class Sanitka {
	...
	branch(input) {
		...
		if (input !== sanitized_branch) {
			console.log(`[error] sanitizing branch failed because '${input}' does not match output '${sanitized_branch}'`);
			return null;
		}
	...
	url(input) {
		...
		console.log("URL not sanitized, contains invalid characters.");
		return null;
	}
