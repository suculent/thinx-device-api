...
module.exports = class Sanitka {
	...
	branch(input) {
		...
		if (input !== sanitized_branch) {
			console.log(`☣️ [error] sanitizing branch failed because '${input}' does not match output '${sanitized_branch}'`);
			return null;
		}
	...
	url(input) {
		...
		console.log("☣️ [error] URL not sanitized, contains invalid characters.");
		return null;
	}
	...
	udid(input) {
		...
		if (valid_udid && (input === sanitized_branch)) {
			...
			console.log("☣️ [error] UDID RegEx and replace failed:", input, sanitized_branch);
		}
		return null;
	}
	...
	owner(input) {
		if (typeof (input) === "undefined") {
			console.log("☣️ [error] Input owner undefined in sanitizer!"); // should throw to debug the stack
		}
		...
		if (valid) {
			...
			console.log("☣️ [error] owner invalid", { input });
		}
		return false;
	}
