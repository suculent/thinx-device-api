...
module.exports = class Sanitka {
	...
	branch(input) {
		...
		if (input.indexOf(sanitized_branch) === 0) return sanitized_branch;
		return null;
	}
	...
	url(input) {
		...
		output = output.replace(/;/g, "");
		output = output.replace(/&/g, "");
		if (input.indexOf(output) === 0) return output;
		return null;
	}
	...
	username(input) {
		...
		sanitized_username = sanitized_username.replace(/&/g, "");
		sanitized_username = sanitized_username.replace(/@/g, "");
		if (input.indexOf(sanitized_username) === 0) return sanitized_username;
		return null;
	}
	...
	deescape(url) {
		...
		sanitized_url = sanitized_url.replace(/;/g, "");
		if (url.indexOf(sanitized_url) === 0) return sanitized_url;
		return null;
	}
	...
	pushToken(token) {
		...
		if (token.length == 64) {
			console.log("[debug] validating as iOS token");
			let valid_ios = /^([a-f0-9]{64,})$/.test(sanitized_token);
			if (valid_ios == sanitized_token) return sanitized_token;
		} else {
			console.log("[debug] validating as Android token");
			let valid_android = /^([a-zA-Z0-9_:\-]+)$/.test(sanitized_token);
			if (valid_android == sanitized_token) return sanitized_token;
		}
