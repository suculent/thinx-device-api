...
module.exports = class Sanitka {
	...
	branch(input) {
		...
		sanitized_branch = sanitized_branch.replace(/;/g, "");
		sanitized_branch = sanitized_branch.replace(/&/g, "");
		if (input.indexOf(sanitized_branch) === -1) {
			console.log("[warning] sanitized branch failed with", input, sanitized_branch);
			return null;
		}
	...
	pushToken(token) {
		...
		if (token.length == 64) {
			//console.log("[debug] validating", token, "as iOS token");
			...
			//console.log("[debug] validating", token, "as Android token");
