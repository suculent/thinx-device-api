...
class Plugins {
  ...
  async load(plugin) {
    ...
    try {
      ...
      console.log(`[info] [plugins] Loaded: '${plugin}'`);
    } catch (e) {
  ...
  async use(path) {
    ...
    return results[results.length-1]; // TODO: In case there are multiple platforms, preference or order should be dealt with
  }
}
