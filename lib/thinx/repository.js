...
module.exports = class Repository {
	...
	process_hook(info) {
		...
		var repositories = Repository.findAllRepositoriesWithFullname(full_name);
	...
	getPlatformFromPath(local_path) {
		...
		yml_path.replace("//thinx.yml", "/thinx.yml");
	...
	check_git_dir_valid(local_path, owner_id, check_callback) {
		...
		if (!fs.lstatSync(local_path).isDirectory() || (git_dirs.length === 0)) {
			console.log("checkRepositoryChange: This is not a git repo: " + __dirname + "(localpath: " + local_path + "/.git )");
			this.checkCallbackWithParams(check_callback, {
				local_path: local_path,
				version: "n/a",
				revision: "n/a",
	...
	safe_git_path(unsanitized_path, repeat, check_callback) {
		...
		if (!check_local_path_early_exit(local_path, check_callback)) return false;

		// now the gitdir should be really valid
		if (!check_git_dir_valid(local_path, owner_id, check_callback)) return false;

		if (!check_git_dir_exists(local_path, check_callback)) return false;
	...
	watch() {
		...
		console.log("[repository.js:watch] » Starting User Webhook Listener on port 9001");
		if (this.gh == null) {
