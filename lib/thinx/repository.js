	...
	var _public = {
		...
		getPlatform: function(local_path, callback) {
			...
			function getINOS(srcpath) {
				...
				allfolders = allfolders.filter(file => file.indexOf(".git" === -1));

				var folder = null;
				for (var findex in allfolders) {
					// TODO: Find all INO files...
					var dirpath = srcpath + "/" + allfolders[findex];
					var inos = fs.readdirSync(dirpath)
						.filter(file => (fs.lstatSync(path.join(dirpath, file)).indexOf(
							".ino") !== -1));
