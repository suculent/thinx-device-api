	...
	var _public = {
			...
			githooked("push", "cd " + local_path + "; git pull", {
				...
				middleware: [
					...
					function(req, res, next) {
						// Do something
						console.log("githooked middleware function called for " +
							local_path);
		...
		checkRepositoryChange: function(local_path, repeat, check_callback) {
			...
			if (!fs.existsSync(gitdir)) {
				console.log("[REPOSITORY][ERROR] Not a git repo (1): " + gitdir);
				return;
			}
		...
		getPlatform: function(local_path, callback) {
			...
			var directories = fs.readdirSync(local_path).filter(
				file = fs.lstatSync(path.join(local_path, file)).isDirectory() // FIXME! =>
			);
			...
			function getINOS(srcpath) {
				return fs.readdirSync(srcpath)
					.filter(file = fs.lstatSync(path.join(srcpath, file)).isDirectory()); // FIXME! =>
			}

			var results = [];

			for (var directory in directories) {
				results.push(getINOS(dirctory));
			}
