...
module.exports = class Repository {
	...
	purge_old_repos_with_full_name(repositories, name) {
		...
		for (let index in device_folders) {
			...
			if (latest_in_device_folder === null) {
				continue;
			}
			...
			src.withRepository(info, local_path, (source_ids) => {
				...
				sources.list(owner_id, (/*success, repos*/) => {
					for (let source_id of source_ids) {
						...
						/* TODO FIXME, but where do we get the signature from?
						...
						if (signature !== null) {
							if (!this.validateSignature(signature, info, repo.secret)) {
								...
								continue; // next source_id, this one is signed but signature does not compute
							}
						}
						*/
