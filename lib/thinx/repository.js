...
var Repository = (function() {
	...
	var _public = {
		...
		checkRepositoryChange: function(local_path, repeat, check_callback) {
			...
			// Search for a git repository or bail...
			...
			if (!fs.existsSync(gitdir)) {
				...
				// find inner path with git folder and break
				for (var dindex in directories) {
					var inner_path = REPO_PATH + "/" + directories[dindex] + "/.git";
					if (fs.lstatSync(inner_path).isDirectory()) {
						local_path = REPO_PATH + "/" + directories[dindex];
						gitdir = inner_path;
						break;
					}
				}
