...
var Repository = (function() {
	...
	var _public = {
		...
		watchRepository: function(local_path, callback) {
			...
			this.checkRepositoryChange(local_path, true, callback);
		},
		...
		checkRepositoryChange: function(local_path, repeat, callback) {
			...
			if (temp.indexOf(nochange) !== -1) {
				...
				console.log("git-watch: " + local_path + " : no changes");
				...
				if (repeat === false) {
					...
					if (typeof(this.watcher) !== "undefined" || this.watcher !== null) {
						...
						var timeoutFunction = function(local_path, repeat, callback) {
							console.log("timeoutFunction for " + local_path + " called");
