...
var Repository = (function() {
	...
	function process_hook(info) {
		...
		for (var rindex in repositories) {
			...
			_checkRepositoryChange(repositories[rindex], false, this.exit_callback);
		}
	}

	function init_githooked(gh) {
	...
	function filterGitDirs(all_dirs, owner) {
		var git_dirs = [];
	...
	function _watcherTimeoutFunction(local_path, repeat, callback) {
		return function() {
			_checkRepositoryChange(local_path, false, callback);
		};
	}

	function _checkRepositoryChange(local_path, repeat, check_callback) {
		...
		var all_dirs = finder.from(local_path).showSystemFiles().findDirectories(
			"\/.git");
		var git_dirs = filterGitDirs(all_dirs, owner_id);
		...
		if (temp.indexOf(nochange) !== -1 && repeat) {
			...
			if (typeof(this.watcher) !== "undefined" || this.watcher !== null) {
				...
				this.timer = setTimeout(_watcherTimeoutFunction(
					local_path,
					repeat,
	...
	var _public = {
		...
		watch: function() {
			...
			if (!gh) {
				...
				gh = githooked("push", process_hook);
				init_githooked(gh);
			}
		...
		watchRepository: function(local_path, callback) {
			...
			if (typeof(callback) === "undefined") {
				console.log(
					...
					_checkRepositoryChange(local_path, true, function(status, response) {
					callback(status, response);
				});
			} else {
				console.log("Starting callback repository watcher on " + local_path);
				_checkRepositoryChange(local_path, true, this.exit_callback);
			}
		...
		checkRepositoryChange: function(local_path, repeat, check_callback) {
			return _checkRepositoryChange(local_path, repeat, check_callback);
		},
