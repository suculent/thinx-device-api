...
module.exports = class Repository {
	...
	process_hook(info) {
		...
		for (let index in device_folders) {
			...
			let latest_in_device_folder = getMostRecentDirectory(device_folder);

			console.log("latest_in_device_folder", device_folder);

			let all_in_device_folder = getDirectories(device_folder);

			console.log("all_in_device_folder", device_folder);

			for (let index in all_in_device_folder) {
				let folder = all_in_device_folder[index];
				if (!folder.indexOf(latest_in_device_folder)) {
		...
		console.log("[repository.js:watch] Kept " + repositories.length + " repositories with name: " + name);
		...
		for (var rindex = 0; rindex < repositories.length; rindex++) {
			...
			let local_path_array = local_path.split("/");
			let device_path = local_path_array.splice(local_path_array.length-2, 2); // remove last two elements to get device path (removes repo_name and build_id inside device folder)
			
			console.log("device path:", device_path);

			let latest = getMostRecentDirectory(device_path);

			console.log("Selected latest (should be per device!)", latest);

			if (local_path.indexOf(latest) === 0) {
				...
				if (!owner_ids.includes(owner_id)) {
					...
					console.log("owner not found", owner_id, "in", owner_ids );
				}
			} else {
				console.log("NOT Removing deprecated repo", local_path);
				/* This deletes still too much...
				fs.rmdir(local_path, { recursive: true }, (err) => {
					if (err) {
					  reject(err);
					}
