...
module.exports = class Repository {
	...
	process_hook(info) {
		...
		for (var rindex = 0; rindex < repositories.length; rindex++) {
			...
			console.log("Webhook in", local_path, "for", info.ref);			
			...
			if (owner_id == null) {
				...
				if (!owner_ids.includes(owner_id)) {
					//console.log("Adding owner_id", owner_id);
					//owner_ids.push(owner_id);

					let source_ids = sources.withRepository(info, local_path); // should return an array (even empty)
