...
var Repository = (function() {
	...
	var _public = {
		...
		checkRepositoryChange: function(local_path, repeat, check_callback) {
			...
			if (!fs.lstatSync(local_path + "/.git").isDirectory()) {
		...
		getRevisionNumber: function() {
			// FIXME: Parametrize, otherwise fails on redeploy
			CMD = "cd /var/root/thinx-device/api; git rev-list HEAD --count";
			var temp = exec.execSync(CMD).toString().replace("\n", "");
		...
		getRevision: function() {
			// FIXME: Parametrize, otherwise fails on redeploy
			CMD = "cd /var/root/thinx-device/api; git rev-parse HEAD";
			var temp = exec.execSync(CMD).toString().replace("\n", "");
			this.revision = temp;
			return temp;
		}
