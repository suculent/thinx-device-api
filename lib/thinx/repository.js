	...
	var _public = {
		...
		watch: function() {
			if (!isMasterProcess()) {
				...
				console.log("» Starting Git Webhook on slave instance: " + process.env.NODE_APP_INSTANCE);

			} else {

				console.log("» Starting Git Webhook Listener on port 9001, instance: " + process.env.NODE_APP_INSTANCE);
		...
		checkRepositoryChange: function(local_path, repeat, check_callback) {
			...
			var CMD = "bash -c \"cd " + local_path +
				"; git status; ls -la; git reset --hard; git pull origin master --recurse-submodules\"";
		...
		getPlatform: function(local_path, callback) {
			...
			console.log("found inos: "+JSON.stringify(inos));
