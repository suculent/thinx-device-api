...
var Repository = (function() {
	...
	var gh = githooked("push", function(info) {
		...
		repositories = finder.from(repositories_path).showSystemFiles().findDirectories(
			full_name +
			"/.git");
	...
	var _public = {
		...
		checkRepositoryChange: function(local_path, repeat, check_callback) {
			...
			var git_dirs = finder.from(local_path).showSystemFiles().findDirectories(
				".git");

			console.log("git_dirs found: " + JSON.stringify(git_dirs));
			...
			if (!fs.existsSync(gitdir)) {
				...
				for (var dindex in directories) {
					...
					if (fs.lstatSync(inner_path).isDirectory()) {
						...
						gitdir = inner_path;
						console.log("setting git dir to " + inner_path);
						break;
					}
				}
			}
			...
			var CMD = "bash -c \"cd " + local_path +
				"; git pull; pushd *; git submodule update --init --recursive; popd\"";
