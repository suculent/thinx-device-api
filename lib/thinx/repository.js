...
module.exports = class Repository {
	...
	validateSignature(signature, body, secret) {
		...
		// consider valid if not set; nothing to compare
	...
	process_hook(req) {
		...
		for (let local_path of repositories) {
			...
			sources.withRepository(info, local_path, (source_ids) => {
				...
				sources.list(owner, (success, repos) => {
					for (let source_id of source_ids) {
						...
						if (signature !== null) {
							if (!validateSignature(signature, info, repo.secret)) {
								console.log("[error] GitHub Secret validation failed for repo", { repo });
