	...
	var _public = {
		...
		watch: function() {
			if (!isMasterProcess()) {
				...
				console.log("[repository.js:watch] » Starting Git Webhook on slave instance: " + process.env.NODE_APP_INSTANCE);
				...
				if (typeof(process.env.CIRCLE_USERNAME) === "undefined") {
					...
					var gh = githooked("push", function(info) {
						...
						console.log("[repository.js:watch] found repos: " + JSON.stringify(repositories));
					...
					try {
						gh.listen(9001); // this is USER GIT webhook!
					} catch (e) {
		...
		checkRepositoryChange: function(local_path, repeat, check_callback) {
			...
			//var CMD = shellescape(["bash", "-c", "cd", local_path + "; git status; ls; git reset --hard; git pull origin master --recurse-submodules"]);
			const local_path_safe = shellescape([local_path]);
			console.log("TODO: use THIS -- local_path_safe: "+local_path_safe);
			var CMD = "bash -c cd " + local_path + "; git status; ls; git reset --hard; git pull origin master --recurse-submodules";
		...
		getPlatform: function(local_path, callback) {
			...
			if (isPlatformio) {
				console.log("Switching to PlatformIO, platformio.ini found in "+local_path);
				isArduino = false;
			}
			...
			console.log("Builder-Detected platform: " + platform);
