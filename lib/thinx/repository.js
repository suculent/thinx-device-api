...
module.exports = class Repository {
	...
	constructor(optional_devices) {
		...
		this.gh = null; // set using watch() trigger
	...
	process_hook(info) {
		...
		for (var rindex in repositories) {
			...
			console.log("Need to match url/ref with source...");
			console.log(info);
	...
	init_githooked(gh) {
		...
		}).listen(app_config.webhook_port);

		githooked().on('delete', (info) => {
			console.log('githooked ref ' + info.ref + ' was deleted. TODO: Prune source.');
			this.process_hook(info);
