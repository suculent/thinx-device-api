	...
	var _public = {
		...
		watch: function() {
			if (!isMasterProcess()) {
				...
				console.log("[repository.js:watch] » Starting Git Webhook Listener on port 9001, instance: " + process.env.NODE_APP_INSTANCE);
				...
				if (typeof(process.env.CIRCLE_USERNAME) === "undefined") {
					...
					var gh = githooked("push", function(info) {
						...
						if (full_name == "suculent/thinx-device-api") {
							console.log(
								"[repository.js:watch] » Skipping own webhook in favour of deploy-hook.sh on port 9009.");
							return;
						}
