...
module.exports = class APIEnv {
	...
	create(owner, key, value, callback) {
		...
		this.client.v4.get("env:" + owner).then((existing) => {
			...
			this.client.v4.set("env:" + owner, JSON.stringify(new_vars));
	...
	fetch(owner, name, callback) {
		this.client.v4.get("env:" + owner).then((json_keys) => {
			if (!json_keys) {
	...
	revoke(owner, changes, callback) {
		...
		this.client.v4.get("env:" + owner).then((json_keys) => {
			...
			if ((json_keys == []) || (typeof (json_keys) === "undefined") || (json_keys === null)) {
				console.log("[ENVVar:revoke:error]:", json_keys);
