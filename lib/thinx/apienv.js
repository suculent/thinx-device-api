...
module.exports = class APIEnv {
	...
	create(owner, key, value, callback) {
		...
		if (typeof(this.client) === "undefined") {
			console.log("this.client does not exist yet! fixing (1)...");
			var redis = require("redis");
			this.client = redis.createClient(Globals.redis_options());
		}

		this.client.get("env:" + owner, function(ferr, json_keys) {

			// Create new owner object if nothing found
			if (ferr) {

				if (typeof(this.client) === "undefined") {
					console.log("this.client does not exist yet! fixing (3)...");
					var redis = require("redis");
					this.client = redis.createClient(Globals.redis_options());
				}
			...
			if (typeof(this.client) === "undefined") {
				console.log("this.client does not exist in this scope! fixing (2)...");
	...
	fetch(owner, name, callback) {
		this.client.get("env:" + owner, function(err, json_keys) {
	...
	revoke(owner, changes, callback) {
		...
		this.client.get("env:" + owner, function(err, json_keys) {
			...
			this.client.set("env:" + owner, JSON.stringify(new_vars), function(eerr, reply) {
				if (eerr) {
					console.log(eerr);
					callback(false, eerr);
				} else {
