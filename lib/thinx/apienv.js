	...
	var _public = {
		...
		create: function(owner, key, value, callback) {
			...
			client.get("env:" + owner, function(err, json_keys) {
				...
				console.log("env_vars old: " + JSON.stringify(env_vars));
				...
				client.set("env:" + owner, JSON.stringify(env_vars),
					function(err) {
						console.log("env_vars new: " + JSON.stringify(env_vars));
						if (err) {
							callback(false, err);
						} else {
							callback(true, env_vars);
						}
					});
		...
		revoke: function(owner, names, callback) {
			...
			client.get("env:" + owner, function(err, json_keys) {
				...
				console.log("Setting new_vars: " + JSON.stringify(new_vars));
