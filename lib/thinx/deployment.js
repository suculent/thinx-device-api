...
module.exports = class Deployment {
	...
	latestFirmwareEnvelope(owner, xudid) {
		...
		if (fs.existsSync(envpath)) { // lgtm [js/path-injection]
			var envelope = JSON.parse(fs.readFileSync(envpath));
			return envelope;
		} else {
	...
	fixAvailableVersion(version) {
		...
		if (!semver.valid(version)) {
			...
			console.log("[hasUpdateAvailable] Deployed version has unexpected semantic versioning: " + version);
			...
			console.log("[hasUpdateAvailable] Deployed version fixed to: " + version);
		}
		return version;
	}
	...
	latestFirmwarePath(in_owner, udid) {
		...
		for (var extension in extensions) {
			let xtension = extensions[extension];
			var files = finder.in(dpath).findFiles();
			if (files.length === 0) continue;
			latest_firmware = files[0];
			//console.log("Searching extension " + xtension + " firmware in files", files);
			// This does not seem to work, providing first valid file...
	...
	hasUpdateAvailable(device) {
		...
		console.log("Getting firmware version for owner ", owner, deviceVersion);
		...
		console.log("semver compare", deviceVersion, available_version, "outdated:", outdated);
		if (outdated) {
			// deviceVersion < availableVersion
			console.log("[hasUpdateAvailable] Device version ", deviceVersion, " is outdated against ", available_version);
		} else {
			// deviceVersion == availableVersion
			...
			if (semver.eq(deviceVersion, available_version)) {
				...
				// deviceVersion > availableVersion; nop
