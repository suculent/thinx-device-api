...
var Deployment = (function() {
	...
	var _private = {
		...
		getFiles: function(dir, files_) {
			...
			console.log("[getFiles] from directory: " + dir);
	...
	var _public = {
		...
		latestFirmwarePath: function(owner, udid) {
			...
			for (var index in files) {
				...
				//console.log("[LFP] supported_extensions: " + JSON.stringify(supported_extensions));
			...
			if (latest_firmware === null) {
				console.log(
					"latestFirmwarePath returns: null; no firmware update available for this device"
				);
			} else {
				console.log("latestFirmwarePath returns: " + latest_firmware);
			}
		...
		latestFirmwareEnvelope: function(owner, udid) {
			...
			if (path) {
				...
				//console.log("[LFP] supported_extensions: " + JSON.stringify(
				//supported_extensions));

				for (var xindex in supported_extensions) {
					var extension = "." + supported_extensions[xindex];
					console.log("Replacing " + extension + " with .json");
				...
				console.log("[LFP] envelope: " + JSON.stringify(
					envelope) + " at path " + envpath);
