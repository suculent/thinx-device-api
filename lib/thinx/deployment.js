...
module.exports = class Deployment {
	...
	_deployPathForOwner(owner) {
		var user_path = app_config.data_root + app_config.deploy_root + "/" +
			owner;
		return user_path;
	}

	_pathForDevice(owner, udid) {
		var user_path = this._deployPathForOwner(owner);
		var device_path = user_path + "/" + udid;
		return device_path;
	}

	_latestFirmwareEnvelope(owner, udid) {
		var path = this._pathForDevice(owner, udid);
	...
	deployPathForOwner(owner) {
		return this._deployPathForOwner(owner);
	}
	...
	getAvailableVersion(owner, udid) {
		// In case of attempt to install completely different firmware, bypasses version check...
		var envelope = this._latestFirmwareEnvelope(owner, udid);
	...
	platformSupportsUpdate(device) {
		...
		if (typeof(device.platform) !== "undefined") {
			console.log("platformSupportsUpdate searches in device platform: "+platform);
	...
	pathForDevice(owner, udid) {
		return this._pathForDevice(owner, udid);
	}

	deploymentPathForDeviceOwner(owner, udid) {
	  return this._deployPathForOwner(owner) + "/" + udid;
	}
	...
	latestFirmwarePath(in_owner, udid) {
		...
		var dpath = this._pathForDevice(owner, udid);
		...
		for (var extension in extensions) {
			...
			latest_firmware = latestFile(files);
		}
		return latest_firmware;
	}

	latestFirmwareEnvelope(owner, udid) {
		this._latestFirmwareEnvelope(owner, udid);
	}
	...
	hasUpdateAvailable(device) {
		...
		var deviceVersion = this.parseDeviceVersion(device.version);
		var available_version = this.getAvailableVersion(device.owner, device.udid);
