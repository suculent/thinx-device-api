...
module.exports = class Deployment {
	...
	latestFirmwareEnvelope(owner, xudid) {
		...
		if (envpath.indexOf(owner) !== -1) {
			if (fs.existsSync(envpath)) { // lgtm [js/path-injection]
				return JSON.parse(fs.readFileSync(envpath)); // js/path-injection
			}
		}
	...
	initWithOwner(v_owner) {
		let owner = sanitka.owner(v_owner);
		...
		mkdirp(user_path); // js/path-injection
	}
	...
	artifact(owner, udid, build_id) {
		...
		return fs.readFileSync(fpath); // js/path-injection
	}
