...
var Deployment = (function() {
	...
	var _private = {
		...
		supportsUpdate: function(device) {
			...
			console.log("Checking update supportability for " + device.platform);

			if ((typeof(device.platform) !== "undefined") && (device.platform == "arduino")) {
				return true;
			}

			if ((typeof(device.platform) !== "undefined") && (device.platform == "platformio")) {
				return true;
			}

			if ((typeof(device.platform) !== "undefined") && (device.platform == "nodemcu")) {
				return true;
			}

			if ((typeof(device.platform) !== "undefined") && (device.platform == "micropython")) {
				return true;
			}

			if ((typeof(device.platform) !== "undefined") && (device.platform == "python")) {
				return true;
			}

			if ((typeof(device.platform) !== "undefined") && (device.platform == "mongoose")) {
				return true;
			}

			if ((typeof(device.platform) !== "undefined") && (device.platform == "nodejs")) {
				return true;
			}

			if ((typeof(device.platform) !== "undefined") && (device.platform == "sigfox")) {
				return false;
			}
	...
	var _public = {
		...
		latestFirmwarePath: function(owner, udid) {
			...
			// console.log("Latest build envelope: " + JSON.stringify(envelope));
			...
			// console.log("Latest firmware path: " + latest_firmware);
		...
		latestFirmwareEnvelope: function(owner, udid) {
			...
			if (path !== null) {
				...
				if (fs.existsSync(envpath) && fs.statSync(envpath).size > 2) {
					...
					//console.log("latestFirmwareEnvelope " + envpath);
					return false;
				}
			} else {
				return false;
