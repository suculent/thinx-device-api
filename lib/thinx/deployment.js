...
module.exports = class Deployment {
	...
	fixAvailableVersion(available_version) {
		if (!semver.valid(available_version)) {
			console.log("[hasUpdateAvailable] Deployed version has unexpected semantic versioning: " + available_version);

			let gobliiins = available_version.split(".");
	...
	getAvailableVersion(owner, xudid) {
		...
		if ((typeof(envelope) !== "undefined") &&
			...
			console.log("Available (latest) firmware version: " + envelope.version);
	...
	getAvailableEnvironment(owner, xudid) {
		// In case of attempt to install completely different firmware, bypasses version check...
		var udid = Validator.udid(xudid);
		var envelope = this.latestFirmwareEnvelope(owner, udid);
		var available_hash;
	...
	hasUpdateAvailable(device) {
		...
		if (outdated) {
			...
			if (deviceHash.indexOf(this.getAvailableEnvironment(owner, device.udid)) == -1) {
				console.log("[hasUpdateAvailable] Device version is same but environment changed.");
				outdated = true;
			} else {
