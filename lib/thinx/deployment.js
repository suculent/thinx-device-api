...
module.exports = class Deployment {
	...
	latestFirmwareEnvelope(owner, xudid) {
		...
		if (fs.existsSync(envpath)) { // lgtm [js/path-injection]
			...
			console.log("[latestFirmwareEnvelope] envelope found at ", envpath, "\n", {envelope});
			return envelope;
		} else {
	...
	fixAvailableVersion(available_version) {
		if (!semver.valid(available_version)) {
			console.log(
				"[hasUpdateAvailable] Deployed version has unexpected semantic versioning: " + available_version);

			var deployment_version;
			if (available_version.indexOf(".") == -1) {
				// in case the input value has no dots, use as last integer
				deployment_version = [0, 0, available_version];
			}	else {
				// incase the input value has dots, use last (or first?) three only
	...
	latestFirmwarePath(in_owner, udid) {
		...
		for (var extension in extensions) {
			...
			console.log("Searching " + extension + " firmware in files", files);
		...
		console.log("Returning envelop path: ", latest_firmware);
		return latest_firmware;
	}
