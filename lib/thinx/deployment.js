...
module.exports = class Deployment {
	...
	_pathForDevice(owner, udid) {
		var user_path = _deployPathForOwner(owner);
		var device_path = user_path + "/" + udid;
		return device_path;
	}

	_latestFirmwareEnvelope(owner, udid) {
		var path = _pathForDevice(owner, udid);
	...
	deployPathForOwner(owner) {
		return _deployPathForOwner(owner);
	}
	...
	getAvailableVersion(owner, udid) {
		...
		var envelope = _latestFirmwareEnvelope(owner, udid);
	...
	initWithDevice(device) {
		...
		initWithOwner(device.owner, null, function(success, response) {
	...
	pathForDevice(owner, udid) {
		return _pathForDevice(owner, udid);
	}

	deploymentPathForDeviceOwner(owner, udid) {
	  return _deployPathForOwner(owner) + "/" + udid;
	}
	...
	latestFirmwarePath(in_owner, udid) {
		...
		var dpath = _pathForDevice(owner, udid);
	...
	hasUpdateAvailable(device) {
		...
		if (platformSupportsUpdate(device) === false) {
