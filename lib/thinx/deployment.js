...
var Deployment = (function() {
	...
	var _public = {
			...
			for (var index in files) {
				...
				if (valid) {
					...
					if (mtime > latest_date) {
						...
						console.log("latestFirmwarePath: skipping old file " + filename);
					}
				} else {
					console.log("latestFirmwarePath: invalid file " + filename);
				}
			}
		...
		latestFirmwareEnvelope: function(owner, udid) {
			...
			if (path) {
				var envpath = path.replace(".bin", ".json"); // FIXME: does not work with non-binary updates!
