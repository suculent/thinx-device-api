...
module.exports = class Deployment {
	...
	latestFirmwareEnvelope(owner, xudid) {
		...
		if (!fs.existsSync(path)) { // lgtm [js/path-injection]
			if (debug_deployment) {
				...
				if (typeof (udid) === "undefined") {
					console.log("[warning] Warning, device identifier undefined!");
				}
			}
			return false;
		}
		...
		if (typeof (path) === "undefined" || (path === null)) {
			console.log("[error] LFE path undefined.");
			return false;
		}
		...
		console.log("[info] Device", udid, "has no firmware available.");
		return false;
	}
	...
	getAvailableVersion(owner, xudid) {
		...
		if ((typeof (envelope) !== "undefined") &&
			...
			console.log("[info] Latest Available Firmware for", xudid, "is", envelope.version);
	...
	getAvailableEnvironmentHash(owner, xudid) {
		...
		if ((typeof (envelope) !== "undefined") &&
			...
			console.log("[info] Available (latest) firmware env_hash: " + envelope.env_hash);
			available_hash = envelope.env_hash;
		}

		if ((typeof (envelope) !== "undefined")) {
			console.log("[info] getAvailableEnvironmentHash envelope", envelope);
		}
	...
	parseDeviceVersion(deviceVersion) {
		...
		if (!pattern_valid) {
			console.log("[warning] [parseDeviceVersion] Device version invalid: " + deviceVersion);
		}
		...
		if (!semver.valid(deviceVersion)) {
			...
			console.log("[warning] [parseDeviceVersion] Invalid semantic versioning in: " + deviceVersion);
			deviceVersion = device_version.join(".");
			console.log("[warning] [parseDeviceVersion] Semantic versioning changed to: " + deviceVersion);
		}
	...
	initWithDevice(device) {
		...
		this.initWithOwner(device.owner, null, (success, response) => {
			console.log("[info] initWithDevice success: " + success + " response " +
				response);
		});
	}
	...
	supportedExtensions() {
		// TODO (9): Extend for other 'platforms' automatically using plugin architecture
		// https://github.com/suculent/thinx-device-api/issues/302
		return ['*.bin', '*.py', '*.lua'];
	}

	latestFirmwarePath(in_owner, udid) {
		if ((typeof (in_owner) === "undefined") || (typeof (udid) === "undefined") || typeOf(in_owner) == "object") {
			console.log(`[error] invalid LFP owner ${in_owner} with udid: ${udid}`);
			return false;
		}
		...
		if (!fs.existsSync(fpath)) { // lgtm [js/path-injection]
			console.log(`[error] Envelope ${fpath} not found.`);
			return false;
		}
	...
	hasUpdateAvailable(device) {
		...
		if (typeof (available_version) === "undefined") {
			if (debug_device) console.log("[info] No firmware update available.");
			return false;
		}
		var outdated = semver.lt(deviceVersion, available_version);

		if (outdated) {
			console.log("[info] Device has:", deviceVersion, ", update available to:", available_version);
		} else {
			if (semver.eq(deviceVersion, available_version)) {
				// versions equal, update may happen
				if (debug_device) console.log("[info] Device version is up-to-date.");

				const deviceHash = device.env_hash;
				if (typeof (deviceHash) !== "undefined" && deviceHash !== null) {
					if (deviceHash.indexOf("cafebabe") === 0) {
						console.log("[warning] Device has default environment, will not perform update (dev version).");
					} else {
						if (deviceHash.indexOf(this.getAvailableEnvironmentHash(owner, device.udid)) == -1) {
							console.log("[info] Device version is same but environment changed -> should provide update.");
							outdated = true;
						}
					}
				}
			} else {
				console.log("[info] Device version is newer than available.");
			}
