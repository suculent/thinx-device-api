...
module.exports = class Deployment {
	...
	fixAvailableVersion(version) {
		...
		if (!semver.valid(version)) {
			...
			console.log("[info] [hasUpdateAvailable] Deployed version collapsed: " + version);
		}
		return version;
	}
	...
	initWithOwner(v_owner) {
		...
		if (typeof (user_path) === "undefined" || user_path === null) {
			console.log("☣️ [error] deployment.js: No deploy path with owner " + owner);
			return;
		}
	...
	latestFirmwarePath(in_owner, udid) {
		var owner = in_owner;
		// TODO (10) FIXME: Find out, why this happens (in_owner is whole object instead of identifier) - log callee
		// https://github.com/suculent/thinx-device-api/issues/303
		if (typeOf(in_owner) == "object") {
			console.log("[debug] Suspicious owner: " + JSON.stringify(in_owner));
			rollbar.info("Suspicious owner", in_owner, udid);
			owner = in_owner.owner;
		}
		if ((typeof (owner) === "undefined") || (typeof (udid) === "undefined")) {
			console.log("[error] latestFirmwarePath: invalid owner: " + owner + " udid: " + udid);
			return false;
		}
		var latest_firmware = false;
		var dpath = this.pathForDevice(owner, udid);
		if (!fs.existsSync(dpath + "/build.json")) { // lgtm [js/path-injection]
			console.log("[warning] Envelope not found.");
			return false;
		}
		const enve = fs.readFileSync(dpath + "/build.json"); // lgtm [js/path-injection]
		var envelope = JSON.parse(enve);
		console.log("[info] LFP envelope: " + JSON.stringify(envelope, false, 2));
