...
var Deployment = (function() {
	...
	var _private = {
		...
		getNewestFolder: function(dir, regexp) {
			...
			if (!fs.existsSync(dir)) {
				dir = dir + "/";
				if (!fs.existsSync(dir)) {
					console.log("No device directory name/ found when trying to fetch newest build for device.");
					return null;
				}
				console.log("No device directory name found when trying to fetch newest build for device.");
				return null;
	...
	var _public = {
		...
		latestFirmwareEnvelope: function(owner, udid) {
			...
			// No device build succeeded yet...
			if (!fs.existsSync(envpath)) {
				console.log("[latestFirmwareEnvelope] pathForDevice does not exist yet... no build envelope available.");
				return false;
			}

			console.log("[latestFirmwareEnvelope] searching for newest folder started at " + new Date().getMilliseconds());

			var path = _private.getNewestFolder(device_path, "*");

			console.log("[latestFirmwareEnvelope] searching for newest folder ended at " + new Date().getMilliseconds());
