...
var Deployment = (function() {
	...
	var _public = {
		...
		latestFirmwareVersion: function() {
			//console.log(this.latestFirmwareEnvelope());
			return this.latestFirmwareEnvelope().version;
		},

		latestFirmwareEnvelope: function() {
			var path = this.latestFirmwarePath().replace(".bin", ".json"); // todo: replace only extension
		...
		hasUpdateAvailable: function(device) {
			...
			if (!semver.valid(deviceVersion)) {
				console.log("deviceVersion semver invalid:" + deviceVersion);
			}
