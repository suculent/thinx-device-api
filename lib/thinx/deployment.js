...
var Deployment = (function() {
	...
	var _public = {
			...
			if (!fs.existsSync(path)) { // No device build succeeded yet...
				if (debug_deployment) {
					if (typeof(owner) === "undefined") {
						throw "Warning, device owner undefined!";
					}
		...
		hasUpdateAvailable: function(device) {
			...
			/* causes repated update after update when device data are not saved...
			var device_flavour = device.firmware;
			var device_flavour_array = device_flavour.split(":");

			var firmware_flavour = envelope.firmware; // fallback to older firmware only
			...
			*/
			...
			if (typeof(available_version) === "undefined") {
				...
				console.log("[hasUpdateAvailable] Available version: " + available_version);
			}
			...
			if (semver.lt(deviceVersion, available_version)) {
				...
				console.log("[hasUpdateAvailable] Device version is latest.");
				return false;
			}
		}
	};
