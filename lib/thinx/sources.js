	...
	var _public = {
		...
		add: function(owner, alias, url, branch, callback) {
			...
			mkdirp(REPO_PATH, function(err) {
				...
				var CMD = "set +e; cd " + OWNER_PATH + "; if $(git clone --recurse-submodules -b " + origin_branch +
					" " + url +
					"); then cd " + REPO_PATH + " && echo { \"basename\":\"$(basename $(pwd))\", \"branch\":\"" +
					origin_branch + "\" } > basename.json; fi";

				if (branch === null) {
					CMD = "set +e; cd " + OWNER_PATH + "; if $(git clone --recurse-submodules " + url +
						"); then cd " + REPO_PATH + " && echo { \"basename\":\"$(basename $(pwd))\", \"branch\":\"" +
