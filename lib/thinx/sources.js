...
module.exports = class Sources {
	...
	upsertOwnerSource(doc, callback) {
		...
		this.userlib.get(doc._id, (gerror, body) => {
			if (gerror) {
			...
			this.userlib.atomic("users", "edit", doc._id, changes, (error, upsert_body) => {
	...
	*/
	...
	add(source, callback) {
		...
		var PREFETCH_CMD = "set +e; mkdir -p " + TEMP_PATH + "; cd " + TEMP_PATH + ";" +
