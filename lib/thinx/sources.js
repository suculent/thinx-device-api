	...
	*/
	...
	normalizedBranch(source, error_callback) {
		...
		if (sanitized_branch === null) {
			error_callback(false, "invalid branch name"); return false;
		}
	...
	validateURL(source, error_callback) {
		...
		//console.log("[validateURL] sanitized_url 1:", sanitized_url);
		if (sanitized_url !== null) {
			//console.log("[validateURL] sanitized_url 4:", sanitized_url);
			if (sanitized_url != source.url) {
				//console.log("[validateURL] sanitized_url 5:", sanitized_url);
	...
	add(source, callback) {
		...
		console.log("Prefetch with sanitized url and branch", sanitized_url, sanitized_branch); // todo: remove me or change to proper log
