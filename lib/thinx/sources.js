	...
	var _public = {
		...
		add: function(owner, alias, url, branch, callback) {
			...
			mkdirp(REPO_PATH, function(err) {
				...
				var CMD = "cd " + REPO_PATH + "; git clone -b " + branch + " " + url +
					"; cd *; echo { \"basename\":\"$(basename $(pwd))\" }"; //  > basename.json; cat basename.json

				// FIXME: Find last '{' and read to the end as JSON...
				var result = exec.execSync(CMD).toString().replace("\n", "");
				var REPO_INNER_PATH = REPO_PATH; // FIXME: Must be last line of what CMD returns
				...
				watcher.checkRepositoryChange(REPO_INNER_PATH, false, function(err,
					...
					source.platform = watcher.getPlatform(repo_path);
