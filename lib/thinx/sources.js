...
module.exports = class Sources {
	...
	*/
	...
	updatePlatform(owner, source_id, platform, complete_callback) {
		...
		this.userlib.get(owner, (err, doc) => {
			console.log("UpdateupdatePlatformPrivate fetched doc", doc);
			if (err) {
				console.log(err);
	...
	updatePrivate(owner, source_id, is_private, complete_callback) {
		this.userlib.get(owner, (err, doc) => {
			...
			this.userlib.destroy(doc._id, doc._rev, (/* destroy_err */) => {
				delete doc._rev;
				doc.repos[source_id].is_private = is_private;
				this.userlib.insert(doc, doc._id, (insert_err, /* body, header */) => {
					if (insert_err) {
						console.log("updatePrivate ERROR:", err, "to:", platform);
						complete_callback(false, insert_err);
						return;
					}
					complete_callback(true);
				});
