	...
	upsertOwnerDocumentRepos(doc, callback) {
		...
		this.userlib.get(doc._id, (error, /* body */) => {
			...
			this.userlib.atomic("users", "edit", doc._id, changes, (eerror, upsert_body) => {
				...
				this.upsertOwnerDocumentRepos(doc, (error, response) => {
	...
	*/
	...
	list(owner, callback) {
		if (typeof (owner) === "undefined") {
			return callback(false, "user_not_defined");
		}
		this.userlib.get(owner, (err, doc) => {
			if (err) {
				console.log("[error] this.userlib.get error", err);
				return callback(false, err);
			}
			if (doc) {
				callback(true, doc.repos);
			} else {
				console.log("[warning] Owner " + owner + " not found.");
	...
	withRepository(info, local_path, callback) {
		...
		this.userlib.get(owner_id, (err, doc) => {
			if ((err != null) || (typeof (doc) === "undefined")) {
				console.log("[error] doc is undefined in withRepository() for owner", owner_id, err);
				return callback([]);
			}
			var source_ids = [];
	...
	*/
	...
	add(source, callback) {
		...
		if (typeof (source) === "undefined") {
			return callback(false, "source_undefined");
		}

		if (typeof (source.owner) === "undefined") {
			return callback(false, "source_owner_undefined");
		}
