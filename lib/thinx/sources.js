	...
	var _public = {
		...
		add: function(owner, alias, url, branch, callback) {
			...
			mkdirp(REPO_PATH, function(err) {
				...
				var CMD = "cd " + OWNER_PATH + "; git clone --recurse-submodules -b " + origin_branch +
					" " + url +
					"; cd " + REPO_PATH + "; echo { \"basename\":\"$(basename $(pwd))\", \"branch\":\"" +
					origin_branch + "\" } > basename.json";

				if (branch === null) {
					CMD = "cd " + OWNER_PATH + "; git clone --recurse-submodules " + url +
						"; cd " + REPO_PATH + "; echo { \"basename\":\"$(basename $(pwd))\", \"branch\":\"" +
				...
				var files = fs.readdirSync(REPO_PATH).filter(
						file => file.indexOf("basename") !== -1
					);

				// try to solve access rights issue by using owner keys...
				
				if ( ( files.count < 2 ) || (result.indexOf("Please make sure you have the correct access rights") !== -1) ) {
