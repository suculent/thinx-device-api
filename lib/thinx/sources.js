...
module.exports = class Sources {
	...
	*/
	...
	ownerIdFromPath(local_path) {
		let owner_path = local_path.replace(app_config.data_root, "");
		owner_path = owner_path.replace(app_config.build_root, "");
		...
		console.log("owner id", owner_id, "from array", {owner_path_array}); // debug, delete
		return owner_id;
	}

	withRepository(info, local_path) {

		let repo_name = info.repository.name; // should be full_name!
		let repo_branch = info.ref.replace("refs/heads/", "origin"); // WTF?
		var source_ids = [];
		...
		userlib.get(owner_id, (err, doc) => {
			...
			console.log("debug owner sources in githook, remove after use: ", sources); // debug, remove...

			let keys = Object.keys(sources);
			console.log("sources keys:", keys);
			...
			for (let i = 0; i < keys.length; i++) {
				...
				if (source.owner.indexOf(owner_id) == -1) {
					// console.log("has different owner_id, skipping...");
					console.log("owner_id does not match");
					continue;
				}
				if (source.url.indexOf(repo_name) == -1) {
					// console.log("repo_name", repo_name, "not found in url", source.url, "skipping...");
					console.log("repo_name does not match");
					continue;
				}
				if (source.branch.indexOf(repo_branch) != -1) {
					console.log("adding key:", key, "of source", source);
					source_ids.push(key);
				} else {
					console.log("branch does not match");
				}
			}
			console.log("final source_ids:", {source_ids});
	...
	*/
	...
	add(source, callback) {
		...
		const OWNER_ROOT = app_config.data_root + app_config.build_root + "/" + source.owner;
		var TEMP_PATH = OWNER_ROOT + "/" + temp_id;
