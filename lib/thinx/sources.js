...
module.exports = class Sources {
	...
	constructor() {
		...
		this.devicelib = require("./couch")(db_uri).use(prefix + "managed_devices");
	...
	*/
	...
	//
	// The old form was `if $(git clone ...)` -- command substitution, which
	// executed the OUTPUT of git clone as a command. It is a plain `if git ...`.
	static prefetchCommand(TEMP_PATH, sanitized_branch, sanitized_url) {
		const escapedPath = shellEscape([TEMP_PATH]);
		...
		return (
			...
			`if git clone -b ${escapedBranch} ${escapedUrl}; then ` +
			`cd * && chmod -R 666 * && ` +
			`printf '{"basename":"%s","branch":"%s"}\n' "$(basename "$(pwd)")" ${escapedBranch} > ../basename.json; fi`
		);
	}
	...
	add(source, callback) {
		...
		if (sanitized_owner === null) {
			console.log("source add rejected: source_owner_insane", source.owner); // remove
		...
		let PREFETCH_CMD = Sources.prefetchCommand(TEMP_PATH, sanitized_branch, sanitized_url);
