...
module.exports = class Sources {
	...
	*/
	...
	ownerIdFromPath(local_path) {
		console.log("[ownerIdFromPath] local_path: ", owner_path);
		let owner_path = local_path.replace(app_config.build_root, "");
		console.log("[ownerIdFromPath] owner_path: ", owner_path);
		...
		console.log("[ownerIdFromPath] owner_path_array: ", owner_path_array);
		let owner_id = owner_path_array[1];
		console.log("[ownerIdFromPath] owner_id: ", owner_id);
		return owner_id;
	}
			...
			for (let i = 0; i < keys.length; i++) {
				...
				if (typeof(source.owner) !== "undefined") {
					if ((source.owner.indexOf(owner_id) != -1) &&
						...
						console.log("not eligible", owner_id, repo_name, repo_branch);
					}
				}
			}
	...
	*/
	...
	getTempPath(owner) {
		const temp_id = uuidV1(); // TODO: FIXME: use source_id instead... or use always same path stored to DB
		const OWNER_ROOT = app_config.build_root + "/" + owner;
		var TEMP_PATH = OWNER_ROOT + "/" + temp_id;
		mkdirp.sync(TEMP_PATH);
		this.cleanupDirectory(TEMP_PATH); // why here?
		return TEMP_PATH;
	}
	...
	add(source, callback) {
		...
		var TEMP_PATH = this.getTempPath(source.owner);
		...
		var PREFETCH_CMD = "set +e; mkdir -p " + TEMP_PATH + "; cd " + TEMP_PATH + "; rm -rf *; " +
			...
			"then cd * && echo { \"basename\":\"$(basename $(pwd))\", \"branch\":\"" + sanitized_branch + "\" } > ../basename.json; fi";
