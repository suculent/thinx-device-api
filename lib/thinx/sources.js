...
module.exports = class Sources {
	...
	*/
	...
	ownerIdFromPath(local_path) {
		...
		let owner_id = null;
		if (owner_path_array.length > 3) {
			if (owner_path_array[0] == null) {
				owner_id = owner_path_array[1]; // skip empty array item after leading or double slash
			} else {
				owner_path_array[0];
			}
		}
		console.log("owner id", owner_path_array[0], "from", {owner_path_array}, "owner", owner_path);
		return owner_path_array[0];
	}

	withRepository(info, local_path) {

		let repo_name = info.repository.full_name;
		...
		userlib.get(owner_id, function (err, doc) {
			...
			if (source.url.indexOf(repo_name) !== -1) {
				...
				if (source.branch.indexOf(repo_branch) !== -1) {
					...
					// result is array of matching source ids...
