...
module.exports = class Sources {
	...
	upsertOwnerSource(doc, callback) {
		...
		userlib.get(doc._id, (gerror, body) => {
			if (!gerror) {
				userlib.atomic("users", "edit", doc._id, changes, (error, body) => {
					if (error) {
						console.log("ERR: " + error + " : " + JSON.stringify(body));
						alog.log(owner, "upsertOwnerSource.", "error");
						callback("upsert_failed");
						return;
					} else {
		...
		/*
		let revision = doc._rev;
		let identifier = doc._id;
		let new_doc = doc;
		delete new_doc._rev;
		userlib.destroy(identifier, revision, (err) => {
			userlib.insert(new_doc, identifier, (err, body, header) => {
				if (err) {
					console.log("upsertOwnerSource", {err}, {body});
				}
				callback(err);
			});
		});*/
	...
	addSourceToOwner(owner, source, temporary_source_path, source_callback) {
		userlib.get(owner, (err, doc) => {
			...
			this.upsertOwnerSource(doc, (err) => {
				if (err) {
					console.log("/api/user/source upsertOwnerSource ERROR:" + err);
	...
	updateUserWithSources(doc, sources, really_removed_repos, callback) {
		userlib.destroy(doc._id, doc._rev, (err) => {
			...
			userlib.insert(doc, doc._id, (err, body, header) => {
				if (err) {
					console.log("/api/user/source updateUserWithSources ERROR:" + err);
