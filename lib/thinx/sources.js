	...
	var _public = {
		...
		add: function(owner, alias, url, branch, callback) {
			...
			mkdirp(REPO_PATH, function(err) {
				...
				CMD = "set +e; cd " + REPO_PATH + "; if $(git clone --recurse-submodules -b '" + origin_branch +
					"' " + url +
					"); then cd * && echo { \"basename\":\"$(basename $(pwd))\", \"branch\":\"" +
					origin_branch + "\" } > basename.json; fi";

				if (branch === null) {
					CMD = "set +e; cd " + REPO_PATH + "; if $(git clone --recurse-submodules " + url +
						"); then cd * && echo { \"basename\":\"$(basename $(pwd))\", \"branch\":\"" +
						branch + "\" } > basename.json; fi";
				}

				var result = null;

				// TODO: Fix shell-injection vulnerability here
				// CMD = shellescape(CMD); does not work because CMD is not an array
