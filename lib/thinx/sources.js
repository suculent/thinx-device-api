	...
	var _public = {
		...
		add: function(owner, alias, url, branch, callback) {
			...
			mkdirp(REPO_PATH, function(err) {
				...
				var CMD = "cd " + REPO_PATH + "; git clone --recurse-submodules -b " + origin_branch +
					" " + url +
					"; cd " + PROJECT_NAME + "; echo { \"basename\":\"$(basename $(pwd))\", \"branch\":\"" +
					origin_branch + "\" } > basename.json; cat basename.json";

				if (branch === null) {
					CMD = "cd " + REPO_PATH + "; git clone --recurse-submodules " + url +
