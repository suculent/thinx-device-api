...
var Sources = (function() {
	...
	var gitFetchWithKeys = function(owner, CMD) {
		var success = false;
	...
	var _public = {
		...
		*/
		...
		add: function(source, callback) {
			...
			if (sanitized_branch.length != source.branch.length) {
				console.log("Invalid sanitized branch name: "+sanitized_branch+" does not equals to "+source.branch.replace("origin/", ""));
				callback(false, "invalid branch name"); return;
			}
			const sanitized_url = sanitka.url(source.url);
			if (sanitized_url.length != source.url.length) {
				callback(false, "Invalid Git URL"); return;
			}
			var CMD = "set +e;" + " cd " + TEMP_PATH + "; " +
			...
			// Try to solve access rights issue by using owner keys...
			const git_success = gitPrefetch(CMD);
			if ( git_success == false || git_success == [] ) {
				if (!gitFetchWithKeys(source.owner, CMD)) {
