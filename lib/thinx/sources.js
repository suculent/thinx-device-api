...
module.exports = class Sources {
	...
	// Private function used only by source removal
	upsertDevice(device) {
	...
	*/
	...
	updatePlatform(owner, source_id, platform, complete_callback) {
		...
		this.userlib.get(owner, (err, doc) => {
			...
			this.userlib.atomic("users", "edit", owner, changes, (uerror/* , abody */) => {
				if (uerror) {
					console.log(`☣️ [error] ${uerror} in platform changes ${JSON.stringify(changes)}`);
					alog.log(owner, "Private update failed.", "error");
					callback(false, "private_update_failed");
					return;
				}
				alog.log(owner, "Private tag updated successfully.", source_id);
	...
	updatePrivate(owner, source_id, is_private, complete_callback) {
		this.userlib.get(owner, (err, doc) => {
			if (err) {
				console.log("[error] updatePrivate", err);
				complete_callback(false);
				return;
			}
			if (!doc) {
			...
			this.userlib.atomic("users", "edit", owner, changes, (uerror/* , abody */) => {
				if (uerror) {
					console.log(`☣️ [error] ${uerror} in privacy changes ${JSON.stringify(changes)}`);
					alog.log(owner, "Private update failed.", "error");
					callback(false, "private_update_failed");
					return;
				}
	...
	updateLastBuild(owner, source_id, last_build, complete_callback) {
		this.userlib.get(owner, (err, doc) => {
			if (err) {
				console.log("[error] updateLastBuild", err);
				complete_callback(false);
				return;
			}
			if (!doc) {
			...
			this.userlib.atomic("users", "edit", owner, changes, (uerror/* , abody */) => {
				if (uerror) {
					console.log(`☣️ [error] ${uerror} in privacy changes ${JSON.stringify(changes)}`);
					alog.log(owner, "Last build update failed.", "error");
					callback(false, "lastbuild_update_failed");
					return;
				}
				alog.log(owner, `last_build updated successfully for source ${source_id})`);
				complete_callback(true);
			});
		});
	}
