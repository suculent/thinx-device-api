			...
			if (typeof(rbody.reset_key) !== "undefined") {
				...
				userlib.view("users", "owners_by_resetkey", {
					...
					if (err === true) {
						...
						if (typeof(rbody.owner) === "undefined") {
							console.log("Reset user: " + JSON.stringify(body));
						}
				...
				if (err) {
					...
					if (user_should_not_exist > 0) {
						if (username !== "test") {
							...
							// WARNING! User 'test' does not return 'email_already_exists' but recreates itself on activation
							userlib.destroy(new_owner_hash);
						}
					}
				}
				...
				userlib.insert(new_user, new_owner_hash, function(err,
					...
					var activationEmail = new Emailer({
						...
						body: "<!DOCTYPE html>Hello " + first_name +
							" " +
							last_name +
							".</p><p> Please <a href='https://rtm.thinx.cloud:7443/api/user/activate?owner=" +
							username + "&activation=" +
