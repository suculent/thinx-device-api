...
module.exports = class Owner {
	...
	constructor() {
		console.log("✅ [info] Loaded module: Owner");
	}
	...
	sendGDPRExpirationEmail24(user, email, callback) {
		...
		console.log("[info] Day before GDPR delete warning: " + user.owner);
		...
		mg.messages.create(app_config.mailgun.domain, deleteIn24Email)
			...
			.catch(err => {
				console.log(`[error] mailgun 24 err ${err}`);
	...
	sendGDPRExpirationEmail168(user, email, callback) {
		...
		console.log("[info] Week before GDPR delete warning: " + user.owner);
		...
		mg.messages.create(app_config.mailgun.domain, deleteIn168Email)
			...
			.catch(err => {
				console.log(`[error] mailgun 168 err ${err}`);
	...
	sendResetEmail(user, email, callback) {
		...
		console.log(`ℹ️ [info] Resetting password for user ${user.owner}`);
		...
		mg.messages.create(app_config.mailgun.domain, resetEmail)
			...
			.catch(err => {
				console.log("[error] mailgun reset password error", err);
	...
	resetUserWithKey(user, email, callback) {
		...
		this.update(user.owner, { reset_key: user.reset_key }, (success, message) => {
			if (!success) {
				console.log("[error] resetUserWithKey update failed", message);
	...
	sendActivationEmail(object, callback) {
		...
		mg.messages.create(app_config.mailgun.domain, activationEmail)
			...
			.catch(err => {
				console.log("[error] Sending activation e-mail failed", err);
	...
	save_avatar(owner, avatar, callback) {
		...
		fs.ensureFile(afile, (err1) => {
			if (err1) {
				console.log(`[error] error creating avatar file: ${err1}`);
				return;
			} 
			fs.writeFile(afile, avatar, (err2) => {
				if (err2) {
					console.log(`[error] error saving avatar file: ${err2}`);
	...
	mqtt_key(owner_id, password, callback) {
		apikey.list(owner_id, (success, api_keys) => {
			
			if (!success) {
				console.log("[error] apikey.list result for mqtt_key: ", { success }, { api_keys });
			...
			if ((typeof (api_keys) !== "undefined") && (api_keys.length > 0)) {
				...
				callback(true, first_key);
				return;
			} 

			console.log("☣️ [error] first API Key not found!");
			if (owner_id.indexOf("thinx") !== -1) {
	...
	update(owner, body, callback) {
		...
		userlib.get(owner, (error/* , user_body */) => {
			...
			userlib.atomic("users", "edit", owner, changes, (uerror, abody) => {
				if (error) {
					console.log("[error] " + uerror + " in changes : " + JSON.stringify(changes));
	...
	password_reset(owner, reset_key, callback) {
		...
		if (typeof (reset_key) === "undefined") {
			console.log("[error] Missing reset key.");
		...
		userlib.view("users", "owners_by_resetkey", {
			...
			if (err) {
				console.log("[error] owners_by_resetkey", err);
				callback(false, err);
			}
			...
			if (reset_key != user_reset_key) {
				console.log("[error] reset_key does not match");
	...
	password_reset_init(email, callback) {
		...
		userlib.view("users", "owners_by_email", {
			...
			if (err) {
				callback(false, "user_not_found");
				console.log("[error] [password_reset_init]", err.toString(), "Found " + body.rows.length + " users matching this e-mail.");
				return;
			} 
			...
			try {
				...
				console.log("[error] Invalid user doc fetched in password reset", body);
			}
			...
			if (typeof (user) === "undefined" || user === null) {
				...
				if (typeof (user) === "undefined" || user === null) {
					console.log("[error] User not found in password reset.");
					callback(false, "user_not_found");
					return;
				}
			}
	...
	activate(ac_owner, ac_key, callback) {
		...
		if (typeof (ac_key) === "undefined") {
			console.log("Developer Error: activation key missing");
			return;
		}

		console.log("[info] [activation] Activation with owner", ac_owner, "and key", ac_key);
		...
		userlib.view("users", "owners_by_activation", {
			...
			console.log("[error] [activation]" + err.toString());
			callback(false, "user_not_found");
		});
	}
	...
	set_password_reset(rbody, callback) {
		userlib.view("users", "owners_by_resetkey", {
			...
			if (typeof (body) === "undefined") {
				console.log("[error] Reset user failing... no body returned.");
				return;
			}
			...
			// TODO: FIXME: Possibly refactor to CouchDB update handler (shorter, faster)
			userlib.destroy(userdoc._id, userdoc._rev, (dt_err) => {

				if (dt_err !== null) {
					console.log(`[error] Cannot destroy document ${userdoc._id}`);
				...
				userlib.insert(userdoc, userdoc.owner, (in_err) => {
					if (in_err) {
						...
						userlib.insert(userdoc, userdoc.owner, (ins_err) => {
							console.log("[critical] Cannot re-insert user on password-set");
	...
	set_password_activation(rbody, callback) {
		userlib.view("users", "owners_by_activation", {
			...
			if (err) {
				console.log("[error] Activation Error: " + err.toString());
			...
			var userdoc = body.rows[0].value;
			console.log("[info] Activating user: " + userdoc.owner);
			...
			userlib.destroy(userdoc._id, userdoc._rev, () => {
				...
				userlib.insert(userdoc, userdoc._id, (ins_err) => {
					if (ins_err) {
						console.log("[error] [activation] insert err", ins_err);
	...
	set_password(rbody, callback) {
		if (rbody.password !== rbody.rpassword) {
		...
		if (typeof (rbody.activation) !== "undefined") {
			console.log("[info] Searching " + rbody.activation + "using owners_by_activation...");
	...
	delete(body, callback, res) {
		...
		userlib.atomic("users", "edit", body.owner, changes, (a_error) => {
			if (a_error) {
				console.log("[error] " + a_error + "  editing : " + JSON.stringify(body));
	...
	create_default_mqtt_apikey(owner_id, callback) {
		...
		if ((typeof (owner_id) === "undefined") || (owner_id === "") || (owner_id === null)) {
			console.log("[error] Cannot create MQTT apikey with invalid owner_id.");
			callback(false);
		}
		...
		userlib.get(new_owner_hash, (user_get_error/* , user_get_body */) => {
			...
			if (process.env.ENVIRONMENT === "test") {
				if (!user_get_error && (username !== "cimrman")) {
					console.log("[warning] User already exists", username, new_owner_hash);
			...
			userlib.view("users", "owners_by_username", {
				...
				this.create_default_mqtt_apikey(new_owner_hash, (success) => {
					...
					if (!success) {
						console.log(`[error] Creating Default MQTT API Key Failed for ${new_owner_hash}`);
