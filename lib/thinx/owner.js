...
module.exports = class Owner {
	...
	// Find user and match user, move to owner.js!
	userExists(username, db_body) {
		if (username.indexOf(db_body.username) == 0) {
			console.log("[userExists] Found user", { db_body });
			...
			console.log("User not found", { username }, "in", { db_body }); // remove when done?
			return null;
		}
	}
