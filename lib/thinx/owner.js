...
module.exports = class Owner {
		...
		fs.ensureFile(afile, (err1) => {
			...
			fs.writeFile(afile, avatar, (err2) => {
				if (err2) {
					...
					return callback(false, "error_saving_avatar");
				}
	...
	set_password_reset(rbody, callback) {
		userlib.view("users", "owners_by_resetkey", {
			...
			userlib.atomic("users", "edit", userdoc._id, changes, (_in_err) => {
				if (_in_err !== null) {
					console.log("Cannot edit user on password-set", {_in_err});
						...
						userlib.insert(new_user, new_owner_hash, (err_u) => {
							if (err_u) {
								if (err_u.statusCode == 409) {
									...
									console.log("[error] owner_error", u_err);
	...
	trackUserLogin(owner_id) {
		...
		alog.log(owner_id, "User logged in...");
		if (Globals.use_sqreen()) {
			this.sqreen.auth_track(true, { username: owner_id });
		}
	}
