...
module.exports = class Owner {
	...
	sendResetEmail(user, email, callback) {
		...
		console.log(`[info] Resetting password for user ${user.owner}`);
	...
	password_reset_init(email, callback) {
		...
		userlib.view("users", "owners_by_email", {
			...
			if (err) {
				callback(false, "user_not_found");
				console.log("[password_reset_init]", err.toString(), "Found " + body.rows.length + " users matching this e-mail.");
				return;
			} 
		...
		userlib.view("users", "owners_by_resetkey", {
			...
			userlib.destroy(userdoc._id, userdoc._rev, (dt_err) => {
				...
				userlib.insert(userdoc, userdoc.owner, (in_err) => {
					...
					console.log(`[info] Password reset successful for ${userdoc._id}`);
					callback(true, {
						success: true,
	...
	create(body, send_activation, res, callback) {
		...
		userlib.get(new_owner_hash, (user_get_error/* , user_get_body */) => {
			...
			userlib.view("users", "owners_by_username", {
				...
				console.log(`🔨 [debug] Creating Default MQTT API Key for ${new_owner_hash}`);
				...
				this.create_default_mqtt_apikey(new_owner_hash, (success) => {
					...
					console.log(`ℹ️ [info] Creating Default MQTT API Key for new_owner_hash ${new_owner_hash}`);
