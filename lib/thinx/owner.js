...
module.exports = class Owner {
	...
	constructor(redis) {		
		...
		this.userlib = require("./couch")(db_uri).use(prefix + "managed_users");
	...
	sendMail(contents, type, callback) {
		mg.messages.create(app_config.mailgun.domain, contents)
			...
			.catch(err => {
				// SEC-PII-01: redact - err carries Mailgun accesstoken; emit only msg + status.
				console.log(`☣️ [error] mailgun ${type} err msg=${err && err.message} status=${err && err.status}`);
	...
	sendResetEmail(user, email, callback) {
		...
		if ((process.env.ENVIRONMENT === "test") || (process.env.ENVIRONMENT === "development")) {
			// SEC-PII-01: log line redacted; callback VALUE stays raw - the chai-http
			// spec at ZZ-AppSessionUserSpec.js:191-198 chains through this raw value.
			console.log("[test] sending reset key directly", Util.redactToken(user.reset_key));
	 ...
	 */
	resetUserWithKey(user, email, client, callback) {
		// Backward-compatible arity: support legacy (user, email, callback) callers.
		if (typeof client === "function") { callback = client; client = undefined; }
		user.reset_key = sha256(email + new Date().toString());
		// AUTH-RESET-ORIGIN: record which console initiated the reset so the GET
		// handler (password_reset) can redirect back to the right path. Only "vue"
		// is recognised; everything else normalises to the legacy default.
		user.reset_console = (client === "vue") ? "vue" : "legacy";
		this.update(user.owner, { reset_key: user.reset_key, reset_console: user.reset_console }, (success, message) => {
			if (!success) {
				console.log("☣️ [error] resetUserWithKey update failed", message);
				callback(false, message);
			} else {
				console.log("ℹ️ [info] will send reset e-mail with key", Util.redactToken(user.reset_key));
	...
	sendActivationEmail(object, callback) {
		...
		if (app_config.public_url.indexOf("localhost") !== -1) {
			// SEC-PII-01: redact activation token in the LOG; callback link stays raw
			// so the admin can still click through. Operator has the raw token on disk.
			const log_link = link.replace(object.new_activation_token, Util.redactToken(object.new_activation_token));
		...
		// Normal user flows
		console.log(`ℹ️ [info] Sending activation e-mail to ${Util.redactEmail(activationEmail.to)} with token ${Util.redactToken(object.new_activation_token)}`);
	...
	async mqtt_key(owner_id, callback) {
	...
	async profile(owner, callback) {
		try {
			const body = await this.userlib.get(owner);
			...
			// Issue E (2026-05-26): legacy console at services/console/src/app/views/profile/account.html:55
			// binds to {{$root.profile.email}} (top-level). Without this flatten, the binding resolves
			// to undefined and the input shows the "john@doe.com" placeholder. CouchDB stores email
			// both at top-level (body.email) and under body.info.email; prefer top-level, fall back to
			// info.email for older user docs that may only have it nested. Vue console treats this as
			// additive (extra field is ignored). See parent Phase 1 SUMMARY / Issue E.
			let email = (typeof (body.email) !== "undefined") ? body.email : (body.info && body.info.email);
	...
	async apply_update(owner, update_key, update_value, callback) {
		let changes = {};
		changes[update_key] = update_value;
		try {
			await this.userlib.get(owner);
		} catch (error) {
			alog.log(owner, "Profile update error " + error, "error");
			return callback(false, error);
		}
		try {
			const abody = await this.userlib.atomic("users", "edit", owner, changes);
	...
	// Public callers (call-graph spot-check per ROADMAP success criterion 3):
	//   - lib/router.profile.js:33  — POST /api/profile (profile editor)
	//   - lib/router.gdpr.js:135    — GDPR purge flow (reset_key-cleanup path)
	// Signature `(owner, body, callback)` MUST stay in this order — both callers
	// rely on the positional binding. Owner.update has NO `this.userlib.*` calls
	// of its own; all DB work is delegated to `apply_update` (async since 07-1)
	// via the existing callback-passthrough. The `async` keyword is added for
	// file-wide consistency with the rest of the converted owner.js surface;
	// the method does no `await`-ing itself by design.
	 ...
	 */
	async validate(username, callback) {
		// Search the user in DB
		try {
			const db_body = await this.userlib.view("users", "owners_by_username", {
				"key": username,
				"include_docs": false
			});
			...
			if ((typeof (db_body.rows) === "undefined") || (db_body.rows.count > 1)) {
				return callback(false);
			}
			callback(db_body);
		} catch (_xerr) {
	...
	async password_reset(owner, reset_key, callback) {
		...
		if (reset_key !== user_reset_key) {
			...
			// AUTH-RESET-ORIGIN: same host (public_url) serves both consoles; only the
			// path differs. The originating console is recorded as user.reset_console at
			// init time (resetUserWithKey). Vue is a hash-mode SPA with no /password-reset
			// nginx fallback, so it must be the `/#/password-reset` hash route. Anything
			// else (legacy AngularJS console, or a key staged before reset_console existed)
			// falls back to the legacy static `/password.html` page that exists on the host.
			const reset_path = (user.reset_console === "vue") ? "/#/password-reset" : "/password.html";
			const url = app_config.public_url + reset_path + "?reset_key=" + reset_key + "&owner=" + owner;
			callback(true, { redirectURL: url });
		}
	}

	async password_reset_init(email, client, callback) {

		// Backward-compatible arity: support legacy (email, callback) callers.
		if (typeof client === "function") { callback = client; client = undefined; }

		let body;
		try {
			body = await this.userlib.view("users", "owners_by_email", {
				"key": email,
				"include_docs": true // might be useless
			});
		} catch (err) {
			// SEC-PII-01: scope err to err.message to avoid leaking CouchDB view envelope.
			console.log("☣️ [error] [password_reset_init]", (err && err.message) ? err.message : String(err), "Found 0 users matching this e-mail.");
			return callback(false, "user_not_found");
		}

		if (body.rows.length !== 1) {
			// SEC-PII-01: redact email; collapse {body} to row-count - the CouchDB view
			// envelope embeds the email as the view key when include_docs=true.
			console.log("☣️ [error] [password_reset_init] email " + Util.redactEmail(email) + " not found in", { rows: body.rows.length });
		...
		this.resetUserWithKey(user, email, client, callback);
	}

	async activate(ac_owner, ac_key, callback) {

		if (typeof (ac_key) === "undefined") {
			return callback(false, "activation_key_missing");
		}

		console.log("ℹ️ [info] [activation] Activation with owner", ac_owner, "and key", Util.redactToken(ac_key));

		try {
			await this.userlib.view("users", "owners_by_activation", {
				"key": ac_key,
				"include_docs": false
			});
			...
			// SEC-PII-01: scope err to err.message - defensive against err carrying activation key.
			console.log("☣️ [error] [activation] " + ((err && err.message) ? err.message : String(err)));
			callback(false, "user_not_found");
		}
	}

	async atomic(doc, changes, action_name, callback) {
		try {
			await this.userlib.atomic("users", "edit", doc, changes);
		} catch (_in_err) {
	...
	async set_password_reset(rbody, callback) {
		let body;
		try {
			body = await this.userlib.view("users", "owners_by_resetkey", {
				"key": rbody.reset_key,
	...
	async set_password_activation(rbody, callback) {
		let body;
		try {
			body = await this.userlib.view("users", "owners_by_activation", {
	...
	// Call-graph spot-check (REFACTOR-04, Phase 7 Plan 03):
	//   Public callers: lib/router.user.js:126 (via Util.responder) — sole caller.
	//   Signature is intentionally (owner, callback, res) — callback BEFORE res.
	//   That order is locked by router.user.js:126 (`user.delete(owner, Util.responder, res)`).
	//   DO NOT "fix" the parameter order; the unusual placement is the public contract.
	...
	async create_default_acl(owner_id, callback) {
	...
	async create_mqtt_access(owner_id, DEFAULT_APIKEY_NAME, callback) {
		this.apikey.create(owner_id, DEFAULT_APIKEY_NAME, (success, object) => {
			if (!success) {
				// Propagate Redis-unavailable / breaker-open signals up so the
				// upstream user-create flow surfaces "creating_mqtt_api_key_failed"
				// instead of orphaning a Mosquitto credential. The 2026-05-31 OOM
				// incident root cause was the previous fall-through path here.
				if (object === "redis_unavailable" || object === "redis_unavailable_backoff") {
					console.log(`☣️ [error] [owner] Default API Key not created — ${object}; skipping add_mqtt_credentials (no orphan will be left).`);
			...
			// Compensation guard: if add_mqtt_credentials throws synchronously,
			// rejects, or never invokes its callback within 5s, revoke the
			// (possibly half-written) credential so no orphan Mosquitto user
			// is left behind. A once-fired flag prevents double-revoke.
			let settled = false;
			const finish = (cb_success, payload) => {
				if (settled) return;
				settled = true;
				cb_success ? callback(true, payload) : callback(false, payload);
			};
			const timeoutMs = 5000;
			const timer = setTimeout(() => {
				if (settled) return;
				console.log(`☣️ [error] [owner] add_mqtt_credentials timed out after ${timeoutMs}ms for ${owner_id} — revoking partial credential`);
				try { this.auth.revoke_mqtt_credentials(owner_id); } catch (_e) { /* best-effort */ }
				finish(false, "mqtt_add_timeout");
			}, timeoutMs);
			...
			try {
				this.auth.add_mqtt_credentials(owner_id, object[0].key /* key, not hash! */, () => {
					...
					this.create_default_acl(owner_id, () => {
						finish(true, object[0]);
					});
				});
			} catch (e) {
				clearTimeout(timer);
				console.log(`☣️ [error] [owner] add_mqtt_credentials threw for ${owner_id} — revoking partial credential:`, e && e.message ? e.message : e);
				try { this.auth.revoke_mqtt_credentials(owner_id); } catch (_e) { /* best-effort */ }
				finish(false, "mqtt_add_threw");
			}
		});
	}

	async create_default_mqtt_apikey(owner_id, callback) {
	 ...
	 */
	async create(body, send_activation, res, callback) {
		...
		// AUTH-USERNAME-FIELD: the legacy register form posts the chosen login name in
		// the `owner` field (input name="owner", labelled "Username (use for login!)"),
		// not `username`. Accept it so the chosen name lands in doc.username and the
		// account is findable at login (Owner.validate -> owners_by_username). A genuinely
		// empty value still falls through to the email-hash default below (OAuth accounts).
		if ((typeof (username) === "undefined") || (username === null) || (username === "")) {
			let ownerField = (typeof (body.owner) === "string") ? body.owner.trim() : "";
			if (ownerField !== "") username = ownerField;
		}
		...
		try {
			...
			// Step A: existence check — userlib.get the hash.
			let user_get_error = null;
			try {
				await this.userlib.get(new_owner_hash);
			} catch (e) {
				user_get_error = e;
			}
			...
			// Step B: Check for existing username, should return error.
			let user_view_body;
			try {
				user_view_body = await this.userlib.view("users", "owners_by_username", {
					"key": username,
					"include_docs": true
				});
			} catch (err) {
				// Should be missing
				if (err && err.toString().indexOf("Error: missing") === -1) {
					console.log("☣️ [error] Is the DB created? New user should not cause: " + err);
				}
				// Synthesize an empty result so the row-count check below proceeds correctly.
				user_view_body = { rows: [] };
			}

			if (user_view_body && user_view_body.rows && user_view_body.rows.length > 0) {
			...
			// Step C: create_default_mqtt_apikey is callback-style (signature preserved per Plan 07-1);
			// wrap in a Promise so we can await it linearly. should not throw.
			...
			// Step D: userlib.insert the new user.
			let insert_err = null;
			try {
				await this.userlib.insert(new_user, new_owner_hash);
			} catch (e) {
				insert_err = e;
			}
			if (insert_err) {
				if (insert_err.statusCode === 409) {
			...
			// Step E: if this fails, should not insert the user (but may be optional in tests or local!).
			// sendActivationEmail is callback-style (NOT converted by Phase 7); wrap in Promise for linear flow.
			const { send_success, result } = await new Promise(resolve =>
				this.sendActivationEmail(activation_email_descriptor, (s, r) => resolve({ send_success: s, result: r }))
			);
			...
			// Defense-in-depth: any unexpected throw inside the create flow funnels to owner_error.
			// The per-step try/catches above handle the expected error cases explicitly.
			console.log("☣️ [error] owner_error", err);
			return callback(res, false, "owner_error");
		}
	...
	async createMesh(owner, mesh_id, mesh_alias, callback) {

		let body;
		try {
			body = await this.userlib.get(owner);
		} catch (err) {
			return callback(false, err);
		}
		...
		// Atomically update
		try {
			await this.userlib.atomic("users", "edit", owner, { mesh_ids: meshes });
		} catch (a_error) {
	...
	async deleteMeshes(owner, mesh_ids, callback) {
		...
		acl.load(async () => {
			...
			// Atomically update
			try {
				await this.userlib.atomic("users", "edit", owner, { mesh_ids: mout });
			} catch (a_error) {
	...
	async listMeshes(owner, callback) {
		let body;
		try {
			body = await this.userlib.get(owner);
		} catch (err) {
	...
	async trackUserLogin(owner_id) {
		try {
			await this.userlib.atomic("users", "checkin", owner_id, {
				last_seen: new Date()
			});
		} catch (error) {
	...
	async updateLastSeen(doc, repeated = false) {
		let response;
		let error = null;
		try {
			response = await this.userlib.atomic("users", "checkin", doc._id, {
				last_seen: new Date()
			});
		} catch (e) {
			error = e;
	...
	async addGitHubAccessToken(owner, token, callback) {
		try {
			await this.userlib.atomic("users", "edit", owner, { gitHubAccessToken: token });
		} catch (a_error) {
