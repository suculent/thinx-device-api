...
module.exports = class Owner {
	...
	constructor(redis) {		
		...
		console.log("[DEBUG] Owner.constructor");
	}
	...
	validate(username, callback) {
		...
		this.userlib.view("users", "owners_by_username", {
			...
			if (typeof (db_body) === "undefined") {
				console.log("⚠️ [warning] [validate] owner body undefined, owner not found:", username, xerr);
	...
	create_default_mqtt_apikey(owner_id, callback) {
		...
		console.log("[DEBUG] create_default_mqtt_apikey (1)");
		...
		console.log("[DEBUG] create_default_mqtt_apikey (2)");

		this.apikey.create(owner_id, "Default MQTT API Key", (success, object) => {
			if (success) {
				console.log("[DEBUG] create_default_mqtt_apikey (3a), success, object", success, JSON.stringify(object));
				console.log(`ℹ️ [info] Adding MQTT credential ${object[0].key}`);
				this.auth.add_mqtt_credentials(owner_id, object[0].key /* key, not hash! */, () => {
					console.log("[DEBUG] create_default_mqtt_apikey (4)");
					this.create_default_acl(owner_id, () => {
						console.log("[DEBUG] create_default_mqtt_apikey (5)");
						callback(true, object[0]);
					});
				});
			} else {
				console.log("[DEBUG] create_default_mqtt_apikey (3b)");
	...
	create(body, send_activation, res, callback) {
		...
		this.userlib.get(new_owner_hash, (user_get_error, user_get_body ) => {
			...
			console.log("[DEBUG] reqesting owner by username", username);
			...
			this.userlib.view("users", "owners_by_username", {
				...
				console.log("[DEBUG] reqesting owner by username", username, "err:", err, "body", user_view_body);
				...
				this.create_default_mqtt_apikey(new_owner_hash, (success) => {
					...
					console.log("[DEBUG] will insert new user (checkpoint)");
