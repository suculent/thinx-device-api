...
module.exports = class Owner {
	...
	addGitHubAccessToken(owner, token, callback) {
		...
		this.userlib.atomic("users", "edit", owner, { gitHubAccessToken: token }, (a_error) => {
			if (a_error) {
				console.log("ERR: " + a_error + "  adding GitHub Access Token in : " + JSON.stringify(body));
				alog.log(body.owner, "GitHub Access Token creation failed.", "error");
				return callback(false, "add_github_token_failed");
			}
			alog.log(body.owner, "GitHub Token Added.", "info");
			callback(true);
		});
	}
