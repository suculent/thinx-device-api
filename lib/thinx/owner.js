...
module.exports = class Owner {
	...
	apply_update(update_key, update_value, callback) {
		var changes = {};
	...
	update(owner, body, callback) {
		...
		console.log("[debug] updating owner with", {body});
		...
		this.process_update(body, callback, (update_key, update_value) => {
			...
			this.apply_update(update_key, update_value, callback);
		});
	}
	...
	validate(username, callback) {
		...
		this.userlib.view("users", "owners_by_username", {
			...
			if (typeof (db_body) === "undefined") {
				console.log("⚠️ [warning] [validate] owner body undefined, owner not found.");
	...
	set_password_reset(rbody, callback) {
		this.userlib.view("users", "owners_by_resetkey", {
			...
			this.userlib.atomic("users", "edit", userdoc._id, changes, (_in_err) => {
				if (_in_err !== null) {
					console.log("Cannot edit user on password-set", {_in_err}, "doc", {userdoc}, "changes", changes);
					// conflict also seems to mean there is no change...
					return callback(false, "password_reset_failed");
				}
	...
	delete(owner, callback, res) {
		...
		this.userlib.atomic("users", "edit", owner, changes, (a_error) => {
			if (a_error) {
				console.log("☣️ [error] " + a_error + "  deleting : " + owner);
				alog.log(owner, "Profile update failed.", "error");
				return callback(false, "delete_failed");
			}
						...
						this.userlib.insert(new_user, new_owner_hash, (err_u) => {
							if (err_u) {
								if (err_u.statusCode == 409) {
									...
									console.log("[error] owner_error", err_u);
