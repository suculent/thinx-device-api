...
module.exports = class Owner {
	...
	sendResetEmail(user, email, callback) {
		...
		this.mailgun.messages().send(resetEmail, (error, body) => {
			if (error) {
				console.log(error);
	...
	sendActivationEmail(object, callback) {
		...
		var link = app_config.public_url + ":7443/api/user/activate?owner=" + object.new_owner_hash +
		...
		this.mailgun.messages().send(activationEmail, function (error, body) {
			if (error) {
				console.log(error);
	...
	update(owner, body, callback) {
		...
		if (typeof(body.avatar) !== "undefined") {
			...
			this.save_avatar(owner, update_value, function(err, response) {
	...
	delete(body, callback) {
		...
		userlib.atomic("users", "edit", body.owner, changes, function(error, body) {
			if (error) {
				console.log("ERR: " + error + " : " + JSON.stringify(body));
