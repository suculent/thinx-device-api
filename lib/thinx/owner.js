	...
	create_default_mqtt_apikey(owner_id, callback) {
		...
		apikey.create(owner_id, "Default MQTT API Key", (success, object) => {
			...
			if (success) {
				...
				console.log("CREATE CREDS (object should contain key): ", {owner_id}, {object});
				auth.add_mqtt_credentials(owner_id, object.key); // key, not hash! also TODO: only call to auth in this class
