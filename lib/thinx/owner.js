...
module.exports = class Owner {
	...
	mqtt_key(owner_id, callback) {
		...
		apikey.list(owner_id, (success, api_keys) => {
			...
			if (!success) {
				console.log("☣️ [error] apikey.list failed in mqtt_key: ", { success }, { api_keys }, "with owner", { owner_id }); 
			...
			this.create_default_mqtt_apikey(owner_id, (zuccess, key) => {
				console.log("[error] 'Default MQTT API Key' not found, creating new one...");
				if (zuccess) {
					callback(true, key);
				} else {
