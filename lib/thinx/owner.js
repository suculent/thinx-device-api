...
var Owner = (function() {
	...
	var _private = {
		...
		avatar: function(owner) {
			var afile = app_config.project_root + "/avatars/" + owner + ".json";
			if (fs.existsSync(afile)) {
				return fs.readFileSync(afile);
			} else {
	...
	var _public = {
		...
		profile: function(owner, callback) {
			...
			userlib.get(owner, function(err, body) {
				...
				var profile = {
					...
					owner: body.owner,
					avatar: _private.avatar(body.owner),
					info: body.info
				};
		...
		update: function(owner, body, callback) {
			...
			if (typeof(body.avatar) !== "undefined") {
				...
				_private.save_avatar(owner, update_value, function(err, response) {
