	...
	var _public = {
		...
		set_password: function(rbody, callback) {
			...
			if (typeof(rbody.reset_key) !== "undefined") {
				...
				userlib.view("users", "owners_by_resetkey", {
					...
					if (err === true) {
						...
						userlib.destroy(userdoc._id, userdoc._rev, function(err) {
							...
							console.log("Saving new userdoc: " + JSON.stringify(userdoc));
				...
				console.log("No reset or activation key defined? Edge assert! Test conditioning error in rbody:");
				console.log(JSON.stringify(rbody));
				callback(false, "change_not_authorized");
			}
		},
			...
			userlib.get(new_owner_hash, function(err, body) {
				...
				if (!err) {
					if (username !== "test") {
						...
						console.log("User should not exist: " + JSON.stringify(body));
					}
				}
				...
				userlib.view("users", "owners_by_username", {
					...
					apikey.create(new_owner_hash, "Default MQTT API Key",
						...
						function(success, object) {
							if (success) {
								var MQTT_PASS_TOOL = exec.execSync("which mosquitto_passwd").toString().replace(
									"\n", "");
								console.log("mosquitto_passwd detection result: " + MQTT_PASS_TOOL);
								if (MQTT_PASS_TOOL.length) {


									fs.ensureFile(app_config.project_root +
										"/mqtt_passwords ", err => {
											if (err) {
												console.log("Error ensuring MQTT PASSWORDS file: " + err);
											}

											var CMD = MQTT_PASS_TOOL + " -b " + app_config.project_root +
											...
											const owner_topic = "topic /" + new_owner_hash + "/#";
											const shared_topic = "topic /thinx/announcements";
									...
									alog.log(new_owner_hash,
										"MQTT Installation failure while saving default password for: " +
										new_owner_hash +
										" with: " + email);
								}
					...
					userlib.insert(new_user, new_owner_hash, function(err,
						...
						var link = "https://rtm.thinx.cloud:7443/api/user/activate?owner=" + new_owner_hash +
							"&activation=" + new_activation_token;
