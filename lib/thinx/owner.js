	...
	mqtt_key(owner, callback) {
		userlib.get(owner, (err, body) => {
			if (err) {
				...
				var api_keys = body.api_keys;
				if (typeof(api_keys) !== "undefined" && api_keys.count > 0) {
					console.log("mqtt_key: listing keys from body, returning first from: ", {api_keys});
					callback(true, api_keys[0]);
				} else {
