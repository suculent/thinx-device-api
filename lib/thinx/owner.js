		...
		delete: function(body, callback) {
			...
			userlib.atomic("users", "edit", username, changes, function(error, body) {
				if (error) {
					...
					alog.log(username, "Owner state changed to deleted.");
						...
						function(success, object) {
							if (success) {
								...
								if (MQTT_PASS_TOOL.length) {
									...
									const user_line = "user " + owner;
									const owner_topic = "topic /" + owner + "/#";
									...
									fs.ensureFile(acl_path, err => {
										...
										fs.appendFile(acl_path, acl, function(err) {
											if (err) {
												...
												alog.log(owner, "Created MQTT ACL record for: " + owner + " email: " +
													email);
											}
										});
									});
									...
									alog.log(owner, "MQTT Installation failure while saving default password for: " +
										new_owner_hash +
										" with: " + email);
								}
