...
const mgDomain = app_config.mailgun.domain;
const mgKey = app_config.mailgun.api_key;
const mg = require('mailgun-js')({mgDomain, mgKey});
...
module.exports = class Owner {
	...
	sendMail(contents, type, callback) {
		mg.messages().send(contents)
			.then((/* msg */) => {
				callback(true, type + "_sent");
			}) // logs response data
			.catch(err => {
				console.log(`☣️ [error] owner mailgun err ${err}`);
	...
	set_password_reset(rbody, callback) {
		this.userlib.view("users", "owners_by_resetkey", {
			...
			alog.log(body.rows[0].value._id, "Attempt to set password with: " + rbody.reset_key, "warning");
