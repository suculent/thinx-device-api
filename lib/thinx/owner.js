...
module.exports = class Owner {
	...
	resetUserWithKey(user, email, callback) {
		console.log("(2) Creating new reset-key...", { user });
	...
	sendActivationEmail(object, callback) {
		...
		// Do not request email confirmation here, makes no sense in test
		if (process.env.ENVIRONMENT === "test") {
			callback(true, link);
			return;
		} else {
			console.log("Email from environment:", process.env.ENVIRONMENT);
		}
	...
	password_reset_init(email, callback) {
		...
		userlib.view("users", "owners_by_email", {
			...
			if (typeof (user) === "undefined" || user === null) {
				console.log("DEBUG: getting value from", body.rows[0]);
	...
	set_password(rbody, callback) {
		...
		if ((typeof (rbody.reset_key) !== "undefined") && (rbody.reset_key !== null)) {
			...
			userlib.view("users", "owners_by_resetkey", {
				...
				console.log("DEBUG2: getting value from", body.rows[0]);
				...
				console.log("DEBUG3: getting value from", body.rows[0]);
		...
		if (typeof (rbody.activation) !== "undefined") {
			...
			userlib.view("users", "owners_by_activation", {
				...
				console.log("DEBUG4: getting value from", body.rows[0]);
	...
	create_default_acl(owner_id, callback) {
		...
		acl.load(() => {
			...
			console.log("[APIKEY:CREATE:REGISTER] ACL Loaded...");
			...
			acl.commit(() => {
				console.log("[APIKEY:CREATE:REGISTER] ACL Saved, calling back...");
	...
	create_default_mqtt_apikey(owner_id, callback) {
		...
		apikey.create(owner_id, "Default MQTT API Key", (success, object) => {
			...
			if (success) {
				...
				console.log("[APIKEY:CREATE:REGISTER] Adding mqtt credentials:", { owner_id }, { object });

				auth.add_mqtt_credentials(owner_id, object[0].key); // key, not hash! also TODO: only call to auth in this class

				console.log("[APIKEY:CREATE:REGISTER] Creating ACL object...");
	...
	create(body, send_activation, callback, res) {
		...
		console.log("Creating owner profile with body", body); // TODO: Remove after fixing test
			...
			userlib.view("users", "owners_by_username", {
				...
				this.create_default_mqtt_apikey(new_owner_hash, (success) => {
					...
					this.sendActivationEmail(activation_email_descriptor, (send_success, result) => {
						...
						// console.log("Creating user with owner hash:", new_owner_hash, {new_user});
