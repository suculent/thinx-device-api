	...
	*/
	...
	createMesh(owner, mesh_id, mesh_alias, callback) {
		...
		userlib.get(owner, (err, body) => {
			...
			console.log("editing meshes", meshes);
	...
	deleteMeshes(owner, mesh_ids, callback) {
		...
		userlib.get(owner, (err, body) => {
			...
			let meshes_out = new Set(meshes);

			console.log("Meshes in:", meshes);
			...
			acl.load(() => {
				...
				for (var index in meshes) {
					...
					for (let delete_index in mesh_ids) {
						...
						if (mesh_id == delete_id) {
							deleted_ids.push(delete_id);
							console.log("Pruning ACL", delete_id);
							meshes_out.delete(mesh_id);
							acl.prune(delete_id);
							deleted = true;
						}
					}
				...
				console.log("Saving meshes after deletion", meshes_out);

				// Atomically update
				userlib.atomic("users", "edit", owner, { mesh_ids: Array.from(meshes_out) }, (a_error) => {
					if (a_error) {
	...
	listMeshes(owner, callback) {
		//console.log("Listing meshes for owner_id:", owner);
		userlib.get(owner, (err, body) => {
			if (err) {
				callback(false, err);
				return;
