...
module.exports = class Devices {
	...
	prefetch_repository(repo_path, source_id, owner) {
		...
		if (fs.existsSync(repo_path)) {
			...
			this.sources.list(owner, (success, response) => {
				...
				// TODO (13): Do not continue if repo path does not exist!
				// add exit on error; check after clone in case repo_path is is empty
				// https://github.com/suculent/thinx-device-api/issues/306
				...
				try {
					...
					if (result !== "Already up to date.") {
						console.log(`[builder] git prefetch result: ${result}`);
					}
				} catch (e) {
	...
	list(owner, callback) {
		...
		console.log("ℹ️  [info] [devices] listing devices for owner '%s'", owner);
	...
	detach(owner, body, callback, res) {
		...
		devicelib.view("devices", "devices_by_udid", {
			...
			console.log(`[info] Detaching repository ${doc.udid} from device ${doc._rev}`);
	...
	detachMesh(owner, body, callback, res) {
		...
		devicelib.view("devices", "devices_by_udid", {
			...
			console.log(`[info] Detaching mesh ${detached_mesh_id} from device ${doc.udid}`);
			...
			devicelib.destroy(doc._id, doc._rev, () => {
				...
				let acl = new ACL(udid);
				acl.load(() => {
					console.log(`[info] Detaching mesh from one device only using acl.removeTopic()`);
