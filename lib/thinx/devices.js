...
module.exports = class Devices {
	...
	// TODO (12). refactor, reuse git.fetch for prefetch action... move all from git_prefetch command to git class...
	// https://github.com/suculent/thinx-device-api/issues/305
	prefetch_repository(repo_path, source_id, owner) {
		...
		this.sources.list(owner, (success, response) => {
			...
			let result = git.prefetch(repo_path, sanitized_url, sanitized_branch, GIT_PREFETCH);
			...
			if (result.indexOf("Please make sure you have the correct access rights") !== -1) {
				console.log("ℹ️ [info] Trying keys...");
				if (git.fetch(owner, GIT_PREFETCH, repo_path)) {
					// update repository privacy status asynchronously
					Sources.updatePrivate(owner, source_id, true, (xuccess, error) => {
						if (xuccess) {
							console.log(`[info] [prefetch] repo privacy status updated to is_private=true; should prevent future public fetches`);
						} else {
			...
			this.platform.getPlatform(repo_path, (update_success, watcher_platform) => {
				if (!update_success) {
					console.log("⚠️ [warning] Failed this.platform.getPlatform() in sources.list()");
				}
				this.sources.updatePlatform(owner, source_id, watcher_platform, (ok, error) => {
					if (!ok) {
						console.log(`[error] failed updating platform ${platform} for ${source_id} with error ${error}`);
					} else {
						console.log(`[info] updated platform ${platform} for ${source_id}`);
					}
				});
	...
	list(owner, callback) {
		...
		devicelib.view("devices", "devices_by_owner", {
			...
			for (var row in rows) {
				...
				// TODO (14): Get current timezone_abbr and return timezone_offset for that zone with current DST
