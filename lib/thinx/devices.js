...
module.exports = class Devices {
	...
	prefetch_repository(repo_path, source_id, owner) {
		...
		if (fs.existsSync(repo_path)) {
			...
			sources.list(owner, (success, response) => {
				...
				// This is feature-envy. GetPlatform should be extracted.
				this.getPlatform(repo_path, (update_success, watcher_platform) => {
					if (!update_success) {
						console.info("Failed watcher.getPlatform() in sources.list()");
					}
					sources.updatePlatform(owner, source_id, watcher_platform);
	...
	udidsWithSource(owner, source_id) {
		let udids = [];
		this.list(owner, function (success, result) {
			if (!success) return udids;
			for (const element in result) {
				//if (sources.hasOwnProperty(element)) {
				const a_device = result[element];
		...
		devicelib.get(udid, (pre_attach_err, pre_attach_body) => {
			...
			devicelib.destroy(doc._id, doc._rev, (err) => {
				...
				devicelib.insert(doc, doc.udid, (reinsert_err, reinsert_body, header) => {
					...
					console.log("TODO: FIXME: repository watcher used instead of recommended webhooks, but prefetch on attach is OK (we need to get supported platform from the repo).");
