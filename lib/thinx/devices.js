...
var Devices = (function() {
	...
	var _private = {
		normalizedMAC: function(mac) {
			var m = mac.toUpperCase().replace(":", "").split("");
	...
	var _public = {
			...
			devicelib.view("devicelib", "devices_by_owner", {
				...
				function(err, body) {
					...
					for (var row in rows) {
						...
						var mac = dvc.mac.toString();
						console.log("MAC:" + mac);
						var xmac = mac.replace(/:/g, "");
						console.log("XMAC:" + xmac);
						console.log("NMAC:" + _private.normalizedMAC(xmac));
		...
		revoke: function(owner, body, callback) {
			...
			function destroy_device(id, rev, owner, callback) {
				...
				devicelib.destroy(id, rev, function(err) {
					...
					if (err) {
						...
						if (fs.existsSync(passwords_path)) {
							let CMD = "mosquitto_passwd -D " + passwords_path + " " + id;
							...
							if (typeof(callback) !== "undefined") callback(true, id);
						}
					}
				});
			}
			...
			devicelib.view("devicelib", "devices_by_owner", {
				...
				function(err, body) {
					...
					for (var dindex in body.rows) {
						...
						if (udids.indexOf(device_udid) != -1) {
					...
					if (devices.count == 1) {
						...
						destroy_device(doc._id, doc._rev, owner, callback);
					} else {
						for (var gindex in devices_for_revocation) {
							doc = devices_for_revocation[gindex].value;
							console.log("Destroying multiple devices: " + doc._id);
							if (dindex < devices.count) {
								destroy_device(doc._id, doc._rev, owner);
							} else {
								console.log("Destroying last device: " + doc._id);
								destroy_device(doc._id, doc._rev, owner, callback);
							}
						}
					}
