...
const Database = require("./database.js"); // potential problem with this... asks too fast in tests without waiting for DB to be created; could be solved by DI(!) - seems to be only a refactoring fix
...
module.exports = class Devices {
	...
	// Builds the shell script handed to git.fetch() -> exec.execSync(). Every
	// interpolated value is shell-escaped here; the sanitizers upstream are the
	// second line of defence, not the first. Mirrors builder.js
	// gitCloneAndPullCommand(). Pure function, exported (static) for testing.
	static gitPrefetchCommand(repo_path, sanitized_branch, sanitized_url) {
		const escapedPath = shellEscape([repo_path]);
		const escapedBranch = shellEscape([sanitized_branch]);
		const escapedUrl = shellEscape([sanitized_url]);
		return (
			`set +e; mkdir -p ${escapedPath}; cd ${escapedPath} || exit 1; rm -rf ./*; ` +
			`git clone -b ${escapedBranch} ${escapedUrl}; ` +
			`cd *; ` +
	...
	prefetch_repository(repo_path, source_id, owner_id) {
		...
		this.sources.list(owner_id, (success, response) => {
			...
			// Sanitka.branch() returns NULL (never false) when it rejects the
			// input. Comparing against false let a rejected branch through and
			// then threw on .replace() below.
			...
			if (sanitized_url === null) {
				console.log("☣️ [error] Invalid repository URL for source", source_id);
				return false;
			}

			let GIT_COMMAND = Devices.gitPrefetchCommand(repo_path, sanitized_branch, sanitized_url);

			if (git.fetch(owner_id, GIT_COMMAND, repo_path)) {
				// sources.update() and sources.updatePlatform() both read the
				// owner document, mutate its `repos` map and write it back. Fired
				// concurrently they raced on one document, and the loser reported
				// "Document update conflict" / atomic_update_failed -- which is
				// how the platform update kept getting dropped. Chain them.
				...
				// Without a usable checkout there is nothing to infer a platform
				// from, and guessing would only overwrite the stored one.
	...
	updatePlatform(repo_path, source_id, owner_id) {
		Platform.getPlatform(repo_path, (ok, watcher_platform) => {
			if (!ok) {
				// On failure getPlatform's second argument is an error string
				// ("unknown", "local_path not defined"), not a platform. Writing
				// it would replace a correct stored platform with a guess.
				console.log(`⚠️ [warning] Platform could not be inferred for ${source_id}; keeping the stored platform`);
				return;
			}
			this.sources.updatePlatform(owner_id, source_id, watcher_platform, (updated, error) => {
				if (!updated) {
	...
	destroy_device(id, rev, owner, destroy_callback) {
		...
		devicelib.destroy(id, rev, (err) => {
			if (err) {
				...
				InfluxConnector.statsLog(owner, EventTaxonomy.NAMES.DEVICE_REVOCATION, id);
				alog.log(owner, logmessage);
				if ((typeof (destroy_callback) !== "undefined") && (destroy_callback != null)) {
	...
	list(in_owner, callback) {
		...
		devicelib.view("devices", "devices_by_owner", {
			...
			if (err) {
				console.log("☣️ [error] /api/user/devices: Error: ", {err}); // no db shards could be opened?
	...
	detach(body, callback, res) {
		...
		devicelib.view("devices", "devices_by_udid", {
			...
			console.log(`ℹ️ [info] Detaching repository ${doc.udid} from device ${doc._id} (rev ${doc.rev})`);
	...
	attachMesh(owner, body, callback, res) {
		...
		if (!udid) return callback(res, false, "invalid_udid");

		devicelib.get(udid, (pre_attach_err, pre_attach_body) => {

			if (pre_attach_err) {
				console.log("☣️ [error] cannot attach to non-existing device: ", udid);
				return callback(res, false, "udid_not_found");
			}

			if (typeof (pre_attach_body) === "undefined") {
				alog.log(owner, "Attempt to attach repository to non-existent device: " + udid);
				const message = "udid_not_found:" + udid;
				console.log("[DEBUG] attachMesh callback (5): res, false, message:", message);
			...
			devicelib.atomic("devices", "modify", doc._id, doc, (reinsert_err) => {
				if (reinsert_err) {
					...
					console.log("[DEBUG] attachMesh callback (6): res, false, attach_mesh_failed", res, false, "attach_mesh_failed");
				...
				const outObject = Array.from(mesh_ids);
				console.log("[DEBUG] attachMesh callback (7): res, true, outObject", res, false, {outObject});
				callback(res, true, outObject);
			});
		});
	}
