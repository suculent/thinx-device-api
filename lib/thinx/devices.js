	...
	attachMesh(owner, body, callback, res) {
		...
		devicelib.get(udid, (pre_attach_err, pre_attach_body) => {
			...
			if (pre_attach_err) {
				...
				console.log("[DEBUG] attachMesh callback (4): res, false, pre_attach_err", res, false, pre_attach_err);
				return callback(res, false, pre_attach_err);
			}
			...
			if (typeof (pre_attach_body) === "undefined") {
				...
				console.log("[DEBUG] attachMesh callback (5): res, false, message", res, false, message);
