	...
	prefetch_repository(repo_path, source_id, owner) {
		...
		this.sources.list(owner, (success, response) => {
			...
			if (result.indexOf("Please make sure you have the correct access rights") !== -1) {
				...
				// TODO: FIXME: git.fetchWithKeys implementation lost!!!
				git.fetchWithKeys(owner, GIT_PREFETCH, repo_path);

				delete source._rev;
				source.is_private = true;
				// updateSource(owner, source_id, data)
				
				// if fetch succeeds, the `source` object is to be set to private asynchronously
			}
			...
			this.platform.getPlatform(repo_path, (update_success, watcher_platform) => {
				...
				this.sources.updatePlatform(owner, source_id, watcher_platform, (success, error) => {
					if (!success) {
						console.log(`[error] failed updating platform ${platform} for ${source_id}`);
					} else {
