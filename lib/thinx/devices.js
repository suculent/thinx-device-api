...
module.exports = class Devices {
	...
	udidsWithSource(owner, source_id) {
		// TODO TEST:
		let udids = [];
		let devices = this.list(owner, function(success, result) {
			if (success) {
				foreach (source in sources) {
					if (device.source_id == source_id) {
						if (device.auto_update == true) {
							console.log("adding", device.udid, "for source", source_id);
							udids.push(device.udid);
						} else {
							console.log(device.udid, "has auto-update disabled.");
						}
					}
				}
			}
