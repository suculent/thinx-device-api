...
module.exports = class Devices {
	...
	prefetch_repository(repo_path, owner) {
		...
		if (fs.existsSync(repo_path)) {
			...
			sources.list(owner, function(success, response) {
				...
				console.log("TODO: use source: "+source);
				...
				const platform = watcher.getPlatform(repo_path, function(success, platform) {
	...
	revoke_devices(owner, udids, body, destroy_callback) {
		...
		if (devices_for_revocation.length === 0) {
			...
			this.destroy_device(doc._id, doc._rev, owner, function(err, status) {
		...
		devicelib.get(udid, (err, body) => {
			...
			devicelib.destroy(doc._id, doc._rev, (err) => {
				...
				devicelib.insert(doc, doc.udid, (err, body, header) => {
					...
					this.prefetch_repository(repo_path, owner);
				});
			});
		});
	}
	...
	push(owner, body, callback) {
		var udids;
		messenger.initWithOwner(owner, null /* no socket */ ,
