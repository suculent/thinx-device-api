...
module.exports = class Devices {
			...
			this.sources.list(owner, (success, response) => {
				...
				try {
					...
					if (first_git_success !== "Already up to date.") {
						console.log(`[builder] git prefetch result: ${first_git_success}`);
					}
	...
	list(owner, callback) {
		...
		if (Validator.owner(owner) === false) {
			console.log(`[error] [list] owner validation failed for '${owner}'`);
			callback(false, {
				success: false,
		...
		if ((typeof (owner) === "string") && (owner === "")) {
			console.log(`[error] [list] owner validation failed for empty owner`);
			callback(false, {
				success: false,
				devices: []
			});
