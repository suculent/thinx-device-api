...
module.exports = class Devices {
	...
	revoke_devices(owner, udids, body, destroy_callback) {
		...
		console.log("devices_for_revocation", {devices_for_revocation});
		...
		if (devices_for_revocation.length === 0) {
			...
			if (typeof(doc) !== "undefined") {
				...
				destroy_callback(false, {devices_for_revocation});
			}
			return;
	...
	list(owner, callback) {
		...
		//console.log("Listing devices for owner_id", owner);
	...
	attachMesh(owner, body, callback) {

		if (typeof (body.mesh_id) === "undefined") {
			callback(false, "missing_mesh_id");
			return;
		}
		...
		alog.log(owner, "Attempt to attach mesh: " + attached_mesh_id + " to device: " + udid);

		console.log("[OID:" + owner + "] [MESH_ATTACH] " + udid);
			...
			devicelib.destroy(doc._id, doc._rev, (err) => {
				...
				devicelib.insert(doc, doc.udid, (reinsert_err, reinsert_body, header) => {
					if (reinsert_err) {
						...
						callback(false, "attach_mesh_failed");
					} else {
						console.log("INSERT: " + JSON.stringify(reinsert_body));
						console.log("[attachMesh] success:", Array.from(mesh_ids));
						callback(true, Array.from(mesh_ids));
					}
				});
			});
		});
	...
	detachMesh(owner, body, callback) {
		...
		alog.log(owner, "Attempt to detach mesh from device: " + udid);
			...
			devicelib.destroy(doc._id, doc._rev, () => 
			{
				...
				devicelib.insert(doc, doc.udid, (detach_reinsert_err) => {
					if (detach_reinsert_err) {
						...
						callback(false, "detach_mesh_failed");
					} else {
						console.log("[detachMesh] success:", message);
						callback(success, Array.from(mesh_ids));
					}
				});
			});
		});
	...
	revoke(owner, body, destroy_callback) {
		...
		devicelib.view("devicelib", "devices_by_owner", {
			...
			console.log("view_body: ", {view_body});
			let array = view_body.rows;
			console.log("view_body: ", {array});
			let doc = array[0].doc;
			console.log("view_body doc: ", {doc});
			let value = array[0].doc;
			console.log("view_body value: ", {value});
