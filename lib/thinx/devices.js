				...
				function(err, body) {
					...
					for (var row in rows) {
						...
						var deviceDescriptor = {
							...
							status: dvc.status,
							tags: dvc.tags,
							timezone_abbr: dvc.timezone_abbr || "UTC",
							timezone_offset: dvc.timezone_offset || 0,
							...
							sources.list(owner, function(success, response) {
								...
								if (success === true) {
									...
									var GIT_PREFETCH = "bash -c \"cd " + repo_path +
										"; echo Cleaning workdir...; rm -rf *; git clone " + source.url +
										"; cd *; git pull origin master --recurse-submodules\"";
										"; cd *; git pull origin master --recurse-submodules\"";
									var nochange = "Already up-to-date.";
									var result = exec.execSync(GIT_PREFETCH).toString().replace(
										"\n", "");
