...
function attach(client, opts) {
    ...
    function postSafely(text) {
        ...
        try {
            // `.send()` returns a Promise; the try/catch only traps sync throws.
            // Catch async rejection too (e.g. Slack webhook `no_team`) so it never
            ...
            if (sendResult && typeof sendResult.catch === 'function') {
                sendResult.catch((e) => console.log('☣️ [error] [redis-health] SlackNotify.send failed (async):', e && e.message ? e.message : e));
            }
        } catch (e) {
