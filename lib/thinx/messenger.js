  ...
  retry_strategy: function (options) {
	...
	var _public = {
		...
		init: function() {
			...
			// console.log("Connecting MQTT with config: " + JSON.stringify(connect_config, false, 4));
			...
			_public.getDevices(owner, function(success, devices) {
				...
				if (!success) {
					...
					_public.apiKeyForOwner(owner, function(success, apikey) {
						...
						if (typeof(apikey.hash) === "undefined") {
						...
						clients[owner] = mqtt.connect([{
							...
							port: 1883,
							username: owner,
							password: apikey.hash
						}]);

						console.log("MESENGER #1 CONNECT: "+owner+": hash = "+JSON.stringify(apikey.hash));
						...
						clients[owner].on("reconnect", function() {
							...
							console.log("MESENGER RECONNECT: "+owner+": hash = "+JSON.stringify(apikey.hash));
						});
