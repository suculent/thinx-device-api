	...
	*/
	...
	var _public = {
		...
		initWithOwner: function(owner, websocket, callback) {
			...
			console.log("Initializing messenger socket with owner " + owner);
					...
					_public.apiKeyForOwner(owner, function(success, apikey) {
						...
						clients[owner].on("message", function(topic, message) {
							...
							if (typeof(message.status) !== "undefined") {
								...
								// TODO: Fetch device info (will cause additional DB load...)
								devicelib.get(udid, function(error, body) {
									...
									if (message.status == "connected") {
										var notification_in = {
											notification: {
												title: "Check-in",
												body: "Device " + body.name + " checked-in.",
												type: "info",
												udid: did
											}
										};
										...
										var notification_out = {
											notification: {
												title: "Check-out",
												body: "Device " + body.name + " disconnected.",
												type: "warning",
												udid: did
											}
										};
