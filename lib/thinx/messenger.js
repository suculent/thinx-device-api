...
module.exports = class Messenger {
	...
	createInstance(redis, password) {
		...
		this.devicelib = require("./couch")(new Database().uri()).use(prefix + "managed_devices");
	...
	_resolveBroker() {
		const server = app_config.mqtt.server;
		const defaultPort = app_config.mqtt.port;

		let host = typeof server === "string" ? server : undefined;
		let port = defaultPort;
		const url = (typeof server === "string" && server.length > 0) ? server : undefined;

		if (typeof server === "string" && server.length > 0) {
			const hasScheme = /^[a-zA-Z][a-zA-Z0-9+\-.]*:\/\//.test(server);
			const parseTarget = hasScheme ? server : `mqtt://${server}`;
			try {
				...
				const stripped = server.replace(/^[a-zA-Z][a-zA-Z0-9+\-.]*:\/\//, "");
  ...
  */
	...
	fetchKeyAndPublish(owner, udid, message) {
		this.user.mqtt_key(owner, (success, apikey) => {
			...
			const broker = this._resolveBroker();
			const connectionHost = (typeof (broker.host) === "string" && broker.host.length > 0) ? broker.host : app_config.mqtt.server;
			const connectionPort = (typeof (broker.port) !== "undefined" && broker.port !== null) ? broker.port : app_config.mqtt.port;
			const targetUrl = (typeof (broker.url) === "string" && broker.url.length > 0) ? broker.url : null;
			const connectOptions = {
				host: connectionHost,
				port: connectionPort,
				username: owner,
				password: apikey.key,
				// Reconnect on transient broker DNS/connection failures (see master
				// connect note) instead of failing the publish outright.
				reconnectPeriod: 2000,
				connectTimeout: 30000
			};
			this.clients[owner] = targetUrl ? mqtt.connect(targetUrl, connectOptions) : mqtt.connect(connectOptions);
	...
	ensureJSON(message) {
		try {
			...
			} catch (_parser_err) { /* if not JSON it's just a message so we can ignore this */ }
		}
		return message;
	}
	...
	setupMqttClient(owner, mqtt_options, callback) {
		...
		const broker = this._resolveBroker();
		if (typeof (broker.host) === "string" && broker.host.length > 0) {
			mqtt_options.host = broker.host;
		}
		if (typeof (broker.port) !== "undefined" && broker.port !== null) {
			mqtt_options.port = broker.port;
		}
		const connectTarget = (typeof (broker.url) === "string" && broker.url.length > 0) ? broker.url : null;

		try {
			this.clients[owner] = connectTarget ? mqtt.connect(connectTarget, mqtt_options) : mqtt.connect(mqtt_options);
		} catch (e) {
	...
	init(password) {
		...
		this.inject_password(mqtt_password, () => {
			...
			const broker = this._resolveBroker();
			const masterHost = (typeof (broker.host) === "string" && broker.host.length > 0) ? broker.host : app_config.mqtt.server;
			const masterPort = (typeof (broker.port) !== "undefined" && broker.port !== null) ? broker.port : app_config.mqtt.port;
			const masterTarget = (typeof (broker.url) === "string" && broker.url.length > 0) ? broker.url : null;

			let mqtt_config = {
				clientId: 'THiNX-API-' + require('crypto').randomBytes(4).toString('hex'),
				host: masterHost,
				port: masterPort,
				username: "thinx",
				password: mqtt_password,
				// Tolerate transient broker DNS/connection failures (e.g. a
				// `getaddrinfo EAI_AGAIN mosquitto` blip under load) by reconnecting
				// instead of surfacing a hard error. mqtt.js re-resolves the host on
				// each reconnect attempt.
				reconnectPeriod: 2000,
				connectTimeout: 30000
			};

			this.master = masterTarget ? mqtt.connect(masterTarget, mqtt_config) : mqtt.connect(mqtt_config);
	...
	initWithOwner(owner, websocket, callback) {
		...
		this.user.mqtt_key(owner, (key_success, apikey) => {
			...
			const broker = this._resolveBroker();
			let mqtt_options = {
				host: (typeof (broker.host) === "string" && broker.host.length > 0) ? broker.host : app_config.mqtt.server,
				port: (typeof (broker.port) !== "undefined" && broker.port !== null) ? broker.port : app_config.mqtt.port,
				username: owner,
				password: api_key,
