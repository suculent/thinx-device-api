  ...
  */
	...
	processActionableNotification(did, topic, message) {
		...
		if (messageFromDevice) {
			...
			console.log(`[info] messageNotFromDevice (notification.response defined) parsing message ${msg}`);

			let nid = message.notification.nid;
			...
			if (typeof(nid) !== "undefined") {
				...
				this.redis.get("nid:" + nid, (error, json_keys) => {
					...
					if (nid_data.length > 0) {
						...
						nid_data.done = true;
						nid_data.response = message.notification.response;
						if (typeof(message.notification.response_type) !== "undefined") {
							nid_data.response_type = message.notification.response_type;
						}
