...
module.exports = class Messenger {
	...
	attachCallback(rtm, err_callback) {
		...
		if (this.rtm == null) {
			console.log("☣️ [error] Slack initialized to late or attachCallback called too soon...");
			return;
		}
			...
			akey.get_first_apikey(oid, (success, apikey) => {
				...
				device.register(
					...
					(reg_success, registration_response) => {
						...
						// TODO (18): the UDID parameter is a reflection here, should be validated instead!
						// https://github.com/suculent/thinx-device-api/issues/311
			...
			if (messageFromDevice) {
				...
				this.client.get(nid, (err, /* json_keys */) => {
					...
					if (!err) {
						...
						console.log("[warning] Actionable notification " + nid +
	...
	setupMqttClient(owner, apikey, mqtt_options, callback) {
		...
		this.clients[owner].on("connect", (error) => {
			...
			console.log("ℹ️ [info] Messenger #1 connected.");
			...
			if (typeof (callback) === "function") {
				...
				console.log("[warning] No callback in messenger.js:578");
			}
		});

		this.clients[owner].on("reconnect", () => {
			console.log("ℹ️ [info] [client messenger] reconnected...");
		});
		...
		this.clients[owner].on('close', () => {
			console.log('[info] [mqtt] Connection closed for owner', owner);
		});
	...
	attach_callbacks(master) {

		master.on("connect", this.connect_callback);

		master.on("error", this.error_callback);

		master.on("message", this.message_callback);
	}
	...
	init(password) {
		...
		this.inject_password(mqtt_password, () => {
			...
			this.attach_callbacks(this.master);
	...
	initWithOwner(owner, websocket, callback) {
		...
		this.getDevices(owner, (success, devices) => {
			...
			this.user.mqtt_key(owner, this.password, (key_success, apikey) => {
				...
				if (!key_success) {
					console.log("[warning] MQTT: API key fetch failed (may happen for new owner, trying to generate some...) " + owner);
				...
				if ((typeof (apikey) === "undefined") || (typeof (apikey.hash) === "undefined")) {
					console.log("☣️ [error] MQTT: No API keys found in Redis for this owner. Database persistency not enabled?");
				}
