...
module.exports = class Messenger {
	...
	async getBotToken() {
		...
		let token = await this.client.getAsync("__SLACK_BOT_TOKEN__")
			.catch((err) => {
			...
			this.web.conversations.list({ limit: 20 })
				.then((response) => {
					for (var c in response.channels) {
						...
						console.log("[debug] [slack] Finding conversation in list...");
	...
	registerDevice(_registration, _auth, _ws, oid) {
		device.register(
			{}, /* req */
			_registration,
			_auth,
	...
	messageResponder(topic, message) {
		...
		this.devicelib.get(did, (error, body) => {
			...
			if (topic.indexOf("/status") !== -1) {
				...
				this.user.get(oid).then((profile) => {
					device.runDeviceTransformers(profile, body, null, null);
				});
			}
		});
	...
	sendSlack(message, chan, callback) {
		...
		if (process.env.ENVIRONMENT == "test") {
			return callback(true, message); // do not spam Slack by tests
		}
	...
	data(owner, udid, callback) {
		console.log("[info] Getting messenger data for owner: " + owner);
		this.client.keys("/*" + owner + "/" + udid + "*", (_err, replies) => {
			if (_err || (replies.length == 0)) return callback(false, "no_data");
			var results = {};
