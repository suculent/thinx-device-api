...
var Messenger = (function(){
  ...
  var redis = require("redis");
	...
	function _publish(owner, udid, message) {
		if (typeof(client[owner]) === "undefined") {
			apiKeyForOwner(owner, function(success, apikey) {
		...
		if (typeof(message.nid) !== "undefined") {
			...
			client.set(nid, JSON.stringify(message), (err) => {
				if (err) {
					console.log(err);
				}
			});
	...
	function attachCallback(rtm, err_callback) {
	...
	var messageResponder = function(topic, message) {
			 ...
			 (message.registration !== null))
		{
			...
			device.register(message.registration, _auth, _ws, function(success, registration_response) {
				...
				_publish(oid, message.registration.udid, registration_response);
			}, null);
		}
		...
		if (typeof(message.notification) !== "undefined") {
			...
			if (messageFromDevice) {
				...
				client.get("nid:" + notification_to.nid, (err, json_keys) => {
						...
						client.set("nid:" + did, JSON.stringify(nid_data), (err) => {
							if (err) {
								console.log("MQTT Action Save Error: " + err);
							}
						});
	...
	client.get("__SLACK_BOT_TOKEN__", (err, token) => {
		...
		function postRandomQuote(quote) {
				...
				web.chat.postMessage({ channel: this.channel, text: message })
					.then((res) => {
			...
			.then((response) => {
				for (var c in response.channels) {
					...
					if (conversation.name == app_config.slack_bot_topic) {
						...
						new GRQ().then((quote) => { postRandomQuote(quote); });
					}
				}
			})
			.catch((error) => {
	...
	var sendSlack = function(message, chan, callback) {
		rtm.sendMessage(message, chan, function(err, response) {
			if (err) {
				console.log(err);
			}
			if (typeof(callback) !== "undefined") {
	...
	var _public = {
		...
		init: function() {
			...
			console.log("MQTT connect with config: "+mqtt_config);

			master = mqtt.connect(app_config.mqtt.server, mqtt_config);
			...
			master.on("message", function(topic, message) {
				...
				client.get(topic + "@" + iso_date, (err, json_keys) => {
					...
					client.set(topic + "@" + iso_date, JSON.stringify(newkeys), (err) => {
							if (err) {
		...
		initWithOwner: function(owner, websocket, callback) {
			...
			_public.getDevices(owner, function(success, devices) {
				...
				this.apiKeyForOwner(owner, function(success, apikey) {
		...
		slack: function(owner, message, callback) {
			...
			if ((typeof(channel) === "undefined") || (channel === null)) {
				client.get("__SLACK_CHANNEL_ID__", (err, ch) => {
