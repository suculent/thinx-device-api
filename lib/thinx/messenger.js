			...
			*/
			...
			master.on("message", function(topic, message) {
				...
				client.get(topic + "@" + iso_date, function(err, json_keys) {
					...
					keys[new Date().getTime().toString()] = enc_message;
					client.set(topic + "@" +
						iso_date, JSON.stringify(keys),
						function(err) {
							if (err) {
				...
				var get_result_or_callback = function(reply, i, callback) {
					...
					client.get(reply, function(err, json_keys) {
						...
						if (err) {
							...
							// TODO: Decode Base64
							console.log("Stringified Keys: " + JSON.stringify(json_keys));

							if ((typeof(json_keys) !== "undefined") && json_keys !== null) {
								var ks = Object.keys(json_keys);
								for (var index in ks) {
									const key = ks[index];
									var value = json_keys[key];
									console.log("Returning key " + key + " with value " + value);
									results[key] = value; // base64.decode(value);
								}
