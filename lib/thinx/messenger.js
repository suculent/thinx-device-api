...
module.exports = class Messenger {
	...
	setupMqttClient(owner, mqtt_options, callback) {
		...
		console.log(`setupMqttClient options ${JSON.stringify(mqtt_options)}`);
		...
		this.clients[owner].on("error", (error) => {
			...
			if (error.toString().indexOf("Not authorized") != -1) {
				console.log(`[error] Owner ${owner} not authorized to access using apikey [redacted]`);
				this.clients[owner].stop();
			}
		});
