...
module.exports = class Messenger {
	...
	initSlack() {
		...
		this.client.get("__SLACK_BOT_TOKEN__", (err, token) => {
			...
			if (typeof(bot_token) === "undefined" || bot_token == null) {
			...
			this.rtm.on('reaction_added', (reaction) => {
				console.log('» Slack API: Reaction added:', reaction);
			});

			this.rtm.on('reaction_removed', (reaction) => {
	...
	messageResponder(topic, message) {
			 ...
			 (message.registration !== null))
		{
			...
			const _ws = null;
			const _auth = apikey.key; // TODO: fetch API key for device here to allow/fix MQTT registration...
	...
	init() {
		...
		// old
		if (typeof(app_config.mqtt.key) !== "undefined") {
			mqtt_password = app_config.mqtt.key;
		}
		...
		var mqtt_config = {
			...
			password: mqtt_password // could be password, but that's dangerous to change now
		};
		...
		this.master.on("reconnect", () => {
			//console.log("[OID:MASTER] listener reconnected with config: ", mqtt_config);
			console.log("[TODO:FIXME] Restart mosquitto or use shell command: mosquitto_password -b ", app_config.mqtt.passwords, app_config.mqtt.username, mqtt_password);
			const CMD = "mosquitto_password -b " + app_config.mqtt.passwords + " " + app_config.mqtt.username + " " + mqtt_password;
			var result = exec.execSync(CMD).toString().replace("\n", "");
			console.log("Overridding THiNX MQTT password for owner with result: "+result);

			console.log("Testing zzz (3s)...");
			zzz(); // should be used after MQTT password is adjusted, but never mind so far... needs ES8
		});
	...
	initWithOwner(owner, websocket, callback) {
		...
		this.getDevices(owner, (success, devices) => {
			...
			user.mqtt_key(owner, (success, apikey) => {
				...
				if (!success) {
					console.log("MQTT: API key fetch failed for owner "+owner);
					return;
				}
