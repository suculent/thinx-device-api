...
module.exports = class Messenger {
	...
	async initSlack() {
		...
		console.log("Initializing Slack with bot token ", bot_token);
	...
	setupMqttClient(owner, apikey, mqtt_options, callback) {
		...
		console.log("Connecting to", app_config.mqtt.server, "with options", {mqtt_options});
		...
		this.clients[owner].on("error", (error) => {
			...
			if (error.toString().indexOf("Not authorized") > 0) {
	...
	init() {
		...
		async function zzz() {
			console.log('[messenger::zzz] Taking a break...');
			await sleep(3000);
			console.log('[messenger::zzz] Two seconds later, showing sleep in a loop...');
