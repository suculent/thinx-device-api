...
module.exports = class Messenger {
		...
		devicelib.get(did, (error, body) => {
			...
			if (topic.indexOf("/status") !== -1) {
				...
				device.edit(oid, changes, (success, a_message) => {
					if (!success) {
						console.log("MQTT Device Edit Success: " + success + " status: " + a_message);
					}
				});
				// TODO: FIXME: Should reuse transformer request code from device's checkinExistingDevice, this is insecure fallback and run_transfomers is deprecated or should be reused for this call with new params
				device.run_transformers(did, body.owner, false, (rerror, rstatus) => {
					console.log("Warning: calling insecure transformer, refactor to containerized call!");
	...
	setupMqttClient(owner, apikey, mqtt_options, callback) {
		...
		this.clients[owner].on("error", (error) => {
			...
			if (error.toString().indexOf("Not authorized") != -1) {
				throw new Error("Valid MQTT connection required. Without that, this app is useless..."); // restart might fix this
			}
			console.log("Killing on critical MQTT error... Broker MUST work in order to start THiNX.");
			process.exit(0);
		});
	...
	init() {
		...
		console.log("[messenger::init] Connecting", app_config.mqtt.server);
		...
		this.master.on("error", (error) => {
			// console.log("[OID:MASTER] MQTT: error '" + error + "' with config: " + JSON.stringify(mqtt_config, false, 2)); // TODO: FIXME: LEAKS CONFIG!
