...
module.exports = class Messenger {
	...
	async initSlack() {
		...
		this.client.get("__SLACK_BOT_TOKEN__", (err, token) => {
			...
			if (typeof (bot_token) === "undefined" || bot_token == null || bot_token == "") {
				console.log("[error] ☣️ Skipping Slack RTM, no bot token available.");
				return;
			}
	...
	init(password) {
		...
		if (process.env.ENVIRONMENT === "test") {
			...
			console.debug("[debug] [messenger] Process environment:", process.env.ENVIRONMENT);
		}

		this.inject_password(password, () => {

			console.log("[info] ℹ️ MQTT credentials refresh complete, initializing Messenger...");
			...
			this.master.on("reconnect", () => {
				...
				console.log(`[critical] 🚫 MQTT reconnecting.`);
			});
			...
			this.master.on("error", (error) => {
				if (error.toString().indexOf("Not authorized") !== -1) {
					...
					console.log("[error] ☣️ MQTT error:", error);
				}
			});
			...
			this.master.on("message", (topic, message) => {
				...
				this.client.get(topic + "@" + iso_date, (err, json_keys) => {
					...
					try {
						...
						console.log("[error] ☣️ parsing redis keys for client.get: " + e);
					}
