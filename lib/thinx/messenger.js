...
var Messenger = (function(){
  ...
  var redis = require("redis");
	...
	function _apiKeyForOwner(owner, callback) {
		apikey.list(owner, function(success, keys) {
			if (success === true) {
				if (typeof(callback) !== "undefined") {
					console.log("API Key list found :" + keys[0]);
	...
	function _publish(owner, udid, message) {
		if (typeof(client[owner]) === "undefined") {
			_apiKeyForOwner(owner, function(success, apikey) {
	...
	var _public = {
		...
		apiKeyForOwner: function(owner, callback) {
			_apiKeyForOwner(owner, callback);
		},
