...
module.exports = class Messenger {
	...
	async getBotToken() {
		...
		return Promise.resolve(
			await this.client.getAsync("__SLACK_BOT_TOKEN__")
				.catch((err) => {
	...
	updateAndTransformDeviceStatus(did, message) {
		this.devicelib.get(did, (error, body) => {
			...
			if (topic.indexOf("/status") !== -1) {
				...
				this.user.get(oid).catch(e => {
					console.log("Transformers failed with", e);
				}).then((profile) => {
	...
	processConnectionChange(message) {
	...
	processActionableNotification(message) {
	...
	processActionableMessages(message) {
		...
		// Match by message.connected (should be optional)
			...
			this.processConnectionChange(message);
			

		// Match by Actionability
		} else if (Util.isDefined(message.notification)) {
			this.processActionableNotification(message);
	...
	messageResponder(topic, message) {
		...
		this.updateAndTransformDeviceStatus(did, message);

		// Process other well-known mesages
		// Match by message.status (should be optional)
		this.processActionableMessages(message);
	}
	...
	message_callback(topic, message) {
		...
		this.client.get(topic + "@" + iso_date, (_err, json_keys) => {
	...
	data(owner, udid, callback) {
		...
		this.client.keys("/*" + owner + "/" + udid + "*", (_err, replies) => {
	...
	slack(_owner, message, callback) {
		...
		if ((typeof (this.channel) === "undefined") || (this.channel === null)) {
			this.client.get("__SLACK_CHANNEL_ID__", (err, ch) => {
				if ((err === null) && (typeof (ch) !== "undefined") && (ch !== null)) {
					this.channel = ch;
