...
module.exports = class Messenger {
	...
	async getBotToken() {
		...
		this.client.getAsync("__SLACK_BOT_TOKEN__").then((err, token) => {
	...
	initSlack() {
		...
		let token = this.getBotToken();

		console.log("Initializing Slack with bot token ", token);
	...
	setupMqttClient(owner, apikey, mqtt_options, callback) {
		...
		this.clients[owner].on("error", (error) => {
			...
			if (error.indexOf("Connection refused: Not authorized") !== -1) {
