...
module.exports = class Messenger {
	...
	async getBotToken() {
		...
		return Promise.resolve(
			await this.redis.getAsync("__SLACK_BOT_TOKEN__")
				.catch((err) => {
	...
	publish(owner, udid, message) {
		...
		if (typeof (message.nid) !== "undefined") {
			...
			this.redis.set(nid, JSON.stringify(message), (err) => {
				console.log("[debug] publishing nid message", message);
				if (err) {
	...
	attachCallbacks() {
		...
		this.rtm.on('ready', (/* rtmStartData */) => {
			this.redis.get("slack-conversation_id", (error, result) => {
				if (!error) {
					this.channels = result;
				} else {
					this.fetchAndUpdateChannel();
				}
			});
		});
	}
	...
	updateAndTransformDeviceStatus(oid, did, message, topic) {
		this.devicelib.get(did, (error, body) => {
			...
			if (topic.indexOf("/status") !== -1) {
				...
				device.edit(changes, (success, a_message) => {
					if (!success) {
				...
				this.user.profile(oid, (profile) => {
					device.runDeviceTransformers(profile, body, null, null, null);
				});
			}
		});
	}
	...
	processActionableNotification(did, topic, message) {
		...
		if (messageFromDevice) {
			...
			this.redis.get(nid, (err, /* json_keys */) => {
				...
				this.redis.set("nid:" + did, not, (set_err) => {
					if (set_err) {
			...
			this.redis.get("nid:" + notification_to.nid, (err, json_keys) => {
				...
				this.redis.set("nid:" + did, JSON.stringify(nid_data), (nid_err) => {
					if (nid_err) {
	...
	message_callback(topic, message) {
		...
		this.redis.get(topic + "@" + iso_date, (_err, json_keys) => {
			...
			this.redis.set(topic + "@" + iso_date, JSON.stringify(newkeys), (set_err) => {
	...
	data(owner, udid, callback) {
		...
		this.redis.keys("/*" + owner + "/" + udid + "*", (_err, replies) => {
	...
	initWithOwner(owner, websocket, callback) {
		...
		this.getDevices(owner, (success, devices) => {
			...
			this.user.mqtt_key(owner, (key_success, apikey) => {
				...
				console.log(`[debug] MQTT: Commencing checks...`);
				...
				console.log(`[debug] MQTT: Commencing setupMqttClient`);
	...
	push(owner, body, callback) {
		...
		for (var dindex in udids) {
			this.publish(owner, udids[dindex], { configuration: body.enviros });
		}
