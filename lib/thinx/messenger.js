	...
	processActionableNotification(did, topic, message) {
		...
		if (messageFromDevice) {
			...
			if (notification_to.nid) {
				...
				console.log(`[info] [messenger] NID message for device ${notification_to.nid}: ${msg}`);
				// Search existing transaction
				this.redis.get("nid:" + notification_to.nid, (error, json_keys) => {
					// {"notification":{"response":false,"nid":"nid-0000"}
					if (error) {
						console.log("☣️ [error] in processActionableNotification: NID not found", error);
						return;
					}
