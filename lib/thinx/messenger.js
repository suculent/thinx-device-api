		...
		init: function() {
			...
			*/
			...
			master.on("message", function(topic, message) {
				...
				// Warning, insecure!
				console.log("[OID:MASTER] MQTT Message String: " + msg_string);

				if (msg_string.length === 0) return;

				const enc_message = base64.encode(message); // prevent attacks on Redis

				var iso_date = dateFormat(new Date(), "isoDate");

				var timestamp = new Date().toString();

				client.set(topic + "@" + timestamp, enc_message, function(err) {
					if (err) {
						console.log("MQTT/Redis Action Save Error: " + err);
					}
				});

				// Keep MQTT Data for 24 hours only
