...
var Messenger = (function() {
	...
	*/
	...
	var bot = new SlackBot({
		token: app_config.slack_bot_token, // Add a bot https://my.slack.com/services/new/bot and put the token
		name: 'THiNX Support'
	});

	bot.on('start', function() {

		console.log("Started user slackbot...");
		...
		bot.postMessageToChannel('thinx', 'Hello!', params);

		// define existing username instead of 'user_name'
		bot.postMessageToUser('suculent', 'meow!', params);

		// If you add a 'slackbot' property,
		// you will post to another user's slackbot channel instead of a direct message
		bot.postMessageToUser('mikulas', 'meow!', {
			'slackbot': true,
			icon_emoji: ':cat:'
		});
	...
	bot.on('message', function(data) {
		...
		// What if this is support reqoest or response?

		// Send the message to respective Slack thread saved in Redis (owner_id)
		console.log("Incoming slack message: " + JSON.stringify(data));

		if (data.type == "desktop_notification") {
			// when someone writes to the webslack app channel
		}
		...
		if (typeof(socket) !== "undefined" && socket !== null) {
			...
			console.log("[messenger] no socket to forward the message... alles kitz.");
	...
	var _public = {
					...
					_public.apiKeyForOwner(owner, function(success, apikey) {
						...
						clients[owner].on("connect", function(error) {
							...
							if (typeof(callback) == "function") {
								if (error.returnCode !== 0) {
									...
									console.log("Messeger using callback with result TRUE.");
									callback(true, error);
									return;
								}
							} else {
						...
						clients[owner].on("message", function(topic, message) {
							...
							if (typeof(bot) !== "undefined") {
								bot.postMessageToChannel('thinx', message, params);
							}
		...
		slack: function(owner, message, callback) {
			// define channel, where bot exist. You can adjust it there https://my.slack.com/services
			bot.postMessageToChannel(owner.username, message, owner, function(data) {
				callback(data);
			});
