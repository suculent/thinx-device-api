	...
	var _public = {
		...
		init: function() {
			...
			master.on("reconnect", function() {
				console.log("[OID:MASTER] listener reconnected with config " + JSON.stringify(connect_config));
			});
				...
				if (!success) {
					...
					_public.apiKeyForOwner(owner, function(success, apikey) {
						...
						clients[owner].on("reconnect", function() {
							...
							console.log("MESENGER RECONNECT: " + owner + ":: key = "+JSON.stringify(apikey.key)+" options: "+JSON.stringify(mqtt_options));
						});
