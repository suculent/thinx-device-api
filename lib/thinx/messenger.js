...
var Messenger = (function() {
	...
	client.get("__SLACK_BOT_TOKEN__", function(err, token) {
		...
		web.conversations.info()
				.then((response) => {
					// Success!
					console.log('Conversations info response:');
					console.log(response);
				})
				.catch((error) => {
					// Error :/
					console.log('Conversations info error:');
					console.log(error);
				});
		...
		rtm.on('ready', (rtmStartData) => {
			...
			console.log("rtmStartData: "+ JSON.stringify(rtmStartData), rtmStartData);

			if (typeof(rtmStartData) === "undefined") {
				return;
			}
