...
module.exports = class Messenger {
	...
	async initSlack(callback) {
		...
		this.rtm.start().then( () => {
			console.log("✅ [info] Slack RTM started...");
			callback(true);
		}).catch(s => {
	...
	inject_password(password, callback) {
		...
		console.log("Injecting MQTT password", password); // FIXME: REMOVE WHEN DONE
	...
	connect_callback(error) {
		if (error) {
			console.log("[error] connect_callback", error);
		} else {
	...
	init(password) {
		...
		this.inject_password(mqtt_password, () => {
			...
			console.log("ℹ️ [info] Connecting MQTT with config", mqtt_config); // TODO: REMOVE ME
