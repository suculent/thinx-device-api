...
module.exports = class Messenger {
	...
	async getBotToken() {
		...
		this.client.getAsync("__SLACK_BOT_TOKEN__")
			.catch((err) => {
				console.log("Error fatching Slack bot token: ", err);
	...
	async initSlack() {
		...
		this.client.get("__SLACK_BOT_TOKEN__", (err, token) => {
			...
			if (typeof (bot_token) === "undefined" || bot_token == null || bot_token == "") {
				console.log("[error] Skipping Slack RTM, no bot token available.");
				return;
			}
	...
	publish(owner, udid, message) {
		...
		if (typeof (this.clients[owner]) === "undefined") {
			...
			this.user.mqtt_key(owner, this.password, (success, apikey) => {
				if (success) {
	...
	setupMqttClient(owner, apikey, mqtt_options, callback) {
		...
		this.clients[owner].on("reconnect", () => {
			console.log("[client messenger] reconnected...");
		});
	...
	inject_password(password, callback) {
		...
		console.log("Initializing MQTT with password", password); // intentional logging for administrative/testing purposes
	...
	init(password) {
		...
		if ((typeof (this.password) === "undefined") || (this.password === null)) {
			console.log("[critical] no this.password in messenger.init!");
		} else {
			console.log("[info] messenger.init with this.password");
			mqtt_password = this.password;
		}

		if ((typeof (password) === "undefined") || (password === null)) {
			console.log("[critical] no password in messenger.init!");
		} else {
			console.log("[info] messenger.init with password");
		...
		if ((process.env.ENVIRONMENT === "test") || (process.env.ENVIRONMENT === "circleci")) {
			mqtt_password = "mosquitto"; // test purposes only; to align with REDIS_PASSWORD variable set on CCI
		}

		this.inject_password(password, () => {

			// TODO: Add a reconnect-timeout and fail service if the connection does not work.
			...
			console.log("messenger.init(password); MQTT connect with config", mqtt_config); // TODO: FIXME! REMOVE AFTER USE!
			...
			this.master.on("reconnect", () => {
				/* Problem description: in test, only one of three calls here has password. Race condition? */
				console.log("[OID:MASTER] MQTT reconnecting as", mqtt_username, mqtt_password);  // TODO: FIXME! REMOVE AFTER USE!
			});

			this.master.on("error", (error) => {
				if (error.toString().indexOf("Not authorized") !== -1) {
					console.log("[OID:MASTER] MQTT connection not authorized!");
				} else {
