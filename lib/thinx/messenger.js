...
module.exports = class Messenger {
	...
	async initSlack(callback) {
		...
		this.client.get("__SLACK_BOT_TOKEN__", (err, token) => {
			...
			if ((err === null) && (typeof (token) !== "undefined") && (token !== null)) {
	...
	registerDevice(_registration, _auth, _ws) {
		device.register(
			{}, /* req */
			_registration,
			_auth,
	...
	messageResponder(topic, message) {
		...
		if ((topic.indexOf("/status") !== -1) &&
			...
			akey.get_first_apikey(oid, (success, apikey) => {
				if (!success) {
					this.user.create_default_mqtt_apikey(owner_id, (result) => {
						let _auth = result.key;
						this.registerDevice(message.registration, _auth, _ws);
					});
				} else {
