...
module.exports = class Messenger {
	...
	sendRandomQuote() {
		...
		new GRQ().then((quote) => {
			...
			console.log("Random quote error", err);
		});
	...
	inject_password(password, callback) {
		...
		console.log("[debug] Initializing MQTT with password", password); // intentional logging for administrative/testing purposes
	...
	init(password) {
		...
		if (process.env.ENVIRONMENT === "test") {
			...
			console.debug("[debug] Process environment:", process.env.ENVIRONMENT);
		}

		this.inject_password(password, () => {

			console.log("[info] MQTT credentials refresh complete, initializing Messenger...");
			...
			this.master.on("reconnect", () => {
				...
				console.log(`[critical] MQTT reconnecting with ${mqtt_username} and ${mqtt_password}`);
			});

			this.master.on("error", (error) => {
				if (error.toString().indexOf("Not authorized") !== -1) {
					console.log(`[critical] MQTT connection not authorized with ${mqtt_username} and ${mqtt_password}`);
				} else {
