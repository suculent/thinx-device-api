...
var Messenger = (function() {
	...
	*/
	...
	var bot_token = process.env.SLACK_BOT_TOKEN || app_config.slack_bot_token;
	...
	client.get("__SLACK_BOT_TOKEN__", function(err, token) {
		if (!err) {
			...
			bot_token = app_config.slack_bot_token || process.env.SLACK_BOT_TOKEN;
		}

		console.log('» Starting Slack with bot_token ' + bot_token);
	...
	var _public = {
		...
		initWithOwner: function(owner, websocket, callback) {
			...
			console.log("Initializing messenger with owner " + owner + " and socket " + websocket);
				...
				if (success) {
					...
					_public.apiKeyForOwner(owner, function(success, apikey) {
						...
						clients[owner].on("connect", function(error) {
							...
							for (var i = _private._devices.length - 1; i >= 0; i--) {
								var udid = _private._devices[i];
