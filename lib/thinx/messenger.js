...
module.exports = class Messenger {
	...
	attachCallback(rtm, err_callback) {
	    ...
	    this.web.conversations.list({ limit: 1 })
	    .then((response) => {
	...
	init() {
		...
		console.log("[DEBUG] Singleton Messenger.init()");
		...
		if (typeof(app_config.mqtt.username) !== "undefined") {
			...
			console.log("[DEBUG] Setting mosquitto username from configuration file.");
		} else {
			if (typeof(process.env.MOSQUITTO_PASSWORD) !== "undefined") {
				mqtt_password = process.env.MOSQUITTO_PASSWORD;
				console.log("[DEBUG] Overriding mosquitto password by environment variable.");
			}
		}

		if (typeof(app_config.mqtt.password) !== "undefined") {
			mqtt_password = app_config.mqtt.password;
			console.log("[DEBUG] Setting mosquitto password from configuration file.");
		} else {
			if (typeof(process.env.MOSQUITTO_USERNAME) !== "undefined") {
				mqtt_username = process.env.MOSQUITTO_USERNAME;
				console.log("[DEBUG] Overriding mosquitto username by environment variable.");
			}
		}
