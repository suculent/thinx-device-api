...
var Messenger = (function() {
	...
	var _public = {
			...
			*/
			...
			master.on("message", function(topic, message) {
				...
				client.get(topic + "@" + iso_date, function(err, json_keys) {
					var keys = json_keys; // JSON.parse(json_keys);
					console.log("Updating keys: " + JSON.stringify(keys));
					if ((keys === null) || (typeof(keys) === "undefined")) {
						keys = [];
					}
					keys.push(enc_message);
					client.set(topic + "@" + iso_date, JSON.stringify(keys), function(err) {
						if (err) {
		...
		data: function(owner, callback) {
			...
			client.keys("/*" + owner + "*", function(err, replies) {
				console.log(replies.length + " replies<:");

				var results = [];

				var get_result_or_callback = function(reply, i, callback) {

					client.get(reply, function(err, keys) {
						if (err) {
							//callback(false, err);
						} else {
							// TODO: Decode Base64
							console.log("Keys: " + json_keys);
							//const json_keys = JSON.decode(keys);
							//console.log("JSON Keys: " + json_keys);

							if ((typeof(json_keys) !== "undefined") && json_keys !== null) {
								for (var index in json_keys) {
									const key = json_keys[index];
									var value = base64.decode(json_keys[key]);
									results[key] = value;
								}
				...
				replies.forEach(function(reply, i) {
					console.log("    " + i + ": " + reply);
					get_result_or_callback(reply, i, callback);
				});
