...
module.exports = class Messenger {
	...
	updateAndTransformDeviceStatus(oid, did, message) {
	...
	processConnectionChange(oid, message) {
	...
	processActionableMessages(message, topic) {
		...
		if (Util.isDefined(message.status)) {
			...
			this.processConnectionChange(oid, message);
	...
	messageResponder(topic, message) {
		...
		this.updateAndTransformDeviceStatus(oid, did, message);

		// Process other well-known mesages
		// Match by message.status (should be optional)
		this.processActionableMessages(message, topic);
	}
