...
module.exports = class Messenger {
	...
	constructor() {
		...
		console.log("APIEnv Redis Client is set.");
	...
	publish(owner, udid, message) {
		if (typeof(client[owner]) === "undefined") {
			apiKeyForOwner(owner, (success, apikey) => {
	...
	setupMqttClient(owner, mqtt_options) {
		...
		this.clients[owner].on("connect", function(error) {
		...
		this.clients[owner].on('close', function () {
			console.log('[mqtt] Connection closed for owner ' + owner);
		});

		// Message Responder
		this.clients[owner].on("message", function(topic, message) {
	...
	initWithOwner(owner, websocket, callback) {
		...
		this.attachCallback(rtm, callback);

		getDevices(owner, (success, devices) => {
			...
			this.apiKeyForOwner(owner, (success, apikey) => {
				...
				this.setupMqttClient(owner, mqtt_options);
