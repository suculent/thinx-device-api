...
module.exports = class Messenger {
		...
		let token = await this.client.getAsync("__SLACK_BOT_TOKEN__")
			...
			.then((saved_token) => {
				if ((typeof (saved_token) === "undefined") || saved_token === null) {
					console.log("[debug] Providing environment bot token...");
					return Promise.resolve(bot_token);
				} else {
					console.log("[debug] Fetched Redis token for Slack...", saved_token);
	...
	async initSlack(callback) {
		...
		console.log("[debug] await getBotToken", { bot_token });

		if ((typeof (process.env.SLACK_BOT_TOKEN) !== "undefined") && (process.env.SLACK_BOT_TOKEN !== null)) {
			bot_token = process.env.SLACK_BOT_TOKEN;
		}

		console.log("[debug] with ENV token", { bot_token });
