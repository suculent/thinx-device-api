  ...
  */
	...
	publish(owner, udid, message) {
		...
		if (typeof (message.nid) !== "undefined") {
			...
			this.redis.v4.set(nid, JSON.stringify(message));
		}
	...
	processActionableNotification(did, topic, message) {
		...
		let messageFromDevice = (typeof (notification.response) ===
		...
		if (messageFromDevice) {
			...
			this.redis.v4.get(nid).then((result) => {
				...
				this.redis.v4.set("nid:" + did, not);
			});

		} else {

			let notification_to = message.notification;

			// Message for device
			if (notification_to.nid) {
				console.log("[messenger] NID message for device :" + notification_to.nid +
					" : " +
					JSON.stringify(message));

				// Search existing transaction
				this.redis.v4.get("nid:" + notification_to.nid).then((json_keys) => {
					...
					this.redis.v4.set("nid:" + did, JSON.stringify(nid_data));
				}).catch((e) => {
					console.log("[messenger] Redis GET Exception:", e);
				});
			}
		}
	...
	message_callback(topic, message) {
		...
		this.redis.v4.get(topic + "@" + iso_date).then((json_keys) => {
			...
			this.redis.v4.set(topic + "@" + iso_date, JSON.stringify(newkeys));
		});

		// Keeps MQTT Data for 24 hours only
		this.redis.v4.expire(topic, 3 * 86400);
	}
	...
	get_result_or_callback(reply, i, callback, results, replies) {
		this.redis.v4.get(reply).then((json_keys) => {
			let keys = {};
			try {
				keys = JSON.parse(json_keys);
			} catch (e) {
	...
	data(owner, udid, callback) {
		...
		this.redis.v4.sendCommand(["KEYS", "/*" + owner + "/" + udid + "*"]).then((replies) => {
