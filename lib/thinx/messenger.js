...
var Messenger = (function() {
	...
	*/
	...
	var bot_token = process.env.SLACK_WEBHOOK_URL || app_config.slack_bot_token;
	...
	rtm.on(CLIENT_EVENTS.RTM.AUTHENTICATED, (rtmStartData) => {
		// debugging only: console.log(JSON.stringify(rtmStartData));
		// Get Channel ID for the THiNX #support channel on Slack
		for (const c of rtmStartData.channels) {
			if (c.is_member && c.name === 'support') {
				channel = c.id;
				console.log("'support' channel has ID: " + channel);
			}
		}
		console.log(
	...
	rtm.on(RTM_EVENTS.RTM_CONNECTION_OPENED, function() {
		if (channel) {
			...
			console.log("No Slack channel connected yet to send Hello message.");
		}
	});
