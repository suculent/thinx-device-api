...
module.exports = class ACL {
    ...
    addTopic(user, mode, topic) {
        ...
        console.log("Updating ACL with", user_array);
    ...
    commit_redis(callback) {
        ...
        console.log("TODO: ACL commit to Redis not implemented!");
        ...
        if (user.length > 0 && user !== null) {
            let user_line = "user " + this.current_user + "\n";
            stream.write(user_line);
            ...
            for (var index in user) {
                ...
                if (topic.startsWith("readwrite ")) {
                    rwacls.put(topic.replace("readwrite ", ""));
                }
                if (topic.startsWith("read ")) {
                    racls.put(topic.replace("read ", ""));
                }
                if (topic.startsWith("write ")) {
                    wacls.put(topic.replace("write ", ""));
                }
            }

            client.set(this.current_user + ":racls", racls);
            client.set(this.current_user + ":wacls", wacls);
            client.set(this.current_user + ":rwacls", rwacls);
