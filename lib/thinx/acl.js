...
module.exports = class ACL {
    ...
    load(callback) {
        ...
        let racls = this.client.sendCommand(['SMEMBERS', query + ":racls"]);
        for (var rindex in racls) {
            user_array.push("read " + racls[rindex]);
        }
    
        let wacls = this.client.sendCommand(['SMEMBERS', query + ":wacls"]);
        for (var windex in wacls) {
            user_array.push("write " + wacls[windex]);
        }
    
        let rwacls = this.client.sendCommand(['SMEMBERS', query + ":rwacls"]);
