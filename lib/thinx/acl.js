...
module.exports = class ACL {
    ...
    prune(topic, callback) {
        console.log(`[info] Pruning ACL topic ${topic}`);
    ...
    addTopic(user, mode, topic) {
        ...
        if (typeof (user) == "undefined") {
            console.log("[error] Invalid user on addTopic!");
        }
        ...
        for (let index in user_array) {
            ...
            if (xtopic.indexOf(topic) !== -1) {
                ...
                console.log(`[info] Updating existing ACL topics for user ${user}`);
            }
        }
    ...
    removeTopic(user, topic) {
        ...
        for (let index in usertopics) {
            ...
            if (xtopic.indexOf(topic) !== -1) {
                ...
                found = true;
                console.log(`[info] Deleted ACL topic ${topic} for user ${user}`);
            }
        }
        if (!found) {
            console.log(`[error] Topic ${topic} for user ${user} not found on removal.`);
            return;
        }
