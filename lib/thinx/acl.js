...
module.exports = class ACL {
    ...
    load(callback) {
        ...
        let racls = this.client.v4.sendCommand(['SMEMBERS', query + ":racls"]);
        for (var rindex in racls) {
            user_array.push("read " + racls[rindex]);
        }
    
        let wacls = this.client.v4.sendCommand(['SMEMBERS', query + ":wacls"]);
        for (var windex in wacls) {
            user_array.push("write " + wacls[windex]);
        }
    
        let rwacls = this.client.v4.sendCommand(['SMEMBERS', query + ":rwacls"]);
    ...
    commit_redis(callback) {
        ...
        if (racls.length > 0) {
            this.client.v4.sAdd(this.current_user + ":racls", racls, (error, reply) => {
                if (error) console.log("RACLS1 failed.", error, reply);
            });
            // Auto-add subscribe ACLS
            this.client.v4.sAdd(this.current_user + ":sacls", racls, (error, reply) => {
                if (error) console.log("SACLS1 failed.", error, reply);
            });
        }

        if (wacls.length > 0) {
            this.client.v4.sAdd(this.current_user + ":wacls", wacls, (error, reply) => {
        ...
        if (rwacls.length > 0) {
            // Add read/write ACLS
            this.client.v4.sAdd(this.current_user + ":rwacls", rwacls, (error, reply) => {
                if (error) console.log("RWACLS failed with error", error, reply);
            });
            // Auto-add subscribe ACLS
            this.client.v4.sAdd(this.current_user + ":sacls", rwacls, (error, reply) => {
                if (error) console.log("SACLS2 failed.", error, reply);
            });
        }

        // Generic subscribe ACL for Mosquitto > 1.5
        this.client.v4.sAdd(this.current_user + ":sacls", ["/#"], (error, reply) => {
