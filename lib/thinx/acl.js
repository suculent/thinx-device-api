...
module.exports = class ACL {
    ...
    load(callback) {
        ...
        this.redis.sendCommand(['SMEMBERS', query + ":racls"], (error, racls) => {
            for (var rindex in racls) {
                user_array.push("read " + racls[rindex]);
            }

            this.redis.sendCommand(['SMEMBERS', query + ":wacls"], (error, wacls) => {
                for (var windex in wacls) {
                    user_array.push("write " + wacls[windex]);
                }

                this.redis.sendCommand(['SMEMBERS', query + ":rwacls"], (error, rwacls) => {
