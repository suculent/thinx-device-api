...
module.exports = class ACL {
    ...
    constructor(user) {
        ...
        if ((typeof (app_config.mqtt) !== "undefined") &&
            (typeof (app_config.mqtt.acl) !== "undefined")) {
            this.path = app_config.mqtt.acl;
            if (!fs.existsSync(this.path)) {
                console.log("[ACL] Warning, app_config error: no ACL file found at", this.path); // could create file on first run, but better warn there is no configuration ready
            }
        }
    ...
    commit_redis(callback) {
        ...
        if ((typeof (user) === "undefined") || (user.length > 0 && user !== null)) return;
