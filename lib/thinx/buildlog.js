	...
	var _public = {
		...
		state: function(build_id, owner, udid, state) {
			...
			if (typeof(owner) === "undefined") {
				console.log("Invalid Log owner: " + owner);
				return;
			}

			if (typeof(build_id) === "undefined") {
				console.log("Invalid build_id: " + build_id);
				return;
			}

			if (typeof(udid) === "undefined") {
				console.log("Invalid udid: " + udid);
				return;
			}
			...
			buildlib.atomic("builds", "edit", build_id, changes, function(error, body) {
				if (error) {
					console.log("Build-log " + build_id + " state update (existing) error: " + JSON.stringify(error));
				}
			});
		},
