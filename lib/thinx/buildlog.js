...
module.exports = class Buildlog {
	...
	state(build_id, owner, udid, state) {
		...
		buildlib.atomic("builds", "edit", build_id, changes, (error, body) => {
			if (error) {
				console.log("blog:state:fail:1", error, body);
	...
	createAtomicLogRecord(record) {
		...
		console.log("Appending atomic log record: " + JSON.stringify(record));
	...
	list(owner, callback) {
		...
		buildlib.view("builds", "latest_builds", {
			...
			if (err) {
				...
				console.log("monthAgo: ", monthAgo, "now", nowMillis, "diff", now - monthAgo);
				
				/* filtering if needed */
				console.log("[buildlog] Listing latest builds for owner...");
				...
				for (var index in documents) {
					...
					if (doc.timestamp < monthAgo) {
						console.log("Pruning old log", doc._id);
						this.prune(doc);
					} else {
						if (log.owner == owner) {
				...
				// sort by timestamp (needs checking, just pseudo-code)
				owner_logs.sort((a, b) => {
					// console.log({a}, {b});
					return b.timestamp - a.timestamp; // (b-a); bigger first
					}
				);
