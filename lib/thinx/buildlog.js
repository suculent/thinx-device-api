...
var Sanitka = require("./sanitka");const { writeFileSync } = require("fs-extra");
...
module.exports = class Buildlog {
	...
	setupTail(websocket, build_log_path, err_callback) {
		...
		if (process.env.ENVIRONMENT === "test") {
			err_callback(true, "tail_started"); // should return in test only...
		}
	}
	...
	logtail(unsafe_build_id, owner, socket, error_callback) {
		...
		console.log("Logtail called for", owner, {unsafe_build_id}, {socket});
		...
		this.fetch(build_id, (err, body) => {
			...
			if (typeof (body.message) !== "undefined") {
				//console.log("[logtail] error when taililng build_id", build_id);
				//console.log("[logtail] error:", body.message);
				//err_callback("[logtail] log not found");
				//return;
			}
			...
			if (typeof(body.udid) === "undefined") {
				console.log("[logtail] udid not found in " + JSON.stringify(body));
				err_callback("[logtail] udid not found");
				//return; do not exit, wait for build log
			}

			if ((typeof (body.log) === "undefined") || body.log === null) {
				console.log("body.log undefined while body = ", body);
				body = {}; // fix error where body has no log because it is a string
				body.log = [{
			...
			if (typeof (body.owner) === "undefined") {
				...
				build.owner = sanitka.udid(body.owner); // invalid case
			}
			...
			if (err) {
				...
				var path = this.pathForDevice(build.owner, sanitka.udid(body.udid));
				var deploy_path = path + "/" + sanitka.branch(build_id); // /owner/device/build_id/ == deployment_path
				...
				if (!fs.existsSync(deploy_path)) {
					...
					console.log("[buildlog] deploy_path found at: " + deploy_path);
				}

				// TODO: FIXME: This path does not align with builder, which writes to ${DEPLOYMENT_PATH}/build.shell.log
				var build_log_path = deploy_path + "/build.log";

				if (!fs.existsSync(build_log_path)) {
					fs.writeFileSync(build_log_path, "--tail-start--\n", { mode: 0x666 });
				}

				// DEPRECATED --> Create file before trying to tail, do not wait for builder to do it...
				/*
				var PRE = "LOG_DIR=`dirname " + build_log_path +
					"`; [ ! -d $LOG_DIR ] && mkdir -p $LOG_DIR; touch " +
					build_log_path;
				// console.log(PRE);
				var presult = exec.execSync(PRE); // lgtm [js/command-line-injection]
