...
var Buildlog = (function() {
  ...
  var app_config = Globals.app_config();
	...
	function atomic_log(atomic_error) {
		if (atomic_error) {
	...
	function wsSend(websocket, data) {
	...
	var _private = {
		...
		setupTail: function(build_log_path, err_callback) {
			var options = {
				fromBeginning: true,
				fsWatchOptions: {},
				follow: true
			...
			_public.fetch(build_id, function(err, body) {
					...
					if (err) {
						...
						if (fs.existsSync(build_log_path)) {
							...
							_private.setupTail(build_log_path, err_callback);
