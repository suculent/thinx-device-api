...
module.exports = class Buildlog {
	...
	state(build_id, owner, udid, state) {
		...
		buildlib.atomic("builds", "edit", build_id, changes, (error1, body1) => {
			if (error1) {
				console.log("blog:state:fail:1", {error1}, {body1});
				console.log("While atomic editing build-log id '" + build_id + "' state update (existing) error: ", { error });

				buildlib.atomic("builds", "edit", build_id, changes, (error2, body2) => {
					if (error2) {
						console.log("blog:state:fail:2", {error2}, {body2});
						console.log("While atomic editing build-log id '" + build_id + "' state update (existing) error: ", { error });
					}
				});
			}
