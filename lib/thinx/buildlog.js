...
module.exports = class Buildlog {
	...
	atomic_log(atomic_error) {
		if (atomic_error) {
			console.log("Log update (existing) error: " + atomic_error);
		} else {
	...
	fetch(build_id, callback) {
		...
		buildlib.get(build_id, function(err, body) {
			...
			var path = pathForDevice(blog.owner, blog.udid);
	...
	logtail(build_id, owner, socket, error_callback) {
		...
		fetch(build_id, function(err, body) {
				...
				if (err) {
					...
					wsSend(websocket, message);

					var path = pathForDevice(build.owner, build.udid);
	...
	pathForDevice(owner, udid) {
		this.owner = owner;
		this.udid = udid;
		var user_path = deployPathForOwner(owner);
