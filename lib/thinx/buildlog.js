...
var Buildlog = (function() {
	...
	var _public = {
		...
		state: function(build_id, owner, udid, state) {
			...
			console.log("Attempt to edit " + build_id + " with state " + state);

			var changes = {};
			changes.state = state;
			buildlib.atomic("builds", "edit", build_id, changes, function(error, body) {
				if (error) {
					console.log("Log state update (existing) error: " + JSON.stringify(error));
				}
			});
		},
			...
			buildlib.view("builds", "latest_builds", {
				...
				if (err) {
					...
					for (var index in logs) {
						...
						//console.log("Parsing log for owner: " + JSON.stringify(log));
						if (log.owner == owner) {
