...
module.exports = class Buildlog {
	...
	list(owner, callback) {
		...
		buildlib.search("builds", "latest_builds", {
			"q": owner
		}, (err, body) => {
			if (err) {
				...
				var monthAgo = new Date().getTime() - 30*86400;

				// filter by timestamp (last month logs only)
				owner_logs.filter(log => log.timestamp > monthAgo);

				console.log("filtered: ", owner_logs);
