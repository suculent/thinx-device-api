...
module.exports = class Buildlog {
	...
	state(build_id, owner, udid, state) {
		...
		buildlib.get(build_id, (err, doc) => {
			console.log("Log find result: ", err, doc);
			var newDoc = doc;
			delete newDoc._rev;
			newDoc.state = state;
			buildlib.destroy(build_id, (derr, dbody) => {
				console.log("Log destroy result: ", derr, dbody);
				buildlib.insert(build_id, newDoc, (ierr, ibody) => {
					console.log("Log edit result: ", ierr, ibody);
				}
			});
