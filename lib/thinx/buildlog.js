...
module.exports = class Buildlog {
	...
	logtail(unsafe_build_id, owner, socket, error_callback) {
		...
		this.fetch(build_id, (err, in_body) => {
			...
			if (err !== true) {
				console.log(`☣️ [error] ⛔️[logtail]: ${err} ${in_body}`);
				err_callback(err);
				return;
			...
			chmodr(deploy_path, 0o776, (cherr) => {
				...
				if (cherr) {
					console.log(`[warning] Failed to execute chmodr in buildlog: ${cherr}`);
				}

				if (fs.existsSync(build_log_path)) {
					console.log(`[info] [buildlog] Tailing ${build_log_path}`);
