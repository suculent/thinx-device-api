...
module.exports = class Buildlog {
	...
	createInitialLogRecord(initial_record, callback) {
		...
		buildlib.insert({ log: [ initial_record ]}, build_id, (insert_error, body /*, header */) => {
	...
	fetch(build_id, callback) {
		...
		buildlib.get(build_id, (err, body) => {
			...
			var path = Filez.deployPathForDevice(blog.owner, blog.udid);
	...
	list(owner, callback) {
		...
		buildlib.view("builds", "latest_builds", {
			...
			for (var index in documents) {
				...
				// BOLA fix: this view query is NOT constrained to `owner` (the
				// key:owner filter above is commented out), so `documents` holds
				// builds for ALL owners. Filter in-memory to the requesting owner —
				// otherwise any authenticated user reads (and, below, prunes) every
				// tenant's build records. (router.logs getBuildLogs passes the
				// session owner; a new account was seeing "105 Builds".)
				if (!doc || doc.owner !== owner) continue;
	...
	/**
	 * GDPR #353: destroy ALL build documents owned by `owner`, regardless of age.
	 * The latest_builds view emits (doc.owner, doc), so keying by owner returns
	 * exactly this owner's builds with full _id/_rev for destruction.
	 * @param {string} owner - 'owner' id
	...
	purgeOwner(owner, callback) {
		buildlib.view("builds", "latest_builds", { "key": owner }, (err, body) => {
			if (err) {
				console.log(`[error] purging builds for owner ${err}`);
			...
			rows.forEach((row) => {
				...
				buildlib.destroy(doc._id, doc._rev, (derr) => {
					if (!derr) destroyed++;
					else console.log(`[warning] build destroy failed for ${doc._id}: ${derr}`);
					if (--remaining === 0) callback(null, destroyed);
				});
			});
		});
	...
	logtail(unsafe_build_id, owner, socket, error_callback) {
		...
		this.fetch(build_id, (err, in_body) => {
			...
			var device_path = Filez.deployPathForDevice(owner, sanitka.udid(build.udid));
