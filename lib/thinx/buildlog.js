		...
		logtail: function(build_id, owner, socket, error_callback) {
			...
			_public.fetch(build_id, function(err, body) {
					...
					if (typeof(build.owner) === "undefined") {
						console.log(
							"[logtail] build has no owner - FIXME: hacking, injecting test user!"
						);
						build.owner =
					...
					if (err) {
						...
						if (!fs.existsSync(build_path)) {
							mkdirp(build_path, function(err) {
								...
								else console.log(build_path + " build_path created.");
							});
						}
