...
module.exports = class Buildlog {
	...
	setupTail(websocket, build_log_path, build_id, terr_callback) {
		...
		tail.on("line", (data) => {
			var logline = "[" + build_id + "]: " + data; // data.toString();
	...
	fetch(build_id, callback) {
		...
		buildlib.get(build_id, (err, body) => {
			...
			if (err !== null) {
				this.fetch(build_id, callback); // try again?
				console.log("[buildlog] Error fetching build log ID ", build_id, err);
	...
	list(owner, callback) {
		...
		buildlib.view("builds", "latest_builds", {
			...
			if (err) {
				console.log("[buildlog] Error listing builds for owner..."); // no db shards could be opened?
				callback(err, body);
				return;
			}
		...
		this.fetch(build_id, (err, in_body) => {
			...
			if ((typeof (in_body) === "undefined") || in_body === null) {
				...
				if (typeof(in_body.log) === "undefined") {
					console.log("[BUG] passing build in_body: ", in_body);
					build = in_body;
				} else {
					console.log("[BUG] passing build in_body.log: ", in_body.log);
					build = in_body.log[0];
				}
			}
