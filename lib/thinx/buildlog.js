	...
	var _public = {
		...
		state: function(build_id, owner, udid, state) {
			...
			if (typeof(build_id) === "undefined") {
				throw ("Invalid build_id: " + build_id);
			}

			if (typeof(udid) === "undefined") {
				throw ("Invalid udid: " + udid);
			}
			...
			console.log("Attempt to mark " + build_id + " with state " + state);

			buildlib.atomic("builds", "state", build_id, state, function(error, body) {
				if (error) {
					console.log("Log state update (existing) error: " + JSON.stringify(error), body);
				}
			});
		},
		...
		log: function(build_id, owner, udid, message, contents) {
			...
			buildlib.get(build_id, function(err, existing) {
				...
				if (err || (typeof(existing) === "undefined")) {
					...
					console.log("Creating initial log record: " + JSON.stringify(initial_record));

					buildlib.insert(initial_record, build_id, function(error,
						body, header) {
						if (error) {
							console.log("Log insert error: " + error, body);

							// Maybe already exists, try to create record...
							buildlib.atomic("builds", "log", build_id, record, function(error,
								body) {
								if (err) {
									console.log("Log update (existing) error: " + err + " :: " + JSON.stringify(body));
								} else {
									console.log("Log updated successfully" + JSON.stringify(body));
								}
							});
