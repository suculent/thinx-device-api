...
module.exports = class Buildlog {
	...
	wsSend(socket, data) {
      ...
      console.log("[buildlog] no socket " + e);
    }
	}
	...
	setupTail(socket, build_log_path, sock_callback) {
      ...
      separator: /[\r]{0,1}\n/,
		...
		tail.on("error", (error) => {
			...
			if (typeof(sock_callback) !== "undefined") {
				sock_callback("fake build log error");
			}
		});
    	...
    	if (process.env.ENVIRONMENT === "test") {
			sock_callback(true, "tail_started"); // should return in test only...
    	}
	}
		...
		buildlib.atomic("builds", "edit", build_id, changes, (error, body) => {
			if (error) {
				if (error.error == "conflict") {
					...
					buildlib.insert(initial_record, build_id, (insert_error, nbody, header) => {
						if (insert_error !== null) {
						  console.log("[buildlog] insert error: " + insert_error, nbody);
						}
					});

				} else {
					buildlib.atomic("builds", "edit", build_id, changes, (aerror, abody) => {
						if (aerror) {
							buildlib.atomic("builds", "edit", build_id, changes, (berror, bbody) => {
								if (berror) {
									console.log("While atomic editing build-log id '" + build_id + "' state update (existing) error: ", {error});
								}
							});	
						}
					});
   ...
   createInitialLogRecord(initial_record) {
     ...
     buildlib.insert({ record: initial_record }, build_id, (insert_error, body, header) => {
       if (insert_error !== null) {
   ...
   createAtomicLogRecord(record) {
	...
	logtail(build_id, owner, socket, error_callback) {
    	...
    	if (typeof(socket) === "undefined" || socket === null) {
      		onsole.log("[logtail] WARNING: Calling logtail without socket...");
   		}
		...
		this.fetch(build_id, (err, body) => {
				...
				if ((typeof(body) === "undefined") || body === null) {
					if (typeof(socket) !== "undefined" && socket !== null) {
						try {
							socket.send(JSON.stringify({log: "Sorry, no log records fetched."}));
          ...
          body = {}; // fix error where body has no log because it is a string
				...
				if (err) {
					...
					if (!fs.existsSync(build_path)) {
						...
						console.log("[logtail] Created build_path: " + build_path);
					} else {
