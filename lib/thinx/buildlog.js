...
module.exports = class Buildlog {
	...
	setupTail(websocket, build_log_path, terr_callback) {
		...
		if (process.env.ENVIRONMENT === "test") {
			terr_callback(true, "tail_started"); // should return in test only...
		}
	}
	...
	logtail(unsafe_build_id, owner, socket, error_callback) {
		...
		this.fetch(build_id, (err, in_body) => {
			...
			let body = in_body.log[[0]];

			// in case something bad happened
			if (typeof (body.message) !== "undefined") {
				console.log("[logtail] error when tailing build_id", build_id);
			...
			if (err) {
				...
				var path = this.pathForDevice(owner, sanitka.udid(body.udid));
				console.log("[logtail] path:", path);

				var deploy_path = path + sanitka.branch(build_id); // /owner/device/build_id/ == deployment_path
				console.log("[logtail] deploy_path (device path+build):", path);
				...
				if (!fs.existsSync(deploy_path)) {
					...
					console.log("[logtail] deploy_path found at: " + deploy_path);
				}
