...
module.exports = class Buildlog {
	...
	fetch(build_id, callback) {
		...
		buildlib.get(build_id, (err, body) => {
			...
			if ((typeof (body.log) === "undefined") || (body.log.count === 0)) {
				console.log("✅ [info] [buildlog] body has no log...");
				callback(false, {});
				return;
			}
	...
	logtail(unsafe_build_id, owner, socket, error_callback) {
		...
		if (build_id == null) {
			throw new Error("☣️ [error] Tailing invalid log ID", unsafe_build_id);
		}
		...
		this.fetch(build_id, (err, in_body) => {
			...
			if (err) {
				console.log("☣️ [error] ⛔️[logtail]: %s %s", err, in_body);
				err_callback(err);
				return;
			...
			chmodr(deploy_path, 0o776, (cherr) => {
				...
				if (cherr) {
					...
					console.log("✅ [info] [builder] BUILD_PATH permission change successful.");
				}
