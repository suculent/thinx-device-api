...
var Buildlog = (function() {
	...
	try {
		...
		console.log("[buildlog] thx_prefix_exception " + e);
	}

	nano.db.create(prefix + "managed_builds", function(err, body, header) {
		if (typeof(err) !== "undefined" && err !== null) {
			if (typeof(err.statusCode) !== "undefined" && err.statusCode != 412) {
		...
		log: function(build_id, owner, udid, message, contents) {
			...
			buildlib.get(build_id, function(err, existing) {
				...
				if (err || (typeof(existing) === "undefined")) {
					...
					console.log("[buildlog] Creating initial log record."); // + JSON.stringify(initial_record));

					buildlib.insert(initial_record, build_id, function(insert_error,
						body, header) {
						if (insert_error) {
							console.log("Log insert error: " + insert_error, body);
