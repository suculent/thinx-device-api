				...
				owner.profile(device.owner, function(status, profile) {
					...
					if (typeof(device.transformers) !== "undefined" && device.transformers !== null) {
						...
						*/
						...
						function update_device(udid, device) {
					...
					var req = http.request(options, (res) => {
						...
						res.on('data', function(chunk) {
							...
							// save processeddevice status
							const response = JSON.parse(Buffer.concat(chunks));
							console.log(JSON.stringify(response));
							const response_status = response.status;
							device.status = response_status;
							update_device(udid, device);
						});
					});
	...
	var _public = {
		...
		revoke: function(udid, callback) {
			devicelib.get(udid, function(err, doc) {
				...
				devicelib.destroy(udid, doc.rev, function(err, callback) {
					if ((typeof(callback) !== "undefined") && callback !== null) {
						callback(err);
					} else {
						console.log("Device document destroyed without callback.");
					}
				});
			});
		}
