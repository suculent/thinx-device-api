...
var Deployment = require('./deployment'); var deployment = new Deployment();
...
module.exports = class Device {
  ...
  constructor() {
		console.log("device.js: Device constructor start...");
    this.client = redis.createClient(Globals.redis_options());
    console.log("device.js: Device constructor end (redis initialized)...");
	}
	...
	_storeOTT(body, callback) {
		...
		this.client.set("ott:" + new_ott, body_string, function(err) {
			if (err) {
	...
	updateFromPath(path, ott, callback) {
		...
		if (platform === "arduino" || platform === "platformio") {
			update_binary(path, ott, callback);
...
 							console.log("Device Update Query successful, updating...");
					...
					update_device_and_respond(udid, device, callback, true, reg);
					return;
				} else {
					// repeat failed
			...
			if (update === false) {
				...
				_storeOTT(reg, function(success, result) {
					...
					registration_response.registration.mac = normalizedMAC(rmac); // do not return MAC to device
			...
			owner.profile(device.owner, function(status, profile) {
				...
				if (typeof(device.transformers) !== "undefined" && device.transformers !== null) {
					...
					if (device.transformers.length == 0) {
						update_device_and_respond(device.udid, device, callback, false, reg);
						return;
					}
						...
						res.on('data', function(chunk) {
							...
							devicelib.get(udid, function(error, existing) {
								if (error || (typeof(existing) === "undefined")) {
									...
									update_device_and_respond(existing.udid, device, callback, false, reg);
									return;
								}
							}); // get
						}); // end
	...
	fetchOTT(ott, callback) {
		this.client.get("ott:" + ott, function(err, json_keys) {
	...
	register(body, api_key, websocket, callback, req) {
		...
		apikey.verify(owner, api_key, req, function(success, message) {
			...
			devicelib.get(udid, function(error, existing) {
				...
				if (!error && (typeof(existing) !== "undefined") && (existing.owner == owner)) {
					...
					checkinExistingDevice(existing, reg, callback);
					return;
								...
								if (typeof(body.rows) === "undefined") {
									...
									if (body.rows.length === 0) {
										...
										if (typeof(xisting) !== "undefined") {
											...
											checkinExistingDevice(xisting, reg, callback);
											return;
										}
									}
								}
	...
	ott_update(ott, callback) {
		...
		this.client.get("ott:" + ott, function(err, info) {
		...
		apikey.verify(owner, api_key, req, function(success, message) {
			...
			devicelib.get(udid, function(err, device) {
				...
				if (updateAvailable === false) {
					...
					this.client.get("nid:" + udid, function(err, json_keys) {
						if (!err) {
