...
var Device = (function() {
	...
	var _public = {
		...
		register: function(reg, api_key, callback) {
			...
			apikey.verify(reg.owner, api_key, function(success, message) {

				if (success === false) {
					alog.log(reg.owner, "Attempt to use invalid API Key: " +
						api_key +
		...
		firmware: function(body, api_key, callback) {
			...
			if (typeof(body.commit) === "undefined") {
		...
		edit: function(owner, changes, callback) {
			...
			for (var changeindex in changes) {
				// FIXME: if (!changes[changeindex].hasOwnProperty(udid)) continue;
