...
module.exports = class Device {
	...
	storeOTT(body, callback) {
		...
		this.client.v4.set("ott:" + new_ott, body_string).then(() => {
			this.client.v4.expire("ott:" + new_ott, 86400).then(() => {
	...
	update_binary(path, ott, upload_callback) {
		...
		try {
			...
			if (typeof (ott) !== "undefined" && ott !== null) {
				this.client.v4.expire("ott:" + ott, 3600); // The OTT is valid for 60 minutes after first use
			}
	...
	runDeviceTransformers(profile, device, callback, reg, res) {
		...
		if (typeof (device.transformers) !== "undefined" && device.transformers !== null) {
			...
			// TODO: FIXME: From HTTP transformer communication to some kind of secure comms (It would require self-signed certificate with only public part available to the transformer for validation)
	...
	fetchOTT(ott, callback) {
		this.client.v4.get("ott:" + ott).then((json_keys) => {
	...
	ott_update(ott, callback) {
		...
		this.client.v4.get("ott:" + ott).then((info) => {

			if (!info) {
				console.log("OTT_UPDATE_NOT_FOUND: ", info);
		...
		this.apikey.verify(firmware_owner, api_key, false, (success, message) => {
			...
			devicelib.get(udid, (err, device) => {
				...
				if (updateAvailable === false) {
					...
					this.client.v4.get("nid:" + udid).then((json_keys) => {
