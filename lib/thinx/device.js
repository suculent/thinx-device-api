...
module.exports = class Device {
	...
	runDeviceTransformers(profile, device, callback, reg, res) {
		...
		if (typeof (device.transformers) !== "undefined" && device.transformers !== null) {
			...
			// No transformers configured for this device. The check-in must STILL
			// persist (lastupdate/status/version) and respond. Without this branch
			// the function returned without ever calling update_device_and_respond,
			// so a device with no `transformers` field never updated its
			// "last connected" timestamp at all.
			if ((typeof (reg) !== "undefined") && (typeof (callback) !== "undefined")) {
				this.update_device_and_respond(device.udid, device, callback, reg, res);
			}
	...
	register(reg, api_key, res, callback) {
		...
		if (typeof (reg.timezone_abbr) === "string") {
			...
			// utcOffset() is minutes east of UTC and already accounts for DST.
			// (The former _tzm is only set when parsing a string that carries an
			// offset, so it was always undefined here and made the result NaN.)
			let device_time = momentTz().tz(timezone_abbr);
			if (device_time.isDST()) {
				timezone_offset = device_time.utcOffset() / 60;
