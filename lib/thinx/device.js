...
var Device = (function() {
	...
	var _public = {
		...
		firmware: function(body, api_key, callback) {
			...
			//
			// OTT Update (requires valid One-Time-Token)
			//
			...
			if (ott === null) {
				...
				_private.fetchOTT(ott, function(err, ott_info) {
					...
					if (err) {
						...
						// In case the OTT does not exist, it has not been created yet.
						if (typeof(body.ott) !== "undefined") {
							// OTT GET request
							console.log("Creating new OTT...");
							storeOTT(ott, callback);
							return;
						}
