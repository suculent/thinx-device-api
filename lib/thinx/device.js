...
module.exports = class Device {
	...
	constructor() {
		...
		console.log("✅ [info] Loaded module: Device");
	}
	...
	_storeOTT(body, callback) {
		...
		this.client.set("ott:" + new_ott, body_string, (err) => {
			if (err) {
				console.log("[error] [device.js] :: storeOTT error");
				callback(false, err);
			} else {
	...
	updateFromPath(path, ott, callback) {
		...
		if (platform === "arduino" || platform === "platformio") {
			...
			console.log("[warning] Multi-file update for NodeMCU not yet fully supported.");
			this.update_multiple(firmware_path, ott, callback);

		} else if (platform === "pine64") {
			console.log("[warning] Multi-file update for Pine64 not yet fully supported.");
			this.update_multiple(firmware_path, ott, callback);

		} else if (platform === "micropython") {
			console.log("[warning] Multi-file update for Micropython not yet fully supported.");
			this.update_multiple(firmware_path, ott, callback);

		} else if (platform === "mongoose") {
			console.log("[warning] Firmware update for MongooseOS not yet supported.");
			this.update_multiple(firmware_path, ott, callback);

		} else if (platform === "nodejs") {
			console.log("[warning] Firmware update for node.js not yet supported.");

		} else {
			console.log("[warning] Firmware update for " + platform + " not yet supported.");
		}
	}
	...
	update_binary(path, ott, upload_callback) {
		...
		try {
			...
			if (buffer.length < 1000) {
				console.log("[warning] Input file too short for a firmware, skipping (" + buffer.length + ")");
				upload_callback(false);
				return;
			}
			...
			if (typeof (upload_callback) !== "undefined" && upload_callback !== null) {
				console.log("[info] Sending firmware update (" + buffer.length + ")");
			...
			console.log("[error] Upload callback failed: " + e);
			if (typeof (upload_callback) !== "undefined" && upload_callback !== null) {
				upload_callback(false);
			}
		}
	...
	update_device_and_respond(udid, device, callback, repeat, reg) {
		...
		devicelib.atomic("devices", "modify", udid, device, (error, /* body */) => {
			...
			if (error) {
				...
				console.log("[error] devicelib.atomic.modify failed with udid:", udid, "device:", device);

				if (!repeat) {
					if (debug_device) {
						console.log("[error] Device Update Query failed, responding...");
					}

				} else {
					// repeat failed
					console.log("[info] Repeating...");
					this.update_device_and_respond(udid, device, callback, true, reg);
					return;
				}

				if (debug_device) {
					console.log("[error] Atomic modification error:", error, body);
				}
			...
			if (update === false) {
				...
				if (callback !== null) {
					...
					if (debug_device) {
						console.log("[info] Check-in reply (existing): " + JSON.stringify(registration_response));
					}
				}
				...
				this._storeOTT(reg, (success, result) => {
					...
					if (typeof (rmac) === "undefined") {
						console.log("[error] Missing MAC in device.js:491");
						return;
					}
					...
					if (callback !== null) {
						console.log("[info] Calling back with", registration_response);
						callback(true, JSON.stringify(registration_response));
					} else {
						console.log("[error] Missing callback in firmware update registration response.");
					}
				}); // store
	...
	runDeviceTransformers(device, callback, reg) {
		...
		if (typeof (device.transformers) !== "undefined" && device.transformers !== null) {
			...
			// TODO (22): FIXME: Communication with Transformer should be done over secure channel (e.g. the Transformer could connect to server using HTTPS SocketIO the same way as Worker does)
			// Connection is always on, the response will be asynchronous (!).
		...
		apikey.verify(registration_owner, api_key, req, (success, message) => {
			...
			devicelib.get(udid, (error, existing) => {
				...
				if (!error && (typeof (existing) !== "undefined") && (existing.owner == registration_owner)) {
					...
					if (debug_device) {
						console.log("[info] Checking as existing device [1]...");
					}
					...
					devicelib.view("devices", "devices_by_mac", {
						...
						(err, body) => {
							...
							if (err) {
								// TODO (11): check if it contains : then it is maybe MAC, UDIDs have - instead)
								// https://github.com/suculent/thinx-device-api/issues/304
								console.log("[info] Device with this UUID/MAC not found. Seems like new one...");
								console.log(err);
							} else {
								if (typeof (body.rows) === "undefined") {
									...
									if (body.rows.length === 0) {
										...
										if (typeof (xisting) !== "undefined") {
											...
											console.log("[info] Checking as existing device [2]...");
							...
							devicelib.insert(device, udid, (create_err) => {
								...
								if (create_err) {
									...
									console.log("[error] Device record update failed." + create_err);
	...
	firmware(req, callback) {
		...
		var udid = Validator.udid(rbody.udid);
        let firmware_owner = Validator.owner(rbody.owner);
		...
		if (typeof (rbody.mac) === "undefined") {
			console.log("[error] missing_mac in " + JSON.stringify(rbody));
			callback(false, {
				success: false,
				status: "missing_mac"
			});
		...
		apikey.verify(firmware_owner, api_key, null, (success, message) => {
			...
			devicelib.get(udid, (err, device) => {
				...
				if (updateAvailable === false) {
					...
					this.client.get("nid:" + udid, (fw_err, json_keys) => {
						...
						if ((typeof (not) !== "undefined") && not.done === true) {
							console.log("[info] Device firmware current, deleting NID notification...");
							this.client.expire("nid:" + udid, 0);
						} else {
							console.log("[info] Keeping nid:" + udid + ", not done yet...");
						}
					});
				} else {
				...
				this.client.get("nid:" + udid, (get_err, json_keys) => {
					if (get_err) {
						console.log("[info] [nid] Device has no NID for actionable notification.");
						// no NID, that's OK...
						// nid will be deleted on successful download/update (e.g. when device is current)
					} else {
						if (json_keys !== null) {
							var not = JSON.parse(json_keys);
							console.log("[info] [nid] Device has NID:" + json_keys);
							if (not.done === true) {
								console.log("[info] [nid] User sent reply.");
								// update allowed by user
							} else {
								console.log("[info] [nid] Device is still waiting for reply.");
	...
	update_device(udid, changes, update_callback) {
		...
		devicelib.get(udid, (err, doc) => {
			...
			devicelib.atomic("devices", "modify", udid, changes, (atomic_err /*, body */) => {
				if (atomic_err) {
					console.log(`[error] [device] ${udid} edit error ${aromic_err}`);
					update_callback(false, {
						success: false,
	...
	edit(device_owner, changes, callback) {
		...
		if ((typeof (changes.udid) === "undefined") || (changes.udid === null)) {
			console.log("[critical] No device update, UDID undefined...");
			callback(false, "changes.udid_undefined");
			return;
		}

		if (typeof (device_owner) === "undefined") {
			console.log("[critical] No device update, owner undefined...");
			...
			devicelib.destroy(udid, doc._rev, (destroy_err) => {
				if (destroy_err) {
					...
					if (err.reason !== 'deleted') {
						console.log("[error] Device destroy error: ", err);
					}
				}
