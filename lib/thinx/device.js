...
var Device = (function() {
  ...
  var rollbar = new Rollbar({
	...
	try {
		...
		console.log("Initializing device.js with " + pfx_path);
	...
	var _private = {
		...
		update_device_and_respond: function(udid, device, callback) {

			console.log("Updating device...");

			// prevent revision collision (even though this may be an issue later) <
			if ( typeof(device._rev) !== "undefined" ) {
				delete device._rev;
			}
			...
			owner.profile(device.owner, function(status, profile) {
				...
				if ( (typeof(reg.ack) !== "undefined") ) {
					console.log("This is a SigFox downlink request.");
						...
						const downlinkdata = device.status.toString('hex').substring(0, 24);
						console.log("Sending "+downlinkdata);
						var downlinkResponse = {
							deviceId: reg.mac.replace("SIGFOX", ""),
							downlinkData: downlinkdata
						};
						rollbar.info(JSON.stringify(downlinkResponse));
						callback(true, JSON.stringify(downlinkResponse));
						callback = null;
				} // COPY A
				 else {
					console.log(JSON.stringify(reg));
				...
				if (typeof(device.transformers) !== "undefined" && device.transformers !== null) {
					...
					if (device.transformers.length == 0) {
						console.log("No transformers.");
						_private.update_device_and_respond(device.udid, device, callback);
						return;
					}
					...
					for (var ti in device.transformers) {
						...
						for (var tindex in profile.info.transformers) {
							if (profile.info.transformers[tindex].utid == utid) {
								...
								var code = base64.decode(descriptor.body);
								if (reg.status) {
						...
						res.on('data', function(chunk) {
							...
							devicelib.get(udid, function(error, existing) {
								if (error || (typeof(existing) === "undefined")) {
									...
									_private.update_device_and_respond(existing.udid, device, callback);
								}
							}); // get
						}); // end
					}); // req
		...
		var _public = {
			...
			push: function(body, api_key, callback) {
				...
				register: function(body, api_key, callback, req) {
								...
								if (!error && (typeof(existing) !== "undefined") && (existing.owner == owner)) {
									...
									function(err, body) {
										...
										"] [DEVICE_NEW] New device: " + JSON.stringify(
											...
											if ( (typeof(reg.ack) !== "undefined") ) {
												console.log("This is a SigFox downlink registration request.");
												//if ( reg.platform == "sigfox" ) {
													...
													rollbar.info({ info: "SigFox Downlink", data: downlinkResponse});
													callback(true, JSON.stringify(downlinkResponse));
													callback = null;
												//}
											} // COPY A
											...
											.replace(
												...
												satelize.satelize({ip: req.ip}, function(err, payload) {
													...
													// TODO: Extract satelize as function with implementation from here >
													if (!err) {
														/*
														{
														    "ip": "46.19.37.108",
													...
													devicelib.insert(device, udid, function(err, body, header) {
																if (!err) {
																	...
																	var device_time = new time.Date(new Date());
																	device_time.setTimezone(timezone);
																	response.registration.timestamp = device_time.getMilliseconds() / 1000;
