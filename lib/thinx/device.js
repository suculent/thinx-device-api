				...
				owner.profile(device.owner, function(status, profile) {
					...
					*/
					...
					if (typeof(reg.status) !== "undefined" && reg.status !== null) {
						...
						// --> TODO: External status parser(s), possibly user configurable. Like:

						// function customParser(owner, device, status, callback(success, result))
						const ststr = reg.status; // input param
						var retval = ststr; // return value
						var warning = "";
						...
						if (tag == "ba") {
							...
							if (voltage < 3.5) {
								retval += "#!"; // show yellow warning sign
							}
							device.status = retval;
						} else {
							// in case return value is undefined, original status will be displayed
						}
						...
						// -- end Status Transformer prototype

						//
						// <----- (will deprecate in favour of Status Transformers implementation)
						//
					}
					...
					// var transform = (function(code, status) {
					//	/* jshint -W061 */
					//	return eval(code).call(reg.status);
					//	/* jshint +W061 */
					// })();
					...
					if ((typeof(profile) !== "undefined") && (profile !== null)) {
						...
						if (typeof(device.transformers) !== "undefined" && device.transformers !== null) {
							for (var ti in device.transformers) {
								...
								for (var tindex in profile.transformers) {
									if (profile.transformers[tindex].utid === utid) {
										var transformer = profile.transformers[utid];
										console.log("Parsing transformer: " + utid + " : " + JSON.stringify(transformer));
										if (typeof(transformer) !== "undefined" && transformer !== null) {
											const alias = transformer.alias;
											const code = utf8.decode(base64.decode(transformer.body));
											console.log("Decoding transformer: " + alias + " : " + code);
											/* jshint -W061 */
											device.status = eval(code).call(reg.status); // may be dangerous if not running in closure
											// device.status = transform(code, status); // v2, but the function must not be pre-defined or gets executed with null code!
											/* jshint +W061 */
										}
									}
								}
					...
					devicelib.atomic("devicelib", "modify", udid, device, function(error, body) {
						if (!error) {
							...
							if (update === false) {
								...
								console.log("CHECKIN_RESPONSE: " + JSON.stringify(
