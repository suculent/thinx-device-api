		...
		checkinExistingDevice: function(device, reg, callback) {
				...
				*/
				...
				if (typeof(reg.status) !== "undefined" && reg.status !== null) {
					...
					//
					// Sample tagged byte->float parser
					// Tag is HEX("ba") and rest is Battery value
					//

					// DUMB EXAMPLE ONLY: Check if this is a battery information
					const tag = ststr.substr(0, 2); // start, length

					// When first byte of the Sigfox message is HEX(ba) let's say this is battery info
					// (You may decide on your own tag length or structure, this is just an example.)
					if (tag == "ba") {
						// Take end of message except for tag byte
						...
						var parseFloat = function(str) {
							...
							for (var j = 0; j < mantissa.length; j++) {
								float += parseInt(mantissa[j]) ? Math.pow(2, exp) : 0;
								exp--;
							}
							return float * sign;
						};

						const voltage = parseFloat(hex_foltage);
						retval = "Battery " + voltage + " V";

						// this should be returned
						device.status = retval; // this is how reg.status becomes device.status through custom formatting using owner's JS function

					} // -- end external status parser prototype
