...
module.exports = class Device {
		...
		if (changed) {
			userlib.atomic("users", "edit", device.owner, {
				...
				if (error) {
					...
					alog.log(device.owner, "Profile update failed.", "error");
		...
		if (typeof (device.transformers) !== "undefined" && device.transformers !== null) {
			...
			let req = http.request(options, (_res) => {
				...
				_res.on('data', (chunk) => {
					...
					devicelib.get(device.udid, (error, existing) => {
						if (error || (typeof (existing) === "undefined")) {
							...
							console.log("[TODO FIXME!] This will never work. Socketing [transformer] registration status needs websocket.");
	...
	register(reg, api_key, res, callback) {
		...
		this.apikey.verify(registration_owner, api_key, true, (success, message) => {
			...
			let mesh_ids = [];
			let udid = uuidV5(); // is returned to device which should immediately take over this value instead of mac for new registration
