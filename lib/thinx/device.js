...
module.exports = class Device {
	...
	updateFromPath(path, ott, callback) {
		...
		if ((typeof (path) === "undefined") || path == null) {
			callback(false);
			console.log("update path must be defined");
			return;
		}
	...
	update_device_and_respond(udid, device, callback, repeat, reg) {
		...
		devicelib.atomic("devices", "modify", udid, device, (error, /* body */) => {
			...
			if (update) {
				...
				console.log(`[info] Device ${udid} has update available and enabled.`);
			} else {
				console.log(`[info] Device ${udid} has auto-update disabled.`);
			}
	...
	register(req, reg, api_key, websocket, callback) {
		...
		if (typeof (reg.timezone_abbr) !== "undefined") {
			...
			if (momentTz().tz(timezone_abbr).isDST()) {
				...
				console.log("🔨 [debug] [device] Ajusting timezone offset based on DST: " + timezone_offset);
			}
		}

		if (debug_device) {
			console.log("🔨 [debug] [device] Timezone offset: " + timezone_offset);
		}
		...
		apikey.verify(registration_owner, api_key, req, (success, message) => {
			...
			if (success === false) {
				...
				if (debug_device) {
					console.log("🔨 [debug] [device] API Key verification failed!");
				}
	...
	firmware(req, callback) {
		...
		apikey.verify(firmware_owner, api_key, null, (success, message) => {
			...
			devicelib.get(udid, (err, device) => {
				...
				console.log(`[info] Getting LFE descriptor for udid ${device.udid}`);
				...
				this.client.get("nid:" + udid, (get_err, json_keys) => {
					if (get_err) {
						console.log("[info] Device has no NID for actionable notification.");
						// no NID, that's OK...
						// nid will be deleted on successful download/update (e.g. when device is current)
					} else {
						if (json_keys !== null) {
							var not = JSON.parse(json_keys);
							console.log("[info] Device has NID:" + json_keys);
							if (not.done === true) {
								console.log("[info] User sent reply.");
								// update allowed by user
							} else {
								console.log("[info] Device is still waiting for reply.");
					...
					if (path === false) {
						console.log(`[info] No update available for udid ${udid}`);
						callback(false, {
							success: false,
