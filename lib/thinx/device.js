		...
		checkinExistingDevice: function(existing, reg, callback) {
				...
				devicelib.destroy(device._id, device._rev,
					...
					function(err) {
						...
						if (err) {
							console.log(
								"[WILL-RETRY] Error destroying existing device on checkin! " +
								device._id + " : " + err);
						...
						devicelib.insert(device, udid, function(err,
							body, header) {
							if (!err) {
	...
	var _public = {
		...
		normalizedMAC: function(mac) {
			...
			if (mac.toString().length == 17) {
				...
				var ms = mac.toString().toUpperCase();
				if (ms.indexOf(":") !== -1) {
					ms = ms.replace(/:/g, "");
				}
				var m = ms.split("");
				var retval = "";
				for (var step = 0; step == m.length; m += 2) {
					retval += m[step].toString() + m[step + 1].toString() + ":";
				}
				/*
		...
		register: function(body, api_key, callback) {
			...
			var reg = body.registration;

			console.log("Registration with API Key: " + api_key + " and data " +
				JSON.stringify(reg));
			...
			console.log("REG (old_protocol_owner):" + JSON.stringify(reg));
			console.log("REG.OWNER:" + reg.owner);
			console.log("REG.JSON.OWNER:" + JSON.parse(reg).owner);
			...
			apikey.verify(owner, api_key, function(success, message) {
				...
				if (update === true) {
					...
					response.mac = _public.normalizedMAC(firmwareUpdateDescriptor.mac);
								...
								if (err) {
									...
									if (typeof(body.rows) === "undefined") {
										...
										for (var expired in body.rows) {
											if (typeof(body.rows[expired]) === "undefined") return;
											console.log("Pruning expired device " + JSON.stringify(
												body
												.rows[
													expired]._id));
											devicelib.destroy(body.rows[expired]._id, body.rows[
													expired]
												._rev);
										}

										if (typeof(existing) !== "undefined") {
											console.log("Checking-in existing device...");
											_private.checkinExistingDevice(existing, reg, callback);
											return;
										} else {
		...
		firmware: function(body, api_key, callback) {
			...
			apikey.verify(owner, api_key, function(success, message) {
				...
				devicelib.get(udid, function(err, device) {
					...
					var firmwareUpdateDescriptor = deploy.latestFirmwareEnvelope(
						device);
					var mac = _public.normalizedMAC(firmwareUpdateDescriptor.mac);
