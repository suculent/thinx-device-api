					...
					*/
					...
					if (typeof(device.transformers) !== "undefined" && device.transformers !== null) {
						for (var ti in device.transformers) {
							...
							console.log("Searching for transformer: " + utid);
								...
								if (profile.transformers[tindex].utid == utid) {
									...
									if (typeof(transformer) !== "undefined" && transformer !== null) {
										...
										if (reg.status) {
											...
											try {
												var transformed_status = transformer(reg.status, device); // may be dangerous if not running in closure with cleaned globals!
												console.log("Processed status: " + transformed_status);
												device.status = transformed_status;
												device.status_raw = reg.status;
											} catch (e) {
												console.log(e);
											}
										}
