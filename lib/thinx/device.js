					...
					function(err) {
						...
						if (err) {
							console.log(
								...
								device._id + " : " + err);
							_public.checkinExistingDevice(existing, reg, callback);
							return;
						} else {
				...
				devicelib.get(udid, function(err, device) {
					...
					if (updateAvailable === false) {
						...
						client.get("nid:" + did, function(err, json_keys) {
							if (!err) {
								var not = JSON.parse(json.keys);
								if (not.done === true) {
									console.log(
										"Device firmware current, deleting NID notification...");
									client.expire("nid:" + did, 0);
								} else {
									console.log("Keeping nid:" + did + ", not done yet...");
								}
							}
						});
					}
