...
var Device = (function() {
	...
	var _private = {
			...
			devicelib.atomic("devicelib", "modify", udid, device, function(error, body) {
				...
				if (update === false) {
					...
					if (callback !== null) {
						...
						console.log("» Check-in reply (existing): " + JSON.stringify(registration_response, null, 4));
					}

				} else {

					console.log("Creating OTT with: " + JSON.stringify(body));
					...
					_public.storeOTT(reg, function(success, result) {
						...
						// registration_response.registration.checksum = firmwareUpdateDescriptor.checksum; // may be used later, very long
		...
		restartStatusTransformer: function() {
			...
			const docker_check_cmd = "docker ps | grep 'thinx-node-transformer' | cut -d' ' -f1";
			...
			const docker_run_cmd = "cd ~/thinx-node-transformer && docker run --user=transformer -d -p " + app_config.lambda +
		...
		checkinExistingDevice: function(device, reg, callback) {
				...
				owner.profile(device.owner, function(status, profile) {
					...
					if (typeof(reg.commit) !== "undefined" && reg.commit !== null) {
						if (typeof(device.commit) !== "undefined") {
							if (device.commit !== reg.commit) {
								device.commit = reg.commit;
								// device updated, switch user tag! in device.owner
								goals.push("build");
								goals.push("update");
								console.log("Adding goal BUILD to " + JSON.stringify(goals));
								userlib.edit(device.owner, {
									"goals": goals
	...
	var _public = {
		...
		push: function(body, api_key, callback) {
			...
			console.log("• Push Registration with API Key: " + api_key + " and body " + JSON.stringify(body, null, 4));
		...
		register: function(body, api_key, websocket, callback, req) {
			...
			console.log("» Registration with API Key: " + api_key + " and body " + JSON.stringify(body, null, 4));
