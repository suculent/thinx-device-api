				...
				owner.profile(device.owner, function(status, profile) {
					...
					if (typeof(reg.firmware) !== "undefined" && reg.firmware !== null) {
						...
						if (typeof(envelope) !== "undefined") {
							if (envelope.firmware == device.firmware) {
								built_by_thinx = true;
							}
						}
					...
					// update goals / target action for master
					if (built_by_thinx == true) {
	...
	var _public = {
								...
								fs.ensureFile(acl_path, function(err) {
									...
									fs.appendFile(acl_path, acl, function(err) {
										if (err) {
											...
											var process_id = result.data.toString();
		...
		ott_update: function(ott, callback) {
			...
			client.get("ott:" + ott, function(err, info) {
				...
				if (path === false) {
					console.log("deploy.latestFirmwarePath returned false instead of path");
					callback(false, {
						success: false,
				...
				devicelib.get(udid, function(err, device) {
					...
					client.get("nid:" + udid, function(err, json_keys) {
						...
						if (updateAvailable) {
							...
							// "update" goal
							var envelope = deploy.latestFirmwareEnvelope(owner, device.udid);
							console.log("Matching envelope: " + JSON.stringify(envelope, null, 4));
							if (envelope.firmware == device.firmware) {
								console.log("TODO: update goal reached but not saved!");
							}
