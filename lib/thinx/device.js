...
module.exports = class Device {
	...
	register(req, reg, api_key, websocket, callback) {
		...
		var registration_owner = Validator.owner(sanitka.owner(reg.owner)); // TODO: refactor to sanitka or expect only boolean result
				...
				if (!error && (typeof (existing) !== "undefined") && (existing.owner == registration_owner)) {
					...
					devicelib.view("devices", "devices_by_mac", {
						...
						(err, body) => {
							...
							if (err) {
								console.log("Device with this UUID/MAC not found. Seems like new one... (TODO: check if it contains : then it is maybe MAC, UDIDs have - instead)");
								console.log(err);
							} else {
	...
	firmware(body, api_key, owner, callback) {
		...
		var said_firmware_owner = Validator.owner(body.owner); // TODO: refactor to sanitka or expect only boolean result
		var firmware_owner = owner; // use own value instead of user supplied-one? the firmware session based on apikey should have its owner set and verified

		if (said_firmware_owner !== firmware_owner) {
			console.log("[critical] 🚫 Firmware owner does not match! %s != %s", firmware_owner, owner);
			callback(false, {
				success: false,
