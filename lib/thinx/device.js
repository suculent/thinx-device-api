...
module.exports = class Device {
  ...
  constructor() {
	...
	checkinExistingDevice(device, reg, callback) {
			...
			owner.profile(device.owner, (status, profile) => {
				...
				if (typeof(reg.udid) !== "undefined") {
					udid = this.validatedUDID(reg.udid);
				}
	...
	push(body, api_key, callback) {
		...
		var push = reg.push;
		var udid = this.validatedUDID(reg.udid);
  ...
  validatedOwner(owner) {
	...
	register(body, api_key, websocket, callback, req) {
		...
		var push = reg.push;
		var alias = reg.alias;
		var owner = this.validatedOwner(reg.owner);
		...
		apikey.verify(owner, api_key, (success, message) => {
				...
				if (!error && (typeof(existing) !== "undefined") && (existing.owner == owner)) {
					...
					devicelib.view("devicelib", "devices_by_mac", {
						...
						(err, body) => {
							...
							console.log("[OID:"+owner+"] [DEVICE_NEW]: ", {reg});
	...
	firmware(body, api_key, callback) {
		...
		var udid = this.validatedUDID(body.udid);
  ...
  update_device(owner, udid, changes, update_callback) {
	...
	revoke(udid, callback) {
		devicelib.get(udid, (err, doc) => {
			...
			devicelib.destroy(udid, doc.rev, (err) => {
				if (err) {
					console.log(err);
				}
