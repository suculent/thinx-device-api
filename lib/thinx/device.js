...
module.exports = class Device {
	...
	updateDeviceDataWithRegistration(reg, device) {
		...
		if (Util.isDefined(reg.status)) {
			device.status = reg.status;
		}

		if (Util.isDefined(reg.snr)) {
			device.snr = reg.snr;
		} else {
			device.snr = null;
		}

		if (Util.isDefined(reg.rssi)) {
			device.rssi = reg.rssi;
		} else {
			device.rssi = null;
		}

		if (Util.isDefined(reg.station)) {
		...
		if (Util.isDefined(reg.lat)) {
			device.lat = reg.lat;
		}

		if (Util.isDefined(reg.lon)) {
			device.lon = reg.lon;
		}
