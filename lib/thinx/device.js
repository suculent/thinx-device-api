...
var Device = (function() {
	...
	var _public = {
		...
		register: function(reg, api_key, callback) {
			...
			var mac = reg.registration.mac;
			var fw = "unknown";
			if (!reg.registration.hasOwnProperty("firmware")) {
				fw = "undefined";
			} else {
				fw = reg.registration.firmware;
				console.log("Setting firmware " + fw);
			}

			console.log("REGL:" + JSON.stringify(reg));

			var push = reg.registration.push;
			var alias = reg.registration.alias;
			var owner = reg.registration.owner;
			var version = reg.registration.version;
			...
			apikey.verify(owner, api_key, function(success, message) {
				...
				var hash = null;
				if (typeof(reg.registration.hash) !== "undefined") {
					hash = reg.registration.hash;
				}

				var checksum = hash;
				if (typeof(reg.registration.checksum) !== "undefined") {
					checksum = reg.registration.checksum;
				}

				var udid = uuidV1(); // is returned to device which should immediately take over this value instead of mac for new registration
				if (typeof(reg.registration.udid) !== "undefined") {
					udid = reg.registration.udid; // overridden
				}
		...
		firmware: function(regbody, api_key, callback) {

			var body = regbody.registration;
