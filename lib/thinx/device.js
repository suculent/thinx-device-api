...
module.exports = class Device {
	...
	updateFromPath(path, ott, callback) {
		...
		if (typeof (path) === "undefined") {
			throw new Error("update path must not be undefined");
		}

		if (typeof (path) == null) {
			throw new Error("update path must not be null");
		}
	...
	update_device_and_respond(udid, device, callback, repeat, reg) {
		...
		console.log("update_device_and_respond udid:", udid, "device:", device);
	...
	run_transformers(udid, transformer_owner, dry, callback) {
		...
		userlib.get(transformer_owner, (error, response) => {
						...
						if (profile.info.transformers[tindex].utid == utid) {
							...
							if (device.status_raw) {
								...
								try {
									...
									// TODO: 3rd party code execution must be strictly audited and sandboxed. This is not:
