		...
		checkinExistingDevice: function(device, reg, callback) {
				owner.profile(device.owner, function(status, profile) {
					...
					if (typeof(device.transformers) !== "undefined" && device.transformers !== null) {
						...
						// --> NEW IMPLEMENTATION (requires save callback when finished)
						//  with:
						//
						...
						*/
						...
						function send_device_response(udid, device) {
					...
					for (var ti in device.transformers) {
						...
						for (var tindex in profile.info.transformers) {
							if (profile.info.transformers[tindex].utid == utid) {
								...
								// Status is the one required thing being processed here with (TODO: limited) details of device
								if (reg.status) {
					...
					var req = http.request(options, (res) => {
						...
						res.on('data', function(chunk) {
							...
							var new_status = response.input;
							if (typeof(response.output !== "undefined")) {
								new_status = response.output;
							}
							console.log("Job response :" + JSON.stringify(response));
							console.log("Fetching current device state revision:");
							devicelib.get(udid, function(error, existing) {
								if (error || (typeof(existing) === "undefined")) {
									...
									console.log("Saving updated status:" + new_status);
									_public.edit(existing.owner, { udid: existing.udid, status: new_status }, function() {
										send_device_response(udid, existing);
									} );
								}
							}); // get
						});
					...
					return;
						...
						// --> DEPRECATED IMPLEMENTATION: Local Processing
					...
					// TODO: Extract to separate function (DEPRECATED)
