...
var Device = (function() {
	...
	var _private = {
		...
		update_device_and_respond: function(udid, device, callback, repeat, registration_request) {
			...
			devicelib.atomic("devicelib", "modify", udid, device, function(error, body) {
				...
				if (error) {
					...
					if (repeat == false) {
						...
						_private.update_device_and_respond(udid, device, callback, true, registration_request);
						return;
					} else {
						// repeat failed
				...
				if (update === false) {
					...
					_public.storeOTT(body.registration, function(success, result) {
