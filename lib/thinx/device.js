...
module.exports = class Device {
	...
	update_multiple(path, ott, callback) {
		...
		var artifact_filenames = new Array();
				...
				if (!error && (typeof (existing) !== "undefined") && (existing.owner == registration_owner)) {
					...
					devicelib.view("devicelib", "devices_by_mac", {
						...
						(err, body) => {
							...
							const device_topic = "topic readwrite /" + registration_owner + "/" + udid + "/#";
							const shared_topic = "topic readwrite /" + registration_owner + "/shared/#";
							const status_topic = "topic readwrite /" + registration_owner + "/status";

							var acl = device_user_line + "\n" + device_topic + "\n" + shared_topic + "\n" + status_topic + "\n\n";

							fs.ensureFile(acl_path, (ensure_err) => {
								if (ensure_err) {
									console.log("Error ensuring ACL file: " + ensure_err);
								}
								fs.appendFile(acl_path, acl, (append_err) => {
									if (append_err) {
										console.log("Error appending ACL file: " + append_err);
										return;
									}
									console.log("New ACL record created.");
									alog.log(registration_owner, "Created MQTT ACL record for: " + udid + " alias: " + alias);
	...
	update_device(udid, changes, update_callback) {
		...
		devicelib.get(udid, (err, doc) => {
			...
			doc = this.apply_edits_to_doc(doc, changes); // nonsense, doc will not be saved
			//doc.changes = changes;
			//delete doc._rev;
			//delete doc.value;
			//delete doc.doc;
