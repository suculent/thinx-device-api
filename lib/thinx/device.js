...
var Device = (function() {
	...
	console.log("device.js: Loading device globals...");

	var Globals = require("./globals.js");

	console.log("device.js: Globals loaded...");

  var app_config = Globals.app_config();
  var prefix = Globals.prefix();
  var rollbar = Globals.rollbar();

	console.log("device.js: Loading fs-extra...");
	var fs = require("fs-extra");

	console.log("device.js: Loading http...");
	var http = require('http');

	console.log("device.js: Loading md5...");
	var md5 = require('md5');

	console.log("device.js: Loading auth...");
	...
	console.log("device.js: Initializing device DBs...");

	var devicelib = require("nano")(db).use(prefix + "managed_devices");
	var userlib = require("nano")(db).use(prefix + "managed_users");

	console.log("device.js: Initializing device requires...");

	console.log("device.js: sha256...");
	var sha256 = require("sha256");

	console.log("device.js: uuid...");
	var uuidV1 = require("uuid/v1");

	console.log("device.js: base64...");
	var base64 = require("base-64");

	console.log("device.js: moment...");
	var moment = require("moment");

	console.log("device.js: redis...");
	var redis = require("redis");

	console.log("device.js: redis client...");
	var client = redis.createClient(Globals.redis_options());

	console.log("device.js: audit...");
	var alog = require("./audit");

	console.log("device.js: deployment...");
	var deploy = require("./deployment");

	console.log("device.js: apikey...");
	var apikey = require("./apikey");

	console.log("device.js: owner...");
	var owner = require("./owner");

	console.log("device.js: child_process...");
	var exec = require("child_process");

	console.log("device.js: base128 (gets stucked on divergent node version)...");
	var base128 = require("base128");

	console.log("device.js: time (gets stucked on divergent node version)......");
	var time = require("time");

	// private
	function _storeOTT(body, callback) {
	...
	var _private = {
		...
		update_multiple: function(path, ott, callback) {
			...
			var extensions = app.config.project_root + "/languages/" + platform_descriptor.language + "/descriptor.json";
...
 							console.log("Device Update Query successful, updating...");
				...
				if (update === false) {
					...
					_storeOTT(reg, function(success, result) {
						...
						registration_response.registration.mac = _private.normalizedMAC(rmac); // do not return MAC to device
	...
	var _public = {
		...
		normalizedMAC: function(_mac) {
			return _private.normalizedMAC(_mac);
		},
		...
		register: function(body, api_key, websocket, callback, req) {
			...
			apikey.verify(owner, api_key, req, function(success, message) {
				...
				devicelib.get(udid, function(error, existing) {
					...
					if (debug_device) {
						console.log("» Device DB info fetch...");
					}
					...
					if (!error && (typeof(existing) !== "undefined") && (existing.owner == owner)) {
						...
						devicelib.view("devicelib", "devices_by_mac", {
								key: _private.normalizedMAC(reg.mac),
								include_docs: true
							},
							function(err, body) {
								...
								if (err) {
									...
									//console.log("Known device identified by MAC address: " + _private.normalizedMAC(reg.mac));
									if (typeof(body.rows) === "undefined") {
										...
										if (body.rows.length === 0) {
											...
											if (typeof(xisting) !== "undefined") {
												...
												console.log("» Checking as existing (2): " + JSON.stringify(xisting));
								...
								if (err && (typeof(req) !== "undefined")) {
									// may require Satelize, which was a security risk due to its dependencies...
		...
		ott_request: function(owner, body, api_key, callback) {
			apikey.verify(owner, api_key, req, function(success, message) {
				...
				if (success) {
					...
					_storeOTT(JSON.stringify(body), callback);
				} else {
		...
		firmware: function(body, api_key, callback) {
			...
			apikey.verify(owner, api_key, req, function(success, message) {
				...
				devicelib.get(udid, function(err, device) {
					...
					mac = _private.normalizedMAC(rmac);
