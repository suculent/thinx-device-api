...
 							console.log("Device Update Query successful, updating...");
	...
	push(body, api_key, callback) {
		...
		devicelib.get(udid, function(error, existing) {
			if (error || (typeof(existing) === "undefined") || (existing === null)) {
				...
				this.edit(existing.owner, changes, function() {
	...
	firmware(body, api_key, callback) {
		...
		apikey.verify(owner, api_key, req, function(success, message) {
			...
			devicelib.get(udid, function(err, device) {
				...
				console.log("Getting latest firmware update descriptor from envelope for: " + JSON.stringify(device));
  ...
  update_device(owner, udid, changes, update_callback) {
    ...
    devicelib.get(udid, function(err, doc) {
      ...
      doc = apply_edits_to_doc(doc, change);
    ...
    if ((typeof(changes.udid) === "undefined") || (changes.udid !== null)) {
      ...
      return;
		...
		update_device(owner, changes.udid, changes, callback);
	}
