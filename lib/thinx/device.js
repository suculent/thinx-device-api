				...
				devicelib.destroy(existing._id, existing._rev,
					...
					function(err) {
						...
						if (err) {
							...
							console.log("Destroyed existing device: " + JSON.stringify(existing));
						}
						...
						if (typeof(reg.udid) === "undefined" || reg.udid === "") {
							...
							if (typeof(device.udid) === "undefined") {
								...
								console.log("Device found with udid: " + device.udid);
							}
						}
						...
						console.log("device udid: " + device.udid);
							...
							function(err, body) {
								...
								if (err) {
									...
									if (typeof(body.rows) === "undefined") {
										...
										existing = body.rows[body.rows.length - 1];

										for (var expired in body.rows) {
											console.log("Pruning device " + JSON.stringify(expired));
											devicelib.destroy(expired._id, expired._rev);
										}
								...
								if (typeof(temp) !== "undefined" && temp.data.toString() !==
									"") {
