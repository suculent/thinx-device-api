	...
	var _public = {
		...
		register: function(reg, api_key, callback) {
			...
			apikey.verify(owner, api_key, function(success, message) {
				...
				alog.log(owner,
					"Attempt to register device: " + hash + " alias: " + alias);
		...
		firmware: function(body, api_key, callback) {
			...
			if (typeof(body.use) !== "undefined") use = body.use;
			if (typeof(body.ott) !== "undefined") ott = body.ott;
			...
			apikey.verify(owner, api_key, function(success, message) {
				...
				devicelib.view("devicelib", "devices_by_id", {
					...
					if (use == "ott") {
						...
						console.log("Fetching OTT record...");

						// ott token must be in query and this will be a get request...
						_private.fetchOTT(ott, function(err, ott_info) {
							if (err) {
								console.log("Fetch ott error...");
							} else {
								console.log(JSON.stringify(ott_info));
								var ott_device = {
									mac: ott_info.mac,
									owner: ott_info.owner,
									version: ott_info.version
								};
								device = ott_device;
								deploy.initWithDevice(ott_device);
								// update availability cannot be forced for OTT?
								// it should rather delete the OTT on success...
								if (deploy.hasUpdateAvailable(device)) {
									var path = deploy.latestFirmwarePath(owner, udid);
