...
var Device = (function() {
	...
	var _public = {
		...
		register: function(reg, api_key, callback) {
			...
			function checkinExistingDevice(existing) {
				...
				devicelib.destroy(existing._id, existing._rev,
					function(err) {
						...
						if (typeof(reg.udid) === "undefined" || reg.udid === "") {
							if (typeof(existing.udid) === "undefined") {
								console.log(
									"Device found but creating new udid (not given in request): " +
									existing.udid);
								existing.udid = uuidV1();
							}
						} else {
							if (typeof(existing.udid) === "undefined") {
								existing.udid = _id;
								console.log("Device found but creating new udid: " + existing._id);
							} else {
								console.log("Device found with udid: " + existing._id);
							}
						}

						if (typeof(reg.firmware) !== "undefined" && reg.firmware !== null) {
							existing.firmware = reg.firmware;
						}
						if (typeof(reg.push) !== "undefined" && reg.push !== null) {
							existing.push = reg.push;
						}
						if (typeof(reg.alias) !== "undefined" && reg.alias !== null) {
							existing.alias = reg.alias;
						}
						...
						var udid = existing._id;

						devicelib.insert(existing, existing._id, function(err,
							body, header) {
							if (!err) {
								callback(true, {
									registration: {
								...
								console.log("Device checkin complete.");
							} else {
								callback(false, {
									registration: {
										success: false,
			...
			apikey.verify(owner, api_key, function(success, message) {
				...
				devicelib.get(udid, function(error, existing) {
					...
					if (!error && (typeof(existing) !== "undefined")) {
						checkinExistingDevice(existing);
					} else {
							...
							function(err, body) {
								...
								if (err) {
									...
									if (typeof(body.rows) === "undefined") {
										...
										console.log("Devices found:" + JSON.stringify(body.rows));
