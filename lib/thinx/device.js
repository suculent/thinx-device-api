...
module.exports = class Device {
	...
	updateFromPath(path, ott, callback) {
		...
		if (platform === "arduino" || platform === "platformio") {
			...
			console.log("⚠️ [warning] Multi-file update for NodeMCU not yet fully supported.");
			this.update_multiple(firmware_path, ott, callback);

		} else if (platform === "pine64") {
			console.log("⚠️ [warning] Multi-file update for Pine64 not yet fully supported.");
			this.update_multiple(firmware_path, ott, callback);

		} else if (platform === "micropython") {
			console.log("⚠️ [warning] Multi-file update for Micropython not yet fully supported.");
			this.update_multiple(firmware_path, ott, callback);

		} else if (platform === "mongoose") {
			console.log("⚠️ [warning] Firmware update for MongooseOS not yet supported.");
			this.update_multiple(firmware_path, ott, callback);

		} else if (platform === "nodejs") {
			console.log("⚠️ [warning] Firmware update for node.js not yet supported.");
	...
	update_device_and_respond(udid, device, callback, repeat, reg) {
		...
		devicelib.atomic("devices", "modify", device._id, { changes: device }, (error, /* body */) => {
			...
			if (update === false) {
				...
				this.storeOTT(reg, (success, result) => {
	...
	checkinExistingDevice(device, reg, api_key, callback) {
		...
		owner.profile(device.owner, (status, profile) => {
			...
			this.runDeviceTransformers(profile, device, callback, reg); // reg = registration request to respond
		}); // profile

	} // checkin

	runDeviceTransformers(profile, device, callback, reg) {
	...
	run_transformers(udid, transformer_owner, dry, callback, /* res */) {
		devicelib.get(udid, (fetch_error, device) => {
			if (fetch_error || (typeof (device) === "undefined") || (device.owner != transformer_owner)) {
				console.log(`[debug] transform u: ${udid} o: ${transformer_owner} d: ${JSON.stringify(device, null, 2)}`);
				callback(false, "no_such_device");
				return;
			}
			userlib.get(transformer_owner).then((profile) => {
				this.runDeviceTransformers(profile, device, callback, null); // registration request to respond with
			});
		});
	}
	...
	update_device(udid, changes, update_callback, errors = null) {
		...
		devicelib.get(udid, (err, doc) => {
			...
			devicelib.atomic("devices", "modify", doc._id, changes, (atomic_err, body) => {
				if (atomic_err) {
					let data = JSON.stringify(changes, null, 2);
					console.log(`☣️ [error] [device] errors: ${errors} device ${udid} edit error ${atomic_err}, ${body} retry with data ${data}, updating doc ${doc}...`);
					if (errors < 100) this.update_device(udid, changes, update_callback, errors++);
				} else {
	...
	envs(_udid, callback) {
		let udid = sanitka.udid(_udid);
		if (udid == null) {
			return callback(false, "getenv_device_id_invalid");
		}
		devicelib.get(udid, (error, device) => {
			if (error || (typeof (device) === "undefined")) {
				console.log("[error] envs:", error);
				if (typeof (callback) !== "undefined") {
	...
	detail(_udid, callback) {
		let udid = sanitka.udid(_udid);
		if (udid == null) {
			return callback(false, "detail_device_id_invalid");
		}
		devicelib.get(udid, (error, device) => {
			if (error || (typeof (device) === "undefined")) {
				console.log("[error] detail searching for udid:", _udid, ", ", error);
				if (typeof (callback) !== "undefined") {
