						...
						for (var ti in device.transformers) {
							...
							for (var tindex in profile.info.transformers) {
								if (profile.info.transformers[tindex].utid == utid) {
									...
									console.log("Parsing transformer: " + utid + " : " + JSON.stringify(descriptor));
									const alias = descriptor.alias;
									var code = utf8.getStringFromBytes(base64.decode(descriptor.body));
									console.log("Decoding transformer: " + alias + " : " + JSON.stringify(code));
									/* jshint -W061 */
									if (reg.status) {
										...
										console.log("No status to process. Space to optimize.");
									}
