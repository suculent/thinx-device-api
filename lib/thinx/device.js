...
module.exports = class Device {
	...
	updateFromPath(path, ott, callback) {
		...
		//console.log("deploy_path: " + deploy_path); // trimmed
	...
	update_multiple(path, ott, callback) {
		...
		var platform_descriptor = JSON.parse(fs.readFileSync(platforms_path + "/descriptor.json")); // TODO: FIXME! This does not update without app restart!
	...
	update_device_and_respond(udid, device, callback, repeat, reg) {
		...
		devicelib.atomic("devicelib", "modify", udid, device, (error, body) => {
			...
			/* seems to be deprecated/unused

			//
			// Timezone
			//

			let timezone; // default should be GMT or Universal
			if (typeof (device.timezone) !== "undefined") {
				timezone = device.timezone;
			} else {
				timezone = "Etc/GMT"; // Etc? UTC?
			}
			*/

			registration_response.registration.timestamp = Math.floor(new Date() / 1000); // device_time.getMilliseconds() / 1000;
			...
			if (update === false) {
				...
				this._storeOTT(reg, (success, result) => {
					...
					console.log("[device] Stored new OTT token: " + result.ott);
	...
	checkinExistingDevice(device, reg, callback) {
		...
		owner.profile(device.owner, (status, profile) => {
			...
			if ((typeof (device.iv) === "undefined") || (device.iv === null)) {
				...
				for (var index in iv) {
					iv[index] = Math.floor(Math.random() * 255);
				}
				var iv_base64 = Buffer.from(iv).toString('base64');
				device.iv = iv_base64;
			}

			/* WTF is key? Find out and document!
			if ((typeof(device.key) === "undefined") || (device.key === null)) {
						// AES Initialization Vector
						let N_BLOCK = 16;
						var key = Array(N_BLOCK);
						for (var kindex in key) {
							key[kindex] = Math.floor(Math.random() * 255);
						}
						var key_base64 = Buffer.from(key).toString('base64');
						device.key = key_base64;
					}*/
	...
	register(req, reg, api_key, websocket, callback) {
		...
		// Until 2.0.0
		if (typeof (reg.owner) === "undefined") {
			console.log("searching for owner in registration request: " + JSON.stringify(reg));
			if (typeof(callback) !== "undefined") {
		...
		var push = reg.push;
		var alias = reg.alias;
		var registration_owner = Validator.owner(reg.owner);
		...
		if (debug_device) {
			//console.log("» Timezone offset: " + timezone_offset);
		}
		...
		apikey.verify(registration_owner, api_key, req, (success, message) => {
			...
			var mqtt = "/" + registration_owner + "/" + udid;
				...
				if (!error && (typeof (existing) !== "undefined") && (existing.owner == registration_owner)) {
					...
					devicelib.view("devicelib", "devices_by_mac", {
						...
						(err, body) => {
							...
							console.log("[OID:" + registration_owner + "] [DEVICE_NEW]: ", JSON.stringify(reg));
							...
							acl.load(() => {
								const device_topic = "/" + registration_owner + "/" + udid + "/#"; // device topics
								const shared_topic = "/" + registration_owner + "/shared/#"; // owner shared topics
								const status_topic = "/" + registration_owner + "/status"; // owner status topic

								acl.addTopic(reg.udid, "readwrite", device_topic);
								acl.addTopic(reg.udid, "readwrite", shared_topic);
								acl.addTopic(reg.udid, "readwrite", status_topic);

								if ( typeof(device.mesh_ids) !== "undefined" ) {
									console.log("Adding Mesh Topics...");
									for (let mindex in device.mesh_ids) {
										let id = device.mesh_ids[mindex];
										if (id !== null) {
											let mesh_topic = "/" + registration_owner + "/" + id;
											console.log("mesh_topic", mesh_topic);
											acl.addTopic(reg.udid, "readwrite", mesh_topic);
										}
									}
								}
	...
	run_transformers(udid, transformer_owner, dry, callback) {
	...
	ott_update(ott, callback) {
		...
		this.client.get("ott:" + ott, (err, info) => {
			...
			//console.log("A) will update from path", path);
	...
	update_device(udid, changes, update_callback) {
		...
		devicelib.get(udid, (err, doc) => {
			...
			//console.log("atomic device updated document (after cleanup)", doc);
			...
			devicelib.atomic("devicelib", "modify", udid, changes, (atomic_err, body) => {
				if (atomic_err) {
					...
					console.log("[atomic modify] atomic device updated with changes", changes);
	...
	edit(device_owner, changes, callback) {
		...
		let acl = new ACL();
		acl.load(() => {
			if (typeof(changes.mesh_ids) !== "undefined") {
				for (let index in changes.mesh_ids) {
					let mesh = mesh_ids[index];
					let mesh_topic = "/" + owner_id + "/" + mesh;
					acl.addTopic(device_owner, "readwrite", mesh_topic);
				}
			}
			acl.commit();
		});

		console.log("[TODO: REMOVE LOG] Device update with: " + JSON.stringify(changes));
