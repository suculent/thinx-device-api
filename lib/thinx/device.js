...
var Device = (function() {
	...
	var _public = {
		...
		updateFromPath: function(path, ott, callback) {
			...
			// TODO: FIXME: This supports only single binary.
			// Multi-file upload goes here - returned buffer must be JSON.
				...
				devicelib.get(udid, function(err, device) {
					...
					client.get("nid:" + did, function(err, json_keys) {
						...
						if (updateAvailable === true) {
							...
							// Start OTT Update
							if (use == "ott") {
								console.log("Starting OTT update...");
								_public.ott_request(owner, body, api_key, callback);

								// Perform OTT Update
							} else if (ott !== null) {
								console.log("Performing OTT update...");
								_public.updateFromPath(path, ott, callback);
		...
		edit: function(owner, changes, callback) {
			...
			udid = change.udid;
			console.log("Processing change " + JSON.stringify(change) + " for udid " +
				udid);
