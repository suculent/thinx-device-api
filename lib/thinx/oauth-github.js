...
module.exports = function (opts) {
  ...
  function callback(code, state, resp, cb) {
    ...
    console.log(("[debug] [oauth-github] Requesting access token with state", {state}, "vs. internalState", {internalState}, "code", {code}));
    ...
    var u = 'https://github.com/login/oauth/access_token'
      ...
      + '&code=' + String(code).replace("&", "").replace("=", "").replace("%00", "").replace("%", "")
      + '&state=' + String(state).replace("&", "").replace("=", "").replace("%00", "").replace("%", "")
      ;
