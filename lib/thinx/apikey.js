	...
	log_invalid_key(apikey, req, owner, callback) {
		...
		if (typeof (callback) !== "undefined") {
			if (typeof (req) === "undefined" || req == null) {
				...
				// HTTP request
				console.log("⚠️ [warning] Invalid API key request from address " + req.ip + " with key " + apikey);
		...
		this.client.get("ak:" + owner, (err, json_keys) => {
			...
			if (err !== null) {
				if (typeof (callback) !== "undefined") {
					...
					throw new Error("[critical] Callback undefined!!! in verify: (1)", err);
				}
				return;
			}
			...
			if ((typeof (json_keys) !== "undefined") && (json_keys !== null)) {
				if (this.key_in_keys(apikey, json_keys)) {
					if (typeof (callback) !== "undefined") {
						...
						throw new Error(`[critical] callback undefined in verify (2)`);
					}
				} else {
			...
			console.log(`☣️ [error] API key '${apikey}' verification failed for owner ${owner}`);
			if (typeof (callback) !== "undefined") {
