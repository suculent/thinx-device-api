...
module.exports = class APIKey {
	...
	save_apikeys(owner_id, api_key_array, api_key_object, callback) {
		this.client.set("ak:" + owner_id, JSON.stringify(api_key_array), (error, result) => {
			if (result !== "OK") {
	...
	create(owner_id, apikey_alias, callback) {
		...
		this.client.get("ak:" + owner_id, (error, json_keys) => {
	...
	verify(owner, apikey, is_http, callback) {
		...
		// Fetch owner keys from redis
		this.client.get("ak:" + owner, (error, json_keys) => {
	...
	revoke(owner, apikey_hashes, callback) {
		...
		// Fetch owner keys from redis
		this.client.get(key_id, (error, json_keys) => {
			...
			this.client.set(key_id, JSON.stringify(new_keys), () => {
