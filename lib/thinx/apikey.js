...
module.exports = class APIKey {
  ...
  constructor() {
    ...
    console.log("ApiKey redis_client instantiated with options: " + JSON.stringify(Globals.redis_options()));
  }
	...
	create(owner_id, apikey_alias, callback) {
		...
		var new_api_key = create_key(owner_id);
		...
		this.client.get("ak:" + owner_id, function(cerr, json_keys) {

			// Create new owner object if nothing found
			if (cerr) {
				this.client.set("ak:" + owner_id, JSON.stringify([api_key_object]), function(nerr) {
					if (nerr) {
			...
			this.client.set("ak:" + owner_id, JSON.stringify(api_keys), function(
				err) {
				if (typeof(callback) !== "undefined") {
					if (err) {
						callback(false, err);
    ...
    console.log("[!] Fetching keys for owner " + owner);
		...
		// Fetch owner keys from redis
		this.client.get("ak:" + owner, function(err, json_keys) {
        ...
        console.log("Fetched keys " + json_keys);
				var keys = JSON.parse(json_keys);
				for (var ki in keys) {
					if (keys[ki].key.indexOf(apikey) !== -1) {
						if (typeof(callback) !== "undefined") {
              console.log("Valid key " + apikey);
							callback(true);
              return; // valid key found, early exit
						} else {
              console.log("Programmer error, callback undefined!");
              return;
            }
					}
				}
	...
	revoke(owner, apikey_hashes, callback) {
		...
		// Fetch owner keys from redis
		this.client.get("ak:" + owner, function(rerr, json_keys) {
			...
			if ((typeof(json_keys) !== "undefined") && (json_keys !== null)) {
				...
				this.client.set("ak:" + owner, JSON.stringify(new_keys), function(err,
					reply) {
					if (err) {
						callback(false);
						console.log(reply);
