...
var APIKey = (function() {
  ...
  console.log("[ApiKey.js] creating redis_client...");
  ...
  console.log("ApiKey redis_client instantiated with options: " + JSON.stringify(Globals.redis_options()));
	...
	var _public = {
		...
		create: function(owner_id, apikey_alias, callback) {
			...
			client.get("ak:" + owner_id, function(cerr, json_keys) {

				// Create new owner object if nothing found
				if (cerr) {
					client.set("ak:" + owner_id, JSON.stringify([api_key_object]), function(nerr) {
						if (nerr) {
      ...
      console.log("[!] Fetching keys for owner " + owner);
		...
		revoke: function(owner, apikey_hashes, callback) {
			...
			// Fetch owner keys from redis
			client.get("ak:" + owner, function(rerr, json_keys) {
