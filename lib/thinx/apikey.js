...
module.exports = class APIKey {
	...
	constructor() {
		...
		console.log("APIKey class initialized with redis options", {options});
	...
	verify(owner, apikey, req, callback) {
		...
		console.log("verify::verify():", {owner}, {apikey});

		if (typeof (callback) === "undefined") {
			console.log("warning: verify callback undefined(1)");
		}

		this.client.keys("*", (error, response) => {
			console.log("redis keys:", error, response);
		});
		...
		this.client.get("ak:" + owner, (err, json_keys) => {
			...
			if (typeof (callback) !== "undefined") {
				callback(false, "apikey_not_found for: ", owner);
			}
		});
	}
