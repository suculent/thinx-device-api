...
module.exports = class APIKey {
  ...
  save_apikeys(owner_id, api_key_array, callback) {
	...
	create(owner_id, apikey_alias, callback) {
        ...
        this.save_apikeys(owner_id, api_keys, null);
      }
		});
	}
  ...
  log_invalid_key(apikey, req, callback) {
    ...
    this.alog.log("OID?", "Attempt to use invalid API Key: " + apikey, "error");
    console.log("API key", apikey, " not found for this owner");
    if (typeof(callback) !== "undefined") {
  ...
  key_in_keys(apikey, json_keys) {
    ...
    for (var ki in keys) {
      ...
      if (value.indexOf(apikey) !== -1) {
		  console.log(apikey, "found in json_keys");
	...
	verify(owner, apikey, req, callback) {
		console.log("verify()", {owner}, {apikey}, {req}, "+callback...");
		if (typeof(callback) === "undefined") {
			console.log("warning: verify callback undefined");
		}
		// Fetch owner keys from redis
		this.client.get("ak:" + owner, (err, json_keys) => {
			console.log("verify()::redis_get", "ak:"+owner, "error:", err, json_keys);
			...
			if ((typeof(json_keys) !== "undefined") && (json_keys !== null)) {
				if (this.key_in_keys(apikey, json_keys)) {
					...
					console.log("[verify] API Key invalid!");
					this.log_invalid_key(apikey, req, callback);
					if (typeof(callback) !== "undefined") {
						callback(false, "apikey_invalid: " + "ak:" + owner);
					}
				}
			}
	...
	list(owner, callback) {
		...
		this.client.get("ak:" + owner, (err, json_keys) => {
			...
			if ((typeof(json_keys) !== "undefined") && (json_keys !== null)) {
				...
				console.log("AKList: success callback");
				callback(true, exportedKeys);
			} else {
				console.log("AKList: failure callback");
				callback(false, []);
			}
		});
	}
