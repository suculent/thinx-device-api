...
module.exports = class APIKey {
	...
	save_apikeys(owner_id, api_key_array, api_key_object, callback) {
		this.client.v4.set("ak:" + owner_id, JSON.stringify(api_key_array)).then((result) => {
			if (result !== "OK") {
	...
	create(owner_id, apikey_alias, callback) {
		...
		this.client.v4.get("ak:" + owner_id).then((json_keys) => {
	...
	verify(owner, apikey, is_http, callback) {
		...
		// Fetch owner keys from redis
		this.client.v4.get("ak:" + owner).then((json_keys) => {
	...
	revoke(owner, apikey_hashes, callback) {
		...
		// Fetch owner keys from redis
		this.client.v4.get(key_id).then((json_keys) => {
			...
			this.client.v4.set(key_id, JSON.stringify(new_keys)).then(() => {
