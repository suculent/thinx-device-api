...
module.exports = class APIKey {
  ...
  key_in_keys(apikey, json_keys) {
    ...
    console.log("Revoking API keys", apikey_hashes, "with owner", owner);
    ...
    let key_id = "ak:" + owner;
      ...
      console.log("Should delete from keys", json_keys);

			for (var ki in keys) {

				var key_hash = keys[ki];

        console.log("key_hash", key_hash, "for key", keys[ki]);
				...
				for (var apikey_hash_index in apikey_hashes) {
					// Skip revoked key(s)
          console.log("Comparing", key_hash, " : ", apikey_hashes[apikey_hash_index]);
      ...
      console.log("Saving new keys...", new_keys);
