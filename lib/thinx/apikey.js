...
module.exports = class APIKey {
	...
	create(owner_id, apikey_alias, callback) {
		...
		if (typeof(new_api_key) === "undefined") console.log("[error] API Key generator error. Check test_ prefix for new_api_key.");
		if (typeof(owner_id) === "undefined") console.log("[error] API Key generator error. Check owner_id");
		var api_key_object = {
			"key": new_api_key,
	...
	log_invalid_key(apikey, req, owner, callback) {
		...
		if (typeof (callback) !== "undefined") {
			if (typeof (req) === "undefined" || req == null) {
				...
				// HTTP request
				console.log("[warning] Invalid API key request from address " + req.ip + " with key " + apikey);
	 ...
	 * @param {req} req - used for callback... req or null; not sent for mqtt; should be is_mqtt; TODO (5): FIXME: REFACTOR https://github.com/suculent/thinx-device-api/issues/298
