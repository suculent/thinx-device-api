			...
			client.get("ak:" + owner, function(err, json_keys) {
				...
				if ((typeof(json_keys) !== "undefined") && (json_keys !== null)) {
					...
					for (var ki in keys) {
						...
						console.log("Seeking " + apikey_hash + " in " + key_hash);
						if (key_hash.indexOf(apikey_hash) != 1) {
							console.log("Revoking key: " + key_hash);
							splice_index = ki;
							break;
						}
					}

					if (typeof(splice_index) !== "undefined") {
						keys.splice(splice_index, 1);
					}

					console.log("After: " + JSON.stringify(keys));

					client.set("ak:" + owner, JSON.stringify(keys), function(err,
						reply) {
						if (err) {
							callback(false);
							console.log(reply);
