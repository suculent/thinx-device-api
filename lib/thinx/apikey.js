...
module.exports = class APIKey {
	...
	save_apikeys(owner_id, api_key_array, api_key_object, callback) {
		this.client.set("ak:" + owner_id, JSON.stringify(api_key_array)).then(() => {
			let response_data;
			if ((typeof (api_key_object) !== "undefined") && (api_key_object !== null)) {
				response_data = [api_key_object];
			}
	...
	create(owner_id, apikey_alias, callback) {
		...
		this.client.get("ak:" + owner_id).then((json_keys) => {
			...
			if ((json_keys === null) || (json_keys == [])) {
				this.save_apikeys(owner_id, [api_key_object], null, callback);
			} else {
