	...
	var _public = {
			...
			client.get("ak:" + owner, function(err, json_keys) {
				...
				if (err) {
					client.set("ak:" + owner, JSON.stringify([api_key_object]), function(
						err) {
						if (err) {
							console.log("API Key save err" + err);
						} else {
		...
		verify: function(owner, apikey, callback) {
			...
			client.get("ak:" + owner, function(err, json_keys) {
				...
				if ((typeof(json_keys) !== "undefined") && (json_keys !== null)) {
					...
					for (var ki in keys) {
						if (!keys.hasOwnProperty(ki)) continue;
						if (!keys[ki].hasOwnProperty("key")) continue;
		...
		revoke: function(owner, apikey_hash, callback) {
			...
			client.get("ak:" + owner, function(err, json_keys) {
				...
				if ((typeof(json_keys) !== "undefined") && (json_keys !== null)) {
					...
					for (var ki in keys) {
						if (!keys.hasOwnProperty(ki)) continue;
						if (!keys[ki].hasOwnProperty("hash")) continue;
		...
		list: function(owner, callback) {
			...
			client.get("ak:" + owner, function(err, json_keys) {
				...
				console.log("Fetched keys: " + JSON.stringify(json_keys));
				...
				if ((typeof(json_keys) !== "undefined") && (json_keys !== null)) {
					...
					for (var index in keys) {
						...
						//if (!api_keys.hasOwnProperty(keys[index])) continue;

						var keydata = api_keys[keys[index]];
						//if (!keys.hasOwnProperty("key")) continue;
						//if (!keys.hasOwnProperty("hash")) continue;
						//if (!keys.hasOwnProperty("alias")) continue;
