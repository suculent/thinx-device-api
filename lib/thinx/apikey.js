...
module.exports = class APIKey {
	...
	save_apikeys(owner_id, api_key_array, api_key_object, callback) {
		this.client.set("ak:" + owner_id, JSON.stringify(api_key_array)).then((result) => {
			console.log("DEBUG save_apikeys result", result);
			callback(true, api_key_object); // must return only original created key object, never the saved array
		});
	}
	...
	create(owner_id, apikey_alias, callback) {
		...
		this.client.get("ak:" + owner_id).then((json_keys) => {
			...
			if (json_keys === null) {
				...
				return this.save_apikeys(owner_id, [api_key_object], null, callback);
			} 
			...
			this.save_apikeys(owner_id, api_keys, api_key_object, callback);
		
		}).catch((e) => {
			console.log("[DEBUG] APIKey.create", e);
			// save anyway
			this.save_apikeys(owner_id, [api_key_object], api_key_object, callback);
		});
	}
	...
	verify(owner, apikey, is_http, callback) {
		...
		// Fetch owner keys from redis
		this.client.get("ak:" + owner).then((json_keys) => {
			...
			console.log("[APIKey][fetch][error]", e);
			callback(false, "apikey_not_found");
		});
	}
	...
	revoke(owner, apikey_hashes, callback) {
		...
		// Fetch owner keys from redis
		this.client.get(key_id).then((json_keys) => {
			...
			this.client.set(key_id, JSON.stringify(new_keys)).then(() => {
				callback(true, deleted_keys);
			});
		}).catch((err) => {
			console.log("[APIKey:revoke:error]:" + err + " revoking " + key_id);
			return callback(false, "owner_not_found");
		});
	}
	...
	list(owner, callback) {
		...
		this.client.get("ak:" + owner).then((json_keys) => {
			...
			console.log("[APIKey:list:error]:" + err);
			return [];
		});
	}
