...
module.exports = class APIKey {
	...
	save_apikeys(owner_id, api_key_array, callback) {
		this.client.set("ak:" + owner_id, JSON.stringify(api_key_array), (nerr) => {
			if (nerr) {
				console.log("[error] [apikey] first key NOT created.");
				callback(false);
				return;
			} 
			this.client.save();
	...
	list(owner, callback) {
		...
		this.client.get("ak:" + owner, (err, json_keys) => {
			...
			var exportedKeys = [];
			console.log("[debug] [apikey] list for", {owner}, {err}, {json_keys});
			if ((err === null) && (typeof (json_keys) !== "undefined") && (json_keys !== null)) {
				console.log("[debug] parsing json_keys", json_keys);
				var api_keys = JSON.parse(json_keys);

				console.log("[debug] api_keys", api_keys);
				...
				for (var index in keys) {
					...
					console.log("[debug] parsing key", {keyname}, {keydata});
	...
	get_first_apikey(owner, callback) {
		this.list(owner, (success, json_keys) => {
			if (!success || (typeof(json_keys[0]) !== "undefined")) {
