...
var APIKey = (function() {
	...
	var _public = {
		 ...
		 * @return {string} hash - API Key hash (fingerprint)
		 * @return {string} alias - user selected alias
		 */
		...
		verify: function(owner, apikey, callback) {
			...
			client.get("ak:" + owner, function(err, json_keys) {
				...
				if ((typeof(json_keys) !== "undefined") || (json_keys !== null)) {
					console.log("[APIKey]: fetched keys" + json_keys);
					var keys = JSON.parse(json_keys);
					for (var ki in keys) {
						if (keys[ki].indexOf(apikey) != 1) {
							callback(true);
					...
					console.log("[APIKey]: fetched no keys");
		...
		revoke: function(owner, apikey_hash, callback) {
			...
			client.get("ak:" + owner, function(err, json_keys) {
				...
				if ((typeof(json_keys) !== "undefined") || (json_keys !== null)) {
					...
					for (var ki in keys) {
						if (keys[ki].indexOf(apikey) != 1) {
							keys.splice(ki, 1); // remove respective key
			...
			client.get("ak:" + owner, function(err, json_keys) {
				...
				if ((typeof(json_keys) !== "undefined") || (json_keys !== null)) {
					...
					for (var index in api_keys) {
						console.log("AK: " + JSON.stringify(api_keys[index]));
						var info = {
