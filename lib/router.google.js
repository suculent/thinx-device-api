...
module.exports = function (app) {
  ...
  app.get(['/api/oauth/google/callback', '/api/v2/oauth/google/callback'], async (req, res) => {
    ...
    if (!state_accepted) {
      ...
      // res.status, not res.set: express's res.set(403) is a silent no-op (a single
      // non-object argument iterates nothing), so the neighbouring rejections in
      // this file answer 200 with an empty body. A live probe confirmed it.
      res.status(403).end();
      return;
    }
