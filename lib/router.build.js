...
module.exports = function (app) {
    ...
    function getArtifactsValidate(owner, udid, build_id, req, res) {
        ...
        if (!device_owner) {
            Util.responder(res, false, "missing_owner");
            retVal = false;
        } else if (!udid) {
            Util.responder(res, false, "missing_udid");
            retVal = false;
        } else if (!build_id) {
            Util.responder(res, false, "missing_build_id");
            retVal = false;
        } else if (owner !== device_owner) {
            Util.responder(res, false, "invalid_request");
            retVal = false;
        } 
