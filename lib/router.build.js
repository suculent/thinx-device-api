...
module.exports = function (app) {
    ...
    function build(req, res) {
        ...
        if ((owner == null) || (udid == null) || (source_id == null)) {
            ...
            console.log("[warning] [build] rejecting request for invalid input:", JSON.stringify({
                owner: owner,
                udid: udid,
                source_id: source_id
            }, null, 2));
    ...
    function getArtifacts(req, res) {
        ...
        if (!device_owner) return Util.responder(res, false, "missing_owner");

        if (!udid) return Util.responder(res, false, "missing_udid");

        if (!build_id) return Util.responder(res, false, "missing_build_id");

        if (owner !== device_owner)  return Util.responder(res, false, "invalid_requet");
