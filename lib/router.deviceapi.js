...
module.exports = function (app) {
  ...
  app.post("/device/register", function (req, res) {
    ...
    const mac = req.body.registration.mac || "unknown";
    console.log(`ℹ️ [info] Device registration attempt for MAC: ${mac}`);
    ...
    device.register(
      ...
      (r, success, response) => {
        ...
        if ((success === true) && (typeof (response.registration) !== "undefined")) {
          ...
          const udid = response.registration.udid || "unknown";
          const owner = response.registration.owner || "unknown";
          console.log(`ℹ️ [info] [OID:${owner}] Device registered successfully, UDID: ${udid}`);
        }
        if (success === false) {
          console.log(`☣️ [error] Device registration failed for MAC: ${mac}, response: ${JSON.stringify(response)}`);
  ...
  app.post("/device/addpush", function (req, res) {
    ...
    device.push(req.body, api_key, (success, response) => {
      if (!success) {
        console.log(`☣️ [error] Device push attach failed: ${JSON.stringify(response)}`);
      }
