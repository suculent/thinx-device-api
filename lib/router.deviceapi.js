  ...
  app.post("/device/register", function (req, res) {
    ...
    if (typeof (req.body) === "undefined") {
      ...
      if (rip !== false) {
        ...
        res.status(400);
        Util.responder(res, false, "request_invalid");
      }
      ...
      try {
        ...
        console.log("[error] Device-API Caught exception: ", err);
      }
      device.register(
        req,
        registration,
        ...
        (success, response) => {
          ...
          if (success === false) {
            console.log("[warning] Device registration failed with response:", response);
