...
function getSourcesList(req, res) {
  ...
  console.log(`ℹ️ [info] [OID:${owner}] GET sources list`);
  sources.list(req.session.owner, (success, response) => {
    if (success !== true) {
      console.log(`☣️ [error] [OID:${owner}] GET sources list failed: ${JSON.stringify(response)}`);
      return Util.respond(res, response);
    }
...
function addSource(req, res) {
  ...
  console.log(`ℹ️ [info] [OID:${owner}] Adding source: ${req.body.alias}`);
  sources.add({
    owner: owner,
  ...
  (_success, response) => {
    if (!_success) {
      console.log(`☣️ [error] [OID:${owner}] Adding source failed: ${JSON.stringify(response)}`);
    }
...
function deleteSource(req, res) {
  ...
  let source_ids = Array.isArray(req.body.source_ids)
    ? req.body.source_ids.map((id) => mongoSanitize(id)) // sanitize individual items in array
    : [mongoSanitize(req.body.source_ids)]; // handle case where source_ids isn't an array

  console.log(
    `ℹ️ [info] [OID:${owner}] Removing sources: ${JSON.stringify(source_ids)}`,
  );

  sources.remove(owner, source_ids, (success, message) => {
    if (!success) {
      console.log(
        `☣️ [error] [OID:${owner}] Removing sources failed: ${JSON.stringify(message)}`,
      );
    }
