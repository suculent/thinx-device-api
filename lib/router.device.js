...
module.exports = function (app) {
  ...
  function editDevice(req, res) {
    if (!Util.validateSession(req)) return res.status(401).end();
    if (typeof (req.body) === "undefined") return Util.respond(res, false, "missing_body");
    if (typeof (req.body.changes) === "undefined") return Util.respond(res, false, "missing_changes");
    let changes = req.body.changes;
    // Manually fixes wronte device docs, regardless why it happens(!)
    if (typeof (changes.doc) !== "undefined") changes.doc = null;
    if (typeof (changes.value) !== "undefined") changes.value = null;
  ...
  function getDeviceDetail(req, res) {
    if (!Util.validateSession(req)) return res.status(401).end();
    if (typeof (req.body) === "undefined") return res.status(400).end();
    if (typeof (req.body.udid) === "undefined") return res.status(400).end();
  ...
  function publishNotification(req, res) {
    ...
    if ((typeof (reply) === "undefined") || (reply == null)) {
      return Util.responder(res, false, "missing_reply");
    }
  ...
  function runTransformer(req, res) {
    ...
    let owner = sanitka.owner(req.session.owner);
    if ((typeof (owner) === "undefined") || (owner === null)) {
      return Util.responder(res, false, "owner_not_found");
    }
    var udid = req.body.device_id;
    if ((typeof (udid) === "undefined") || (udid === null)) {
      return Util.responder(res, false, "udid_not_found");
    }
    device.run_transformers(udid, owner, false, Util.responder, res);
  }
  ...
  app.get("/api/device/data/:udid", function (req, res) {
    ...
    if (typeof (app.messenger) !== "undefined") {
      ...
      Util.respond(res, false, "messenger_not_available");
    }
  });
};