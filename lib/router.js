  ...
  */
  ...
  function rejectLogin(req, user_data, password, stored_response) {
    ...
    if ((typeof (deleted) !== "undefined") && (deleted === true)) {
      console.log("ℹ️ [info] user deleted.");
    ...
    if (password.indexOf(user_data.password) === -1) {
      ...
      if (typeof (p) === "undefined" || p === null) {
        console.log("⚠️ [warning] [LOGIN_INVALID] (not activated/no password)");
  ...
  app.post("/api/login", function (req, res) {
    ...
    user.validate(username, (db_body) => {
      ...
      console.log("[debug] session with short cookie domain", short_domain);
      ...
      res.cookie("x-thx-ws-session", maxAge, {
        maxAge: maxAge,
        httpOnly: false,
        secure: false,
        domain: short_domain
      ...
      // this works
      res.cookie("x-thx-ws-httponly", maxAge, {
        maxAge: maxAge,
        httpOnly: true,
        secure: false
      });
