...
module.exports = function(app) {
  ...
  function checkDirtyRequests(req, res, next) {
    ...
    if (BLACKLIST.includes(client_ip)) {
      console.log(client_ip, "BLACKLISTED! such requests may/should fail in future!");
    }
  ...
  app.get("/api/mesh/list", function(req, res) {
    console.log("req.session A", req.session);
    user.listMeshes(req.session.owner, function(success, mesh_ids) {
      console.log("A list success:", success, "ids", mesh_ids);
      respond(res, {
        success: success,
        mesh_ids: mesh_ids
      });
  ...
  app.post("/api/mesh/create", function(req, res) {
    // owner_id is session_id
    // mesh_id is required parameter
    // alias is optional parameter

    console.log("Called api/mesh/create...", req.body);
    ...
    user.createMesh(owner_id, mesh_id, mesh_alias, function(success, status) {
      console.log("Mesh create success:", success, "ids", mesh_ids);
  ...
  app.post("/api/mesh/delete", function(req, res) {
    ...
    console.log("Called api/mesh/delete...", req.body);
    ...
    let mesh_ids;
    if (typeof(req.body.mesh_ids) === "undefined") {
      respond(res, { success: false, status: "Mesh ID missing in request body."});
    } 

    user.deleteMeshes(owner_id, mesh_ids, function(success, status) {
      console.log("Mesh delete success:", success, "ids", mesh_ids);
      respond(res, { success: success, status: status });
    });
