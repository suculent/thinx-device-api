...
module.exports = function (app) {
  ...
  function validateSession(req) {
    return Util.validateSession(req);
  }
  ...
  app.post("/api/user/rsakey/revoke", function (req, res) {
    ...
    if (typeof (req.body.filenames) !== "undefined") {
      ...
      let r_responder = (res, success, result) => {
        Util.responder(res, success, result);
      };
  ...
  app.post("/api/mesh/delete", function (req, res) {
    ...
    if (typeof(req.body) !== "object") {
      respond(res, { success: false, status: "Invalid request format." });
      return;
    }
  ...
  function performOAuthLogin(req, res, oauth) {
    ...
    redis_client.get(oauth, function (err, userWrapper) {
      if (err) {
        console.log("[oauth] [LOGIN_INVALID] takeover failed");
        ...
        if ((typeof (wrapper) !== "undefined") && wrapper !== null) {
          ...
          console.log("[login] [LOGIN_INVALID] user wrapper error: ", { userWrapper });
          if (wrapper === null) {
  ...
  app.post("/api/login", function (req, res) {
    ...
    user.validate(username, (db_body) => {
      ...
      if ((typeof (db_body.rows) === "undefined") || (db_body.rows.length == 0)) {
        console.log("⚠️ [warning] [LOGIN_INVALID] No body rows returned on login while validating username", username); // in case DB is broken
