...
module.exports = function (app) {
  ...
  */
  ...
  app.get('/api/oauth/github', function (req, res) {
    console.log("/api/oauth/github will destroy old session...");
    if (typeof (req.session) !== "undefined") {
      req.session.destroy();
    }
    if (typeof (githubOAuth) !== "undefined") {
      console.log('[debug] [oauth] [github] Starting Login...');
  ...
  app.get('/api/oauth/gcb', function (req, res) {

    let query = req.query;
    console.log("GitHub OAuth request (code, state): ", { query }); // TODO (4): get some ID for global_response array to allow multiple requests at once without compromising security/availability
    console.log("Saving global_response in /oauth/gcb");
    global_response = res;
    global_responses[req.query.code] = res;
    // TODO: create login_token[req.query.code] = token and then here:
    // const login_token = login_token[req.query.code]

    console.log("Github OAuth2 Callback (TODO (4): validate redis oa:*:g token)...");
  ...
  app.get('/api/oauth/google', function (req, res) {
    ...
    require("crypto").randomBytes(48, (err, buffer) => {
      ...
      console.log("GET [oauth/google] started with UNSAVED TOKEN", token, "redirecting to", authorizationUri);
      res.redirect(authorizationUri);
    });
  });

  app.get('/api/oauth/cb', async (req, res) => {
    ...
    try {
      ...
      var gat_url = 'https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token=' + accessToken;
