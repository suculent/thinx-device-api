...
module.exports = function(app) {
  ...
  function checkDirtyRequests(req, res, next) {
    ...
    if (BLACKLIST.includes(client_ip)) {
      console.log(client_ip, "BLACKLISTED! such requests may/should fail in future:", req.headers, req.body);
    }
  ...
  app.post("/api/login", function(req, res) {
    ...
    console.log("Password with prefix", prefix, "and body", req.body.password);
    ...
    userlib.view("users", "owners_by_username", {
      ...
      if (password.indexOf(user_data.password) !== -1) {
        ...
        if (client_type == "device") {
          ...
          userlib.get(user_data.owner, function(error, udoc) {
            ...
            if (error) {
              ...
              console.log("atomic edit doc", user_data.owner, udoc);
        ...
        if (typeof(p) === "undefined" || p === null) {
          ...
          console.log("[LOGIN_INVALID] Password ", password, "mismatch for: " + username);
  ...
  app.get("/slack/direct_install", function(req, res) {
    ...
    res.redirect(
      "https://slack.com/oauth/authorize?client_id=" + slack_client_id + "&scope=bot&state=Online&redirect_uri=" + app_config.public_url + ":7443/slack/redirect"
    );
  });
  ...
  app.get("/slack/redirect", function(req, xres) {
    ...
    var options = {
      ...
      hostname: 'slack.com',
      port: 443,
      path: '/api/oauth.access?client_id=' + slack_client_id + '&client_secret=' + slack_client_secret + '&redirect_uri=' + app_config.public_url + ':7443/slack/redirect&scope=bot&code=' +
        req.query.code
    };
