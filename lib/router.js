...
module.exports = function (app) {
  ...
  */
  ...
  // validates required params for authenticated session: internal owner or external owner_id+api_key that will be checked later
  ...
  app.get('/api/oauth/github/callback', function (req, res) {
    require("crypto").randomBytes(48, (err, buffer) => {
      ...
      redis_client.set("oa:github:" + token, 60);
      // Authorization uri definition
      let query = req.query;
      console.log("GitHub OAuth request (code, state): ", { query }, "with token", token);
      githubOAuth.callback(req, res, (gerr) => {
        if (gerr) {
          console.log("githubOAuth.callback cberr: ", { gerr });
          return;
        }
  ...
  app.get('/api/oauth/google', function (req, res) {
    ...
    require("crypto").randomBytes(48, (err, buffer) => {
      var token = buffer.toString('hex');
      redis_client.set("oa:google:" + token, 60); // auto-expires in 1 minute
      // Authorization uri definition
      ...
      console.log("GET [oauth/google] started with token", token, "redirecting to", authorizationUri);
  ...
  app.get('/api/oauth/google/callback', async (req, res) => {
    ...
    console.log("[debug] Google OAuth Callback incoming code:", code);
    ...
    try {
      ...
      console.log("[debug] fetching userinfo with Google Access Token:", accessToken);
      let gat = accessToken.token.access_token;

      const gat_url = 'https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token=' + gat;

      console.log({gat}, {gat_url});
      ...
      https.get(gat_url, (res3) => {
        ...
        res3.on('end', () => {
          ...
          console.log("[debug] [oauth/cb] fetched GAT: ", {odata});
