...
module.exports = function (app) {
  ...
  function enforceACLHeaders(res, req) {
    ...
    if ((typeof (requestOrigin) === "string") && (requestOrigin.length > 0) && (requestOrigin !== "device")) {
      res.header("Access-Control-Allow-Origin", requestOrigin);
      res.header("Vary", "Origin");
    } else {
      // deepcode ignore TooPermissiveCorsHeader: fallback restricted to CORS_ALLOWED_ORIGINS allowlist
      const allowedOrigins = (process.env.CORS_ALLOWED_ORIGINS || app_config.public_url || '').split(',').map(s => s.trim()).filter(Boolean);
      const allowed = allowedOrigins.length === 0 ? app_config.public_url : allowedOrigins[0];
    ...
    res.header("Access-Control-Allow-Headers", "Content-type,Accept,X-Access-Token,X-Key,X-XSRF-TOKEN");
  }
