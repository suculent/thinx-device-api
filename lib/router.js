...
module.exports = function (app) {
  ...
  function validateSecureGETRequest(req, res) {
    if (req.method !== "GET") {
      console.log("⚠️ [warning] validateSecure: Not a get request." + JSON.stringify(req.query.params));
  ...
  function validateSecurePOSTRequest(req, res) {
    if (req.method !== "POST") {
      console.log("⚠️ [warning] validateSecure: Not a post request.");
  ...
  function validateSession(req) {
    ...
    console.log("⚠️ [warning] Session seems invalid (no-owner).");
  ...
  function secureGithubCallbacks(original_response) {
    ...
    githubOAuth.on('error', (err) => {
      ...
      if (typeof (original_response) !== "undefined") original_response.end(666);
    });
