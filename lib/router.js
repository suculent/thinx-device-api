...
module.exports = function (app) {
  ...
  const auth = new Auth(); // constructor must be called to do the job
  ...
  function respond(res, object) {
    console.log("Responding with", {object}, {res});
  ...
  function enforceACLHeaders(res, req) {
    ...
    // this is common for worldwide devices and users so far...
  ...
  app.all("/*", function (req, res, next) {
    ...
    try {
      logAccess(req, client);
    } catch (e) {
      //console.log("Failed logging request", req);
      next();
      return;
    }
    ...
    if (typeof (req.body) !== "undefined") {
      ...
      if (typeof (xowner) !== "undefined" && typeof (api_key) !== "undefined") {
        // Using Owner/API Key
        apikey.verify(sanitka.udid(xowner), sanitka.udid(api_key), req, function (vsuccess, vmessage) {
          if (vsuccess) {
            next();
          } else {
            respond(401, {
  ...
  app.post("/api/device/mesh/detach", function (req, res) {
    ...
    devices.detachMesh(owner, body, function (success, status) {
  ...
  app.get("/api/device/data/:udid", function (req, res) {
    ...
    app.messenger.data("", udid, function (success, response) {
      respond(res, {
        success: success,
        response: response
      });
  ...
  app.post("/api/user/create", function (req, res) {
    user.create(req.body, true, responder, res);
  });
  ...
  app.post("/api/device/envs", function (req, res) {
    device.envs(req.body.udid, function (success, response) {
      respond(res, response);
    });
  });
  ...
  app.post("/api/device/detail", function (req, res) {
    device.detail(req.body.udid, function (success, response) {
      respond(res, response);
    });
  });
  ...
  app.get("/api/mesh/list", function (req, res) {
    user.listMeshes(req.session.owner, function (success, mesh_ids) {
      respond(res, {
        success: success,
        mesh_ids: mesh_ids
      });
  ...
  app.post("/api/mesh/list", function (req, res) {
    ...
    let owner_id = req.body.owner_id; // TODO: change to session validation and/or JWT, this is insecure
    user.listMeshes(owner_id, function (success, mesh_ids) {
  ...
  function performOAuthLogin(req, res, oauth) {
    ...
    redis_client.get(oauth, function (err, userWrapper) {
      if (err) {
        ...
        console.log("[debug] [performOAuthLogin] fecthing from userlib:", owner_id);
        ...
        userlib.get(owner_id, function (gerr, doc) {
          ...
          if (gerr) {            
            ...
            console.log("[oauth] owner_id not found, ", gerr, "creating: ", owner_id);
            ...
            console("Responding with reedirectURL to /app");
  ...
  function redirectWithToken(res, token, skip_gdpr_page) {
    const ourl = app_config.acl_url + "/auth.html?t=" + token + "&g=" + skip_gdpr_page;
    let rbody = { 
      "status": "OK",
      "success": true,
      "redirectURL": ourl
    };
    console.log("[debug] [loginWithGDPR] Responding with redirectURL: ", ourl, " and rbody ", {rbody}, "using response", {res});
    res.status(200).end(JSON.stringify(rbody));
  }
  ...
  // TODO: Fix sideeffect returning token (can only throw)
  function newTokenWithUserData(user_data) {
    let token = sha256(user_data.email + ":" + user_data.activation_date);
    console.log("Saving login token (also as global!) a.k.a. UserWrapper!", token, {user_data});
    redis_client.set(token, JSON.stringify(user_data));
    redis_client.expire(token, 60);
    return token;
  }

  function loginWithGDPR(req, res, user_data) {

    if (typeof(user_data.owner) !== "undefined") {
      console.log("Overriding req.session.owner from user_data! This is suspicious vulnerability.");
      req.session.owner = user_data.owner;
    }

    if (typeof (req.session.owner) === "undefined") {
      console.log("[warning] [loginWithGDPR] Flushing session: " + JSON.stringify(req.session));
      failureResponse(res, 403, "unauthorized");
      req.session.destroy(function (err) {
        if (err) {
    ...
    const token = newTokenWithUserData(user_data);

    console.log("[debug] [loginWithGDPR] [redirectWithToken] response should not be 404", {res}); // already broken here
    
    return redirectWithToken(res, token, willSkipGDPR(user_data));
  ...
  app.post("/api/login", function (req, res) {
    ...
    if ((typeof (oauth) !== "undefined") && (oauth !== null)) {
        console.log("Going the OAuth login way...");
    ...
    console.log("Going the U/P login way...", {debugRequestBody});
    ...
    userlib.view("users", "owners_by_username", {
      "key": username,
      "include_docs": true // required for password, but this anyway returns whole doc so wtf? maybe this creates the additional issue somewhere later
    }, (err, db_body) => {
      ...
      var all_users = db_body.rows;

      var user_data = null;
      for (var index in all_users) {
        var all_user_data = all_users[index];
        if (username.indexOf(all_user_data.value.username) == 0) {
          console.log("Found user", {all_user_data});
      ...
      if (user_data === null) {
        ...
        console.log("No user data, ", username, " not authorized against", db_body);
        failureResponse(res, 403, "unauthorized");
        return;
      }
      ...
      if (password.indexOf(user_data.password) !== -1) {
        ...
        console.log("Refreshing owner default ACLs...");

        user.create_default_acl(user_data.owner, () => {

          console.log("[info] ACL refresh complete...");

          if (typeof (req.session) !== "undefined") {

            console.log("[info] Setting session owner...");
          ...
          // User validated, now respond or redirect
          ...
          if (client_type == "webapp") {
            ...
            console.log("[debug] [login] Fetching doc", user_data.owner);
  
            // Make note on user login
            userlib.get(user_data.owner, (error, udoc) => {
              if (error) {
                console.log("[OID:" + user_data.owner + "] owner get error: " + error);
              } else {
                userlib.atomic("users", "checkin", user_data.owner, {
                  last_seen: new Date()
                }, (aerr, response) => {
                  if (aerr) {
                    console.log("Last-seen update failed (2): " + aerr, response);
                  }
                  console.log("[debug] [login] going to loginWithGDPR...");
                  req.session.owner = user_data.owner;

                  console.log("[debug] [login] [last-seen] updated...");
                  alog.log(udoc.owner, "Last seen updated.");

                  console.log("loginWithGDPR with res", {res}); // FIXME: {res} should NOT be 404[debug] [loginWithGDPR] [redirectWithToken]...

                  return loginWithGDPR(req, res, user_data); // response should not be 404 here
                });
              }
            });
  ...
  app.post("/api/device/push", function (req, res) {
    if (!(validateSecurePOSTRequest(req) || validateSession(req, res))) return;
    var owner = req.session.owner;
    devices.push(owner, req.body, function (success, response) {
      respond(res, {
        success: success,
        status: response
      });
  ...
  app.get("/slack/direct_install", function (req, res) {
    const slack_client_id = "233115403974.233317554391";  // FIXME: SECURITY BUG, needs to be configuration variable app_config.slack.*
    res.redirect(
  ...
  app.get("/slack/redirect", function (req, xres) {
    ...
    const slack_client_secret = "ccbaae01e5259ed283ef63321be597da"; // FIXME: SECURITY BUG, needs to be configuration variable in app_config.slack.*
    const slack_client_id = "233115403974.233317554391"; // FIXME: SECURITY BUG, needs to be configuration variable app_config.slack.*
