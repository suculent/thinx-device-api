  ...
  */
  ...
  function validateSession(req, res) {
    ...
    if (typeof (req.session.owner) !== "undefined") {
      ...
      if (typeof (req.session) !== "undefined") {
        if (typeof (req.session.owner) !== "undefined") {
          console.log("[debug] destroying existing session", sess);
          req.session.destroy(function (err) {
            if (err) {
  ...
  app.all("/*", function (req, res, next) {
    ...
    if (req.method == "POST") {
      if (typeof (req.body) !== "undefined") {
        let body = req.body;
        console.log("[debug] [next] Incoming post body", { body });
        let owner = req.session.owner;
        if (typeof (owner) !== "undefined") {
          let xowner = sanitka.owner(req.body.owner); // owner can be undefined here?
  ...
  app.get("/api/user/devices", function (req, res) {
    ...
    console.log("🔨 [debug] devices session owner:", session);
  ...
  app.post("/device/firmware", function (req, res) {
    ...
    if ((typeof (req.body.use) !== "undefined") && (req.body.use == "ott")) {
      ...
      device.firmware(req.body, req.headers.authentication, req.owner,
        (success, response) => {
        ...
        userlib.get(owner_id, function (gerr, doc) {
          ...
          if (gerr) {
            ...
            user.create(wrapper, false, res, (response, success, status) => {
              ...
              alog.log(owner_id, "OAuth User created: " + wrapper.first_name + " " + wrapper.last_name);
            ...
            // disabled to see if it has affects login issues
            req.session.cookie.secure = !app_config.debug.allow_http_login || true;
            req.session.cookie.secure = false; // overrided, fixme!
            req.session.cookie.httpOnly = false;

            console.log(`[OID:${owner_id}] [NEW_SESSION] [oauth]`);

            alog.log(owner_id, "New session.", "info");
            ...
            updateLastSeen(doc);
            console.log("🔨 [debug] Responding with redirectURL to /app");
  ...
  app.post("/api/login", function (req, res) {
    ...
    if ((typeof (token) !== "undefined") && (token !== null)) {
      let debug_session = req.session;
      console.log("ℹ️ [info] [/api/login] Performing Token login with session", { debug_session });
    ...
    user.validate(username, (db_body) => {
      ...
      if (typeof (db_body) === "undefined" || (db_body == null) || (db_body === false)) {
        console.log("☣️ [error] [/api/login] No db_body, ", db_body, "for user", username);
      ...
      if (user_data === null) {
        ...
        console.log(`☣️ [error] No user data, '${username}' not authorized.`);
      ...
      if (typeof (req.session) !== "undefined") {
        console.log("🔨 [debug] [/api/login] assigning owner cookie to existing session DB (%s, %s)", req.session.owner, user_data.owner); // here the request/response seems to be still OK
        req.session.owner = user_data.owner;
