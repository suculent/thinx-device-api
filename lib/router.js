...
module.exports = function (app) {
  ...
  function loginRedirectWithToken(req, res, user_data) {
    ...
    if (typeof (req.session.owner) === "undefined") {
      console.log("[warning] [loginRedirectWithToken] Flushing session: " + JSON.stringify(req.session));
      failureResponse(res, 403, "unauthorized");
      req.session.destroy(function (err) {
        if (err) {
    ...
    //console.log("[debug] [loginRedirectWithToken] [redirectWithToken] response should not be 404", { res });

    let token = newTokenWithUserData(user_data);
    let redirectURL = app_config.acl_url + "/auth.html?t=" + token + "&g=" + willSkipGDPR(user_data);
    
    console.log("[debug] [loginRedirectWithToken] redirecting to", redirectURL);
  ...
  app.post("/api/login", function (req, res) {
    ...
    console.log("[/api/login][0] request body", req.body);
    ...
    user.validate(username, function(db_body) {
      ...
      if (client_type == "webapp") {
        ...
        user.create_default_acl(user_data.owner, function(ignored) {
          ...
          console.log("webapp loginRedirectWithToken with res", { cached_response }); // FIXME: first broken here {res} should NOT be 404[debug] [loginRedirectWithToken] [redirectWithToken]...
          auditLogin(user_data.owner); // audit can be done later
          
        });

        loginRedirectWithToken(req, cached_response, user_data); // response should not be 404 here
