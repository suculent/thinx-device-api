...
module.exports = function (app) {
  ...
  app.all("/*", function (req, res, next) {
    ...
    if ((typeof (req.headers['authorization']) !== "undefined") || (typeof (req.headers['Authorization']) !== "undefined")) {
      login.verify(req, (error, payload) => {
        // console.log("[debug] JWT Secret verification result:", { error }, { payload });
        if (error == null) {
          req.session.owner = payload.username;
          next();
        } else {
          res.status(403).end(); // TODO: FIXME: Change to 401 Unauthorized in tests as well!
        }
      });
      return;
    }
  ...
  app.post("/api/transformer/run", function (req, res) {
    ...
    device.run_transformers(udid, owner, false, responder, res);
  });
