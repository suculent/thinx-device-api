...
module.exports = function (app) {
  ...
  */
  ...
  if (typeof (github_ocfg) !== "undefined" && github_ocfg !== null) {
    ...
    console.log(`[debug] github-oauth with specs: ${specs}`);
    try {
  ...
  if (typeof (google_ocfg) !== "undefined" && google_ocfg !== null) {
    ...
    app.get('/api/oauth/cb', function (req, ores) {

      console.log("[debug] Google OAuth Callback incoming code:", req.query.code);

      let c = req.query.code || "";
      if (typeof (c) !== "string") {
        return;
      } else {
        if (c.length > 255) return; // should not DoS the regex now; lgtm [js/type-confusion-through-parameter-tampering]
      }
      ...
      console.log("[debug] GET [oauth/cb] started with options", { options });
      ...
      var tok = oauth2.authorizationCode.getToken(options, (error, result) => {
        ...
        console.log("[debug] GET [oauth/cb] creating token", { result });
        return oauth2.accessToken.create(result);
      });

      console.log("[debug] GET [oauth/cb] token", { tok });

      tok.then(res2 => {

        console.log("[debug] GET [oauth/cb] assigning/storing global_token: ", { global_token }); // WTF?

        global_token = res2.access_token; // TODO (4): FIXME: remove global_token requirement or map to something; c
        ...
        console.log("Oauth error: " + err); // TODO: FIXME: fails here
        ores.redirect(
          app_config.public_url + '/error.html?success=failed&title=OAuth-Error&reason=' +
          err.message);
      });
