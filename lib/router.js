...
module.exports = function (app) {
      ...
      if (err) {
        ...
        userlib.get(owner_id, function (gerr, doc) {
          ...
          if (gerr) {
            ...
            // FIXME: create owner _only_ if does not exist
  ...
  app.post("/api/login", function (req, res) {
    ...
    //console.log("[/api/login] client_type:", client_type, "res:", res); // OK here
    ...
    if ((typeof (oauth) !== "undefined") && (oauth !== null)) {
      console.log("[/api/login] Going the OAuth login way...");
    ...
    user.validate(username, function(db_body) {
      ...
      let rows = db_body.rows;

      console.log("[D5]", {stored_response}, {rows});

      // Find user and match password
      let user_data = db_body.rows[0].value;
      
      console.log("[D6]", {user_data}, {stored_response}); // at this point res is wrong...
      ...
      if (typeof (req.session) !== "undefined") {
        console.log("[/api/login] [D5] assigning owner from DB"); // here the request/response seems to be still OK
        req.session.owner = user_data.owner;
        /*
      ...
      console.log("[D7]", {stored_response}); // here the response is already fault

      if (client_type == "webapp") {

        //let cached_response = Object.assign({}, stored_response);

        console.log("[api/login] Refreshing owner default ACLs...");
        user.create_default_acl(user_data.owner, function(ignored) {
          console.log("ACL created async.");
          console.log("webapp loginWithGDPR with res", { cached_response }); // FIXME: first broken here {res} should NOT be 404[debug] [loginWithGDPR] [redirectWithToken]...
          loginWithGDPR(req, cached_response, user_data); // response should not be 404 here
