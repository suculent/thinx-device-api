...
module.exports = function (app) {
  ...
  */
  ...
  var global_response = null; // was stored in GET /api/oauth/github; continued by checkUserWithResponse() in githubOAuth.on('token',...);
  ...
  console.log("ℹ️ [info] Bootstrapping completed.");
  ...
  function validateSession(req) {
    ...
    if (typeof (req.body) !== "undefined") {
      if ((typeof (req.body.owner_id) !== "undefined") && (typeof (req.body.api_key) !== "undefined")) {
        console.log("Session seems valid (apikey).");
        return true;
      }
    }

    console.log("Session seems invalid (no-owner).");
  ...
  console.log(`[debug] [oauth] [github] init with redirect_uri ${redirect_uri} and baseURL ${github_ocfg.base_url}`);
  ...
  if (typeof (process.env.GITHUB_CLIENT_SECRET) !== "undefined" && process.env.GITHUB_CLIENT_SECRET !== null) {
    const specs = {
      ...
      scope: 'bot user email'
    };
    console.log(`[debug] [oauth] [github] with specs: ${JSON.stringify(specs)}`);
    try {
      githubOAuth = require('./thinx/oauth-github')(specs);
    } catch (e) {
      console.log(`[debug] [oauth] [github] github_ocfg init error: ${e}`);
    }
  }

  // called from checkUserWithResponse()
  ...
  // TODO: fixme: not used with google, the trackUserLogin must be called...
      ...
      if (typeof (req.body) !== "undefined") {
        ...
        if (typeof (owner) !== "undefined") {
          ...
          if ((typeof (xowner) !== "undefined") && (xowner !== null) && (typeof (api_key) !== "undefined") && (api_key !== null)) {
            // Using Owner/API Key
            apikey.verify(sanitka.owner(xowner), sanitka.udid(api_key), req, (vsuccess, vmessage) => {
              if (vsuccess) {
                next();
              } else {
                respond(401, {
  ...
  app.get("/api/user/devices", function (req, res) {
    ...
    if (!(validateSecureGETRequest(req) || validateSession(req))) { 
      res.status(401).end();
      return;
    }
  ...
  app.post("/api/device/attach", function (req, res) {
    if (!(validateSecurePOSTRequest(req) || validateSession(req))) return;
    ...
    apikey.create(owner, new_api_key_alias, (success, all_keys) => {
      if (success) {
        if (all_keys.length == 0) {
          ...
          console.log(`[info] Created API key ${new_api_key_alias}`);
          respond(res, {
            success: success,
            api_key: object.key,
  ...
  app.get("/api/user/rsakey/create", function (req, res) {
    ...
    if (!validateSession(req)) return;

    let owner = sanitka.owner(req.session.owner);
    const ownerValid = this.validateOwner(owner);
    if (!ownerValid) {
  ...
  app.post("/device/addpush", function (req, res) {
    ...
    if ((typeof(tstring) === "undefined") || (tstring === null)) {
      respond(res, {
        success: false,
        status: "no_token"
      });
  ...
  app.post("/api/mesh/create", function (req, res) {
    ...
    // TODO (2): change to session validation and/or JWT, this is insecure
    // https://github.com/suculent/thinx-device-api/issues/295
    ...
    redis_client.get(oauth, function (err, userWrapper) {
      if (err) {
        ...
        userlib.get(owner_id, function (gerr, doc) {
          ...
          req.session.cookie.secure = false;  // fixes HTTP login
  ...
  function rejectLogin(req, user_data, password, stored_response) {
    ...
    if (typeof (user_data) === "undefined" || (user_data === null) || (user_data === false)) {
      console.log("☣️ [error] [/api/login] No user_data");
    ...
    if ((typeof (deleted) !== "undefined") && (deleted === true)) {
      console.log("ℹ️ [info] [D6] user deleted.");
    ...
    if (password.indexOf(user_data.password) === -1) {
      ...
      if (typeof (p) === "undefined" || p === null) {
        console.log("[LOGIN_INVALID] (not activated/no password)");
        ...
        console.log("[LOGIN_INVALID] Password mismatch.");
  ...
  function checkMqttKeyAndLogin(req, cached_response, user_data) {
    ...
    user.mqtt_key(user_data.owner, function (success, key) {
      ...
      user.create_default_acl(user_data.owner, () => {
        console.log("🔨 [debug] [api/login] Refreshed owner default ACLs...");
        // audit
  ...
  app.post("/api/login", function (req, res) {
    ...
    user.validate(username, (db_body) => {
      ...
      if (typeof(db_body.rows[0]) === "undefined") {
        console.log("No body rows returned on login while validating username", username); // in case DB is broken
        return;
      }

      let user_data = db_body.rows[0].value;

      if (rejectLogin(req, user_data, password, stored_response)) return;
    ...
    var areq = https.get(options, function (res) {
      ...
      res.on('data', function (chunk) {
        ...
        if (typeof (token) !== "undefined") {
          ...
          // TODO (3): Tell messenger when new Slack bot token is received.
          // https://github.com/suculent/thinx-device-api/issues/296
  ...
  if (typeof (githubOAuth) !== "undefined") {
    ...
    githubOAuth.on('token', (oauth_token_string, serverResponse) => {

      console.log("[debug] [oauth] [github] githubOAuth on token", {oauth_token_string}, {serverResponse});

      let oauth_token_array = oauth_token_string.split("&");

      console.log("[debug] [oauth] [github] oauth_token_array", {oauth_token_array});
      ...
      if (typeof (access_token) === "undefined") {
        console.log("[debug] [github] [token] Fetching token failed.", serverResponse);
        return;
      }

      console.log("[debug] [github] [token] request successful:", { oauth_token });
      ...
      console.log("[debug] [github] [token] getting token with", {request_options});
      ...
      https.get(request_options, (res) => {
        ...
        res.on('end', () => {
          ...
          try {
            ...
            console.log("[error] [github] [token] error parsing e-mail: " + e + " email: " + email);
            res.redirect(
          ...
          console.log("[debug] [github] [token] checkUserWithResponse with token:", token);
  ...
  app.get('/api/oauth/github/callback', function (req, res) {
    require("crypto").randomBytes(48, (err, buffer) => {
      if (err) throw new Error("[critical] crypto failure");
      var token = buffer.toString('hex');
      let query = req.query; // TODO: FIXME: unused; what's inside?
      console.log("[debug] [oauth] [github] callback query:", query);
      redis_client.set("oa:github:" + token, 60); // TODO: FIXME: Does not set any value; is never read
      console.log("[debug] [oauth] [github] request with incoming query ", { query }, "tagged with new token", token);
      githubOAuth.callback(req, res /*, (gerr) => {
        if (gerr) {
          console.log("[debug] [oauth] [github] callback error", { gerr });
          return;
        }
        console.log(`[debug] [oauth] [github] callback should login with token ${token} now...`);
        const rurl = app_config.acl_url + "/auth.html?t=" + token + "&g=" + false; // require GDPR consent
        res.redirect(rurl);
      }*/ );
    });
  });
  ...
  app.get('/api/oauth/google', function (req, res) {
    ...
    require("crypto").randomBytes(48, (err, buffer) => {
      ...
      const authorizationUri = oauth2.authorizationCode.authorizeURL({
        ...
        scope: 'email',
        state: token // returned upon auth provider call back
      });
  ...
  app.get('/api/oauth/google/callback', async (req, res) => {
    ...
    if (typeof (code) !== "string") {
      ...
      if (code.length > 255) return; // should not DoS the regex now; lgtm [js/type-confusion-through-parameter-tampering]
    }
      ...
      res3.on('end', () => {
        ...
        userlib.get(owner_id, function (error, udoc) {
          if (error) {
            ...
            processGoogleCallbackError(error, ores, udoc, req, odata, userWrapper, accessToken);
            return;
          }
          updateLastSeen(udoc);
          alog.log(owner_id, "OAuth2 User logged in...");
