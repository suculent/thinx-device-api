...
module.exports = function (app) {
  ...
  function redirectWithToken(res, user_data) {
    const token = newTokenWithUserData(user_data);
    const ourl = app_config.acl_url + "/auth.html?t=" + token + "&g=" + willSkipGDPR(user_data);
    let rbody = {
      "status": "OK",
      "success": true,
      "redirectURL": ourl
    };
    console.log("[debug] [loginWithGDPR] Responding with rbody:", { rbody }, "using response", { res });
  ...
  function loginWithGDPR(req, res, user_data) {
    ...
    redirectWithToken(res, user_data);
  ...
  function userDataFromDB(username, db_body) {
     // Find user and match password
     let all_users = db_body.rows;
     let user_data = null;
     for (var index = 0; index < all_users.count; index++) {
       let this_user_data = all_users.rows[index].value;
       console.log("this_user_data", {this_user_data});
       if (username.indexOf(all_users.rows[index].key) == 0) {
         console.log("Found user", { this_user_data });
         // cleanup deleted conflicts
         if (typeof (user_data._deleted_conflicts) !== "undefined") {
           delete user_data._deleted_conflicts;
         }         
         user_data = this_user_data;
         return user_data;
       }
     }
  }
  ...
  app.post("/api/login", function (req, res) {
    ...
    console.log("[D1] POST api/login", { res }); // here the res seems OK (no header)
    ...
    console.log("client_type:", client_type, "res:", res);
    ...
    user.validate(username, (db_body) => {
      ...
      if (typeof(db_body) === "undefined" || (db_body == null)) {
        console.log({xerr});
        console.log("No db_body, ", db_body, "for user", username);
      ...
      if ((typeof(err) !== "undefined") && (err !== null)) {
        console.log("Userlib view Error: ", {err});
        failureResponse(res, 403, "unauthorized");
      ...
      */
      ...
      // Find user and match password
      var user_data = userDataFromDB(username, db_body);

      if (user_data === null) {
        var err_string = "Attempt to login with non-existent user " + username + "!";
        thinx_slack.alert({ text: err_string });
        console.log("No user data,'", username, "'not authorized against", db_body.rows[0].value);
        failureResponse(res, 403, "unauthorized");
        return;
      }
      ...
      if (client_type == "webapp") {
        ...
        user.create_default_acl(user_data.owner, () => {
          console.log("ACL created async.");
        });

        console.log("webapp loginWithGDPR with res", { res }); // FIXME: first broken here {res} should NOT be 404[debug] [loginWithGDPR] [redirectWithToken]...
  ...
  app.post("/api/device/push", function (req, res) {
    if (!(validateSecurePOSTRequest(req) || validateSession(req, res))) return;
    var owner = req.session.owner;
    devices.push(owner, req.body, (success, response) => {
      respond(res, {
        success: success,
        status: response
      });
