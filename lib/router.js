...
module.exports = function (app) {
  ...
  function failureResponse(res, code, reason) {
    console.log("[debug] Responding with FAILURE", { reason });
    res.writeHead(code, {
  ...
  function validateSecureGETRequest(req, res) {
    if (req.method !== "GET") {
      console.log("[warning] validateSecure: Not a get request." + JSON.stringify(req.query.params));
  ...
  function validateSecurePOSTRequest(req, res) {
    if (req.method !== "POST") {
      console.log("[warning] validateSecure: Not a post request.");
  ...
  function validateSession(req) {
    ...
    console.log("[warning] Session seems invalid (no-owner).");

    sess.destroy(function (err) {
      if (err) {
        console.log("[error] Session destroy error: " + JSON.stringify(err));
      }
    });
  ...
  app.all("/*", function (req, res, next) {
    ...
    if (typeof(req.headers['Authorization']) !== "undefined") {
      login.verify(req, (result) => {
        console.log("[debug] JWT Secret verification result:", {result});
        if (result) {
          next();
        } else {
          res.status(401).end();
    ...
    if ((typeof (app_config.debug.ingress) !== "undefined") && (app_config.debug.ingress !== false)) {
      console.log("[debug] Ingress from IP: ", getClientIp(req));
    }

    if (!checkDirtyRequests(req, res, next)) {
      console.log("[info] Exit after checkDirtyRequests in router:508; not running next.");
      res.end(401);
      return;
    }
  ...
  // TODO: refactor to /build/:id (in UI as well)
  ...
  app.get("/api/slack/redirect", function (req, xres) {
    ...
    var areq = https.get(options, function (res) {
      ...
      res.on('data', function (chunk) {
        ...
        console.log('[debug] [slack] BODY: ' + body);
  ...
  function secureGithubCallbacks(original_response) {
    ...
    if (typeof (githubOAuth) === "undefined") return;
