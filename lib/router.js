  ...
  */
  ...
  app.post("/api/login", function (req, res) {
    ...
    user.validate(username, (db_body) => {
      ...
      if (typeof (req.session) !== "undefined") {
        req.session.owner = user_data.owner;
        console.log("🔨 [debug] [/api/login] assigning owner cookie to existing session owner (%s, %s)", req.session.owner, user_data.owner); // here the request/response seems to be still OK
