...
module.exports = function(app) {
  ...
  console.log("» Bootstrapping modules completed.");
  ...
  app.post("/api/device/mesh/attach", function(req, res) {
    ...
    devices.attachMesh(owner, body, function(success, status) {
      respond(res, {
        success: success,
        attached: status
      });
  ...
  app.post("/api/device/data", function(req, res) {
    ...
    app.messenger.data(owner, udid, function(success, response) {
      respond(res, {
        success: success,
        response: response
      });
  ...
  app.post("/device/register", function(req, res) {
    ...
    if (typeof(req.body) === "undefined") {
      ...
      device.register(req, registration, req.headers.authentication, websocket, function(success, response) {
  ...
  app.get("/api/mesh/list", function(req, res) {
    user.listMeshes(req.session.owner, function(success, mesh_ids) {
      //console.log("Mesh list result:", success, "ids", mesh_ids);
      respond(res, {
        success: success,
        mesh_ids: mesh_ids
      });
  ...
  app.post("/api/build", function(req, res) {
    ...
    if (typeof (existing_sockets) !== "undefined") {
      ...
      if (typeof(sowner) !== "undefined") {
        let xocket = existing_sockets[sowner] || null;
        if ((typeof (xocket) !== "undefined") || (xocket !== null)) {
          socket = xocket;
        }
      }
    }
  ...
  app.post("/api/login", function(req, res) {
    ...
    userlib.view("users", "owners_by_username", {
      ...
      if (user_data === null) {
        ...
        console.log("No user data, ", username, " not authorized against", body);
        failureResponse(res, 403, "unauthorized");
        return;
      }
      ...
      if (password.indexOf(user_data.password) !== -1) {
        ...
        // This is a FAIL! Causes breaking MQTT on each login. Should use Default MQTT API Key as created on registration.
        //console.log("Refreshing owner MQTT credentials...");
        //auth.add_mqtt_credentials(user_data.owner, password);
        ...
        if (client_type == "device") {
          ...
          console.log("Fetching doc", user_data.owner);
