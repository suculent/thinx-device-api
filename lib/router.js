...
module.exports = function (app) {
  ...
  */
  ...
  app.post("/api/transfer/accept", function (req, res) {
    ...
    if (typeof (Validator.udid(req.body.transfer_id)) !== "undefined") {
      respond(res, {
        success: false,
        status: "transfer_id_missing"
      });
    ...
    if (typeof (Validator.owner(req.body.owner)) === "undefined") {
      respond(res, {
        success: false,
        status: "owner_missing"
      });
    ...
    redis_client.get(oauth, function (err, userWrapper) {
      if (err) {
        ...
        userlib.get(owner_id, function (gerr, doc) {
          ...
          // deepcode ignore WebCookieSecureDisabledExplicitly: not expected to be secure between app and Traefik HTTPS terminator, or load-balancer
          req.session.cookie("x-thx-session", fortnight, {
  ...
  app.post("/api/login", function (req, res) {
    ...
    user.validate(username, (db_body) => {
      ...
      if (client_type == "webapp") {
        ...
        console.log("🔨 [debug] [api/login] Refreshing owner default ACLs...");
  ...
  if (typeof (google_ocfg) !== "undefined" && google_ocfg !== null) {
    ...
    app.get('/oauth/cb', function (req, ores) {
      ...
      tok.then(res2 => {
        ...
        global_token = res2.access_token; // TODO (4): FIXME: remove global_token requirement or map to something; console.log("assigning/storing global_token: ", { global_token }); // WTF?
