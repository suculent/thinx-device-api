...
module.exports = function (app) {
  ...
  */
  ...
  function validateSession(req) {
    ...
    console.log("[debug] validating session", sesion);

    if (typeof (sess.owner) !== "undefined") return true;
  ...
  app.get("/api/user/devices", function (req, res) {
    if (!(validateSecureGETRequest(req) || validateSession(req))) return;
    ...
    redis_client.get(oauth, function (err, userWrapper) {
      if (err) {
        ...
        userlib.get(owner_id, function (gerr, doc) {
          ...
          let sess = req.session;
          console.log("Redirect with session", { sess }, "res", { res });
