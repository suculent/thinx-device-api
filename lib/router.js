...
module.exports = function (app) {
  ...
  function respond(res, object) {
    console.log("[respond] with", { object }, { res });
  ...
  function loginWithGDPR(req, res, user_data) {
    ...
    //console.log("[debug] [loginWithGDPR] [redirectWithToken] response should not be 404", { res });

    let token = newTokenWithUserData(user_data);
    let redirectURL = app_config.acl_url + "/auth.html?t=" + token + "&g=" + willSkipGDPR(user_data);
    
    console.log("[debug] [loginWithGDPR] redirecting to", redirectURL);
  ...
  app.post("/api/login", function (req, res) {
    ...
    console.log("[/api/login][0] [D4]", {res});
    ...
    user.validate(username, function(db_body) {
      ...
      console.log("[/api/login][0] [D5]" rows, {rows});

      // Find user and match password
      let user_data = db_body.rows[0].value;
      
      console.log("[/api/login][0] [D6] response", {stored_response}); // at this point res is wrong...
