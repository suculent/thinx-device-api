...
module.exports = function (app) {
  ...
  */
  ...
  console.log("[info] ℹ️ (Bootstrapping) modules completed.");
  ...
  app.get("/api/user/devices", function (req, res) {
    ...
    console.log("[debug] [router] listing devices for sanitized session owner '%s'", owner);
  ...
  app.post("/api/login", function (req, res) {
    ...
    user.validate(username, (db_body) => {
      ...
      if (typeof (req.session) !== "undefined") {
        console.log("[debug] [/api/login] [D7] assigning owner to existing session DB"); // here the request/response seems to be still OK
        req.session.owner = user_data.owner;
