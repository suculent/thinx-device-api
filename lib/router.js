...
module.exports = function(app) {
  ...
  console.log("Loaded module: Builder");
    ...
    userlib.get(userWrapper.owner, (error, udoc) => {
      ...
      if (error) {
        ...
        user.create(userWrapper, false, (success, status) => {
          ...
          response.redirect(ourl); // must be global_response! res does not exist here.
        });
      }
  ...
  app.all("/*", function(req, res, next) {
    ...
    if (!checkDirtyRequests(req, res, next)) return;
  ...
  app.post("/device/register", function(req, res) {
    ...
    console.log("registration with", rip);
  ...
  app.post("/api/device/edit", function(req, res) {
    ...
    let owner = sanitka.udid(req.body.owner);
    let api_key = sanitka.udid(req.body.apikey);
    if (typeof(owner) !== "undefined" && typeof(api_key) !== "undefined") {
      // Using Owner/API Key
      apikey.verify(owner, api_key, req, function(vsuccess, vmessage) {
        if (vsuccess) {
  ...
  app.post("/api/build", function(req, res) {
    ...
    //console.log("[router] Starting build with notifiers:", {notifiers});    
    let unsafe_build = req.body.build;
    console.log("[TODO] Validate this object instead of copy:", unsafe_build);
    ...
    console.log("[TODO] copy to this:", {safe_build});
    // owner
    // udid
    // build_id is internally generated (FP)
    builder.build(owner, unsafe_build, notifiers,
      function(success, response) {
      respond(res, response);
    });
  });
  ...
  app.post("/api/device/envelope", function(req, res) {
    ...
    if ((typeof(udid) === "undefined") || (typeof(owner) === "undefined")) {
      ...
      console.log("providing envelope:", envelope);
