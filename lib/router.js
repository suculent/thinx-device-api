...
module.exports = function(app) {
  ...
  function checkUserWithResponse(global_response, token, userWrapper) {
    ...
    console.log("[oauth][github] searching for owner with wrapper: ", { userWrapper });
    ...
    userlib.get(userWrapper.owner, (error, udoc) => {
      ...
      if (error) {
        ...
        if (Globals.use_sqreen()) {
          Sqreen.auth_track(false, { doc: userWrapper.owner });
        }
        ...
        if (error.toString().indexOf("Error: deleted") !== -1) {
          ...
          user.create(userWrapper, false, (success, status) => {
            ...
            console.log("[OID:" + userWrapper.owner + "] [NEW_SESSION] [oauth] 2485:");

            alog.log(userWrapper.owner, "OAuth User created. ");
            ...
            if (Globals.use_sqreen()) {
              Sqreen.signup_track({ username: userWrapper.owner });
            }
      ...
      trackUserLogin(userWrapper.owner);
        ...
        userlib.get(owner_id, function(err, doc) {
          ...
          if (err) {
            ...
            user.create(wrapper, false, function(success, status) {
              ...
              console.log("[OID:" + req.session.owner + "] [NEW_SESSION] [oauth] 1828: ");
            ...
            console.log("alog: OAuth User logged in:", doc.username);
  ...
  if (typeof(githubOAuth) !== "undefined") {
    ...
    githubOAuth.on('token', function(oauth_token, serverResponse) {
      ...
      console.log("[githubOAuth] GitHub token request successfull:", {oauth_token});
      ...
      https.get(request_options, (res) => {
        ...
        res.on('end', () => {
          ...
          if ((typeof(hdata.name) !== "undefined") && hdata.name !== null) {
            ...
            console.log("Warning: no name in GitHub access token response: ", {hdata}); // logs personal data in case the user has no name!
          }
          ...
          console.log("checkUserWithResponse with sessionID", req.sessionID);
          console.log("checkUserWithResponse from global_responses", { global_responses });
  ...
  if (typeof(github_ocfg) !== "undefined" && github_ocfg !== null) {
    ...
    app.get('/oauth/gcb', function(req, res) {
      ...
      let query = req.query;
      console.log("GtHub OAuth request: ", { query }); // TODO: get some ID for global_response array to allow multiple requests at once without compromising security/availability
      console.log("Saving global_response in /oauth/gcb");
      global_response = res;
      global_responses[req.sessionID] = res;
      ...
      githubOAuth.callback(req, res, function(err) {
        if (!err) {
          ...
          if (global_token !== null) {
            ...
            console.log("global token null on gcb!, err: ", { err });
          }
        } else {
