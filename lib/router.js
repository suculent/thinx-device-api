...
module.exports = function (app) {
  ...
  function responder(res, success, message) {
    console.log("Responding with RESPONDER", {object}, {res});
    respond(res, {
      success: success,
      status: message
    });
  ...
  function failureResponse(res, code, reason) {
    console.log("Responding with FAILURE", {object}, {res});
    res.writeHead(code, {
  ...
  function performOAuthLogin(req, res, oauth) {
    ...
    console.log("[debug] [performOAuthLogin] Searching for Auth token:", oauth, "with res", {res});
  ...
  function newTokenWithUserData(user_data) {
    ...
    console.log("Saving login token (also as global!)", token, {user_data});
  ...
  function loginWithGDPR(req, res, user_data) {
    ...
    console.log("[debug] [loginWithGDPR]", {res}); // already broken here
    ...
    redirectWithToken(res, token, willSkipGDPR(user_data));
  ...
  app.post("/api/login", function (req, res) {
    ...
    console.log("[debug] [removeme] Incoming login response (OK here):", {res}); // TODO: FIXME: Highly dangerous, leaks user credentials!

    let saved_response = res;
    ...
    console.log("Going the U/P login way...");
            ...
            userlib.get(user_data.owner, (error, udoc) => {
              if (error) {
                ...
                userlib.atomic("users", "checkin", user_data.owner, {
                  ...
                  console.log("loginWithGDPR with res", {saved_response}); // the {res} is already wrong here...

                  loginWithGDPR(req, saved_response, user_data);
                });
              }
            });
