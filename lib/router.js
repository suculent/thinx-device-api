...
module.exports = function (app) {
  ...
  function respondRedirectWithToken(req, res, user_data) {
    ...
    if (typeof (req.session.owner) === "undefined") {
      console.log("[D99] [warning] [respondRedirectWithToken] Flushing session: " + JSON.stringify(req.session));
      failureResponse(res, 403, "unauthorized");
      req.session.destroy(function (err) {
        if (err) {
    ...
    //console.log("[debug] [respondRedirectWithToken] [redirectWithToken] response should not be 404", { res });

    let token = newTokenWithUserData(user_data);
    let redirectURL = app_config.acl_url + "/auth.html?t=" + token + "&g=" + willSkipGDPR(user_data);
    
    console.log("[D99] [debug] [respondRedirectWithToken] redirecting to", redirectURL);
  ...
  app.post("/api/login", function (req, res) {
    ...
    console.log("[/api/login][0] request body", req.body); // FIXME! leaks password on login, delete.
    ...
    user.validate(username, function(db_body) {
      ...
      if (typeof (req.session) !== "undefined") {
        console.log("[/api/login] [D7] assigning owner from to existing session DB"); // here the request/response seems to be still OK
        req.session.owner = user_data.owner;
      ...
      // Why is this here? Devices are not expected to login. Bells & whistles?
      ...
      console.log("[D8]", {stored_response}); // here the response is already fault

      if (client_type == "webapp") {

        console.log("[D9] [api/login] Refreshing owner default ACLs...");

        user.create_default_acl(user_data.owner, function(ignored) {
          console.log("[D10a] ACL created async.");
          console.log("[D10a] webapp respondRedirectWithToken with res", { cached_response }); // FIXME: first broken here {res} should NOT be 404[debug] [respondRedirectWithToken] [redirectWithToken]...
