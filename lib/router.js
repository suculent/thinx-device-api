      ...
      for (var bindex in body.rows) {
        ...
        if (typeof(row.value.log) === "undefined") {
          ...
          for (var dindex in row.value.log) {
            ...
            let monthAgo = new Date().getMilliseconds() - 30 * 86400;
            if (body.rows.length > 20) {
              if (row.value.log[dindex].timestamp < monthAgo) continue; // trim old logs in response > 20 rows
            }
            console.log(new Date(row.value.log[dindex].timestamp));
            // get the latest record by timestamp
            if (buildlog.date < row.value.log[dindex].timestamp) {
              buildlog = {
                message: row.value.log[dindex].message,
                date: row.value.log[dindex].timestamp,
                udid: row.value.log[dindex].udid,
                build_id: row.value.log[dindex].build_id
              };
            }
          }
