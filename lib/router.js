...
module.exports = function (app) {
  ...
  */
  ...
  // TODO: FIXME: Take CouchDB admin user/password from environment and remove this from config.json (20 spots surgery)
  ...
  app.post("/api/user/source", function (req, res) {
    ...
    sources.add(object, (error, response) => {
      //console.log("sources:add error", { error });
  ...
  app.post("/device/addpush", function (req, res) {
    ...
    let token = sanitka.pushToken(req.body.push);
  ...
  app.post("/api/login", function (req, res) {
    ...
    user.validate(username, (db_body) => {
      ...
      if (typeof (req.session) !== "undefined") {
        // console.log("[debug] remove me db_body", {db_body});        
        //console.log("user data", user_data, "db_body", db_body);
        req.session.owner = user_data.owner;
  ...
  if (typeof (google_ocfg) !== "undefined" && google_ocfg !== null) {
    ...
    app.get('/oauth/cb', function (req, ores) {
      ...
      if (req.query.code.length > 255) return; // should not DoS the regex now; lgtm [js/type-confusion-through-parameter-tampering]
      ...
      tok.then(res2 => {
        ...
        global_token = res2.access_token; // TODO: FIXME: remove global_token requirement or map to something; console.log("assigning/storing global_token: ", { global_token }); // WTF?
  ...
  app.post('/gdpr/transfer', function (req, res) {
    ...
    var owner_id = Validator.owner(req.session.owner); // TODO: refactor to sanitka or expect only boolean result
    userlib.get(owner_id, function (error, udoc) {
      if (error) {
