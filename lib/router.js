...
module.exports = function (app) {
  ...
  function performOAuthLogin(req, res, oauth) {
    ...
    console.log("[debug] [performOAuthLogin] Searching for OAuth token:", oauth);
  ...
  function loginWithGDPR(req, res, user_data, client_type) {
    ...
    if (typeof (req.session.owner) !== "undefined") {
      console.log("[debug] [loginWithGDPR] session valid (has owner), redirecting to", ourl);
      // Device or WebApp... requires  valid session
      if (client_type == "device") {
        return;
      } else if (client_type == "webapp") {
        console.log("[debug] [loginWithGDPR] Responding with redirectURL:", ourl);
        res.end({ "redirectURL": ourl});
      }
    } else {
