...
[![Build Status](https://travis-ci.org/tzapu/EAVManager.svg?branch=master)](https://travis-ci.org/suculent/EAVManager)
...
__Currently version 0.8+ works with release 2.0.0 or newer of the [ESP8266 core for Arduino](https://github.com/esp8266/Arduino)__
 ...
 - search for EAVManager
  ![EAVManager package](http://i.imgur.com/18yIai8.png)
...
#include <EAVManager.h>          //https://github.com/tzapu/EAVManager WiFi Configuration Magic
```
...
EAVManager.autoConnect("AP-NAME", "AP-PASSWORD");
```
if you just want an unsecured access point
```cpp
EAVManager.autoConnect("AP-NAME");
```
...
Also see [examples](https://github.com/tzapu/EAVManager/tree/master/examples).
...
```cpp
EAVManager.autoConnect("AutoConnectAP", "password")
```
...
See [AutoConnectWithFSParameters Example](https://github.com/tzapu/EAVManager/tree/master/examples/AutoConnectWithFSParameters).
```cpp
EAVManager.setSaveConfigCallback(saveConfigCallback);
```
...
```cpp
EAVManager.setConfigPortalTimeout(180);
```
...
void loop() {
  ...
  if ( digitalRead(TRIGGER_PIN) == LOW ) {
    EAVManager EAVManager;
    EAVManager.startConfigPortal("OnDemandAP");
    Serial.println("connected...yeey :)");
  }
}
```
See example for a more complex version. [OnDemandConfigPortal](https://github.com/tzapu/EAVManager/tree/master/examples/OnDemandConfigPortal)

#### Custom Parameters
You can use EAVManager to collect more parameters than just SSID and password.
...
- add the custom parameters to EAVManager using
 ```cpp
 // id/name, placeholder/prompt, default, length
 EAVManagerParameter custom_mqtt_server("server", "mqtt server", mqtt_server, 40);
 EAVManager.addParameter(&custom_mqtt_server);

 ```
- if connection to AP fails, configuration portal starts and you can set /change the values (or use on demand configuration portal)
- once configuration is done and connection is established [save config callback]() is called
- once EAVManager returns control to your application, read and save the new values using the `EAVManagerParameter` object.
 ```cpp
...
- Save and load custom parameters to file system in json form [AutoConnectWithFSParameters](https://github.com/tzapu/EAVManager/tree/master/examples/AutoConnectWithFSParameters)
...
EAVManager.setAPStaticIPConfig(IPAddress(10,0,1,1), IPAddress(10,0,1,1), IPAddress(255,255,255,0));
```

##### Custom Station (client) Static IP Configuration
This will make use the specified IP configuration instead of using DHCP in station mode.
```cpp
EAVManager.setSTAStaticIPConfig(IPAddress(192,168,0,99), IPAddress(192,168,0,1), IPAddress(255,255,255,0));
```
...
```cpp
EAVManager.setCustomHeadElement("<style>html{filter: invert(100%); -webkit-filter: invert(100%);}</style>");
```
- inject a custom bit of html in the configuration form
```cpp
EAVManagerParameter custom_text("<p>This is just a text paragraph</p>");
EAVManager.addParameter(&custom_text);
```
- inject a custom bit of html in a configuration form element
Just add the bit you want added as the last parameter to the custom parameter constructor.
```cpp
EAVManagerParameter custom_mqtt_server("server", "mqtt server", "iot.eclipse", 40, " readonly");
```

#### Filter Networks
You can filter networks based on signal quality and show/hide duplicate networks.

- If you would like to filter low signal quality networks you can tell EAVManager to not show networks below an arbitrary quality %;
```cpp
EAVManager.setMinimumSignalQuality(10);
```
