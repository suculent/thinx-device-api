...
describe("Device", function () {
  ...
  it("(04b) prior check-in persisted top-level lastupdate and left no nested `changes`", function (done) {
    // Regression (read-only — relies on the check-in done by spec (04) above, so
    // this adds no extra async churn): update_device_and_respond used to pass
    // `{ changes: device }` to the devices/modify handler, which flat-merges
    // top-level fields — so it wrote a nested `doc.changes` blob and never
    // refreshed the real top-level lastupdate the console reads. After the fix a
    // check-in refreshes top-level lastupdate and leaves no `changes` field.
    device.detail(udid, (ok, after) => {
      ...
      // lastupdate must be recent (this run), proving the check-in refreshed the
      // top-level field rather than burying it in a nested `changes` blob.
