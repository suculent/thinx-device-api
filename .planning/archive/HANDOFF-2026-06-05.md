...
3. **Deploy (operator-push; NOT auto):** once CI green builds+pushes `thinxcloud/api:latest`, force the rollout:
   `ssh micro 'docker service update --detach --force --image thinxcloud/api:latest thinx_api'`
...
- Swarm nodes: `micro` (Leader; runs thinx_api + thinx_mosquitto + thinx_couchdb) and `core` (manager). SSH: `ssh -o IdentitiesOnly=yes root@<ip> -p2020` (micro) / `-p 2020` (core).
