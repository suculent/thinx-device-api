...
3. **OPS-01** (Phase 3) — Restored swarm-side autoredeploy on `micro` via Rung 1 force-restart of `swarmpit_app` (silent watcher degradation, Swarmpit 1.9). Push-observe SLA: 63s vs ≤300s target (237s under budget). Zero source-code commits. Canonical runbook persisted to `.planning/runbooks/swarm.md`.
