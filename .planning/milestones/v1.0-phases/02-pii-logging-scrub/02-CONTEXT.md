...
- **Email:** Replace with first-char + `***` + `@` + domain length, e.g., `u***@11c.com` for `user@example.com`. Reasoning: enough signal to disambiguate which user in incident triage; insufficient to enumerate. Implementation: a small `Util.redactEmail(email)` helper if `lib/thinx/util.js` doesn't already have one; otherwise inline.
