# Requirements Archive: v1.0 v1 GA Backend Closures
...
- **SEC-PII-02**: Historic entries in the CouchDB `managed_logs` database (`thinx_couchdb` on swarm host `micro`, ~658,808 docs as of 2026-05-26) still contain raw 64-character reset_keys from before Phase 2's fix landed. Sample evidence: lines like `Attempt to set password with: <reset_key redacted — 64-hex>`. Phase 2's `SEC-PII-01` fix prevents the leak from continuing; cleanup of the historic data is a separate concern. Defer to v1.x/v2. Possible remediation: (a) one-time `_bulk_docs` UPDATE with redacted message strings, (b) bulk delete of audit entries older than a retention window, (c) introduce an `audit_log` TTL going forward. GDPR-adjacent.
