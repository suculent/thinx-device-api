...
status: planning
last_updated: "2026-06-28T21:51:17.764Z"
...
| ci_red | CircleCI 5269/5270 — `00-AppSpec /api/login (invalid)` 503 | **RESOLVED.** Root cause confirmed: CouchDB `owners_by_username` design-view index-build race on the fresh CI DB — first request raced the index → `Owner.validate()` transient view error → `router.auth.js:287` 503. Not a v1.11 code regression (v1.11 touched no auth/owner/couch code; only runtime dep delta @hapi/wreck is off this path); pre-existing race exposed by v1.11 startup-timing shift. **Fix applied** (`spec/jasmine/00-AppSpec.js`): warm the users views with retry after `thx.init()`. **Verified green** on `thinx-unit` (pipeline 5271, test-only, no deploy). Full diagnosis: `.planning/debug/resolved/ci-login-503-couchdb.md`. |
| follow_on | Land fix on thinx-staging + deploy 15/16 | The view-warmup fix + all v1.11 commits are on local HEAD and on `thinx-unit` (green). Pushing to `thinx-staging` triggers the deploy pipeline (CI → image → Swarmpit autoredeploy of 15/16+fix to prod) — **deliberate operator step**, not yet done. `thinx_api` is micro-pinned (co-located w/ mosquitto), so the rollout stays co-located. |
